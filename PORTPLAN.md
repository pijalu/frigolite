...
- **Closed goal: P8.RECOVER (2026-09-07, T0-T7 + WR-WRITE + WR-FIX)** —
  ...
  (dbdata reader, schema walk, iField remap, lost_and_found, sequence);
  DB.RecoverSQL in-process API; UCL fixtures s1_1..s4_1 (3.51.0 oracle).
  The WITHOUT ROWID engine write-path gap was closed as part of this goal
  (WR-WRITE da88258c2: 0x0A roots, PK-first cells, PK-addressed
  update/delete) and the 15 pass→fail regressions it introduced were
  repaired in the WR-FIX tranche (interior-root/affinity scan remap, join
  right-side scan, WR-aware DROP COLUMN rebuild, identity-addressed DML,
  REPLACE conflict order, FK scan remap) — all pinned by
  frigolite_wr_storage_native_test.go. Full detail in
  plan/goals/P8.RECOVER.md close sections.
- **Closed goal: P8.PAGER (2026-09-07, batches 1-17, 0 FAIL)** —
  24 packages (bigfile..stmtrand): pager/page-format/btree validation (SQL
  surface). Committed/pushed: 2ff1c238 (un-skip + regen), 7104478c (batch 1),
  f19e260a (batch 2: SQLITE_READONLY write path), 327b2514 (batch 3:
  filefmt+pagesize header validation), b6d7233c (batch 4: pendingrace
  hot-journal playback), d645cd94/b3e85605/5118141b (batches 5/5b/6: memdb1/
  memdb2/memdb serialize + snapshot + signature), c96f4283 (batch 7: cache
  pager_cache_size seam + spill lock model), batch 8 (value-builtin do_test
  dispatch + SQLITE_TOOBIG bind mapping + deterministic regen emission).
  Score 18/24 — 6 red: sqllimits1 (6 result-mismatches), memsubsys1/2,
  bitvec (hang), oserror, sqldiff1. Full per-batch status, census table and
  next-step plans live in `plan/goals/P8.PAGER.md`. **Harness note**: the
  `.goa` goal tool was removed by the user — goal tracking = sub-plan + this
  file's live markers + agent todo list; commits are the durable takeover
  record.
- **Previous goal: P8.PRAGMA closed (2026-09-06)** — 10/10 target packages
  green (pragma, pragma2-6, quota, quota2, quota_glob, tkt2686; serial
  0.2-17.9s). Engine: full `internal/quota` port of test_quota.c (groups,
  quotaStrglob, Initialize/Shutdown MISUSE semantics, stdio file API),
  pager quota hooks + ValidateHeader narrowing + flush-failure rollback,
  max_page_count (default 1073741823, clamp, SQLITE_FULL enforcement),
  integrity_check/quick_check row-format corruption reports,
  TempStore/Synchronous pragma.c contracts, real-rowid UNIQUE conflict
  messages. UCL: frigolite_maxpage_test.go (5 tests) +
  frigolite_quota_test.go (4 tests), oracle-verified. Ledger
  re-seeded 2026-09-06T06:33:30Z; `tools/status --check` PASS. Full
  detail in `plan/goals/P8.PRAGMA.md` §5e Close Status.
- Previous goal: P8.INCRVACUUM closed (2026-09-04, S7 status) — 5/5 packages pass the
  testgen verify command (3.4s / 0.2s / 1.2s / 0.6s / 0.4s serial). Two engine
  fixes closed the long-standing 4.1/4.2.1/4.3 failures: a433c318 aligned the
  btree divider convention with SQLite (btree.c:8813), and 001af0a8 made
  `PRAGMA wal_checkpoint` honor its default PASSIVE mode (pragma.c
  PragTyp_WAL_CHECKPOINT, wal.c walCheckpoint). UCL tests
  `TestNativeBtreeDividerMatchesSQLite`, `TestNativeWalCheckpointHonorsMode`,
  and `TestNativeWritePtrmapRollbackFidelity` pin all three fixes locally.
  5 stale `unsupportedTestFiles` JSON-harness entries rewritten with new
  evidence (harness uses `:memory:` pager that rejects WAL). Full detail in
  `plan/goals/P8.INCRVACUUM.md` § S7 Close Status.
...
- Queue position: §5a item 10 (P8.STORAGE — CORRUPT, ENCODING, INCRVACUUM,
  MISC, PRAGMA ✅; **P8.PAGER active next**, then RECOVER, ROLLBACK,
  VACUUM), then item 11 (full-suite drift triage — now owns the five
  P8.INCRVACUUM-window regressions), then P9.
...
| `P6.FTS5` | [`P6.FTS5.md`](plan/goals/P6.FTS5.md) | 144 | ⚠ **live 107/144 GREEN (2026-09-14: 77 pass + 30 adjudicated N-A/superseded + 37 class-indexed reds)** — fts5 engine ported (internal/fts5/: config+tokenizers+storage+inverted index+full MATCH query language+rank/bm25+highlight/snippet+special queries); T33 test-support tranche: fts5vocab module, fts5tok eponymous vtab, fts5_rowid/fts5_decode, fts5_expr print engine, fts5_test_* aux family (internal/fts5/{vocab,tokvtab,rowid,exprprint,auxfunc}.go) + vtab TVF form (FROM vtab(args)); 74 green (5 newly green + 7 superseded with native anchors in frigolite_fts5_testfn_test.go); 37 reds class-indexed in the goal file (2026-09-14 adjudication: 8 escalated engine gaps, 6 oracle-drift cases, 6 harness artifacts, 9 architectural/too-deep incl. pending-hash + contentless_delete storage, 8 slow/PERF-class) | fts5 full-text engine |
...
| Full-suite drift (§5a item 11) | **GREEN-LEDGER re-based on the FIRST REAL run (2026-09-06T06:33:30Z): 700 pass / 235 fail / 271 skip / 13 timeout-suspects** — the prior "2026-09-05" baseline was a re-stamped copy of the 2026-09-03 run (all durations byte-identical; discovered at P8.PRAGMA close). §4 rows carry `⚠ live N/M` markers. Newly attributed regressions (worktree-bisected 2026-09-06, fail at 4 pre-P8.PRAGMA commits): **lock, pcache, corruptB, fts3corrupt4, tkt_fc62af4523** — introduced in the P8.INCRVACUUM T6/S6 window (2026-09-04: ade63308 CrossConnLockError PRAGMA gating / 3b2d74ef pager revert / f5f79c7d freelist chain head). fts3snippet + fts4langid healed (pass in the 2026-09-06 run); fts4opt status to re-confirm. Verified classes remain: (a) pre-squash drift (select1/insert/where fail at `ba771a6b`); (b) visible-window regressions | `tools/status/ledger.json`, `tools/status/last_run.json`, §2 DRIFT ALERT, worktree probes at f5f79c7d/1d90f62f/a213f805/1abce648 | Triage the 5 INCRVACUUM-window regressions FIRST (btree RelocatePage/updateParentChildPtr + CrossConnLockError classes) in per-goal tranches with `tools/status --check` between tranches; then class (a); never skip without NA_EVIDENCE |
...
| P7 planner follow-up | ✅ CLOSED (2026-09-12, T27): the vtab xBestIndex contract is implemented end-to-end (`internal/vtab/indexinfo.go` + `internal/execquery/vtab_bestindex*.go` + `internal/exec/vtab_bestindex.go`); bestindex1-9/A-G + autoanalyze1 reclassified N-A with native evidence (`frigolite_bestindex_test.go`); residual `skipscan1-8.1eqp` OR-with-skip-scan EQP divergence stays documented | `plan/goals/P7.PLANNER.md` T27, `portplan/NA_EVIDENCE.md` §P7.PLANNER.bestindex, `tools/tcl2go/skiptestfiles.go` | — |
| P7.SHAREDCACHE (candidate) | Shared-cache engine layer NOT ported by design (P7.WAL-G7 slice 5, 2026-09-14): the shared*.test contract needs the global sqlite3_enable_shared_cache C-API + shared pager cache/schema registry + btree TABLE-level locks ("database table is locked: abc") — a separate subsystem from the WAL shm layer, with zero wal.c overlap; deprecated upstream since 3.43.0 (SQLITE_OMIT_SHARED_CACHE). N-A evidence: portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 5 + frigolite_shared_test.go (oracle-verified lock contract) | btree.c shared-cache locking, pager.c lockTable, test/shared*.test | Only pursue if a downstream user needs shared-cache; the WAL multi-connection machinery (P7.WAL-G7 slices 1-4) is the supported concurrency model |
...
| Goal | Sub-plan | # Pkgs | Focus |
|------|----------|--------|-------|
| `P8.CORRUPT` | [`P8.CORRUPT.md`](plan/goals/P8.CORRUPT.md) | 13 | ⚠ live 6/13 (2026-09-03) — ✅ 8/8 in-scope packages green (corrupt, corrupt2, corrupt3..8). Engine: checkTreePage walker (cross-tree first-reference tracking + orphan detection + freelist exclusion), checkFreelistCount, markOverflowChain. Transpiler: tclAtoi / tclReadFileWithLen / tclBinaryScanBigUint16 / tclChannelAppendAt, tclExecSQL row-vs-cell separator (rows `\n`, cells space), processSeek folding, processSet registering activeFileChannels. Native: TestCorrupt2CheckTreePage crafts duplicate-page-ref + orphan DB via /usr/bin/sqlite3. Files split: pragma_quickcheck_trees.go (gocognit/gocyclo compliance). Pre-existing tools/tcl2go/ complexity findings deferred to final closure per §2a. Verify command exits 0 (at closure, over the then-8-pkg scope). The 5 remaining packages (corrupt9/corruptC/corruptF/corruptL/corruptN) are IN SCOPE and currently FAIL (see live marker) — the sub-plan verify command now covers all 13; their repair belongs to the FULL-SUITE-DRIFT tranches. | Database corruption detection (SQL surface) |
| `P8.ENCODING` | [`P8.ENCODING.md`](plan/goals/P8.ENCODING.md) | 8 | ✅ **complete** (2026-08-30) — 8/8 in-scope packages green (enc, enc2, enc4, securedel, securedel2, symlink, symlink2) + 1 superseded-N-A (enc3, UTF-16 storage) per Pure-Go supersession policy. 3 further target packages (uri, uri2, utf16align) are IN SCOPE, currently whole-file skipped on 'deep-engine applicable gap' reasons (no NA_EVIDENCE basis for a UTF-16/URI N/A) — un-skip + verify during FULL-SUITE-DRIFT tranches. Engine: PRAGMA secure_delete getter/setter with per-schema map (mainSecureDelete field for MAIN, secureDeletes map for attached DBs, defaultSecureDelete = 2 = SQLITE_FAST_SECURE_DELETE build-option equivalent — securedel.test DEFAULT_SECDEL=2); execddl.execAttach inherits MAIN's current secure_delete for the new Btree (src/attach.c:207-208 sqlite3BtreeSecureDelete inheritance). Transpiler: processIncr recognizes `detect_blob` (and `[detect_blob ...]` form) as a function-call increment and stubs the return value to 0 — makes the `incr n [detect_blob {} $i]` line a no-op (securedel2 1.5.2/1.6.2 expect n=0). Native oracle-verified tests: TestNativeSecureDeleteContract (default=2 / main setter / db2 inheritance / no-schema setter / DETACH+re-ATTACH), TestNativeEncodingMismatchContract (writes UTF-16le magic to aux header directly to validate checkAttachEncoding), TestNativeSymlinkContract (ATTACH of missing file creates it). Symlink.test 1.4/1.5 (PATH_MAX truncation) and 1.1.4 (-nofollow flag) are VFS-layer N-A in pure-Go Frigolite. TestParseSkipMaps floor lowered 298→293 (5 fully un-skipped + 2 re-skipped with N-A evidence). Zero staticcheck new findings; -race green; SOLID green; no regression (P8.CORRUPT verify command still exits 0 + only pre-existing TestNolockNoCrossConnectionLocking fail outside the verify scope). Full verify command exits 0. | Encoding (UTF-16), secure-delete, URI |
| `P8.INCRVACUUM` | [`P8.INCRVACUUM.md`](plan/goals/P8.INCRVACUUM.md) | 5 | ✅ **complete** (2026-09-04, S7 close) — ⚠ live 5/5 GREEN (run stamp 2026-09-03T16:50:35Z last_run.json; all 5 packages serial PASS 2026-09-04 — 3.4s / 0.2s / 1.2s / 0.6s / 0.4s). `autovacuum`, `autovacuum2`, `incrvacuum`, `incrvacuum3` GREEN since S6; **`incrvacuum2` GREEN since S7** (was FAIL: pre-existing WAL/overflow bugs in tests 4.1 / 4.2.1 / 4.3 — all 3 fixed in a433c318 4.1 btree divider, 001af0a8 4.2.1 wal_checkpoint PASSIVE mode, 317521c8 lessons). S6 work: threaded `nFin` through `BTree.IncrVacuumStep` (btree.c:4010-4104), added `vacuumSkipPages` for the C post-block do-while (btree.c:4096-4102), and made `SetAutoVacuum` + `schema.Init` persist `LargestBTreePage=1` in the on-disk header (btree.c:3537) so a fresh DB created with `PRAGMA auto_vacuum=1` survives close/reopen. S6 also closed the 5.2.5 DROP+PRAGMA incremental_vacuum infinite-loop. S7 work: a433c318 (incrvacuum2 4.1) aligned btree divider convention with SQLite (btree.c:8813 MAX(left) for table btrees, MIN(right) for index btrees, findChildPageForInsert + seekInInteriorTable < key strict comparison, reparentPageOverflowChains skips interior pages); 001af0a8 added `Pager.CheckpointMode` PASSIVE/FULL/RESTART/TRUNCATE (wal.c walCheckpoint) so default `PRAGMA wal_checkpoint` honors PASSIVE semantics (incrvacuum2 4.2.1 `file size test.db-wal == 1104`). UCL added: `TestNativeBtreeDividerMatchesSQLite` (4.1), `TestNativeWalCheckpointHonorsMode` (4.2.1), `TestNativeWritePtrmapRollbackFidelity` (ROLLBACK fidelity across the 5 WritePtrmap sites in `internal/btree/btree_alloc.go`). NA_EVIDENCE audit: 5 stale `unsupportedTestFiles` entries rewritten with new evidence (JSON harness uses `:memory:` pager which rejects WAL; testgen package is the authoritative green source). Verify command exits 0; build/vet/SOLID green; -race green; 0 new staticcheck/gocognit/gocyclo findings from S7. | Incremental and auto-vacuum |
| `P8.MISC` | [`P8.MISC.md`](plan/goals/P8.MISC.md) | 3 | ✅ **complete** (2026-09-05) — 2/3 packages un-skipped and green (cksumvfs full 8500 randomblob insert loop 217s; tkt3080 recursive-SQL-UDF case green); tkt3718 re-skipped w/ NA_EVIDENCE citing nested-statement-journal gap (UDF-driven INSERT inside INSERT SELECT does not participate in parent pager snapshot rollback; tkt3718-4.3 cannot be reproduced until the UDF's separate db.Exec is incorporated into the parent's statement journal). Engine: added `DB.EvalExecSQL(sqlStr, sep)` public surface (engine.evalExecSQL wrapper) so test-harness UDFs can recursively run SQL. tcl2go: `db function execsql execsql` (tkt3080 — recursive SQL UDF mirroring ext/misc/eval.c eval(), wraps BeginActiveStatement/EndActiveStatement so DROP TABLE inside triggers the OP_Destroy interlock); `db func sql [list sql]` (tkt3718 — conditional SQL-execute UDF, swallows catchsql errors so the outer INSERT survives); proc-body shape detector for f1/f2 (tkt3718 f1 recursive SELECT f2(\$a); f2 identity with three → error("Three!!")). UCL native tests: `frigolite_misc_native_test.go` (TestNativeMiscUDFFromHarnessExecutesSQL, TestNativeMiscUDFF1F2, TestNativeMiscCksumvfsPragmaStub) — all green. Verify command exits 0; build/vet/SOLID/race green; no regression in pre-existing tests (misc8/aggorderby/update confirmed via git-stash baseline). Commit 81ef62cd on main. | Misc recovery: cksumvfs, harness UDFs |
| `P8.PAGER` | [`P8.PAGER.md`](plan/goals/P8.PAGER.md) | 24 | ✅ **complete** (2026-09-07, batches 1-17, 0 FAIL) — 19 packages green + 5 N/A-with-evidence (bitvec: test-only C Bitvec self-test + memdebug injection; oserror: C VFS syscall injection, file self-skips; memsubsys1/2: SQLITE_CONFIG_MALLOC allocator + STATUS_ counters; sqldiff1: external tool binary seam — all NA_EVIDENCE-backed). Engine work: greedy interior tail-split (balance_nonroot packing parity — census byte-identical to the 3.51.0 reference build: leaf=699, interior=7, overflow=984), rowid-cache UPDATE corruption fix (nested-INSERT row loss), schema-btree freelist reuse at the page cap, O(rows x tree) UPDATE loops (DeleteCellByRowID + unique-column guard), reserved-space feature, SQL-length/column-term/strftime/identifier limits, SQLITE_OPEN_READONLY, faithful catch semantics, TK_ILLEGAL lexer contracts. Run stamp 2026-09-07T00:35:51Z: 710/255/254 of 1219; ledger re-seeded; check PASS. | Pager/page-format/btree validation (SQL surface) |
| `P8.PRAGMA` | [`P8.PRAGMA.md`](plan/goals/P8.PRAGMA.md) | 10 | ✅ **complete** (2026-09-06) — ⚠ live 10/10 GREEN (run stamp 2026-09-06T06:33:30Z). Engine: `internal/quota` full port of test_quota.c (quota groups keyed by quotaStrglob patterns, per-file size tracking, Initialize/Shutdown MISUSE semantics, callback-extension round-trip, stdio-like FOpen/FWrite/FRead/FSeek family with C-exact semantics incl. `nmemb=(iEnd-iOfst)/size` cap and `zGlob-1` recursion for `*[...]`); pager quota hooks (Register/CheckGrowth/Unregister-always-in-Close), ValidateHeader narrowed to lazy freelist reads, flush-failure journal rollback + execRollback on COMMIT flush error; max_page_count (default 1073741823, setter echo clamp MAX(N,NumPages), ≤0/junk = getter, pager SQLITE_FULL enforcement); integrity_check/quick_check report corruption as rows never errors (checkFreelistCount btree.c checkList/checkRef messages); TempStore/TempStoreDirectory/Synchronous pragma.c contracts (invalidateTempStorage, in-transaction guards); lock_status temp "closed"/"unknown" via tempBtreeOpen; real-rowid threading in UNIQUE conflict errors (rowid-alias PK stores NULL); empty-join short-circuit skips TVF operands. Transpiler: sqlite3_quota_* command family (19 commands), tclConnRegistry runtime dispatch for arbitrary connection names, catch+while loop propagation, db_restore_and_reopen/db_delete_and_reopen, file_control_vfsname. UCL: frigolite_maxpage_test.go (5) + frigolite_quota_test.go (4), oracle-verified (/usr/bin/sqlite3 3.51.0). Residual skips: none in scope (pendingrace un-skipped as collateral — visible FAIL owned by P8.PAGER). Verify command exits 0; build/vet/staticcheck/SOLID/race green (0 DATA RACE); `tools/status --check` PASS; ledger re-seeded 2026-09-06. quality_gate pager.go >1000-line FAIL pre-existing (1763 at 1abce648), deferred per §5c. | PRAGMA edge cases, max_page_count, quota |
| `P8.RECOVER` | [`P8.RECOVER.md`](plan/goals/P8.RECOVER.md) | 1 | ✅ **complete** (2026-09-07, T7 close) — `recover_pkg` fully green incl. 2.1.1 (WR round-trip) and 2.4.1 (WR orphans → lost_and_found_1 with id NULL + storage-order fields). Engine: full `internal/recover` port of ext/recover (dbdata page reader, schema walk, row reassembly with PK-first iField remap, lost_and_found collision naming, sqlite_sequence); `DB.RecoverSQL` in-process API replacing the external-CLI subprocess; oracle fixtures testdata/recoverconformance s1_1..s4_1. Blocker closed by the WR-WRITE engine goal (da88258c2: WITHOUT ROWID index-btree write path) and the WR-FIX regression tranche (15 pass→fail flips from da88258c2 repaired: interior-root scan remap + affinity ordering, join right-side scans, WR-aware DROP COLUMN rebuild, identity-addressed DELETE/REPLACE/UPDATE-OR-REPLACE, REPLACE conflict ordering, FK parent-scan remap — all pinned by frigolite_wr_storage_native_test.go). | .recover (corrupt-db recovery) |
| `P8.ROLLBACK` | [`P8.ROLLBACK.md`](plan/goals/P8.ROLLBACK.md) | 4 | ✅ **complete** (2026-09-07) — 4/4 target packages green (exclusive, exclusive2, rollback, rollback2; re-confirmed PASS in the 2026-09-11T00:09:07Z full run). Engine: lockreg persistent-SHARED marks so locking_mode=EXCLUSIVE holds locks across transactions (autocommit writes fail immediately, explicit-tx writes at COMMIT); transpiler: TCL sqlite3-rebind-close semantics, t1sig fingerprint inlining, pager change-counter helpers, nil-safe resErrString in Errorf emissions, format-proc db-func registrations. Detail in plan/goals/P8.ROLLBACK.md close sections. | Rollback journal semantics |
| `P8.VACUUM` | [`P8.VACUUM.md`](plan/goals/P8.VACUUM.md) | 7 | ✅ **complete** (2026-09-07, T7 close) — all 7 packages green (vacuum, vacuum2, vacuum3, vacuum4, vacuum5, vacuum6, vacuum_into) plus collateral greens memdb1, without_rowid1, reservebytes, backup. Engine: plain VACUUM as logical rebuild + FULL-IMAGE-REPLACE copy-back (Backup.FullImageReplace resets a populated destination empty — that is what compacts, memdb1 115→2); sqlite_sequence preserved via the data copy (vacuum.c); file change counter pinned at pre+1 (one commit per VACUUM, oracle-verified 3/6/7); external-change reload adopts header page size (16..17) and reserve (20) so a second connection re-reads the image correctly (vacuum3-4.5); `VACUUM <schema>` grammar wired (rule 250 existed in the LALR tables, action added) and copyViaBackup targets the VACUUMed schema (VACUUM x2 shrinks test2.db only); INTO target as expression (VacuumStmt.IntoExpr + Engine.EvalExpr: "non-text filename" / resolution errors / UDF+subquery targets); nVdbeActive>1 guard ("cannot VACUUM - SQL statements in progress"); file_control_reservebytes is a REQUEST applied by VACUUM (Pager.SetReservedBytes/ApplyReservedBytes split); WR autoindex positional semantics with WITHOUT ROWID PK absorption (t48 → only sqlite_autoindex_t48_2, oracle-verified); integrity_check walks the schema btree (page 1) and decodes cells with UsableSize (reserve≠0 overflow accounting). tcl2go: expected-position hexio read hoist (_wantBaseN), string-const UDF pre-pass (collectStringConstFuncs), array-get counter bodies emitted side-effects-only (VFS-layer instrumentation). Verify command exits 0; build/vet/SOLID green; ledger re-seeded 2026-09-07. Residual: frigolite.go >1000-line hard gate pre-existing; TestP1InsertUpsert/TestP3* standard-suite failures pre-date the goal (fail at da88258c2) — FULL-SUITE-DRIFT scope. | VACUUM and VACUUM INTO |
...
| 10 | `P8.STORAGE` — CORRUPT (P8.CORRUPT residue: corrupt9 PASS, corrupt/C/F/L/N FAIL — FULL-SUITE-DRIFT scope), ENCODING (uri/uri2 re-skipped w/ sharpened converter-gap reasons, utf16align un-skipped PASS — T5 2026-09-09), INCRVACUUM ✅ complete (2026-09-04, S7 close; ⚠REGRESSION WATCH: `autovacuum` 2.4.5 FAILs in the 2026-09-11 run — re-triage under FULL-SUITE-DRIFT), MISC ✅ complete (2026-09-05), PRAGMA ✅ complete (2026-09-06; ⚠REGRESSION WATCH: `pragma2` page_size=16384+cache_spill FAILs in the 2026-09-11 run — re-triage under FULL-SUITE-DRIFT); PAGER ✅ complete (2026-09-07, batches 1-17) → RECOVER ✅ (2026-09-07) → ROLLBACK ✅ (2026-09-07, row updated 2026-09-11) → VACUUM ✅ (2026-09-07); UCL: hexdump/ptrmap fixtures + corrupted-DB corpus | `P8.*.md` | Sub-plan verify commands |
