...
- **Active goal: P8.PAGER (2026-09-06, tranche census)** — 24 packages
  (bigfile..stmtrand): pager/page-format/btree validation (SQL surface).
  T1 done: 23 skip entries removed + testgen regenerated (suite-wide delta =
  QLDBG debug print cleanup); failure census running. Known FAIL to fix:
  `pendingrace` (integrity_check on locked db must error "database is
  locked"). Highest regression risk of the queue (pager/btree edits) —
  native seam-pinning tests before refactors; ledger `--check` zero-flip
  gate at close. Sub-plan + live census table:
...
| `P8.PAGER` | [`P8.PAGER.md`](plan/goals/P8.PAGER.md) | 24 | ⚠ **ACTIVE** (2026-09-06) — T1 un-skip + regen done; failure census running; pendingrace FAIL known (integrity_check on locked db). Baseline + census table live in sub-plan. | Pager/page-format/btree validation (SQL surface) |
