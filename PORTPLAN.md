...
| Goal | Sub-plan | # Pkgs | Focus |
|------|----------|--------|-------|
| `P8.CORRUPT` | [`P8.CORRUPT.md`](plan/goals/P8.CORRUPT.md) | 13 | ⏸ DEFERRED — 6/13 packages un-skipped and passing (corrupt3..8). Engine fixes landed: transpiler r+ mode bug (`set fd [open FILE r+]` now registers the channel, was leaking across test files), parent-split recovery (`findParentOfChild` correctly locates the original child across both halves after interior split), cell-too-large retry path on empty-leaf pages (corrupt-5.2). 7/13 packages (corrupt + corrupt2 + corrupt9 + corruptC + corruptF + corruptL + corruptN) remain DEFERRED with deeper engine work: corruption detection (balance_deeper per-cell pointer validation, schema-load on corrupt btree), integrity_check Tree X page Y cell Z / Page Y: never used message format, freelist size accounting. Verify-command scope (corrupt..corrupt8) is exit-non-zero — corrupt and corrupt2 still assert. | Database corruption detection (SQL surface) |
...
| 8 | `P7.CONCURRENCY` 🔄 **P7.LOCK-A active** (goa goal `plucky.pike`, resumed 2026-08-28; baseline 2/10 green: `lock`+`lock3` pass, 8 fail on two-process emulation + VFS locking styles — see `plan/goals/P7.LOCK-A.md` Session 2026-08-28) | AUTOINDEX, LOCK-A/B/C, WAL-A/B/C/D/E, SNAPSHOT; UCL: WAL frame decoder (src/wal.c) + oracle-generated -wal/-journal fixtures BEFORE any pager edit | `P7.*.md` | Sub-plan verify commands |
