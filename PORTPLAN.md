...
- **Active goal: P8.INCRVACUUM (queue item 10), 4/5 green** after S6
  (autovacuum, autovacuum2, incrvacuum, incrvacuum3 pass; incrvacuum2
  still fails on pre-existing WAL/overflow bugs in tests 4.1/4.2.1/4.3,
  out of S6 scope). S6 fix details in
  `plan/goals/P8.INCRVACUUM.md` § S6 Close Status. The S6 work
  threaded `nFin` through `BTree.IncrVacuumStep` per
  `src/btree.c:4010-4104`, added a `vacuumSkipPages` helper to mirror
  the C post-block do-while (btree.c:4096-4102), and persisted
  `auto_vacuum` in the on-disk `header[52:56]` so a fresh DB
  created with `PRAGMA auto_vacuum=1` survives close/reopen with
  the flag intact (incrvacuum-12.4 expected 1, got 0 — now 1).
  See `plan/goals/P8.INCRVACUUM.md` for the per-package S6 fix
  table and the residual-risk analysis.
...
| `P8.INCRVACUUM` | [`P8.INCRVACUUM.md`](plan/goals/P8.INCRVACUUM.md) | 5 | 🔄 **ACTIVE (§5a item 10)** — ⚠ live 4/5 (2026-09-04 after S6): `autovacuum`, `autovacuum2`, `incrvacuum`, `incrvacuum3` GREEN; `incrvacuum2` still FAILs on pre-existing WAL/overflow bugs (test 4.1/4.2.1/4.3 — out of S6 scope; residual risk documented in sub-plan § S6 Close Status). S6 work: threaded `nFin` through `BTree.IncrVacuumStep` (btree.c:4010-4104), added `vacuumSkipPages` for the C post-block do-while (btree.c:4096-4102), and made `SetAutoVacuum` + `schema.Init` persist `LargestBTreePage=1` in the on-disk header (btree.c:3537) so a fresh DB created with `PRAGMA auto_vacuum=1` survives close/reopen (incrvacuum-12.4 expected 1, got 0 — now 1). S6 also closed the 5.2.5 DROP+PRAGMA incremental_vacuum infinite-loop. The 0/5 → 1/5 jump in T6 came from the phase15 `WritePtrmap`-at-allocation work; the 1/5 → 4/5 jump in S6 came from the per-test targeted fixes in `btree_vacuum.go` + `schema.go` + `pager.go::SetAutoVacuum`. | Incremental and auto-vacuum |
...
| 10 | `P8.STORAGE` 🔄 **ACTIVE** — CORRUPT ✅ (⚠ live 6/13), ENCODING ✅ (live green), **INCRVACUUM 🔄 live 4/5 after S6 (autovacuum, autovacuum2, incrvacuum, incrvacuum3 GREEN; incrvacuum2 still FAILs on pre-existing WAL/overflow bugs in tests 4.1/4.2.1/4.3 — out of S6 scope; next: icrvacuum2 overflow-page chain + WAL backfill counters per `plan/goals/P8.INCRVACUUM.md` § S6 Close Status)**; MISC→PRAGMA→PAGER→RECOVER→ROLLBACK→VACUUM queued; UCL: hexdump/ptrmap fixtures + corrupted-DB corpus | `P8.*.md` | Sub-plan verify commands |
