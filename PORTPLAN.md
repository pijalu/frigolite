...
- **Live full-suite baseline (2026-09-07, `tools/status` run
  2026-09-07T00:35:51Z: 1,219 testgen packages, 60s/pkg timeout, 8 workers):
  710 PASS (58.2%), 255 FAIL, 254 SKIPPED** — ledger re-seeded from this run
  at the P8.PAGER close; prior 06:33Z baseline superseded (its deltas are
  enumerated in the P8.PAGER sub-plan T4 worklist: net +10 pass and 5
  evidence-backed N/A classifications against the previous baseline). CORRECTION (2026-09-06): the prior "2026-09-05T00:00Z"
  baseline was a re-stamped copy of the 2026-09-03 run (all 1,219
  per-package durations byte-identical) — no fresh run had occurred; the
  ledger seeded from it inherited stale states. The ledger is now seeded
  from the 2026-09-06T06:33:30Z run. Direct serial package runs supersede
  cached details; refresh `tools/status` before selecting a new tranche.
...
| 10 | `P8.STORAGE` — CORRUPT ✅ (⚠ live 6/13), ENCODING ✅ (live green), INCRVACUUM ✅ complete (2026-09-04, S7 close), MISC ✅ complete (2026-09-05), **PRAGMA ✅ complete (2026-09-06 — 10/10 live GREEN, `internal/quota` engine, run stamp 2026-09-06T06:33:30Z)**; PAGER active next → RECOVER → ROLLBACK → VACUUM queued; UCL: hexdump/ptrmap fixtures + corrupted-DB corpus | `P8.*.md` | Sub-plan verify commands |
...
> P8.RECOVER ✅ (2026-09-07) → P8.ROLLBACK ✅ (2026-09-07) → P8.VACUUM ✅ (2026-09-07) → FULL-SUITE-DRIFT (active next) → P6.FTS-RESIDUE →
