...
### Step 8: Fix SQL formatting for `sqlite_master` queries
...
**Fix:**
1. Ensure CREATE TABLE stores the original SQL text (not a re-serialised version).
2. ALTER TABLE only modifies the renamed tokens in the original text.
3. Views, triggers, and indexes store their original SQL text.

## Files Modified

| File | Change |
|------|--------|
| `internal/sql/ast.go` | Add TokenInfo to identifier nodes |
| `internal/sql/lexer.go` | Expose byte offsets on tokens |
| `internal/sql/parser.go` | Copy token positions to AST nodes |
| `internal/exec/rename.go` (NEW) | RenameContext, FindRenameTokens, ApplyRenames |
| `internal/exec/engine.go` | Rewrite execAlterTableRename, execAlterTableRenameColumn; add legacy_alter_table |
