...
func (s *cliState) handleDotCommand(cmdLine string) bool {
	...
	case ".databases":
		s.output("database: %s\n", s.db.Path())
	case ".headers", ".mode", ".separator", ".output", ".import":
		s.handleArgCommand(args)
...
// handleArgCommand dispatches the dot-commands that take arguments.
func (s *cliState) handleArgCommand(args []string) {
	switch args[0] {
	case ".headers":
...
// createOrDropTableCtx resolves a TABLE/INDEX/VIEW/TRIGGER token that follows
// CREATE/DROP to the table context when it is the last token.
func createOrDropTableCtx(tokens []string, i int) completionCtx {
...
// orderOrGroupByCtx resolves a BY token that follows ORDER/GROUP to the column
// context when it is the last token.
func orderOrGroupByCtx(tokens []string, i int) completionCtx {
