...
func TestP1CreateIfNotExists(t *testing.T) {
	...
	// A duplicate CREATE with a DIFFERENT schema should fail. (A verbatim
	// re-create of the identical schema is silently skipped for the compat
	// harness, which models TCL "db close; forcedelete; sqlite3 db" resets
	// as plain SQL — see TestDoubleCreateTable.)
	res = db.Exec("CREATE TABLE t(a INTEGER, b TEXT)")
	if res.Error == nil {
		t.Errorf("expected error for duplicate CREATE TABLE with different schema")
	}
