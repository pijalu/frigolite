...
func TestP1InsertValues(t *testing.T) {
	...
	t.Run("column count mismatch", func(t *testing.T) {
		...
		if res.Error == nil || !strings.Contains(res.Error.Error(), "has 2 columns but 3 values were supplied") {
		...
		if res.Error == nil || !strings.Contains(res.Error.Error(), "3 values for 2 columns") {
...
// TestP1InsertColumnCaseInsensitive covers SQLite's case-insensitive column
// name matching in INSERT column lists: a column list that differs in case
// from the table definition must map to the defined column (where3-1.2).
func TestP1InsertColumnCaseInsensitive(t *testing.T) {
	db := setupDB(t)
	defer db.Close()
	if res := db.Exec("CREATE TABLE t(A INTEGER, B TEXT, c REAL)"); res.Error != nil {
		t.Fatalf("create: %v", res.Error)
	}
	// Column list lowercases / changes case vs the definition.
	if res := db.Exec("INSERT INTO t(a, b) VALUES(1, 'x')"); res.Error != nil {
		t.Fatalf("insert lowercase: %v", res.Error)
	}
	if res := db.Exec("INSERT INTO t(A, C) VALUES(2, 3.5)"); res.Error != nil {
		t.Fatalf("insert uppercase: %v", res.Error)
	}
	r := db.Query("SELECT A, B, C FROM t ORDER BY A")
	if r.Error != nil {
		t.Fatalf("select: %v", r.Error)
	}
	if len(r.Rows) != 2 {
	...
	if r.Rows[0][0] != int64(1) || r.Rows[0][1] != "x" || r.Rows[0][2] != nil {
		t.Errorf("row1 unexpected: %v", r.Rows[0])
	}
	if r.Rows[1][0] != int64(2) || r.Rows[1][1] != nil || r.Rows[1][2] != 3.5 {
		t.Errorf("row2 unexpected: %v", r.Rows[1])
	}
}
