...
func TestP1TypesInsertCoercion(t *testing.T) {
	...
	want := [][]string{{"5", "integer", "3.0", "real", "hi", "6869"}}
	if !reflect.DeepEqual(got, want) {
...
func TestP1TypesRowidFloatBoundary(t *testing.T) {
	...
	// Frigolite evaluates rowid comparisons with the full (unoptimized)
	// int-vs-float comparison, matching SQLite's behavior when the rowid seek
	// optimization is disabled (WHERE +rowid = ...). The seek optimization's
	// boundary quirk (rowid = -2^63.0 with a plain rowid predicate returns no
	// row) is a SQLite codegen artifact frigolite does not replicate.
	//
	// 2^63-1.0 and 2^63.0 as REAL both round to 2^63, which is one more than
	// the maximum rowid, so no row matches.
	...
	// -2^63-1.0 rounds to -2^63, which equals MinInt64's rowid.
