...
// TestP2SubqueryInCollation verifies the IN operator applies the subquery
// column's collation when the LHS is a non-column literal (nulls2 2.x.1).
func TestP2SubqueryInCollation(t *testing.T) {
	db, _ := Open("")
	defer db.Close()
	mustExec(t, db, `
	...
	`)
	// Scalar IN: 'three' matches the stored 'THREE' under the column's
	// NOCASE collation (a non-column LHS falls back to the RHS collation).
	rows := p2QueryRows(t, db, "SELECT 'three' IN (SELECT c FROM t1);")
	if got := flattenRows(rows); got != "1" {
		t.Errorf("scalar IN with subquery collation got %q want 1", got)
	}
	// Row-value IN: (NULL,'two','three') vs ('one','two','THREE') is UNKNOWN
	// (NULL = 'one' undecided, 'two'='two' and 'three'='THREE' under NOCASE),
	// so the IN result is NULL and IS NULL is true.
	rows = p2QueryRows(t, db, "SELECT (NULL, 'two', 'three') IN (SELECT a, b, c FROM t1) IS NULL;")
	if got := flattenRows(rows); got != "1" {
		t.Errorf("row-value IN with NULL and subquery collation got %q want 1", got)
	}
	// An exact row matches.
	rows = p2QueryRows(t, db, "SELECT ('one', 'two', 'three') IN (SELECT a, b, c FROM t1);")
	if got := flattenRows(rows); got != "1" {
		t.Errorf("row-value IN exact match with subquery collation got %q want 1", got)
	}
}
