...
// TestP3Index_PartialIndexNonDeterministicRollback verifies that a failed
// CREATE INDEX (non-deterministic function in the partial-index WHERE clause,
// e.g. date('now') on a row whose indexed value is 'now') rolls back the
// schema entry: the index name must be reusable afterward (date2-410/420).
func TestP3Index_PartialIndexNonDeterministicRollback(t *testing.T) {
	db := openTestDB(t)
	defer db.Close()

	r := db.Exec(`CREATE TABLE t4(a INTEGER PRIMARY KEY,b);
	  INSERT INTO t4(a,b) VALUES(1, julianday('2017-07-01')+1);
	  INSERT INTO t4(a,b) VALUES(500,'now');`)
	if r.Error != nil {
		t.Fatalf("setup: %v", r.Error)
	}

	// date('now') is non-deterministic when the row's b is the text 'now'.
	r = db.Exec(`CREATE INDEX t4b1 ON t4(b) WHERE date(b) BETWEEN '2017-06-01' AND '2017-08-31';`)
	if r.Error == nil || !strings.Contains(r.Error.Error(), "non-deterministic use of date() in an index") {
		t.Fatalf("410: expected non-deterministic error, got: %v", r.Error)
	}

	// The failed CREATE INDEX must not leak a schema entry: the name is
	// reusable after the offending row is deleted (date2-420).
	r = db.Exec(`DELETE FROM t4 WHERE a=500;`)
	if r.Error != nil {
		t.Fatalf("delete: %v", r.Error)
	}
	r = db.Exec(`CREATE INDEX t4b1 ON t4(b) WHERE date(b) BETWEEN '2017-06-01' AND '2017-08-31';`)
	if r.Error != nil {
		t.Fatalf("420: %v", r.Error)
	}

	// Index maintenance also rejects non-deterministic inserts (date2-430).
	r = db.Exec(`INSERT INTO t4(a,b) VALUES(9999,'now');`)
	if r.Error == nil || !strings.Contains(r.Error.Error(), "non-deterministic use of date() in an index") {
		t.Fatalf("430: expected non-deterministic error, got: %v", r.Error)
	}
}
