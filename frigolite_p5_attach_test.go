...
func TestP5AttachDatabaseList(t *testing.T) {
	...
	for _, row := range r.Rows {
		if valuesEqual(row[1], "aux") {
			found = true
			// SQLite always reserves seq 1 for the temp database, so an
			// attached database appears at seq 2 (PRAGMA database_list).
			if !valuesEqual(row[0], int64(2)) {
				t.Errorf("aux seq: got %v want 2", row[0])
			}
