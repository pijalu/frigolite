...
// TestP5PragmaPageSizeReopen verifies a database created with a non-default
// PRAGMA page_size (1024) can be closed and reopened: the pager must read the
// real page size from the file header rather than assuming the default 4096.
func TestP5PragmaPageSizeReopen(t *testing.T) {
	path := filepath.Join(t.TempDir(), "page1024.db")

	db, err := Open(path)
	if err != nil {
		t.Fatal(err)
	}
	r := db.Exec("PRAGMA auto_vacuum=OFF; PRAGMA page_size = 1024; CREATE TABLE t1(x); INSERT INTO t1 VALUES(1.1);")
	if r.Error != nil {
	...
	defer db2.Close()
	r2 := db2.Exec("SELECT * FROM t1")
	if r2.Error != nil {
		t.Fatalf("reopen SELECT: %v", r2.Error)
	}
	if len(r2.Rows) != 1 || r2.Rows[0][0] != 1.1 {
		t.Fatalf("reopen rows: %v", r2.Rows)
	}

	// A page-size-1024 file is also readable by the oracle.
