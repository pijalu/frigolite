...
// TestP5VtabGenerateSeriesEponymousOnly verifies series.c's eponymous-only
// module contract (tabfunc01-1.3, oracle-verified): the module name is usable
// directly in a FROM clause, but CREATE VIRTUAL TABLE USING generate_series
// fails with "no such module" because series.c registers without xCreate.
func TestP5VtabGenerateSeriesCreateDrop(t *testing.T) {
	...
	if res := db.Exec("CREATE VIRTUAL TABLE s USING generate_series(1,4);"); res.Error == nil ||
		!strings.Contains(res.Error.Error(), "no such module: generate_series") {
		t.Fatalf("create vtab: want 'no such module' error, got %v", res.Error)
	}

	// The module name works as an implicit table in FROM.
	r := db.Query("SELECT value FROM generate_series(1,4)")
	if r.Error != nil {
		t.Fatal(r.Error)
	}
	if len(r.Rows) != 4 || r.Rows[0][0] != int64(1) || r.Rows[3][0] != int64(4) {
		t.Errorf("SELECT FROM generate_series(1,4): got %v", r.Rows)
	}

	// PRAGMA table_info on the implicit instance reports only the visible
	// column (value); start/stop/step are HIDDEN.
	r = db.Query("PRAGMA table_info(generate_series)")
	if r.Error != nil {
		t.Fatal(r.Error)
	}
	if len(r.Rows) != 1 || r.Rows[0][1] != "value" {
		t.Errorf("table_info(generate_series): got %v want column 'value'", r.Rows)
	}

	// No schema entry is created for the eponymous instance.
	r = db.Query("SELECT name FROM sqlite_master WHERE name='generate_series'")
	if r.Error != nil {
		t.Fatal(r.Error)
	}
	if len(r.Rows) != 0 {
		t.Errorf("sqlite_master should not list eponymous module: %v", r.Rows)
	}

	// DROP of the never-created name reports no such table.
