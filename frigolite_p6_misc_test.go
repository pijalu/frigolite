...
// TestP6_AggregateInDefault covers the table package: aggregate functions in
// DEFAULT expressions are accepted at CREATE time (SQLite defers evaluation)
// and rejected with "unknown function: <name>()" when the default is actually
// evaluated (INSERT ... DEFAULT VALUES). Verified against sqlite3 3.51.
func TestP6_AggregateInDefault(t *testing.T) {
	...
	for _, ddl := range []string{
		...
		r := db.Exec(ddl)
		if r.Error != nil {
			t.Errorf("%s: expected CREATE to succeed, got: %v", ddl, r.Error)
		}
	}
	// Evaluating the default at INSERT time reports the unknown function
	// (SQLite: "unknown function: avg()").
	for _, ddl := range []string{
