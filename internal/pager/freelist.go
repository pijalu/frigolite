...
// free-page count (header[36:40]). ok=false when no header is loaded.
// Caller holds p.mu.
func (p *Pager) freelistCountLocked() (head, n uint32, ok bool) {
	if len(p.header) < 40 {
		return 0, 0, false
	}
	return binary.BigEndian.Uint32(p.header[32:36]), binary.BigEndian.Uint32(p.header[36:40]), true
}

// trunkMatches reports whether a trunk page satisfies the
// allocateBtreePage searchList target predicate: the trunk itself (exact,
// or at-or-below nearby under BTALLOC_LE).
func trunkMatches(cur, nearby uint32, le bool) bool {
	return cur == nearby || (le && cur < nearby)
}
...
func (p *Pager) allocateFreelistLocked() uint32 {
	head, n, ok := p.freelistCountLocked()
	if !ok || n == 0 {
		return 0
	}
...
func (p *Pager) allocateFreelistNearLocked(nearby uint32, le bool) uint32 {
	...
	for cur != 0 {
		...
		if trunkMatches(cur, nearby, le) {
			return p.allocTrunkTargetLocked(cur, prev, t, n)
		}
		if matched, leaf := p.allocLeafMatchLocked(t, nearby, n, le); matched {
			return leaf
		}
		prev = cur
		cur = t.next
...
// allocTrunkTargetLocked allocates the matched trunk page itself. Source:
// btree.c allocateBtreePage src/btree.c:6430-6490 (k==0 path unlinks the
// trunk; k>0 path promotes leaf[0] to a new trunk). Mirrors the C behavior
// exactly: the unlink for k==0 advances the chain to the next trunk; the
// k>0 path REPLACES the chain head with the new trunk, not with t.next
// (which would orphan the new trunk when the old one was the only trunk —
// P8.INCRVACUUM.S6 regression: the chain header was set to t.next=0 instead
// of the promoted leaf, leaving 16 free pages reachable only by a
// dead-on-disk trunk). Caller holds p.mu.
func (p *Pager) allocTrunkTargetLocked(cur, prev uint32, t freelistTrunk, n uint32) uint32 {
	p.journalPageBeforeLocked(1)
	binary.BigEndian.PutUint32(p.header[36:40], n-1)
	p.mirrorHeaderToPage1Locked()
	if len(t.leaves) == 0 {
...
// allocLeafMatchLocked pops the first leaf of t satisfying the
// allocateBtreePage searchList leaf predicate (exact, or at-or-below nearby
// under BTALLOC_LE): the count is decremented and the LAST leaf moves into
// the freed slot (memcpy(&aData[8+closest*4], &aData[4+k*4], 4)). matched
// is false when no leaf qualifies (no chain mutation). Caller holds p.mu.
func (p *Pager) allocLeafMatchLocked(t freelistTrunk, nearby, n uint32, le bool) (matched bool, pgno uint32) {
	for i, leaf := range t.leaves {
...
func (p *Pager) FreePage(pageNum uint32) error {
	...
	p.invalidateFreelistSetLocked()
	if p.freeLeafAddLocked(pageNum, nFree) {
		return nil
	}
	p.freeNewHeadTrunkLocked(pageNum)
	return nil
}

// freeLeafAddLocked adds the freed page as a leaf of the existing head trunk
// while it has room (freePage2's leaf-add branch: a head trunk exists, is
// readable, is not the page being freed, and its leaf count is below
// usableSize/4 - 8). Reports whether the page was placed there. Caller holds
// p.mu.
func (p *Pager) freeLeafAddLocked(pageNum, nFree uint32) bool {
	if nFree == 0 {
		return false
	}
	head := binary.BigEndian.Uint32(p.header[32:36])
	t, ok := p.readFreelistTrunkLocked(head)
	if !ok || head == pageNum || uint32(len(t.leaves)) >= p.maxTrunkLeaves() {
		return false
	}
...
// freeNewHeadTrunkLocked turns the freed page into the new head trunk:
// [0:4] = previous head, [4:8] = 0 leaves, header[32] = the freed page
// (freePage2's tail block). Caller holds p.mu.
func (p *Pager) freeNewHeadTrunkLocked(pageNum uint32) {
...
func (p *Pager) freelistSetLocked() map[uint32]struct{} {
	...
	if _, n, ok := p.freelistCountLocked(); ok {
		cur := binary.BigEndian.Uint32(p.header[32:36])
		out = p.buildFreelistSetLocked(cur, n)
	}
	p.freeSet = out
	return out
}

// buildFreelistSetLocked walks the on-disk chain from cur, collecting trunk
// and leaf pages into the membership set. Cycle- and count-bounded so a
// corrupt chain cannot spin the rebuild. Caller holds p.mu.
func (p *Pager) buildFreelistSetLocked(cur, n uint32) map[uint32]struct{} {
	out := make(map[uint32]struct{})
...
func (p *Pager) freelistPagesAboveLocked(n uint32) uint32 {
	_, total, ok := p.freelistCountLocked()
	if !ok {
		return 0
	}
	removed := uint32(0)
	...
	for cur != 0 {
		...
		kept, removedLeaves := partitionLeavesAbove(t.leaves, n)
		...
		if cur > n {
			...
			p.spliceTrunkAboveLocked(prev, t)
			cur = t.next
			continue
		}
...
// partitionLeavesAbove splits a trunk's leaf list at the truncation point:
// leaves above n are dropped and counted; leaves at or below n are kept in
// order (in-place filter, as the caller writes the compacted list back).
func partitionLeavesAbove(leaves []uint32, n uint32) (kept []uint32, removed uint32) {
	kept = leaves[:0]
...
// spliceTrunkAboveLocked unlinks the truncated trunk t from the chain: the
// previous link (header[32] when prev==0, else prev trunk's next) is pointed
// at t.next. Caller holds p.mu.
func (p *Pager) spliceTrunkAboveLocked(prev uint32, t freelistTrunk) {
	if prev == 0 {
		p.journalPageBeforeLocked(1)
		binary.BigEndian.PutUint32(p.header[32:36], t.next)
		p.mirrorHeaderToPage1Locked()
