...
func (p *Parser) yyFindShiftAction(lookahead int, stateNo ParserAction) ParserAction {
	...
	idx := p.shiftLookupIndex(stateNo, lookahead)
	if idx >= 0 && t.Lookahead[idx] == lookahead {
		if idx < len(t.Action) {
			return t.Action[idx]
		}
		return t.Default[stateNo]
	}
	// Lookahead mismatch: apply fallback-token substitution exactly like
	// SQLite's yy_find_shift_action (unconditional), then wildcard matching.
	if idx >= 0 {
		if act, ok := p.shiftAlternate(lookahead, stateNo, idx); ok {
			return act
		}
	}
	return t.Default[stateNo]
...
// shiftLookupIndex returns the action-table index for a terminal lookahead
// in the given state, or -1 when the state has no shift entry for it.
func (p *Parser) shiftLookupIndex(stateNo ParserAction, lookahead int) int {
	t := p.tables
...
// shiftAlternate resolves a non-matching lookahead via fallback-token
// substitution, then wildcard matching, returning the resolved action.
func (p *Parser) shiftAlternate(lookahead int, stateNo ParserAction, idx int) (ParserAction, bool) {
	t := p.tables
...
func (p *Parser) Parse(yymajor int, yyminor interface{}) ParseResult {
	...
	//lint:ignore SA4006 standard lemon state-machine loop: yyact is a
	// scratch register; the loop recomputes it from the stack top each pass.
	...
	for {
		...
		if yyact >= ParserAction(t.YYMinReduce) {
			...
			//lint:ignore SA4006 standard lemon state-machine loop: the
			// reduce's effect is the stack mutation; the next pass recomputes
