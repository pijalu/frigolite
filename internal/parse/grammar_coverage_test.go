...
func TestGrammarCoverage(t *testing.T) {
	...
	for i, sqlStr := range grammarCoverageCorpus {
		...
		tok := sql.NewTokenizer(strings.ReplaceAll(strings.ReplaceAll(strings.ReplaceAll(strings.ReplaceAll(sqlStr, "GENERATED ALWAYS", ""), "generated always", ""), ") VIRTUAL", ")"), ") STORED", ")"))
