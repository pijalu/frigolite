...
type CTEDef struct {
	...
	// ScopeDepth records the enclosing-WITH scope index at which this CTE
	// was defined (set at execution time, not parse time). A CTE body must
	// resolve names against the scopes up to and including its defining
	// scope — deeper scopes from the reference site must not leak in
	// (with1 17.5: x2 defined in the outer WITH must see the outer x1, not
	// a same-named x1 shadowed inside x3's nested WITH).
	ScopeDepth int
}
...
type WindowDef struct {
	...
	BaseName   string        // referenced base window name (nm ... in OVER/windowdefn)
	...
	Frame      *WindowFrame  // structured frame spec (nil if none)
}

// WindowFrame is the structured form of a window frame clause
// (ROWS/RANGE/GROUPS BETWEEN <start> AND <end> [EXCLUDE ...]).
type WindowFrame struct {
	Type    string     // "ROWS", "RANGE", "GROUPS", or "" (no explicit frame)
	Between bool       // true when the BETWEEN ... AND ... form is used
	Start   FrameBound // start boundary
	End     FrameBound // end boundary (valid only when Between)
	Exclude string     // "NO OTHERS", "CURRENT ROW", "GROUP", "TIES", or ""
}

// FrameBound is one window frame boundary.
type FrameBound struct {
	Kind string // "UNBOUNDED PRECEDING", "CURRENT ROW", "UNBOUNDED FOLLOWING", "PRECEDING", "FOLLOWING"
	Expr Expr   // offset expression for PRECEDING/FOLLOWING bounds (nil for the fixed kinds)
}
...
func (w *WindowDef) String() string {
	...
	if len(w.Partitions) > 0 {
		result += "PARTITION BY "
		result += strings.Join(formatExprListParts(w.Partitions), ", ")
	}
	...
	if len(w.OrderBy) > 0 {
		...
		result += "ORDER BY "
		result += strings.Join(formatOrderByParts(w.OrderBy), ", ")
	}
	result += ")"
	return result
}
...
type TableRef struct {
	...
	// IsTabFunc marks FROM terms written with function-call syntax
	// ("FROM name(...)"), including a zero-argument call. SQLite's resolver
	// raises "'name' is not a function" when such a term resolves to an
	// ordinary table, view or CTE instead of a table-valued module
	// (tabfunc01-1.21).
	IsTabFunc bool
...
type InsertStmt struct {
	Table        string
	Alias        string // optional table alias (INSERT INTO t AS alias)
	Columns      []string
...
type OnConflictClause struct {
	ConflictColumn []string // optional conflict target columns (order-insensitive set)
	TargetExpr     []string // raw text of each target term (expression targets; empty for plain columns)
	TargetWhere    Expr     // optional conflict-target WHERE (partial-index predicate)
	Action         ConflictAction
	Assignments    []Assignment      // for DO UPDATE SET
	Where          Expr              // optional WHERE for DO UPDATE
	Next           *OnConflictClause // next chained ON CONFLICT clause in statement order
}
...
type UpdateStmt struct {
	Table           string
	Alias           string // optional table alias (UPDATE t AS alias SET ...)
	...
	FromJoins       []JoinClause // JOINed tables in UPDATE ... FROM (with type/ON)
	Where           Expr
	OrderBy         []OrderByTerm
	Limit           Expr
	Offset          Expr
	...
	HasReturning    bool
	RawSQL          string // original statement text (for post-parse fixups)
	// IndexedBy is the INDEXED BY index name, "NOT INDEXED" for the NOT
	// INDEXED clause, or "" when absent. SQLite rejects both forms inside
	// trigger bodies.
	IndexedBy string
}
...
type DeleteStmt struct {
	Table        string
	Alias        string // optional table alias (DELETE FROM t AS alias WHERE ...)
	Where        Expr
	OrderBy      []OrderByTerm
	Limit        Expr
	Offset       Expr
	...
	// IndexedBy is the INDEXED BY index name, "NOT INDEXED" for the NOT
	// INDEXED clause, or "" when absent. SQLite rejects both forms inside
	// trigger bodies ("the INDEXED BY / NOT INDEXED clause is not allowed
	// on UPDATE or DELETE statements within triggers").
	IndexedBy string
}
...
type CreateViewStmt struct {
	...
	// IfNotExists is set for "CREATE VIEW IF NOT EXISTS ...".
	IfNotExists bool
...
type AttachStmt struct {
	...
	Schema     string
	SchemaExpr Expr // expression for the schema name (DETACH argument)
