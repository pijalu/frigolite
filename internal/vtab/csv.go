...
func csvApplySchema(v *csvVTab, schema string) error {
	names, types, err := columnNamesAndTypesFromSchema(schema)
	if err != nil {
		return err
	}
	v.columns = names
...
// Every field is stored as TEXT: csv.c's xFilter returns each field with
// sqlite3_result_text (csv.c:780) — no numeric type inference. Numeric
// predicates (WHERE a=9) still match through the declared column affinity
...
func columnNamesFromSchema(schema string) ([]string, error) {
	names, _, err := columnNamesAndTypesFromSchema(schema)
	return names, err
}

// columnNamesAndTypesFromSchema extracts column names and their declared
// types from a "CREATE TABLE x(a INT, b TEXT)" style schema argument: the
// identifier before the first space of each top-level comma-separated term
// is the name and the remainder is the declared type (csv.c hands the
// schema to sqlite3_declare_vtab verbatim).
func columnNamesAndTypesFromSchema(schema string) ([]string, []string, error) {
	open := strings.Index(schema, "(")
...
// ColumnTypes implements ColumnTypeInfo. A schema= argument declares types
// verbatim (csv.c passes the schema straight to sqlite3_declare_vtab);
// generated declarations (header/columns naming) append " TEXT" to every
// column (csv.c:582/589/598).
func (v *csvVTab) ColumnTypes() []string {
	if len(v.colTypes) == len(v.columns) {
		return v.colTypes
	}
