# TCL Research — Tier 1 failing packages (source of truth: /Users/muaddib/dev/sqlite)

**Date**: 2026-08-01
**Scope**: For each of the 31 failing Tier 1 testgen packages, locate the TCL
source in `/Users/muaddib/dev/sqlite/test/`, explain the TCL mechanics and the
expected semantics, and classify the failure as **engine bug** vs **harness
(tcl2go transpiler) bug** vs **both**. Each entry gives the file:line, the test
name, the failing assertion, and a recommended fix location.

> The verified engine reality (this session, `go test -tags testgen ...`):
> failures reproduced individually. Root-suite regression list is identical to
> HEAD — no pending-batch regressions.

---

## 0. TCL test framework → generated Go test mapping
...
3. **whereG** — `/Users/muaddib/dev/sqlite/test/whereG.test:76,91,106`
   ...
   braces from expected values (like the existing `normalizeExpectedWord` for
   whitespace; needs a brace-unwrapping step).
...
9. **strict** — `/Users/muaddib/dev/sqlite/test/strict1.test:21-36`
...
10. **select7** — `/Users/muaddib/dev/sqlite/test/select7.test:167-177` (6.6/6.7)
...
11. **intpkey** — `/Users/muaddib/dev/sqlite/test/intpkey.test:646-668` (18.x)
...
12. **istrue** — `/Users/muaddib/dev/sqlite/test/istrue.test:80-97,150-155` (500/510/700)
    ...
    `IS NOT TRUE`/`IS NOT FALSE` operators (they are TRUE when x is 1/any
    non-zero, FALSE only for 0, NULL stays NULL).

### C. ENGINE bugs — expressions / values / CAST

13. **cse** — `/Users/muaddib/dev/sqlite/test/cse.test:257-300` (3.2-3.5)
...
14. **numcast (engine part)** — `/Users/muaddib/dev/sqlite/test/numcast.test:27-35`
...
15. **whereM** — `/Users/muaddib/dev/sqlite/test/whereM.test:19-105` (1.0-1.5)
...
16. **affinity** — `/Users/muaddib/dev/sqlite/test/affinity3.test:97-118` (200-260)
...
17. **values** — `/Users/muaddib/dev/sqlite/test/values.test` (engine part)
...
18. **cast** — `/Users/muaddib/dev/sqlite/test/cast.test:516-526` (10.1-10.4)
...
19. **returning** — `/Users/muaddib/dev/sqlite/test/returning1.test:21-27` (1.0/1.1)
...
20. **select1** — `/Users/muaddib/dev/sqlite/test/select1.test:1206-1212` (21.1)
...
21. **selectD** — `/Users/muaddib/dev/sqlite/test/selectD.test:28-104`
...
22. **whereE** — `/Users/muaddib/dev/sqlite/test/whereE.test:63-68` (1.3/1.4)
...
23. **whereH** — `/Users/muaddib/dev/sqlite/test/whereH.test` (lines ~74-122)
...
24. **whereF** — `/Users/muaddib/dev/sqlite/test/whereF.test:303-310` (7.3)
...
25. **whereD** — `/Users/muaddib/dev/sqlite/test/whereD.test:34-52` (1.2-1.9)
...
26. **whereI** — `/Users/muaddib/dev/sqlite/test/whereI.test:96` (3.0)
...
27. **where** — `/Users/muaddib/dev/sqlite/test/where2.test:461-468` (6.14.1)
...
28. **whereL** — `/Users/muaddib/dev/sqlite/test/whereL.test:83-88` (200/201)
...
### F. ENGINE bugs — DELETE / triggers

29. **delete3** — `/Users/muaddib/dev/sqlite/test/delete3.test:21-48` (1.1)
    ...
    loop / hang** in `internal/exec/insert.go:847 hasConflictAt` →
    `scanForConflict` → `findRowByUniqueCols` (stack trace in the timeout
    dump). The INTEGER PRIMARY KEY conflict check on every INSERT...SELECT
    row scans and never terminates (or is O(n²) catastrophic) once the table
    grows. **Fix (engine)**: for INTEGER PRIMARY KEY, check rowid conflict
    via direct lookup (or skip the unique scan when the PK is the rowid);
    ensure the loop terminates.

30. **delete_pkg** — `/Users/muaddib/dev/sqlite/test/delete.test:436-442` (12.0)
...
31. **values (trigger RAISE)** — `/Users/muaddib/dev/sqlite/test/values.test`
...
2. **Custom test-only SQL functions**: `test_getsubtype` (test_func.c:623),
   `intreal` (test1.c:981), `udf` (proc in selectC.test), TCL `int()/log()` in
...
4. **Multi-row VALUES compound** is broken in every context (INSERT, CREATE
   ...
   (10.1-10.4), and parts of select1/selectC. This is a single root cause:
   the engine's VALUES compound only keeps one tuple (first or last).
...
## 3. RECOMMENDED FIX ORDER (highest value / lowest risk first)

1. **tcl2go `$var`-in-SQL transpilation** (harness) → unblocks numcast parse
   errors, values 1.2.5/1.2.6, between's `tclExprWith` fallback. Smallest
   surface, broad impact.
2. **cse bool→0/1** (engine value model) — tiny, isolated.
3. **whereG braces in expected values** (harness normalize) — engine already
   correct.
4. **delete3 infinite loop** (insert.go unique-conflict on INTEGER PRIMARY KEY)
   — crash-class; reproduce with
   `go test -tags testgen ./testgen/delete3/ -count=1 -timeout 300s`.
5. **multi-row VALUES compound** (values/cast/select1/selectC) — single engine
   root cause with wide blast radius.
6. **RETURNING execution** (returning) — parse exists, exec missing.
7. **custom functions** (subtype/intreal/udf/implies_nonnull_row + evalSimpleArith)
   — harness + one engine function.
8. **STRICT datatype enforcement** (strict) — DDL-only.
9. **DQS off for double-quoted strings** (select7) — parser.
10. **cast REAL/NUMERIC numeric-prefix parsing** (numcast engine part).
11. **EQP planner**: automatic index (whereE), temp b-tree (whereH), opcodes
    (whereF), ATTACH (selectD).
12. **OR-optimization row order** (whereD, whereI, where2 NOCASE).
13. **affinity**: column affinity on INSERT + comparisons (whereM, affinity3,
    whereL collation).
14. **ON-clause view-column resolution** (select1, whereL).
15. **intpkey rowid extremes** (MinInt64/MaxInt64).
16. **DELETE WHERE scalar subquery with LIMIT/OFFSET** (delete_pkg).

## 4. Verify loop (per fix)

```bash
go test -tags testgen ./testgen/<pkg>/ -count=1
# regression neighbors (the 27 currently green):
