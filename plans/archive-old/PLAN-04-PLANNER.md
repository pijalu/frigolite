...
| File | Change |
|------|--------|
| `internal/exec/engine.go` | Real ANALYZE computeIndexStat; parseIndexColumns strips ASC/DESC; cost-based index selection via statLookup+estimateCostUsingStats; DROP TABLE cascades to indexes; EXPLAIN QUERY PLAN output with SEARCH/SCAN |
| `frigolite_harness_test.go` | Support `~/.../` regex patterns; handle TCL glob-style expects |

## P4 Implementation Summary

### Completed
1. **ANALYZE statistics computation** (Step 1) — `computeIndexStat` counts distinct prefix values from index b-tree, stores in `sqlite_stat1` with format "N D1 D2 ..."
2. **Cost-based index selection using ANALYZE stats** (Step 2 partial) — `estimateCostUsingStats` uses stat data for equality constraints (`col = value`). Range operators (`<`, `>`, `BETWEEN`) fall back to heuristic.
3. **EXPLAIN QUERY PLAN output** (Step 3) — Correctly outputs SCAN/SEARCH USING INDEX with constraint conditions
4. **parseIndexColumns fix** — Strips ASC/DESC qualifiers from index column names; handles COLLATE clauses
5. **DROP TABLE cascade** — Removes associated indexes when a table is dropped
6. **statLookup fix** — Uses `util.UnwrapColumnValue` to correctly read values from `buildRowMap`
7. **Auto-ANALYZE infrastructure** — `InitStatTable` creates `sqlite_stat1` on database open; engine reads stats for planning

### Remaining
- **Histogram data (sqlite_stat4)** — Range selectivity estimation needs actual min/max values or histogram for accurate cost model
- **Reverse index scan cost penalty** — DESC indexes have higher cost for `<` range scans
- **Multi-column index cost model** — Better handling of equality-on-first-column + range-on-second-column combinations
- **JOIN planning** — Nested loop vs hash join selection (P5 scope)
- Test data conversion artifacts in JSON test files (TCL glob patterns not fully portable to Go regexp)

## Completion Check

Current state:
- `analyzeE` — 13 → **0 failures** (all fixed: DESC index parsing, stat lookup, cost model)
- `analyze7` — 15 failures (test data conversion: `/*...*/` patterns are TCL globs, not valid Go regexps)
- `analyzeC` — ~5 failures (mostly pattern format issues)
- `autoanalyze1`, `analyze6`, `analyze8`, `analyzeD` — pre-existing failures outside planner scope

All test suites (hand-written, exec, btree, storage, util) pass. SOLID architecture tests pass. Build passes. Staticcheck clean.
