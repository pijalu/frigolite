...
func Test_aggorderby(t *testing.T) {
	...
	{ // "aggorderby-7.1" — skipped: json_group_array (JSON1 extension) not supported (SQL side effects only)
	...
	{ // "aggorderby-9.1" — skipped: json_group_array (JSON1 extension) not supported (SQL side effects only)
		_res = db.Exec("\n  WITH c(x,y) AS (VALUES\n    ('[4,4]', 4),\n    ('{a:3}', 3),\n    ('[4,4]', 4),\n    ('[1,1]', 1),\n    ('[4,4]', 4),\n    ('{x:2}', 2))\n  SELECT json_group_array(DISTINCT json(x) ORDER BY y) FROM c;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "aggorderby-9.2" — skipped: json_group_array (JSON1 extension) not supported (SQL side effects only)
		_res = db.Exec("\n  WITH c(x,y) AS (VALUES\n    ('{a:3}', 3),\n    ('[1,1]', 1),\n    ('[4,4]', 4),\n    ('{x:2}', 2))\n  SELECT json_group_array(json(x) ORDER BY json(x)) FROM c;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "aggorderby-9.3" — skipped: json_group_array (JSON1 extension) not supported (SQL side effects only)
