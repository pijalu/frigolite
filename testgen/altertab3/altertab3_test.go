...
func Test_altertab3(t *testing.T) {
	...
	vtab.TclVarSet("testprefix", "", "altertab3")
	...
	{ // "7.1.2"
		...
		got := flatten(r)
		want := "CREATE TABLE \"t1x\"(a,b,c) CREATE TRIGGER AFTER INSERT ON \"t1x\" BEGIN\n    SELECT a, rank() OVER w1 FROM \"t1x\"\n    WINDOW w1 AS (PARTITION BY b, percent_rank() OVER w1);\n  END"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	...
	for _idx0 := 0; _idx0+3 <= len(_items0); _idx0 += 3 {
		...
		_ = _idx0
			{ // "altertab3-19." + tn + ".1" — skipped: window function (GROUPS frame) rename validation not supported (SQL side effects only)
		...
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		{ // "altertab3-24.1" — skipped: JOIN USING column rename validation not supported (SQL side effects only)
		...
		{ // "altertab3-24.3" — skipped: JOIN USING column rename validation not supported (SQL side effects only)
			_res = db.Exec("\n  DROP TRIGGER x;\n  CREATE TRIGGER x AFTER INSERT ON v2 WHEN (\n    0 AND (SELECT rowid FROM v0)\n  ) BEGIN\n    DELETE FROM v2;\n  END;\n")
			_ = _res.Error // tolerate unsupported-feature errors in skipped tests
		}
		{ // "altertab3-24.4" — skipped: JOIN USING column rename validation not supported (SQL side effects only)
		...
		{ // "29.7"
			...
			got := flatten(r)
			want := "CREATE TRIGGER Trigger1 DELETE ON \"t3\" \n  BEGIN \n    SELECT \"t3\".*, \"t3\".z FROM \"t3\" ORDER BY \"t3\".z;\n  END CREATE TRIGGER tr2 AFTER DELETE ON \"t3\" BEGIN\n    SELECT z, y FROM (\n      SELECT \"t3\".* FROM \"t3\"\n    );\n  END"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
