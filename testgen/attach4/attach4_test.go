	...
	{ // do_test "1.2.1"
		...
		for _idx0 := 0; _idx0+2 <= len(_items0); _idx0 += 2 {
			...
			for _idx1 := 0; _idx1+2 <= len(_items1); _idx1 += 2 {
				...
				got := tclListFlatten(L)
				want := tclListFlatten(files)
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "1.2.1")
				}
			}
			{ // "1.2.2"
				_res = db.Exec("\n  ATTACH 'x.db' AS next;\n")
			...
			{ // do_test "1.3"
				...
				{ // do_test "1.4"
					...
					for _idx8 := 0; _idx8+2 <= len(_items8); _idx8 += 2 {
						...
						_ = _idx8
							if "" == "journaltest" {
								vtab.TclVarSet("mode", "", "delete")
								mode = "delete"
								_ = mode // suppress unused warning
							} else {
								vtab.TclVarSet("mode", "", "wal")
								mode = "wal"
								_ = mode // suppress unused warning
							}
							...
							{ // do_test "1.7"
								...
								{ // do_test "1.8"
									...
									for _idx11 := 0; _idx11+2 <= len(_items11); _idx11 += 2 {
										...
										want := tclListFlatten(files)
										if got != want {
											t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "1.8")
										}
									}
									db.Close()
									// foreach {name f} files
									_items12 := tclSplitList(files)
									for _idx12 := 0; _idx12+2 <= len(_items12); _idx12 += 2 {
										name := _items12[_idx12+0]
										_ = name // suppress unused warning
										f := _items12[_idx12+1]
										_ = f // suppress unused warning
										_ = _idx12
