...
func Test_capi3d(t *testing.T) {
	...
	// proc definition (not transpiled)
	// db func eval evalsql (db-eval passthrough — built-in eval used)
	...
	{ // "capi3d-4.2.1" (prepare-step internals; SQL side effects only)
		_r = tclPrepareStmt(db, "s1", "ROLLBACK", -1)
		// prepared s1: ROLLBACK (bind/step emulation)
		_ = s1 // prepared statement handle
		_r = tclStepStmt(db, "s1")
	}
