...
func Test_capi3e(t *testing.T) {
	...
	// proc definition (not transpiled)
	db.Close()
	vtab.TclVarSet("names", "", "t 1 t. 1. t.d 1.d t-1 1-1 t.db ä.db ë.db ö.db ü.db ÿ.db")
	...
	_ = name // suppress unused warning
		...
		{ // do_test "capi3e-1.1." + i
			...
			_r = db2.LastErrCode()
			if _r != "SQLITE_OK" {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", _r, "SQLITE_OK", "capi3e-1.1." + i)
			}
		}
		{ // do_test "capi3e-1.2." + i
			_r = tclCloseDB(db2)
			if _r != "SQLITE_OK" {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", _r, "SQLITE_OK", "capi3e-1.2." + i)
			}
		}
