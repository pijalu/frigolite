// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package corruptA

import (
"github.com/pijalu/frigolite"
"github.com/pijalu/frigolite/internal/vtab"
"os"
"strconv"
"testing"
)

func Test_corruptA(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var unreadable_version string
	_ = unreadable_version // pre-declared from TCL source
	var Id_ string
	_ = Id_ // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	// do_not_use_codec (unsupported command, not transpiled)
	// database_may_be_corrupt (unsupported command, not transpiled)
	{ // do_test "corruptA-1.1"
		_res = db.Exec("\n    CREATE TABLE t1(x);\n    INSERT INTO t1(x) VALUES(1);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TABLE t1(x);\n    INSERT INTO t1(x) VALUES(1);\n  ")
		}
		// expr [file size test.db]>=1024 → runtime compare
		_r = tclBool01(toInt(strconv.Itoa(tclFileSize("test.db")))  >=  1024)
	}
	_res = db.Exec("PRAGMA integrity_check")
	if _res.Error != nil { t.Errorf("integrity check: %v", _res.Error) }
	db.Close()
	tclFileCopy("test.db", "test.db-template")
	vtab.TclVarSet("unreadable_version", "", "02")
	unreadable_version = "02"
	_ = unreadable_version // suppress unused warning
	vtab.TclVarSet("unreadable_version", "", "03")
	unreadable_version = "03"
	_ = unreadable_version // suppress unused warning
	{ // do_test "corruptA-2.1"
		tclFileCopy("test.db-template", "test.db")
		tclHexioWrite("test.db", int64(19), unreadable_version)
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		_res = db.Exec("SELECT * FROM t1")
		_ = _res // catchsql
	}
	{ // do_test "corruptA-2.2"
		db.Close()
		tclFileCopy("test.db-template", "test.db")
		tclHexioWrite("test.db", int64(21), "41")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		_res = db.Exec("SELECT * FROM t1")
		_ = _res // catchsql
	}
	{ // do_test "corruptA-2.3"
		db.Close()
		tclFileCopy("test.db-template", "test.db")
		tclHexioWrite("test.db", int64(22), "1f")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		_res = db.Exec("SELECT * FROM t1")
		_ = _res // catchsql
	}
	{ // do_test "corruptA-2.4"
		db.Close()
		tclFileCopy("test.db-template", "test.db")
		tclHexioWrite("test.db", int64(23), "21")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		_res = db.Exec("SELECT * FROM t1")
		_ = _res // catchsql
	}
}
