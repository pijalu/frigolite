...
func Test_corruptI(t *testing.T) {
	...
	vtab.TclVarSet("testprefix", "", "corruptI")
	testprefix = "corruptI"
	...
	{ // do_test "2.0"
		...
		got := tclListFlatten(pg)
		want := tclListFlatten("5")
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "2.0")
		}
	}
	{ // do_test "2.1"
		db.Close()
	...
	{ // do_test "5.0"
		...
		for true {
			...
			if func() bool { r := db.Query("PRAGMA page_count"); if r.Error != nil || len(r.Rows) == 0 || len(r.Rows[0]) == 0 { return false }; l, err := strconv.ParseFloat(tclRenderCell(r.Rows[0][0]), 64); if err != nil { return false }; rr, rerr := strconv.ParseFloat("$i", 64); if rerr != nil { return false }; return l > rr }() {
				break
			}
			// incr i 1
			{
	...
	{ // do_test "5.2"
		db.Close()
		tclHexioWrite("test.db", int64(512*(toInt(nPage)-1)), tclFormat("%.8X%.8X%.8X", "0", "1", tclExprWith("$nPage+1", map[string]string{"nPage": nPage})))
	}
	{ // do_test "5.3"
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
