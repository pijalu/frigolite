...
func Test_cost(t *testing.T) {
	...
	vtab.TclVarSet("testprefix", "", "cost")
	testprefix = "cost"
	...
	for _idx0 := 0; _idx0+3 <= len(_items0); _idx0 += 3 {
		...
		_ = _idx0
			...
			vtab.TclVarSet("sql1", "", "SELECT * FROM t1 WHERE likelihood(k=?, " + p1 + ") AND " + w)
			sql1 = "SELECT * FROM t1 WHERE likelihood(k=?, " + p1 + ") AND " + w
			_ = sql1 // suppress unused warning
			vtab.TclVarSet("sql2", "", "SELECT * FROM t1 WHERE likelihood(k=?, " + p2 + ") AND " + w)
