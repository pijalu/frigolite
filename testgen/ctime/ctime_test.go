...
func Test_ctime(t *testing.T) {
	...
	for _idx0 := 0; _idx0+3 <= len(_items0); _idx0 += 3 {
		...
		_ = opt // suppress unused warning
			{ // do_test "ctime-2.5." + tc
				N = tclExprWith("$tc-1", map[string]string{"tc": tc})
	...
	_ = msg // suppress unused warning
		...
		{ // do_test "ctime-3.0.1"
			...
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "PRAGMA compile_options")
			}
			found := false
			for _, _row := range r.Rows {
				for _, _cell := range _row {
					if fmt.Sprint(_cell) == "DIRECT_OVERFLOW_READ" {
						found = true
					}
				}
			}
			want := tclBool(res)
			if found != want {
				t.Errorf("result mismatch\n  got:  [%v]\n  want: [%v]\n  body: do_test %s", found, want, "ctime-3.0.1")
			}
		}
}
