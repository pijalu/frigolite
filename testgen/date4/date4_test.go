// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package date4

import (
"github.com/pijalu/frigolite"
"github.com/pijalu/frigolite/internal/vtab"
"os"
"strconv"
"testing"
)

func Test_date4(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var FMT string
	_ = FMT // pre-declared from TCL source
	var i string
	_ = i // pre-declared from TCL source
	var TS string
	_ = TS // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	if tcl_platform_os == "Linux" {
		if "" == tclStrftime("%P", "1") {
			vtab.TclVarSet("FMT", "", "%d,%e,%F,%H,%I,%j,%m,%M,%u,%w,%W,%Y,%%,%p,%U,%V,%G,%g")
			FMT = "%d,%e,%F,%H,%I,%j,%m,%M,%u,%w,%W,%Y,%%,%p,%U,%V,%G,%g"
			_ = FMT // suppress unused warning
		} else {
			vtab.TclVarSet("FMT", "", "%d,%e,%F,%H,%k,%I,%l,%j,%m,%M,%u,%w,%W,%Y,%%,%P,%p,%U,%V,%G,%g")
			FMT = "%d,%e,%F,%H,%k,%I,%l,%j,%m,%M,%u,%w,%W,%Y,%%,%P,%p,%U,%V,%G,%g"
			_ = FMT // suppress unused warning
		}
	} else {
		vtab.TclVarSet("FMT", "", "%d,%e,%F,%H,%I,%j,%p,%R,%u,%w,%W,%%")
		FMT = "%d,%e,%F,%H,%I,%j,%p,%R,%u,%w,%W,%%"
		_ = FMT // suppress unused warning
	}
	vtab.TclVarSet("i", "", "0")
	i = "0"
	_ = i // suppress unused warning
	for func() bool { i_n, _i_e := strconv.Atoi(i); if _i_e != nil { return false }; return i_n <= 24858 }() {
		TS = tclExprWith("$i*86390", map[string]string{"i": i})
		_ = TS // suppress unused warning
		{ // "date4-" + i
			r = db.Query("\n    SELECT strftime(" + sqlLiteral(FMT) + "," + sqlLiteral(TS) + ",'unixepoch');\n  ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT strftime(" + sqlLiteral(FMT) + "," + sqlLiteral(TS) + ",'unixepoch');\n  ")
				return
			}
			got := flatten(r)
			want := tclStrftime(FMT, TS)
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		// incr i 1
		{
			_n, _err := strconv.Atoi(i)
			if _err == nil {
				i = strconv.Itoa(_n + 1)
			}
		}
	}
}
