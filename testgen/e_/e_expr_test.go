...
func Test_e_expr(t *testing.T) {
	...
	// proc definition (not transpiled)
	// proc definition (not transpiled)
	for _, op1 := range []string{oplist} {
		for _, op2 := range []string{oplist} {
	...
	for _, op := range []string{oplist} {
		if op == "AND" || op == "OR" {
			continue
		}
		// foreach {n1 rhs} literals
		_items := []string{literals}
		for _idx := 0; _idx+2 <= len(_items); _idx += 2 {
		n1 := _items[_idx+0]
		rhs := _items[_idx+1]
	...
	expr := _items[_idx+1]
		...
		if tclBool("*EXPR* $expr") {
			...
			for _, el := range []string{elist} {
				for _, e := range []string{" cname \"34+22\" "} {
