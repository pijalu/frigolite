...
func Test_e_fkey(t *testing.T) {
	...
	t.Skipf("TODO: %s not implemented in frigolite", "drop_all_tables")
	for _, fk := range []string{"list OFF ON"} {
	...
	_items := []string{"list 2 c1 3 c2 4 c3"}
	for _idx := 0; _idx+2 <= len(_items); _idx += 2 {
	tn := _items[_idx+0]
	c := _items[_idx+1]
	...
	{ // do_test "e_fkey-25.5"
		_r := tclList(append([]string{}, tclSplitList("execsql { SELECT rowid FROM track WHERE trackartist = 5 }")..., tclSplitList("catchsql { DELETE FROM artist WHERE artistid = 5 }")...))
		_ = _r
	}
	{ // do_test "e_fkey-25.6"
		_r := tclList(append([]string{}, tclSplitList("execsql { SELECT rowid FROM track WHERE trackartist = 7 }")..., tclSplitList("catchsql { DELETE FROM artist WHERE artistid = 7 }")...))
		_ = _r
	}
	{ // do_test "e_fkey-25.7"
	...
	for _, zMatch := range []string{"list SIMPLE PARTIAL FULL Simple parTIAL FuLL"} {
		t.Skipf("TODO: %s not implemented in frigolite", "drop_all_tables")
	...
	for _, recursive_triggers_setting := range []string{"list 0 1 ON OFF"} {
		t.Skipf("TODO: %s not implemented in frigolite", "drop_all_tables")
