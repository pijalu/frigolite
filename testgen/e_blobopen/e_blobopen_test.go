// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package e_blobopen

import (
"github.com/pijalu/frigolite"
"github.com/pijalu/frigolite/internal/vtab"
"os"
"strconv"
"testing"
)

func Test_e_blobopen(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	var incrblob_1 *frigolite.Blob
	_ = incrblob_1
	var incrblob_2 *frigolite.Blob
	_ = incrblob_2
	var incrblob_3 *frigolite.Blob
	_ = incrblob_3
	var incrblob_4 *frigolite.Blob
	_ = incrblob_4
	var incrblob_5 *frigolite.Blob
	_ = incrblob_5
	var incrblob_6 *frigolite.Blob
	_ = incrblob_6
	var incrblob_7 *frigolite.Blob
	_ = incrblob_7
	var incrblob_8 *frigolite.Blob
	_ = incrblob_8
	var incrblob_9 *frigolite.Blob
	_ = incrblob_9
	var incrblob_10 *frigolite.Blob
	_ = incrblob_10
	var incrblob_11 *frigolite.Blob
	_ = incrblob_11
	var incrblob_12 *frigolite.Blob
	_ = incrblob_12
	var incrblob_13 *frigolite.Blob
	_ = incrblob_13
	var incrblob_14 *frigolite.Blob
	_ = incrblob_14
	var incrblob_15 *frigolite.Blob
	_ = incrblob_15
	var incrblob_16 *frigolite.Blob
	_ = incrblob_16
	var incrblob_17 *frigolite.Blob
	_ = incrblob_17
	var incrblob_18 *frigolite.Blob
	_ = incrblob_18
	var incrblob_19 *frigolite.Blob
	_ = incrblob_19
	var incrblob_20 *frigolite.Blob
	_ = incrblob_20
	var incrblob_21 *frigolite.Blob
	_ = incrblob_21
	var incrblob_22 *frigolite.Blob
	_ = incrblob_22
	var incrblob_23 *frigolite.Blob
	_ = incrblob_23
	var incrblob_24 *frigolite.Blob
	_ = incrblob_24
	var incrblob_25 *frigolite.Blob
	_ = incrblob_25
	var incrblob_26 *frigolite.Blob
	_ = incrblob_26
	var incrblob_27 *frigolite.Blob
	_ = incrblob_27
	var incrblob_28 *frigolite.Blob
	_ = incrblob_28
	var incrblob_29 *frigolite.Blob
	_ = incrblob_29
	var incrblob_30 *frigolite.Blob
	_ = incrblob_30
	var incrblob_31 *frigolite.Blob
	_ = incrblob_31
	var incrblob_32 *frigolite.Blob
	_ = incrblob_32
	var incrblob_33 *frigolite.Blob
	_ = incrblob_33
	var incrblob_34 *frigolite.Blob
	_ = incrblob_34
	var incrblob_35 *frigolite.Blob
	_ = incrblob_35
	var incrblob_36 *frigolite.Blob
	_ = incrblob_36
	var incrblob_37 *frigolite.Blob
	_ = incrblob_37
	var incrblob_38 *frigolite.Blob
	_ = incrblob_38
	var incrblob_39 *frigolite.Blob
	_ = incrblob_39
	var incrblob_40 *frigolite.Blob
	_ = incrblob_40
	var incrblob_41 *frigolite.Blob
	_ = incrblob_41
	var incrblob_42 *frigolite.Blob
	_ = incrblob_42
	var incrblob_43 *frigolite.Blob
	_ = incrblob_43
	var incrblob_44 *frigolite.Blob
	_ = incrblob_44
	var incrblob_45 *frigolite.Blob
	_ = incrblob_45
	var incrblob_46 *frigolite.Blob
	_ = incrblob_46
	var incrblob_47 *frigolite.Blob
	_ = incrblob_47
	var incrblob_48 *frigolite.Blob
	_ = incrblob_48
	var incrblob_49 *frigolite.Blob
	_ = incrblob_49
	var incrblob_50 *frigolite.Blob
	_ = incrblob_50
	var incrblob_51 *frigolite.Blob
	_ = incrblob_51
	var incrblob_52 *frigolite.Blob
	_ = incrblob_52
	var incrblob_53 *frigolite.Blob
	_ = incrblob_53
	var incrblob_54 *frigolite.Blob
	_ = incrblob_54
	var incrblob_55 *frigolite.Blob
	_ = incrblob_55
	var incrblob_56 *frigolite.Blob
	_ = incrblob_56
	var incrblob_57 *frigolite.Blob
	_ = incrblob_57
	var incrblob_58 *frigolite.Blob
	_ = incrblob_58
	var incrblob_59 *frigolite.Blob
	_ = incrblob_59
	var incrblob_60 *frigolite.Blob
	_ = incrblob_60
	var incrblob_61 *frigolite.Blob
	_ = incrblob_61
	var incrblob_62 *frigolite.Blob
	_ = incrblob_62
	var incrblob_63 *frigolite.Blob
	_ = incrblob_63
	var incrblob_64 *frigolite.Blob
	_ = incrblob_64
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var testprefix string
	_ = testprefix // pre-declared from TCL source
	var nByte string
	_ = nByte // pre-declared from TCL source
	var data string
	_ = data // pre-declared from TCL source
	var tn string
	_ = tn // pre-declared from TCL source
	var iRow string
	_ = iRow // pre-declared from TCL source
	var flags string
	_ = flags // pre-declared from TCL source
	var n string
	_ = n // pre-declared from TCL source
	var B string
	_ = B // pre-declared from TCL source
	var expected string
	_ = expected // pre-declared from TCL source
	var res string
	_ = res // pre-declared from TCL source
	var cannot string
	_ = cannot // pre-declared from TCL source
	var stmt string
	_ = stmt // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source
	var zDb string
	_ = zDb // pre-declared from TCL source
	var zTab string
	_ = zTab // pre-declared from TCL source
	var zCol string
	_ = zCol // pre-declared from TCL source
	var errcode string
	_ = errcode // pre-declared from TCL source
	var errmsg string
	_ = errmsg // pre-declared from TCL source
	var B2 string
	_ = B2 // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	vtab.TclVarSet("testprefix", "", "e_blobopen")
	testprefix = "e_blobopen"
	_ = testprefix // suppress unused warning
	os.Remove("test.db2")
	{ // "1.0"
		_res = db.Exec("\n  ATTACH 'test.db2' AS aux;\n\n  CREATE TABLE main.t1(a INTEGER PRIMARY KEY, b TEXT, c BLOB);\n  CREATE TEMP TABLE t1(a INTEGER PRIMARY KEY, b TEXT, c BLOB);\n  CREATE TABLE aux.t1(a INTEGER PRIMARY KEY, b TEXT, c BLOB);\n\n  CREATE TABLE main.x1(a INTEGER PRIMARY KEY, b TEXT, c BLOB);\n  CREATE TEMP TABLE x2(a INTEGER PRIMARY KEY, b TEXT, c BLOB);\n  CREATE TABLE aux.x3(a INTEGER PRIMARY KEY, b TEXT, c BLOB);\n\n  INSERT INTO main.t1 VALUES(1, 'main one', X'0101');\n  INSERT INTO main.t1 VALUES(2, 'main two', X'0102');\n  INSERT INTO main.t1 VALUES(3, 'main three', X'0103');\n  INSERT INTO main.t1 VALUES(4, 'main four', X'0104');\n  INSERT INTO main.t1 VALUES(5, 'main five', X'0105');\n\n  INSERT INTO main.x1 VALUES(1, 'x main one', X'000101');\n  INSERT INTO main.x1 VALUES(2, 'x main two', X'000102');\n  INSERT INTO main.x1 VALUES(3, 'x main three', X'000103');\n  INSERT INTO main.x1 VALUES(4, 'x main four', X'000104');\n  INSERT INTO main.x1 VALUES(5, 'x main five', X'000105');\n\n  INSERT INTO temp.t1 VALUES(1, 'temp one', X'0201');\n  INSERT INTO temp.t1 VALUES(2, 'temp two', X'0202');\n  INSERT INTO temp.t1 VALUES(3, 'temp three', X'0203');\n  INSERT INTO temp.t1 VALUES(4, 'temp four', X'0204');\n  INSERT INTO temp.t1 VALUES(5, 'temp five', X'0205');\n\n  INSERT INTO temp.x2 VALUES(1, 'x temp one', X'000201');\n  INSERT INTO temp.x2 VALUES(2, 'x temp two', X'000202');\n  INSERT INTO temp.x2 VALUES(3, 'x temp three', X'000203');\n  INSERT INTO temp.x2 VALUES(4, 'x temp four', X'000204');\n  INSERT INTO temp.x2 VALUES(5, 'x temp five', X'000205');\n\n  INSERT INTO aux.t1 VALUES(1, 'aux one', X'0301');\n  INSERT INTO aux.t1 VALUES(2, 'aux two', X'0302');\n  INSERT INTO aux.t1 VALUES(3, 'aux three', X'0303');\n  INSERT INTO aux.t1 VALUES(4, 'aux four', X'0304');\n  INSERT INTO aux.t1 VALUES(5, 'aux five', X'0305');\n\n  INSERT INTO aux.x3 VALUES(1, 'x aux one', X'000301');\n  INSERT INTO aux.x3 VALUES(2, 'x aux two', X'000302');\n  INSERT INTO aux.x3 VALUES(3, 'x aux three', X'000303');\n  INSERT INTO aux.x3 VALUES(4, 'x aux four', X'000304');\n  INSERT INTO aux.x3 VALUES(5, 'x aux five', X'000305');\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  ATTACH 'test.db2' AS aux;\n\n  CREATE TABLE main.t1(a INTEGER PRIMARY KEY, b TEXT, c BLOB);\n  CREATE TEMP TABLE t1(a INTEGER PRIMARY KEY, b TEXT, c BLOB);\n  CREATE TABLE aux.t1(a INTEGER PRIMARY KEY, b TEXT, c BLOB);\n\n  CREATE TABLE main.x1(a INTEGER PRIMARY KEY, b TEXT, c BLOB);\n  CREATE TEMP TABLE x2(a INTEGER PRIMARY KEY, b TEXT, c BLOB);\n  CREATE TABLE aux.x3(a INTEGER PRIMARY KEY, b TEXT, c BLOB);\n\n  INSERT INTO main.t1 VALUES(1, 'main one', X'0101');\n  INSERT INTO main.t1 VALUES(2, 'main two', X'0102');\n  INSERT INTO main.t1 VALUES(3, 'main three', X'0103');\n  INSERT INTO main.t1 VALUES(4, 'main four', X'0104');\n  INSERT INTO main.t1 VALUES(5, 'main five', X'0105');\n\n  INSERT INTO main.x1 VALUES(1, 'x main one', X'000101');\n  INSERT INTO main.x1 VALUES(2, 'x main two', X'000102');\n  INSERT INTO main.x1 VALUES(3, 'x main three', X'000103');\n  INSERT INTO main.x1 VALUES(4, 'x main four', X'000104');\n  INSERT INTO main.x1 VALUES(5, 'x main five', X'000105');\n\n  INSERT INTO temp.t1 VALUES(1, 'temp one', X'0201');\n  INSERT INTO temp.t1 VALUES(2, 'temp two', X'0202');\n  INSERT INTO temp.t1 VALUES(3, 'temp three', X'0203');\n  INSERT INTO temp.t1 VALUES(4, 'temp four', X'0204');\n  INSERT INTO temp.t1 VALUES(5, 'temp five', X'0205');\n\n  INSERT INTO temp.x2 VALUES(1, 'x temp one', X'000201');\n  INSERT INTO temp.x2 VALUES(2, 'x temp two', X'000202');\n  INSERT INTO temp.x2 VALUES(3, 'x temp three', X'000203');\n  INSERT INTO temp.x2 VALUES(4, 'x temp four', X'000204');\n  INSERT INTO temp.x2 VALUES(5, 'x temp five', X'000205');\n\n  INSERT INTO aux.t1 VALUES(1, 'aux one', X'0301');\n  INSERT INTO aux.t1 VALUES(2, 'aux two', X'0302');\n  INSERT INTO aux.t1 VALUES(3, 'aux three', X'0303');\n  INSERT INTO aux.t1 VALUES(4, 'aux four', X'0304');\n  INSERT INTO aux.t1 VALUES(5, 'aux five', X'0305');\n\n  INSERT INTO aux.x3 VALUES(1, 'x aux one', X'000301');\n  INSERT INTO aux.x3 VALUES(2, 'x aux two', X'000302');\n  INSERT INTO aux.x3 VALUES(3, 'x aux three', X'000303');\n  INSERT INTO aux.x3 VALUES(4, 'x aux four', X'000304');\n  INSERT INTO aux.x3 VALUES(5, 'x aux five', X'000305');\n")
		}
	}
	// proc definition (not transpiled)
	{ // do_test "1.1.1"
		// read_blob main t1 b 1 (unsupported command, not transpiled)
	}
	{ // do_test "1.1.2"
		// read_blob main t1 c 1 (unsupported command, not transpiled)
	}
	{ // do_test "1.1.3"
		// read_blob temp t1 b 1 (unsupported command, not transpiled)
	}
	{ // do_test "1.1.4"
		// read_blob temp t1 c 1 (unsupported command, not transpiled)
	}
	{ // do_test "1.1.6"
		// read_blob aux t1 b 1 (unsupported command, not transpiled)
	}
	{ // do_test "1.1.7"
		// read_blob aux t1 c 1 (unsupported command, not transpiled)
	}
	{ // do_test "1.2.1"
		// read_blob main t1 b 4 (unsupported command, not transpiled)
	}
	{ // do_test "1.2.2"
		// read_blob main t1 c 4 (unsupported command, not transpiled)
	}
	{ // do_test "1.2.3"
		// read_blob temp t1 b 4 (unsupported command, not transpiled)
	}
	{ // do_test "1.2.4"
		// read_blob temp t1 c 4 (unsupported command, not transpiled)
	}
	{ // do_test "1.2.6"
		// read_blob aux t1 b 4 (unsupported command, not transpiled)
	}
	{ // do_test "1.2.7"
		// read_blob aux t1 c 4 (unsupported command, not transpiled)
	}
	{ // do_test "1.3.1"
		// read_blob main x1 b 2 (unsupported command, not transpiled)
	}
	{ // do_test "1.3.2"
		// read_blob main x1 c 2 (unsupported command, not transpiled)
	}
	{ // do_test "1.3.3"
		// read_blob temp x2 b 2 (unsupported command, not transpiled)
	}
	{ // do_test "1.3.4"
		// read_blob temp x2 c 2 (unsupported command, not transpiled)
	}
	{ // do_test "1.3.6"
		// read_blob aux x3 b 2 (unsupported command, not transpiled)
	}
	{ // do_test "1.3.7"
		// read_blob aux x3 c 2 (unsupported command, not transpiled)
	}
	{ // do_test "2.1"
		_rc := "0"
		{
			var _catchErr error
			B = "incrblob_1"
			incrblob_1, _berr = db.OpenBlob("test.db", "t1", "b", tclRowID(1), tclBlobWritable(0))
			if _berr != nil {
				incrblob_1 = nil
				_r = ""
				_catchErr = _berr
			} else {
				_r = "B"
			}
			if _catchErr != nil { msg = _catchErr.Error() } else { msg = "" }
			if _catchErr != nil { _rc = "1" }
		}
		_list := tclList([]string{_rc, msg})
		_ = _list
		_r = _list
	}
	{ // do_test "2.2"
		_rc := "0"
		{
			var _catchErr error
			B = "incrblob_2"
			incrblob_2, _berr = db.OpenBlob("test.db2", "t1", "b", tclRowID(1), tclBlobWritable(0))
			if _berr != nil {
				incrblob_2 = nil
				_r = ""
				_catchErr = _berr
			} else {
				_r = "B"
			}
			if _catchErr != nil { msg = _catchErr.Error() } else { msg = "" }
			if _catchErr != nil { _rc = "1" }
		}
		_list := tclList([]string{_rc, msg})
		_ = _list
		_r = _list
	}
	// foreach {tn iRow flags} "1 1   0\n  2 2   1\n  3 3  -1\n  4 4   2147483647\n  5 5  -2147483648"
	_items0 := tclSplitList("1 1   0\n  2 2   1\n  3 3  -1\n  4 4   2147483647\n  5 5  -2147483648")
	for _idx0 := 0; _idx0+3 <= len(_items0); _idx0 += 3 {
		tn := _items0[_idx0+0]
		_ = tn // suppress unused warning
		iRow := _items0[_idx0+1]
		_ = iRow // suppress unused warning
		flags := _items0[_idx0+2]
		_ = flags // suppress unused warning
		_ = _idx0
			{ // do_test "3." + tn + ".1"
				B = "incrblob_3"
				incrblob_3, _berr = db.OpenBlob("main", "x1", "c", tclRowID(iRow), tclBlobWritable(flags))
				if _berr != nil {
					incrblob_3 = nil
					_r = ""
				} else {
					_r = "B"
				}
				n = strconv.Itoa(incrblob_3.Bytes())
				_ = n // suppress unused warning
				if _r2, _err2 := incrblob_3.Read(tclBlobInt(0), tclBlobInt(n)); _err2 != nil {
					_r = ""
				} else {
					_r = string(_r2)
				}
			}
			if func() bool { flags_n, _flags_e := strconv.Atoi(flags); if _flags_e != nil { return false }; return flags_n == 0 }() {
				{ // do_test "3." + tn + ".2"
					_rc := "0"
					{
						var _catchErr error
						if _err2 := incrblob_3.Write(tclBlobInt(0), []byte("xxx"), tclBlobInt(3)); _err2 != nil {
							_r = ""
							_catchErr = _err2
						} else {
							_r = ""
						}
						if _catchErr != nil { msg = _catchErr.Error() } else { msg = "" }
						if _catchErr != nil { _rc = "1" }
					}
					_list := tclList([]string{_rc, msg})
					_ = _list
					_r = _list
				}
				{ // "3." + tn + ".3"
					r = db.Query("\n      SELECT c FROM x1 WHERE a=" + sqlLiteral(iRow) + ";\n    ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n      SELECT c FROM x1 WHERE a=" + sqlLiteral(iRow) + ";\n    ")
						return
					}
					got := flatten(r)
					want := tclListFlatten((string([]byte{byte(tclBlobInt(0))}) + string([]byte{byte(tclBlobInt(1))}) + string([]byte{byte(tclBlobInt(iRow))})))
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
			} else {
				{ // do_test "3." + tn + ".4"
					_rc := "0"
					{
						var _catchErr error
						if _err2 := incrblob_3.Write(tclBlobInt(0), []byte("xxx"), tclBlobInt(3)); _err2 != nil {
							_r = ""
							_catchErr = _err2
						} else {
							_r = ""
						}
						if _catchErr != nil { msg = _catchErr.Error() } else { msg = "" }
						if _catchErr != nil { _rc = "1" }
					}
					_list := tclList([]string{_rc, msg})
					_ = _list
					_r = _list
				}
				{ // "3." + tn + ".5"
					r = db.Query("\n      SELECT c FROM x1 WHERE a=" + sqlLiteral(iRow) + ";\n    ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n      SELECT c FROM x1 WHERE a=" + sqlLiteral(iRow) + ";\n    ")
						return
					}
					got := flatten(r)
					want := "xxx"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
			}
			incrblob_3.Close()
		}
		db.Close()
		os.Remove("test.db")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		{ // "4.0"
			_res = db.Exec("\n  CREATE TABLE t1(x, y);\n  INSERT INTO t1 VALUES('abcd', 152);\n  INSERT INTO t1 VALUES(NULL, X'00010203');\n  INSERT INTO t1 VALUES('', 154.2);\n\n  CREATE TABLE t2(x PRIMARY KEY, y) WITHOUT ROWID;\n  INSERT INTO t2 VALUES(1, 'blob');\n\n  CREATE TABLE t3(a PRIMARY KEY, b, c, d, e, f, UNIQUE(e, f));\n  INSERT INTO t3 VALUES('aaaa', 'bbbb', 'cccc', 'dddd', 'eeee', 'ffff');\n  CREATE INDEX t3b ON t3(b);\n\n  CREATE TABLE p1(x PRIMARY KEY);\n  INSERT INTO p1 VALUES('abc');\n\n  CREATE TABLE c1(a INTEGER PRIMARY KEY, b REFERENCES p1);\n  INSERT INTO c1 VALUES(45, 'abc');\n")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t1(x, y);\n  INSERT INTO t1 VALUES('abcd', 152);\n  INSERT INTO t1 VALUES(NULL, X'00010203');\n  INSERT INTO t1 VALUES('', 154.2);\n\n  CREATE TABLE t2(x PRIMARY KEY, y) WITHOUT ROWID;\n  INSERT INTO t2 VALUES(1, 'blob');\n\n  CREATE TABLE t3(a PRIMARY KEY, b, c, d, e, f, UNIQUE(e, f));\n  INSERT INTO t3 VALUES('aaaa', 'bbbb', 'cccc', 'dddd', 'eeee', 'ffff');\n  CREATE INDEX t3b ON t3(b);\n\n  CREATE TABLE p1(x PRIMARY KEY);\n  INSERT INTO p1 VALUES('abc');\n\n  CREATE TABLE c1(a INTEGER PRIMARY KEY, b REFERENCES p1);\n  INSERT INTO c1 VALUES(45, 'abc');\n")
			}
		}
		// proc definition (not transpiled)
		// test_blob_open 1 nosuchdb t1 x 1 0 SQLITE_ERROR no such table: nosuchdb.t1 (unsupported command, not transpiled)
		// test_blob_open 2 main tt1 x 1 0 SQLITE_ERROR no such table: main.tt1 (unsupported command, not transpiled)
		// test_blob_open 3 main t2 y 1 0 SQLITE_ERROR cannot open table without rowid: t2 (unsupported command, not transpiled)
		// test_blob_open 4 main t1 z 2 0 SQLITE_ERROR no such column: \"z\" (unsupported command, not transpiled)
		// test_blob_open 5 main t1 y 6 0 SQLITE_ERROR no such rowid: 6 (unsupported command, not transpiled)
		// test_blob_open 6 main t1 x 2 0 SQLITE_ERROR cannot open value of type null (unsupported command, not transpiled)
		// test_blob_open 7 main t1 y 1 0 SQLITE_ERROR cannot open value of type integer (unsupported command, not transpiled)
		// test_blob_open 8 main t1 y 3 0 SQLITE_ERROR cannot open value of type real (unsupported command, not transpiled)
		// test_blob_open 8.1.1 main t3 a 1 0 SQLITE_OK not an error (unsupported command, not transpiled)
		// test_blob_open 8.1.2 main t3 b 1 0 SQLITE_OK not an error (unsupported command, not transpiled)
		// test_blob_open 8.1.3 main t3 c 1 0 SQLITE_OK not an error (unsupported command, not transpiled)
		// test_blob_open 8.1.4 main t3 d 1 0 SQLITE_OK not an error (unsupported command, not transpiled)
		// test_blob_open 8.1.5 main t3 e 1 0 SQLITE_OK not an error (unsupported command, not transpiled)
		// test_blob_open 8.1.6 main t3 f 1 0 SQLITE_OK not an error (unsupported command, not transpiled)
		vtab.TclVarSet("cannot", "", "cannot open indexed column for writing")
		cannot = "cannot open indexed column for writing"
		_ = cannot // suppress unused warning
		// test_blob_open 8.2.1 main t3 a 1 8 SQLITE_ERROR $cannot (unsupported command, not transpiled)
		// test_blob_open 8.2.2 main t3 b 1 8 SQLITE_ERROR $cannot (unsupported command, not transpiled)
		// test_blob_open 8.2.3 main t3 c 1 8 SQLITE_OK not an error (unsupported command, not transpiled)
		// test_blob_open 8.2.4 main t3 d 1 8 SQLITE_OK not an error (unsupported command, not transpiled)
		// test_blob_open 8.2.5 main t3 e 1 8 SQLITE_ERROR $cannot (unsupported command, not transpiled)
		// test_blob_open 8.2.6 main t3 f 1 8 SQLITE_ERROR $cannot (unsupported command, not transpiled)
		// test_blob_open 9.1 main c1 b 45 0 SQLITE_OK not an error (unsupported command, not transpiled)
		// test_blob_open 9.2 main c1 b 45 1 SQLITE_OK not an error (unsupported command, not transpiled)
		r = db.Query(" PRAGMA foreign_keys = ON ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA foreign_keys = ON ")
		}
		// test_blob_open 9.3 main c1 b 45 0 SQLITE_OK not an error (unsupported command, not transpiled)
		// test_blob_open 9.4 main c1 b 45 1 SQLITE_ERROR cannot open foreign key column for writing (unsupported command, not transpiled)
		{ // "11.1"
			_res = db.Exec("\n  CREATE TABLE b1(a INTEGER PRIMARY KEY, b, c UNIQUE);\n  INSERT INTO b1 VALUES(1, '1234567890', 1);\n  INSERT INTO b1 VALUES(2, '1234567890', 2);\n  INSERT INTO b1 VALUES(3, '1234567890', 3);\n  INSERT INTO b1 VALUES(4, '1234567890', 4);\n  INSERT INTO b1 VALUES(5, '1234567890', 5);\n  INSERT INTO b1 VALUES(6, '1234567890', 6);\n\n  CREATE TABLE b2(a INTEGER PRIMARY KEY, b, c UNIQUE);\n  INSERT INTO b2 VALUES(1, '1234567890', 1);\n  INSERT INTO b2 VALUES(2, '1234567890', 2);\n  INSERT INTO b2 VALUES(3, '1234567890', 3);\n  INSERT INTO b2 VALUES(4, '1234567890', 4);\n  INSERT INTO b2 VALUES(5, '1234567890', 5);\n  INSERT INTO b2 VALUES(6, '1234567890', 6);\n")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE b1(a INTEGER PRIMARY KEY, b, c UNIQUE);\n  INSERT INTO b1 VALUES(1, '1234567890', 1);\n  INSERT INTO b1 VALUES(2, '1234567890', 2);\n  INSERT INTO b1 VALUES(3, '1234567890', 3);\n  INSERT INTO b1 VALUES(4, '1234567890', 4);\n  INSERT INTO b1 VALUES(5, '1234567890', 5);\n  INSERT INTO b1 VALUES(6, '1234567890', 6);\n\n  CREATE TABLE b2(a INTEGER PRIMARY KEY, b, c UNIQUE);\n  INSERT INTO b2 VALUES(1, '1234567890', 1);\n  INSERT INTO b2 VALUES(2, '1234567890', 2);\n  INSERT INTO b2 VALUES(3, '1234567890', 3);\n  INSERT INTO b2 VALUES(4, '1234567890', 4);\n  INSERT INTO b2 VALUES(5, '1234567890', 5);\n  INSERT INTO b2 VALUES(6, '1234567890', 6);\n")
			}
		}
		{ // do_test "11.2.1"
			B = "incrblob_4"
			incrblob_4, _berr = db.OpenBlob("main", "b1", "b", tclRowID(2), tclBlobWritable(0))
			if _berr != nil {
				incrblob_4 = nil
				_r = ""
			} else {
				_r = "B"
			}
			if _r2, _err2 := incrblob_4.Read(tclBlobInt(0), tclBlobInt(10)); _err2 != nil {
				_r = ""
			} else {
				_r = string(_r2)
			}
		}
		{ // do_test "11.2.2"
			_res = db.Exec(" DELETE FROM b1 WHERE a = 1 ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " DELETE FROM b1 WHERE a = 1 ")
			}
			if _r2, _err2 := incrblob_4.Read(tclBlobInt(0), tclBlobInt(10)); _err2 != nil {
				_r = ""
			} else {
				_r = string(_r2)
			}
		}
		{ // do_test "11.2.3"
			_res = db.Exec(" DELETE FROM b1 WHERE a = 2 ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " DELETE FROM b1 WHERE a = 2 ")
			}
			_rc := "0"
			{
				var _catchErr error
				if _r2, _err2 := incrblob_4.Read(tclBlobInt(0), tclBlobInt(10)); _err2 != nil {
					_r = ""
					_catchErr = _err2
				} else {
					_r = string(_r2)
				}
				if _catchErr != nil { msg = _catchErr.Error() } else { msg = "" }
				if _catchErr != nil { _rc = "1" }
			}
			_list := tclList([]string{_rc, msg})
			_ = _list
			_r = _list
		}
		{ // do_test "11.2.4"
			incrblob_4.Close()
		}
		{ // do_test "11.3.1"
			B = "incrblob_5"
			incrblob_5, _berr = db.OpenBlob("main", "b1", "b", tclRowID(3), tclBlobWritable(0))
			if _berr != nil {
				incrblob_5 = nil
				_r = ""
			} else {
				_r = "B"
			}
			if _r2, _err2 := incrblob_5.Read(tclBlobInt(0), tclBlobInt(10)); _err2 != nil {
				_r = ""
			} else {
				_r = string(_r2)
			}
		}
		{ // do_test "11.3.2"
			_res = db.Exec(" UPDATE b1 SET c = 42 WHERE a=4 ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " UPDATE b1 SET c = 42 WHERE a=4 ")
			}
			if _r2, _err2 := incrblob_5.Read(tclBlobInt(0), tclBlobInt(10)); _err2 != nil {
				_r = ""
			} else {
				_r = string(_r2)
			}
		}
		{ // do_test "11.3.3"
			_res = db.Exec(" UPDATE b1 SET c = 43 WHERE a=3 ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " UPDATE b1 SET c = 43 WHERE a=3 ")
			}
			_rc := "0"
			{
				var _catchErr error
				if _r2, _err2 := incrblob_5.Read(tclBlobInt(0), tclBlobInt(10)); _err2 != nil {
					_r = ""
					_catchErr = _err2
				} else {
					_r = string(_r2)
				}
				if _catchErr != nil { msg = _catchErr.Error() } else { msg = "" }
				if _catchErr != nil { _rc = "1" }
			}
			_list := tclList([]string{_rc, msg})
			_ = _list
			_r = _list
		}
		{ // do_test "11.3.4"
			incrblob_5.Close()
		}
		{ // do_test "11.4.1"
			B = "incrblob_6"
			incrblob_6, _berr = db.OpenBlob("main", "b1", "b", tclRowID(6), tclBlobWritable(0))
			if _berr != nil {
				incrblob_6 = nil
				_r = ""
			} else {
				_r = "B"
			}
			if _r2, _err2 := incrblob_6.Read(tclBlobInt(0), tclBlobInt(10)); _err2 != nil {
				_r = ""
			} else {
				_r = string(_r2)
			}
		}
		{ // do_test "11.4.2"
			_res = db.Exec(" INSERT OR REPLACE INTO b1 VALUES(10, 'abcdefghij', 5) ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " INSERT OR REPLACE INTO b1 VALUES(10, 'abcdefghij', 5) ")
			}
			if _r2, _err2 := incrblob_6.Read(tclBlobInt(0), tclBlobInt(10)); _err2 != nil {
				_r = ""
			} else {
				_r = string(_r2)
			}
		}
		{ // do_test "11.4.3"
			_res = db.Exec(" INSERT OR REPLACE INTO b1 VALUES(11, 'abcdefghij', 6) ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " INSERT OR REPLACE INTO b1 VALUES(11, 'abcdefghij', 6) ")
			}
			_rc := "0"
			{
				var _catchErr error
				if _r2, _err2 := incrblob_6.Read(tclBlobInt(0), tclBlobInt(10)); _err2 != nil {
					_r = ""
					_catchErr = _err2
				} else {
					_r = string(_r2)
				}
				if _catchErr != nil { msg = _catchErr.Error() } else { msg = "" }
				if _catchErr != nil { _rc = "1" }
			}
			_list := tclList([]string{_rc, msg})
			_ = _list
			_r = _list
		}
		{ // do_test "11.4.4"
			incrblob_6.Close()
		}
		{ // do_test "11.4.1"
			B = "incrblob_7"
			incrblob_7, _berr = db.OpenBlob("main", "b2", "b", tclRowID(2), tclBlobWritable(1))
			if _berr != nil {
				incrblob_7 = nil
				_r = ""
			} else {
				_r = "B"
			}
		}
		{ // do_test "11.4.2"
			_res = db.Exec(" DELETE FROM b2 WHERE a = 1 ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " DELETE FROM b2 WHERE a = 1 ")
			}
		}
		{ // do_test "11.4.3"
			_res = db.Exec(" DELETE FROM b2 WHERE a = 2 ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " DELETE FROM b2 WHERE a = 2 ")
			}
			_rc := "0"
			{
				var _catchErr error
				if _catchErr != nil { msg = _catchErr.Error() } else { msg = "" }
				if _catchErr != nil { _rc = "1" }
			}
			_list := tclList([]string{_rc, msg})
			_ = _list
			_r = _list
		}
		{ // do_test "11.4.4"
			incrblob_7.Close()
		}
		{ // do_test "11.5.1"
			B = "incrblob_8"
			incrblob_8, _berr = db.OpenBlob("main", "b2", "b", tclRowID(3), tclBlobWritable(1))
			if _berr != nil {
				incrblob_8 = nil
				_r = ""
			} else {
				_r = "B"
			}
		}
		{ // do_test "11.5.2"
			_res = db.Exec(" UPDATE b2 SET c = 42 WHERE a=4 ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " UPDATE b2 SET c = 42 WHERE a=4 ")
			}
		}
		{ // do_test "11.5.3"
			_res = db.Exec(" UPDATE b2 SET c = 43 WHERE a=3 ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " UPDATE b2 SET c = 43 WHERE a=3 ")
			}
			_rc := "0"
			{
				var _catchErr error
				if _catchErr != nil { msg = _catchErr.Error() } else { msg = "" }
				if _catchErr != nil { _rc = "1" }
			}
			_list := tclList([]string{_rc, msg})
			_ = _list
			_r = _list
		}
		{ // do_test "11.5.4"
			incrblob_8.Close()
		}
		{ // do_test "11.6.1"
			B = "incrblob_9"
			incrblob_9, _berr = db.OpenBlob("main", "b2", "b", tclRowID(6), tclBlobWritable(1))
			if _berr != nil {
				incrblob_9 = nil
				_r = ""
			} else {
				_r = "B"
			}
		}
		{ // do_test "11.6.2"
			_res = db.Exec(" INSERT OR REPLACE INTO b2 VALUES(10, 'abcdefghij', 5) ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " INSERT OR REPLACE INTO b2 VALUES(10, 'abcdefghij', 5) ")
			}
		}
		{ // do_test "11.6.3"
			_res = db.Exec(" INSERT OR REPLACE INTO b2 VALUES(11, 'abcdefghij', 6) ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " INSERT OR REPLACE INTO b2 VALUES(11, 'abcdefghij', 6) ")
			}
			_rc := "0"
			{
				var _catchErr error
				if _catchErr != nil { msg = _catchErr.Error() } else { msg = "" }
				if _catchErr != nil { _rc = "1" }
			}
			_list := tclList([]string{_rc, msg})
			_ = _list
			_r = _list
		}
		{ // do_test "11.6.4"
			incrblob_9.Close()
		}
		{ // "12.1"
			_res = db.Exec("\n  CREATE TABLE b3(x INTEGER PRIMARY KEY, y TEXT, z INTEGER);\n  INSERT INTO b3 VALUES(22, '..........', NULL);\n")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE b3(x INTEGER PRIMARY KEY, y TEXT, z INTEGER);\n  INSERT INTO b3 VALUES(22, '..........', NULL);\n")
			}
		}
		{ // do_test "12.2"
			B = "incrblob_10"
			incrblob_10, _berr = db.OpenBlob("main", "b3", "y", tclRowID(22), tclBlobWritable(1))
			if _berr != nil {
				incrblob_10 = nil
				_r = ""
			} else {
				_r = "B"
			}
			if _err2 := incrblob_10.Write(tclBlobInt(0), []byte("xxxxx"), tclBlobInt(5)); _err2 != nil {
				_r = ""
			} else {
				_r = ""
			}
		}
		{ // "12.3"
			_res = db.Exec("\n  UPDATE b3 SET z = 'not null';\n")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  UPDATE b3 SET z = 'not null';\n")
			}
		}
		{ // do_test "12.4"
			_rc := "0"
			{
				var _catchErr error
				if _err2 := incrblob_10.Write(tclBlobInt(5), []byte("xxxxx"), tclBlobInt(5)); _err2 != nil {
					_r = ""
					_catchErr = _err2
				} else {
					_r = ""
				}
				if _catchErr != nil { msg = _catchErr.Error() } else { msg = "" }
				if _catchErr != nil { _rc = "1" }
			}
			_list := tclList([]string{_rc, msg})
			_ = _list
			_r = _list
		}
		{ // "12.5"
			r = db.Query("\n  SELECT * FROM b3;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM b3;\n")
				return
			}
			got := flatten(r)
			want := "22 xxxxx..... not null"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // do_test "12.5"
			incrblob_10.Close()
		}
		{ // "12.6"
			r = db.Query("\n  SELECT * FROM b3;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM b3;\n")
				return
			}
			got := flatten(r)
			want := "22 xxxxx..... not null"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "13.1"
			_res = db.Exec("\n  CREATE TABLE c2(i INTEGER PRIMARY KEY, j);\n  INSERT INTO c2 VALUES(10, zeroblob(24));\n")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE c2(i INTEGER PRIMARY KEY, j);\n  INSERT INTO c2 VALUES(10, zeroblob(24));\n")
			}
		}
		{ // "13.2" (prepare-step internals; SQL side effects only)
			// prepared stmt: INSERT INTO c2 VALUES(11, ?) (bind/step emulation)
			_ = stmt // prepared statement handle
			// sqlite3_bind_zeroblob $stmt 1 45 (unsupported command, not transpiled)
			_res = db.Exec("INSERT INTO c2 VALUES(11, ?)")
			if _res.Error != nil { db.SetLastErr(_res.Error.Error(), db.ErrorCodeFor(_res.Error)) }
			_ = _res // step result (SQLITE_ROW/SQLITE_CONSTRAINT) is C-API state; side effect only
			tclFinalizePrepared("stmt")
			// sqlite3_finalize $stmt
		}
		{ // do_test "13.3.1"
			B = "incrblob_11"
			incrblob_11, _berr = db.OpenBlob("main", "c2", "j", tclRowID(10), tclBlobWritable(1))
			if _berr != nil {
				incrblob_11 = nil
				_r = ""
			} else {
				_r = "B"
			}
			B2 = "incrblob_12"
			incrblob_12, _berr = db.OpenBlob("main", "c2", "j", tclRowID(11), tclBlobWritable(1))
			if _berr != nil {
				incrblob_12 = nil
				_r = ""
			} else {
				_r = "B2"
			}
			_list := tclList([]string{"sqlite3_blob_bytes $B", "sqlite3_blob_bytes $B2"})
			_ = _list
			_r = _list
		}
		{ // do_test "13.3.2"
			if _r2, _err2 := incrblob_11.Read(tclBlobInt(0), tclBlobInt(24)); _err2 != nil {
				_r = ""
			} else {
				_r = string(_r2)
			}
		}
		{ // do_test "13.3.3"
			if _r2, _err2 := incrblob_12.Read(tclBlobInt(0), tclBlobInt(45)); _err2 != nil {
				_r = ""
			} else {
				_r = string(_r2)
			}
		}
		{ // do_test "13.4.1"
		}
		{ // do_test "13.4.2"
		}
		{ // do_test "13.5"
			incrblob_11.Close()
			incrblob_12.Close()
			r = db.Query(" SELECT j FROM c2 ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " SELECT j FROM c2 ")
			}
		}
}
