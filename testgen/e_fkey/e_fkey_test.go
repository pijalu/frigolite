		...
		_ = fk // suppress unused warning
			...
			for _idx2 := 0; _idx2+2 <= len(_items2); _idx2 += 2 {
				...
				{ // do_test "e_fkey-25.5"
					...
					_r_tcl = append(_r_tcl, tclSplitList(tclCatchsqlStr(db, "DELETE FROM artist WHERE artistid = 5"))...)
				...
				{ // do_test "e_fkey-25.6"
					...
					_r_tcl = append(_r_tcl, tclSplitList(tclCatchsqlStr(db, "DELETE FROM artist WHERE artistid = 7"))...)
					_r_tcl_str := tclList(_r_tcl)
					_ = _r_tcl_str
					_ = _r_tcl
				}
				{ // do_test "e_fkey-25.7"
					...
					_r_tcl = append(_r_tcl, tclSplitList(tclCatchsqlStr(db, "DELETE FROM artist WHERE artistid = 6"))...)
					_r_tcl_str := tclList(_r_tcl)
					_ = _r_tcl_str
					_ = _r_tcl
				}
				...
				for _idx3 := 0; _idx3+2 <= len(_items3); _idx3 += 2 {
					...
					_ = _idx3
						...
						delete = tclEQP(db, "DELETE FROM parent WHERE 1") + " " + tclEQP(db, "SELECT rowid FROM child WHERE a = ? AND b = ?")
						_ = delete // suppress unused warning
						update = tclEQP(db, "UPDATE parent SET x=?, y=?") + " " + tclEQP(db, "SELECT rowid FROM child WHERE a = ? AND b = ?") + " " + tclEQP(db, "SELECT rowid FROM child WHERE a = ? AND b = ?")
						...
						{ // do_test "e_fkey-26." + tn + ".1"
							_r = tclEQP(db, "DELETE FROM parent WHERE 1")
							if _r != delete {
								t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", _r, delete, "e_fkey-26." + tn + ".1")
							}
						}
						{ // do_test "e_fkey-26." + tn + ".2"
							_r = tclEQP(db, "UPDATE parent set x=?, y=?")
							if _r != update {
								t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", _r, update, "e_fkey-26." + tn + ".2")
							}
						}
					...
					{ // do_test "e_fkey-27.2"
						_r = tclEQP(db, "INSERT INTO artist VALUES(?, ?)")
					}
					...
					for _idx4 := 0; _idx4+3 <= len(_items4); _idx4 += 3 {
						...
						for _idx5 := 0; _idx5+3 <= len(_items5); _idx5 += 3 {
							...
							_ = _idx5
								{ // "e_fkey-40." + tn — skipped: PRAGMA foreign_key_list multi-word action cell bracing not modeled in flatten harness N-A
								}
							}
							_res = db.Exec("PRAGMA foreign_keys = OFF")
							for _, _t := range db.Query("SELECT name, type FROM sqlite_master WHERE type IN('table','view')").Rows {
