...
func Test_eqp(t *testing.T) {
	...
	// det 2.2.4 SELECT DISTINCT * FROM t1, t2 {\n  QUERY PLAN\n  |--SCAN t1\n  |--SCAN t2\n  `--U...} (unsupported command, not transpiled)
	...
	// det 2.4.1 SELECT * FROM t1 WHERE rowid=? {\n  QUERY PLAN\n  `--SEARCH t1 USING INTEGER PRIMA...} (unsupported command, not transpiled)
	{ // "3.1.1"
	...
	// det 3.2.1 {\n  SELECT * FROM (SELECT * FROM t1 ORDER BY x LIM...} {\n  QUERY PLAN\n  |--CO-ROUTINE (subquery-xxxxxx)\...} (unsupported command, not transpiled)
	// det 3.2.2 {\n  SELECT * FROM \n    (SELECT * FROM t1 ORDER BY...} {\n  QUERY PLAN\n  |--CO-ROUTINE x1\n  |  |--SCAN t...} (unsupported command, not transpiled)
	// det 3.2.3 {\n  SELECT * FROM (SELECT * FROM t1 ORDER BY x LIM...} {\n  QUERY PLAN\n  |--CO-ROUTINE (subquery-xxxxxx)\...} (unsupported command, not transpiled)
	// det 3.3.1 {\n  SELECT * FROM t1 WHERE y IN (SELECT y FROM t2)...} {\n  QUERY PLAN\n  |--SCAN t1\n  `--LIST SUBQUERY x...} (unsupported command, not transpiled)
	// det 3.3.2 {\n  SELECT * FROM t1 WHERE y IN (SELECT y FROM t2 ...} {\n  QUERY PLAN\n  |--SCAN t1\n  `--CORRELATED LIST...} (unsupported command, not transpiled)
	// det 3.3.3 {\n  SELECT * FROM t1 WHERE EXISTS (SELECT y FROM t...} {\n  QUERY PLAN\n  |--SCAN t1\n  `--SCAN t2 EXISTS\...} (unsupported command, not transpiled)
	{ // "4.1.1"
	...
	if false {
		...
		// det 5.5 SELECT t1.a, t2.c FROM t2, t1 WHERE t1.a=1 AND t1.... {\n  0 0 1 {SEARCH t1 USING COVERING INDEX i2 (a=? ...} (unsupported command, not transpiled)
		{ // "5.5.0"
		...
		// det 5.7 SELECT c, d FROM t2 ORDER BY c {\n  0 0 0 {SCAN t2}\n  0 0 0 {USE TEMP B-TREE FOR ...} (unsupported command, not transpiled)
		{ // "5.8.0"
		...
		if tclBool("!" + "nonzero_reserved_bytes") {
			...
			// proc definition (not transpiled)
			// do_peqp_test 6.1 {\n    SELECT a, b FROM t1 EXCEPT SELECT d, 99 FROM...} [string trimleft {\n1 0 0 SCAN t1 USING COVERING I... (unsupported command, not transpiled)
		}
	}
	// drop_all_tables (unsupported command, not transpiled)
	{ // "7.0"
	...
	// det 8.1.2 SELECT * FROM t2 WHERE rowid=? {\n  QUERY PLAN\n  `--SEARCH t2 USING INTEGER PRIMA...} (unsupported command, not transpiled)
	// det 8.1.3 SELECT count(*) FROM t2 {\n  QUERY PLAN\n  `--SCAN t2\n} (unsupported command, not transpiled)
	// det 8.2.1 SELECT * FROM t1 {\n  QUERY PLAN\n  `--SCAN t1\n} (unsupported command, not transpiled)
	// det 8.2.2 SELECT * FROM t1 WHERE b=? {\n  QUERY PLAN\n  `--SEARCH t1 USING PRIMARY KEY (...} (unsupported command, not transpiled)
