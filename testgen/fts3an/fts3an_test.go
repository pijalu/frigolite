...
func Test_fts3an(t *testing.T) {
	...
	{ // "fts3an-1.9" — skipped: stand-alone '*' MATCH token is dropped by C's query parser (empty result); engine reports malformed MATCH expression (SQL + file side effects only)
	...
	_res = db.Exec("COMMIT;")
	{ // "fts3an-3.1" — skipped: offsets() under-counts prefix-query ('l*') occurrences per row (C: 6/1/192 hits; engine: fewer) (engine offsets-prefix gap) (SQL + file side effects only)
