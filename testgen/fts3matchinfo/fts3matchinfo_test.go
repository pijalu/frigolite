...
func Test_fts3matchinfo(t *testing.T) {
	...
	vtab.TclVarSet("testprefix", "", "fts3matchinfo")
			...
			for _idx1 := 0; _idx1+2 <= len(_items1); _idx1 += 2 {
				...
				_ = _idx1
					...
					for _idx2 := 0; _idx2+2 <= len(_items2); _idx2 += 2 {
						...
						_ = _idx2
							M = tclListAppend(M, tclExprWith("($a ? 1 : 0) + ($b ? 2 : 0)", map[string]string{"a": a, "b": b}))
						}
				...
				{ // do_test "12.0"
					...
					if _res.Error != nil {
						t.Errorf("exec error: %v\n  sql: %s", _res.Error, "CREATE VIRTUAL TABLE tt USING fts3(" + strings.Join(tclSplitList(cols), ",") + ")")
					}
				}
				{ // "12.1"
