...
func Test_fts3query(t *testing.T) {
	...
	vtab.TclVarSet("testprefix", "", "fts3query")
	...
	{ // do_test "7.2"
		...
		for _idx1 := 0; _idx1+3 <= len(_items1); _idx1 += 3 {
			...
			_ = _idx1
				_dbeval2 := tclExecSQL(db, "SELECT rowid FROM t4 WHERE rowid BETWEEN " + sqlLiteral(iFirst) + " AND " + sqlLiteral(iLast))
				res = _dbeval2
				...
				_dbeval3 := tclExecSQL(db, "SELECT rowid FROM t4 WHERE rowid BETWEEN " + sqlLiteral(iFirst) + " AND " + sqlLiteral(iLast) + " \n     ORDER BY +rowid DESC")
				res = _dbeval3
				_ = res // suppress unused warning
