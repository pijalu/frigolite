...
func Test_fts4onepass(t *testing.T) {
	...
	vtab.TclVarSet("testprefix", "", "fts4onepass")
	...
	for _idx0 := 0; _idx0+3 <= len(_items0); _idx0 += 3 {
		...
		_ = _idx0
			{ // "1." + tn (prepare-step internals; SQL side effects only)
				// sql_uses_stmt db $$sql (statement-journal probe skipped; SQL executes)
				_res = db.Exec(sql)
				_ = _res
			}
		}
		...
		for _idx1 := 0; _idx1+3 <= len(_items1); _idx1 += 3 {
			...
			_ = uses // suppress unused warning
			_ = _idx1
				{ // "2." + tn (prepare-step internals; SQL side effects only)
					// sql_uses_stmt db $$sql (statement-journal probe skipped; SQL executes)
					_res = db.Exec(sql)
					_ = _res
				}
