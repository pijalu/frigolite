// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package fts5fault8

import (
"github.com/pijalu/frigolite"
"github.com/pijalu/frigolite/internal/vtab"
"os"
"testing"
)

func Test_fts5fault8(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testprefix string
	_ = testprefix // pre-declared from TCL source
	var TMPDBERROR string
	_ = TMPDBERROR // pre-declared from TCL source
	var testdir string
	_ = testdir // pre-declared from TCL source
	var testresult string
	_ = testresult // pre-declared from TCL source

	vtab.TclVarSet("testprefix", "", "fts5fault8")
	testprefix = "fts5fault8"
	_ = testprefix // suppress unused warning
	// foreach_detail_mode $testprefix {\n\n  fts5_aux_test_functions db\n  do_execsql_tes...} (unsupported command, not transpiled)
	{ // "4.0"
		_res = db.Exec("\n  CREATE VIRTUAL TABLE x2 USING fts5(a);\n  INSERT INTO x2(x2, rank) VALUES('crisismerge', 2);\n  INSERT INTO x2(x2, rank) VALUES('pgsz', 32);\n  INSERT INTO x2 VALUES('a b c d');\n  INSERT INTO x2 VALUES('e f g h');\n  INSERT INTO x2 VALUES('i j k l');\n  INSERT INTO x2 VALUES('m n o p');\n  INSERT INTO x2 VALUES('q r s t');\n  INSERT INTO x2 VALUES('u v w x');\n  INSERT INTO x2 VALUES('y z a b');\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", resErrString(_res), "\n  CREATE VIRTUAL TABLE x2 USING fts5(a);\n  INSERT INTO x2(x2, rank) VALUES('crisismerge', 2);\n  INSERT INTO x2(x2, rank) VALUES('pgsz', 32);\n  INSERT INTO x2 VALUES('a b c d');\n  INSERT INTO x2 VALUES('e f g h');\n  INSERT INTO x2 VALUES('i j k l');\n  INSERT INTO x2 VALUES('m n o p');\n  INSERT INTO x2 VALUES('q r s t');\n  INSERT INTO x2 VALUES('u v w x');\n  INSERT INTO x2 VALUES('y z a b');\n")
		}
	}
	// db_save_and_close: snapshot test.db* under sv_ prefix
	for _, _sf := range tclSplitList(tclGlob("test.db*")) {
		tclFileCopy(_sf, "sv_"+_sf)
	}
	db.Close()
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	tcl_nullvalue = "{}" // fresh connection resets nullvalue
	// do_faultsim_test 4 -faults oom-* -prep {\n  faultsim_restore_and_reopen\n} -body {\n  execsql { I... (unsupported command, not transpiled)
	vtab.TclVarSet("TMPDBERROR", "", "1 {unable to open a temporary database file for storing temporary tables}")
	TMPDBERROR = "1 {unable to open a temporary database file for storing temporary tables}"
	_ = TMPDBERROR // suppress unused warning
	// do_faultsim_test 5 -faults oom-t* -prep {\n  faultsim_restore_and_reopen\n  execsql { PRAGM...} -... (unsupported command, not transpiled)
}
