// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package gencol1

import (
"github.com/pijalu/frigolite"
"github.com/pijalu/frigolite/internal/vtab"
"os"
"strconv"
"strings"
"testing"
)

func Test_gencol1(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var tn string
	_ = tn // pre-declared from TCL source
	var schema string
	_ = schema // pre-declared from TCL source
	var typelist string
	_ = typelist // pre-declared from TCL source
	var cnt string
	_ = cnt // pre-declared from TCL source
	var t1 string
	_ = t1 // pre-declared from TCL source
	var t2 string
	_ = t2 // pre-declared from TCL source
	var x1 string
	_ = x1 // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	{ // "gencol1-100"
		r = db.Query("\n  CREATE TABLE t0(c0 AS(TYPEOF(c1)), c1);\n  INSERT INTO t0(c1) VALUES(0);\n  CREATE TABLE t1(x AS (typeof(y)), y);\n  INSERT INTO t1 SELECT * FROM t0;\n  SELECT * FROM t1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t0(c0 AS(TYPEOF(c1)), c1);\n  INSERT INTO t0(c1) VALUES(0);\n  CREATE TABLE t1(x AS (typeof(y)), y);\n  INSERT INTO t1 SELECT * FROM t0;\n  SELECT * FROM t1;\n")
			return
		}
		got := flatten(r)
		want := "integer 0"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	// foreach {tn schema} "1 {\n   CREATE TABLE t1(\n     a INT,\n     b TEXT,\n     c ANY,\n     w INT GENERATED ALWAYS AS (a*10),\n     x TEXT AS (typeof(c)),\n     y TEXT AS (substr(b,a,a+2))\n   );\n  }\n2 {\n   CREATE TABLE t1(\n     w INT GENERATED ALWAYS AS (a*10),\n     x TEXT AS (typeof(c)),\n     y TEXT AS (substr(b,a,a+2)),\n     a INT,\n     b TEXT,\n     c ANY\n   );\n  }\n3 {\n   CREATE TABLE t1(\n     w INT GENERATED ALWAYS AS (a*10),\n     a INT,\n     x TEXT AS (typeof(c)) STORED,\n     b TEXT,\n     y TEXT AS (substr(b,a,a+2)),\n     c ANY\n   );\n  }\n4 {\n   CREATE TABLE t1(\n     a INTEGER PRIMARY KEY,\n     w INT GENERATED ALWAYS AS (a*10),\n     b TEXT,\n     x TEXT AS (typeof(c)),\n     y TEXT AS (substr(b,a,a+2)) STORED,\n     c ANY\n   );\n  }\n5 {\n   CREATE TABLE t1(\n     w INT GENERATED ALWAYS AS (a*10),\n     a INT,\n     x TEXT AS (typeof(c)),\n     b TEXT,\n     y TEXT AS (substr(b,a,a+2)) STORED,\n     c ANY,\n     PRIMARY KEY(a,b)\n   ) WITHOUT ROWID;\n  }\n6 {\n   CREATE TABLE t1(\n     w INT GENERATED ALWAYS AS (m*5),\n     m INT AS (a*2) STORED,\n     a INT,\n     x TEXT AS (typeof(c)),\n     b TEXT,\n     y TEXT AS (substr(b,m/2,m/2+2)) STORED,\n     c ANY,\n     PRIMARY KEY(a,b)\n   );\n  }\n7 {\n   CREATE TABLE t1(\n     w INT GENERATED ALWAYS AS (m*5),\n     m INT AS (a*2) NOT NULL,\n     a INT,\n     x TEXT AS (typeof(c)) CHECK (x<>'blank'),\n     b TEXT,\n     y TEXT AS (substr(b,m/2,m/2+2)) STORED,\n     c ANY,\n     PRIMARY KEY(b,a)\n   ) WITHOUT ROWID;\n  }"
	_items0 := tclSplitList("1 {\n   CREATE TABLE t1(\n     a INT,\n     b TEXT,\n     c ANY,\n     w INT GENERATED ALWAYS AS (a*10),\n     x TEXT AS (typeof(c)),\n     y TEXT AS (substr(b,a,a+2))\n   );\n  }\n2 {\n   CREATE TABLE t1(\n     w INT GENERATED ALWAYS AS (a*10),\n     x TEXT AS (typeof(c)),\n     y TEXT AS (substr(b,a,a+2)),\n     a INT,\n     b TEXT,\n     c ANY\n   );\n  }\n3 {\n   CREATE TABLE t1(\n     w INT GENERATED ALWAYS AS (a*10),\n     a INT,\n     x TEXT AS (typeof(c)) STORED,\n     b TEXT,\n     y TEXT AS (substr(b,a,a+2)),\n     c ANY\n   );\n  }\n4 {\n   CREATE TABLE t1(\n     a INTEGER PRIMARY KEY,\n     w INT GENERATED ALWAYS AS (a*10),\n     b TEXT,\n     x TEXT AS (typeof(c)),\n     y TEXT AS (substr(b,a,a+2)) STORED,\n     c ANY\n   );\n  }\n5 {\n   CREATE TABLE t1(\n     w INT GENERATED ALWAYS AS (a*10),\n     a INT,\n     x TEXT AS (typeof(c)),\n     b TEXT,\n     y TEXT AS (substr(b,a,a+2)) STORED,\n     c ANY,\n     PRIMARY KEY(a,b)\n   ) WITHOUT ROWID;\n  }\n6 {\n   CREATE TABLE t1(\n     w INT GENERATED ALWAYS AS (m*5),\n     m INT AS (a*2) STORED,\n     a INT,\n     x TEXT AS (typeof(c)),\n     b TEXT,\n     y TEXT AS (substr(b,m/2,m/2+2)) STORED,\n     c ANY,\n     PRIMARY KEY(a,b)\n   );\n  }\n7 {\n   CREATE TABLE t1(\n     w INT GENERATED ALWAYS AS (m*5),\n     m INT AS (a*2) NOT NULL,\n     a INT,\n     x TEXT AS (typeof(c)) CHECK (x<>'blank'),\n     b TEXT,\n     y TEXT AS (substr(b,m/2,m/2+2)) STORED,\n     c ANY,\n     PRIMARY KEY(b,a)\n   ) WITHOUT ROWID;\n  }")
	for _idx0 := 0; _idx0+2 <= len(_items0); _idx0 += 2 {
		tn := _items0[_idx0+0]
		_ = tn // suppress unused warning
		schema := _items0[_idx0+1]
		_ = schema // suppress unused warning
		_ = _idx0
			{
				var _catchErr error
				_ = _catchErr // suppress unused warning
				db.Close()
			}
			db, err = frigolite.Open("")
			if err != nil { t.Fatal(err) }
			_res = db.Exec(schema)
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, schema)
			}
			{ // "gencol1-2." + tn + ".100"
				r = db.Query("\n    INSERT INTO t1(a,b,c) VALUES(1,'abcdef',5.5),(3,'cantaloupe',NULL);\n    SELECT w, x, y, '|' FROM t1 ORDER BY a;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    INSERT INTO t1(a,b,c) VALUES(1,'abcdef',5.5),(3,'cantaloupe',NULL);\n    SELECT w, x, y, '|' FROM t1 ORDER BY a;\n  ")
					return
				}
				got := flatten(r)
				want := "10 real abc | 30 null ntalo |"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "gencol1-2." + tn + ".101"
				r = db.Query("\n    SELECT w, x, y, '|' FROM t1 ORDER BY w;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT w, x, y, '|' FROM t1 ORDER BY w;\n  ")
					return
				}
				got := flatten(r)
				want := "10 real abc | 30 null ntalo |"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "gencol1-2." + tn + ".102"
				r = db.Query("\n    SELECT a FROM t1 WHERE w=30;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t1 WHERE w=30;\n  ")
					return
				}
				got := flatten(r)
				want := "3"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "gencol1-2." + tn + ".103"
				r = db.Query("\n    SELECT a FROM t1 WHERE x='real';\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t1 WHERE x='real';\n  ")
					return
				}
				got := flatten(r)
				want := "1"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "gencol1-2." + tn + ".104"
				r = db.Query("\n    SELECT a FROM t1 WHERE y LIKE '%tal%' OR x='real' ORDER BY b;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t1 WHERE y LIKE '%tal%' OR x='real' ORDER BY b;\n  ")
					return
				}
				got := flatten(r)
				want := "1 3"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "gencol1-2." + tn + ".110"
				r = db.Query("\n    CREATE INDEX t1w ON t1(w);\n    SELECT a FROM t1 WHERE w=10;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    CREATE INDEX t1w ON t1(w);\n    SELECT a FROM t1 WHERE w=10;\n  ")
					return
				}
				got := flatten(r)
				want := "1"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "gencol1-2." + tn + ".120"
				r = db.Query("\n    CREATE INDEX t1x ON t1(x) WHERE w BETWEEN 20 AND 40;\n    SELECT a FROM t1 WHERE x='null' AND w BETWEEN 20 AND 40;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    CREATE INDEX t1x ON t1(x) WHERE w BETWEEN 20 AND 40;\n    SELECT a FROM t1 WHERE x='null' AND w BETWEEN 20 AND 40;\n  ")
					return
				}
				got := flatten(r)
				want := "3"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "gencol1-2." + tn + ".121"
				r = db.Query("\n    SELECT a FROM t1 WHERE x='real';\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t1 WHERE x='real';\n  ")
					return
				}
				got := flatten(r)
				want := "1"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "gencol1-2." + tn + ".130" — skipped: VACUUM not implemented (P8.VACUUM)
				_res = db.Exec("\n    VACUUM;\n    PRAGMA integrity_check;\n  ")
				_ = _res
			}
			{ // "gencol1-2." + tn + ".140"
				r = db.Query("\n    UPDATE t1 SET a=a+100 WHERE w<20;\n    SELECT a, w, '|' FROM t1 ORDER BY w;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    UPDATE t1 SET a=a+100 WHERE w<20;\n    SELECT a, w, '|' FROM t1 ORDER BY w;\n  ")
					return
				}
				got := flatten(r)
				want := "3 30 | 101 1010 |"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "gencol1-2." + tn + ".150"
				r = db.Query("\n    INSERT INTO t1 VALUES(4,'jambalaya','Chef John'),(15,87719874135,0);\n    SELECT w, x, y, '|' FROM t1 ORDER BY w;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    INSERT INTO t1 VALUES(4,'jambalaya','Chef John'),(15,87719874135,0);\n    SELECT w, x, y, '|' FROM t1 ORDER BY w;\n  ")
					return
				}
				got := flatten(r)
				want := "30 null ntalo | 40 text balaya | 150 integer {} | 1010 real {} |"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
		}
		db.Close()
		db, err = frigolite.Open("")
		if err != nil { t.Fatal(err) }
		{ // "gencol1-3.100"
			r = db.Query("\n  PRAGMA foreign_keys = true;\n  CREATE TABLE t0(c0 PRIMARY KEY, c1, c2 AS (c0+c1-c3) REFERENCES t0, c3);\n  INSERT INTO t0 VALUES (0, 0, 0), (11, 5, 5);\n  UPDATE t0 SET c1 = c0, c3 = c0;\n  SELECT *, '|' FROM t0 ORDER BY +c0;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  PRAGMA foreign_keys = true;\n  CREATE TABLE t0(c0 PRIMARY KEY, c1, c2 AS (c0+c1-c3) REFERENCES t0, c3);\n  INSERT INTO t0 VALUES (0, 0, 0), (11, 5, 5);\n  UPDATE t0 SET c1 = c0, c3 = c0;\n  SELECT *, '|' FROM t0 ORDER BY +c0;\n")
				return
			}
			got := flatten(r)
			want := "0 0 0 0 | 11 11 11 11 |"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-3.110"
			_res = db.Exec("\n  UPDATE t0 SET c1 = c0, c3 = c0+1;\n")
			if _res.Error == nil || !strings.Contains(_res.Error.Error(), "FOREIGN KEY constraint failed") {
				t.Errorf("expected error containing %q, got: %v\n  sql: %s", "FOREIGN KEY constraint failed", _res.Error, "\n  UPDATE t0 SET c1 = c0, c3 = c0+1;\n")
			}
		}
		db.Close()
		db, err = frigolite.Open("")
		if err != nil { t.Fatal(err) }
		{ // "gencol1-4.100"
			r = db.Query("\n  CREATE TABLE t0 (\n    c0,\n    c1 a UNIQUE AS (1),\n    c2,\n    c3 REFERENCES t0(c1)\n  );\n  PRAGMA foreign_keys = true;\n  INSERT INTO t0(c0,c2,c3) VALUES(0,0,1);\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t0 (\n    c0,\n    c1 a UNIQUE AS (1),\n    c2,\n    c3 REFERENCES t0(c1)\n  );\n  PRAGMA foreign_keys = true;\n  INSERT INTO t0(c0,c2,c3) VALUES(0,0,1);\n")
			}
		}
		{ // "gencol1-4.110" — skipped: REPLACE FK-vs-UNIQUE error ordering not implemented N-A (no-side-effects)
		}
		db.Close()
		db, err = frigolite.Open("")
		if err != nil { t.Fatal(err) }
		{ // "gencol1-5.100"
			r = db.Query("\n  PRAGMA foreign_keys=ON;\n  CREATE TABLE t1(\n    gcb AS (b*1),\n    a INTEGER PRIMARY KEY,\n    gcc AS (c+0),\n    b UNIQUE,\n    gca AS (1*a+0),\n    c UNIQUE\n  ) WITHOUT ROWID;\n  INSERT INTO t1 VALUES(1,2,3);\n  INSERT INTO t1 VALUES(4,5,6);\n  INSERT INTO t1 VALUES(7,8,9);\n  CREATE TABLE t1a(\n    gcx AS (x+0) REFERENCES t1(a) ON DELETE CASCADE,\n    id,\n    x,\n    gcid AS (1*id)\n  );\n  INSERT INTO t1a VALUES(1, 1);\n  INSERT INTO t1a VALUES(2, 4);\n  INSERT INTO t1a VALUES(3, 7);\n  DELETE FROM t1 WHERE b=5;\n  SELECT id,x,'|' FROM t1a ORDER BY id;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  PRAGMA foreign_keys=ON;\n  CREATE TABLE t1(\n    gcb AS (b*1),\n    a INTEGER PRIMARY KEY,\n    gcc AS (c+0),\n    b UNIQUE,\n    gca AS (1*a+0),\n    c UNIQUE\n  ) WITHOUT ROWID;\n  INSERT INTO t1 VALUES(1,2,3);\n  INSERT INTO t1 VALUES(4,5,6);\n  INSERT INTO t1 VALUES(7,8,9);\n  CREATE TABLE t1a(\n    gcx AS (x+0) REFERENCES t1(a) ON DELETE CASCADE,\n    id,\n    x,\n    gcid AS (1*id)\n  );\n  INSERT INTO t1a VALUES(1, 1);\n  INSERT INTO t1a VALUES(2, 4);\n  INSERT INTO t1a VALUES(3, 7);\n  DELETE FROM t1 WHERE b=5;\n  SELECT id,x,'|' FROM t1a ORDER BY id;\n")
				return
			}
			got := flatten(r)
			want := "1 1 | 3 7 |"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-6.10"
			_res = db.Exec("\n  DROP TABLE IF EXISTS t0;\n  CREATE TABLE t0(c0 NOT NULL AS(c1), c1);\n  REPLACE INTO t0(c1) VALUES(NULL);\n")
			if _res.Error == nil || !strings.Contains(_res.Error.Error(), "NOT NULL constraint failed: t0.c0") {
				t.Errorf("expected error containing %q, got: %v\n  sql: %s", "NOT NULL constraint failed: t0.c0", _res.Error, "\n  DROP TABLE IF EXISTS t0;\n  CREATE TABLE t0(c0 NOT NULL AS(c1), c1);\n  REPLACE INTO t0(c1) VALUES(NULL);\n")
			}
		}
		db.Close()
		os.Remove("test.db")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		{ // "gencol1-7.10"
			r = db.Query("\n  CREATE TABLE t0 (c0 GENERATED ALWAYS AS (1), c1 UNIQUE, c2 UNIQUE);\n  INSERT INTO t0(c1) VALUES (1);\n  SELECT quote(0 = t0.c2 OR t0.c1 BETWEEN t0.c2 AND 1) FROM t0;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t0 (c0 GENERATED ALWAYS AS (1), c1 UNIQUE, c2 UNIQUE);\n  INSERT INTO t0(c1) VALUES (1);\n  SELECT quote(0 = t0.c2 OR t0.c1 BETWEEN t0.c2 AND 1) FROM t0;\n")
				return
			}
			got := flatten(r)
			want := "NULL"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-7.11"
			r = db.Query("\n  DROP TABLE t0;\n  CREATE TABLE t0(c0 NOT NULL DEFAULT 'xyz', c1 AS(c0) NOT NULL);\n  REPLACE INTO t0(c0) VALUES(NULL);\n  SELECT * FROM t0;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE t0;\n  CREATE TABLE t0(c0 NOT NULL DEFAULT 'xyz', c1 AS(c0) NOT NULL);\n  REPLACE INTO t0(c0) VALUES(NULL);\n  SELECT * FROM t0;\n")
				return
			}
			got := flatten(r)
			want := "xyz xyz"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-7.12"
			r = db.Query("\n  DROP TABLE t0;\n  CREATE TABLE t0(c0 NOT NULL DEFAULT 'xyz', c1 AS(c0) STORED NOT NULL);\n  REPLACE INTO t0(c0) VALUES(NULL);\n  SELECT * FROM t0;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE t0;\n  CREATE TABLE t0(c0 NOT NULL DEFAULT 'xyz', c1 AS(c0) STORED NOT NULL);\n  REPLACE INTO t0(c0) VALUES(NULL);\n  SELECT * FROM t0;\n")
				return
			}
			got := flatten(r)
			want := "xyz xyz"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-7.20"
			r = db.Query("\n  CREATE TABLE t1(\n   a NOT NULL DEFAULT 'aaa',\n   b AS(c) NOT NULL,\n   c NOT NULL DEFAULT 'ccc');\n  REPLACE INTO t1(a,c) VALUES(NULL,NULL);\n  SELECT * FROM t1;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t1(\n   a NOT NULL DEFAULT 'aaa',\n   b AS(c) NOT NULL,\n   c NOT NULL DEFAULT 'ccc');\n  REPLACE INTO t1(a,c) VALUES(NULL,NULL);\n  SELECT * FROM t1;\n")
				return
			}
			got := flatten(r)
			want := "aaa ccc ccc"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-7.21"
			r = db.Query("\n  DROP TABLE t1;\n  CREATE TABLE t1(\n   a NOT NULL DEFAULT 'aaa',\n   b AS(c) STORED NOT NULL,\n   c NOT NULL DEFAULT 'ccc');\n  REPLACE INTO t1(a,c) VALUES(NULL,NULL);\n  SELECT * FROM t1;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE t1;\n  CREATE TABLE t1(\n   a NOT NULL DEFAULT 'aaa',\n   b AS(c) STORED NOT NULL,\n   c NOT NULL DEFAULT 'ccc');\n  REPLACE INTO t1(a,c) VALUES(NULL,NULL);\n  SELECT * FROM t1;\n")
				return
			}
			got := flatten(r)
			want := "aaa ccc ccc"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-7.30"
			r = db.Query("\n  CREATE TABLE t2(\n   a NOT NULL DEFAULT 'aaa',\n   b AS(a) NOT NULL,\n   c NOT NULL DEFAULT 'ccc');\n  REPLACE INTO t2(a,c) VALUES(NULL,NULL);\n  SELECT * FROM t2;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t2(\n   a NOT NULL DEFAULT 'aaa',\n   b AS(a) NOT NULL,\n   c NOT NULL DEFAULT 'ccc');\n  REPLACE INTO t2(a,c) VALUES(NULL,NULL);\n  SELECT * FROM t2;\n")
				return
			}
			got := flatten(r)
			want := "aaa aaa ccc"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-7.31"
			r = db.Query("\n  DROP TABLE t2;\n  CREATE TABLE t2(\n   a NOT NULL DEFAULT 'aaa',\n   b AS(a) STORED NOT NULL,\n   c NOT NULL DEFAULT 'ccc');\n  REPLACE INTO t2(a,c) VALUES(NULL,NULL);\n  SELECT * FROM t2;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE t2;\n  CREATE TABLE t2(\n   a NOT NULL DEFAULT 'aaa',\n   b AS(a) STORED NOT NULL,\n   c NOT NULL DEFAULT 'ccc');\n  REPLACE INTO t2(a,c) VALUES(NULL,NULL);\n  SELECT * FROM t2;\n")
				return
			}
			got := flatten(r)
			want := "aaa aaa ccc"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-7.40"
			r = db.Query("\n  CREATE TABLE t3(a NOT NULL DEFAULT 123, b AS(a) UNIQUE);\n  REPLACE INTO t3 VALUES(NULL);\n  SELECT * FROM t3;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t3(a NOT NULL DEFAULT 123, b AS(a) UNIQUE);\n  REPLACE INTO t3 VALUES(NULL);\n  SELECT * FROM t3;\n")
				return
			}
			got := flatten(r)
			want := "123 123"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-7.41"
			r = db.Query("\n  SELECT * FROM t3 WHERE b=123;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t3 WHERE b=123;\n")
				return
			}
			got := flatten(r)
			want := "123 123"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-7.50"
			r = db.Query("\n  CREATE TABLE t4(a NOT NULL DEFAULT 123, b AS(a*10+4) STORED UNIQUE);\n  REPLACE INTO t4 VALUES(NULL);\n  SELECT * FROM t4;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t4(a NOT NULL DEFAULT 123, b AS(a*10+4) STORED UNIQUE);\n  REPLACE INTO t4 VALUES(NULL);\n  SELECT * FROM t4;\n")
				return
			}
			got := flatten(r)
			want := "123 1234"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-7.51"
			r = db.Query("\n  SELECT * FROM t4 WHERE b=1234;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t4 WHERE b=1234;\n")
				return
			}
			got := flatten(r)
			want := "123 1234"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-8.10"
			r = db.Query("\n  DROP TABLE IF EXISTS t0;\n  CREATE TABLE t0(\n    c0 AS (('a', 9) < ('b', c1)),\n    c1 AS (1),\n    c2 CHECK (1 = c1)\n  );\n  INSERT INTO t0 VALUES (0),(99);\n  SELECT * FROM t0;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE IF EXISTS t0;\n  CREATE TABLE t0(\n    c0 AS (('a', 9) < ('b', c1)),\n    c1 AS (1),\n    c2 CHECK (1 = c1)\n  );\n  INSERT INTO t0 VALUES (0),(99);\n  SELECT * FROM t0;\n")
				return
			}
			got := flatten(r)
			want := "1 1 0 1 1 99"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-8.20"
			_res = db.Exec("\n  DROP TABLE IF EXISTS t0;\n  CREATE TABLE t0(\n    c0,\n    c1 AS(c0 + c2),\n    c2 AS(c1) CHECK(c2)\n  );\n  UPDATE t0 SET c0 = NULL;\n")
			if _res.Error == nil || !strings.Contains(_res.Error.Error(), "generated column loop on \"c2\"") {
				t.Errorf("expected error containing %q, got: %v\n  sql: %s", "generated column loop on \"c2\"", _res.Error, "\n  DROP TABLE IF EXISTS t0;\n  CREATE TABLE t0(\n    c0,\n    c1 AS(c0 + c2),\n    c2 AS(c1) CHECK(c2)\n  );\n  UPDATE t0 SET c0 = NULL;\n")
			}
		}
		db.Close()
		os.Remove("test.db")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		{ // "gencol1-9.10"
			r = db.Query("\n  PRAGMA foreign_keys=OFF;\n  CREATE TABLE t1(aa , bb AS (17) UNIQUE);\n  INSERT INTO t1 VALUES(17);\n  CREATE TABLE t2(cc);\n  INSERT INTO t2 VALUES(41);\n  SELECT * FROM t2 JOIN t1 WHERE t1.bb=t1.aa AND t1.bb=17;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  PRAGMA foreign_keys=OFF;\n  CREATE TABLE t1(aa , bb AS (17) UNIQUE);\n  INSERT INTO t1 VALUES(17);\n  CREATE TABLE t2(cc);\n  INSERT INTO t2 VALUES(41);\n  SELECT * FROM t2 JOIN t1 WHERE t1.bb=t1.aa AND t1.bb=17;\n")
				return
			}
			got := flatten(r)
			want := "41 17 17"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-9.20"
			r = db.Query("\n  CREATE TABLE t3(aa INT PRIMARY KEY, bb UNIQUE AS(aa));\n  INSERT INTO t3 VALUES(1);\n  SELECT 100, * FROM t3;\n  DELETE FROM t3 WHERE (SELECT bb FROM t3);\n  SELECT 200, * FROM t3;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t3(aa INT PRIMARY KEY, bb UNIQUE AS(aa));\n  INSERT INTO t3 VALUES(1);\n  SELECT 100, * FROM t3;\n  DELETE FROM t3 WHERE (SELECT bb FROM t3);\n  SELECT 200, * FROM t3;\n")
				return
			}
			got := flatten(r)
			want := "100 1 1"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		db, err = frigolite.Open("")
		if err != nil { t.Fatal(err) }
		{ // "gencol1-10.10"
			r = db.Query("\n  CREATE TABLE t1(aa,bb);\n  CREATE TABLE IF NOT EXISTS t1(aa, bb AS (aa+1));\n  PRAGMA integrity_check;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t1(aa,bb);\n  CREATE TABLE IF NOT EXISTS t1(aa, bb AS (aa+1));\n  PRAGMA integrity_check;\n")
				return
			}
			got := flatten(r)
			want := "ok"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		db, err = frigolite.Open("")
		if err != nil { t.Fatal(err) }
		{ // "gencol1-11.10"
			r = db.Query("\n  PRAGMA foreign_keys = true;\n  CREATE TABLE t0(\n    c0,\n    c1 INTEGER PRIMARY KEY,\n    c2 BLOB UNIQUE DEFAULT x'00',\n    c3 BLOB GENERATED ALWAYS AS (1), \n    FOREIGN KEY(c1) REFERENCES t0(c2)\n  );\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  PRAGMA foreign_keys = true;\n  CREATE TABLE t0(\n    c0,\n    c1 INTEGER PRIMARY KEY,\n    c2 BLOB UNIQUE DEFAULT x'00',\n    c3 BLOB GENERATED ALWAYS AS (1), \n    FOREIGN KEY(c1) REFERENCES t0(c2)\n  );\n")
			}
		}
		{ // "gencol1-11.20"
			_res = db.Exec("\n  INSERT OR REPLACE INTO t0(c0, c1) VALUES (2, 1), (1, 0)\n")
			if _res.Error == nil || !strings.Contains(_res.Error.Error(), "FOREIGN KEY constraint failed") {
				t.Errorf("expected error containing %q, got: %v\n  sql: %s", "FOREIGN KEY constraint failed", _res.Error, "\n  INSERT OR REPLACE INTO t0(c0, c1) VALUES (2, 1), (1, 0)\n")
			}
		}
		{ // "gencol1-11.30"
			_res = db.Exec("\n  DROP TABLE t0;\n  CREATE TABLE t0(\n    c0,\n    c1 INTEGER PRIMARY KEY,\n    c3 BLOB GENERATED ALWAYS AS (1), \n    c2 BLOB UNIQUE DEFAULT x'00',\n    FOREIGN KEY(c1) REFERENCES t0(c2)\n  );\n")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  DROP TABLE t0;\n  CREATE TABLE t0(\n    c0,\n    c1 INTEGER PRIMARY KEY,\n    c3 BLOB GENERATED ALWAYS AS (1), \n    c2 BLOB UNIQUE DEFAULT x'00',\n    FOREIGN KEY(c1) REFERENCES t0(c2)\n  );\n")
			}
		}
		{ // "gencol1-11.40"
			_res = db.Exec("\n  INSERT OR REPLACE INTO t0(c0, c1) VALUES (2, 1), (1, 0)\n")
			if _res.Error == nil || !strings.Contains(_res.Error.Error(), "FOREIGN KEY constraint failed") {
				t.Errorf("expected error containing %q, got: %v\n  sql: %s", "FOREIGN KEY constraint failed", _res.Error, "\n  INSERT OR REPLACE INTO t0(c0, c1) VALUES (2, 1), (1, 0)\n")
			}
		}
		{ // "gencol1-11.50"
			_res = db.Exec("\n  DROP TABLE t0;\n  CREATE TABLE t0(\n    c0,\n    c3 BLOB GENERATED ALWAYS AS (1), \n    c1 INTEGER PRIMARY KEY,\n    c2 BLOB UNIQUE DEFAULT x'00',\n    FOREIGN KEY(c1) REFERENCES t0(c2)\n  );\n")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  DROP TABLE t0;\n  CREATE TABLE t0(\n    c0,\n    c3 BLOB GENERATED ALWAYS AS (1), \n    c1 INTEGER PRIMARY KEY,\n    c2 BLOB UNIQUE DEFAULT x'00',\n    FOREIGN KEY(c1) REFERENCES t0(c2)\n  );\n")
			}
		}
		{ // "gencol1-11.60"
			_res = db.Exec("\n  INSERT OR REPLACE INTO t0(c0, c1) VALUES (2, 1), (1, 0)\n")
			if _res.Error == nil || !strings.Contains(_res.Error.Error(), "FOREIGN KEY constraint failed") {
				t.Errorf("expected error containing %q, got: %v\n  sql: %s", "FOREIGN KEY constraint failed", _res.Error, "\n  INSERT OR REPLACE INTO t0(c0, c1) VALUES (2, 1), (1, 0)\n")
			}
		}
		{ // "gencol1-11.70"
			_res = db.Exec("\n  DROP TABLE t0;\n  CREATE TABLE t0(\n    c3 BLOB GENERATED ALWAYS AS (1), \n    c0,\n    c1 INTEGER PRIMARY KEY,\n    c2 BLOB UNIQUE DEFAULT x'00',\n    FOREIGN KEY(c1) REFERENCES t0(c2)\n  );\n")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  DROP TABLE t0;\n  CREATE TABLE t0(\n    c3 BLOB GENERATED ALWAYS AS (1), \n    c0,\n    c1 INTEGER PRIMARY KEY,\n    c2 BLOB UNIQUE DEFAULT x'00',\n    FOREIGN KEY(c1) REFERENCES t0(c2)\n  );\n")
			}
		}
		{ // "gencol1-11.80"
			_res = db.Exec("\n  INSERT OR REPLACE INTO t0(c0, c1) VALUES (2, 1), (1, 0)\n")
			if _res.Error == nil || !strings.Contains(_res.Error.Error(), "FOREIGN KEY constraint failed") {
				t.Errorf("expected error containing %q, got: %v\n  sql: %s", "FOREIGN KEY constraint failed", _res.Error, "\n  INSERT OR REPLACE INTO t0(c0, c1) VALUES (2, 1), (1, 0)\n")
			}
		}
		db, err = frigolite.Open("")
		if err != nil { t.Fatal(err) }
		{ // "gencol1-12.10"
			r = db.Query("\n  CREATE TABLE t0 (c0, c1 NOT NULL AS (c0==0));\n  INSERT INTO t0(c0) VALUES (0);\n  PRAGMA integrity_check;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t0 (c0, c1 NOT NULL AS (c0==0));\n  INSERT INTO t0(c0) VALUES (0);\n  PRAGMA integrity_check;\n")
				return
			}
			got := flatten(r)
			want := "ok"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-13.10"
			r = db.Query("\n  CREATE TABLE t1(x, y AS(x+1));\n  INSERT INTO t1 VALUES(10);\n  SELECT y FROM t1 JOIN t1 USING (y,y);\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t1(x, y AS(x+1));\n  INSERT INTO t1 VALUES(10);\n  SELECT y FROM t1 JOIN t1 USING (y,y);\n")
				return
			}
			got := flatten(r)
			want := "11"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-13.11"
			r = db.Query("\n  SELECT 123 FROM t1 JOIN t1 USING (x);\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT 123 FROM t1 JOIN t1 USING (x);\n")
				return
			}
			got := flatten(r)
			want := "123"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-13.11"
			r = db.Query("\n  SELECT 456 FROM t1 JOIN t1 USING (x,x);\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT 456 FROM t1 JOIN t1 USING (x,x);\n")
				return
			}
			got := flatten(r)
			want := "456"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-13.20"
			r = db.Query("\n  CREATE INDEX t1y ON t1(y);\n  SELECT y FROM t1 JOIN t1 USING (y,y);\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE INDEX t1y ON t1(y);\n  SELECT y FROM t1 JOIN t1 USING (y,y);\n")
				return
			}
			got := flatten(r)
			want := "11"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-13.21"
			r = db.Query("\n  CREATE INDEX t1x ON t1(x);\n  SELECT 123 FROM t1 JOIN t1 USING (x);\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE INDEX t1x ON t1(x);\n  SELECT 123 FROM t1 JOIN t1 USING (x);\n")
				return
			}
			got := flatten(r)
			want := "123"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-13.22"
			r = db.Query("\n  SELECT 456 FROM t1 JOIN t1 USING (x,x);\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT 456 FROM t1 JOIN t1 USING (x,x);\n")
				return
			}
			got := flatten(r)
			want := "456"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		db, err = frigolite.Open("")
		if err != nil { t.Fatal(err) }
		{ // "gencol1-14.10"
			r = db.Query("\n  CREATE TABLE t0(c0 AS(1 >= 1), c1 UNIQUE AS(TYPEOF(c0)), c2);\n  INSERT INTO t0 VALUES(0);\n  REINDEX;\n  SELECT * FROM t0;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t0(c0 AS(1 >= 1), c1 UNIQUE AS(TYPEOF(c0)), c2);\n  INSERT INTO t0 VALUES(0);\n  REINDEX;\n  SELECT * FROM t0;\n")
				return
			}
			got := flatten(r)
			want := "1 integer 0"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-14.10"
			_res = db.Exec("\n  INSERT INTO t0 VALUES(2);\n")
			if _res.Error == nil || !strings.Contains(_res.Error.Error(), "UNIQUE constraint failed: t0.c1") {
				t.Errorf("expected error containing %q, got: %v\n  sql: %s", "UNIQUE constraint failed: t0.c1", _res.Error, "\n  INSERT INTO t0 VALUES(2);\n")
			}
		}
		db.Close()
		os.Remove("test.db")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		{ // "gencol1-15.10" — skipped: C-API db deserialize hexdb image not supported N-A (no-side-effects)
		}
		{ // "gencol1-15.20" — skipped: C-API db deserialize hexdb image not supported N-A (no-side-effects)
		}
		db.Close()
		os.Remove("test.db")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		{ // "gencol1-16.10" — skipped: C-API db deserialize hexdb image not supported N-A (no-side-effects)
		}
		{ // "gencol1-16.20" — skipped: C-API db deserialize hexdb image not supported N-A (no-side-effects)
		}
		{ // "gencol1-16.30" — skipped: C-API db deserialize hexdb image not supported N-A (no-side-effects)
		}
		{ // "gencol1-16.40" — skipped: C-API db deserialize hexdb image not supported N-A (no-side-effects)
		}
		db.Close()
		os.Remove("test.db")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		{ // "gencol1-17.10"
			r = db.Query("\n  CREATE TABLE t0(c0 REAL AS(1) UNIQUE, c1 INT);\n  INSERT INTO t0 VALUES('');\n  SELECT quote(c0), quote(c1) from t0;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t0(c0 REAL AS(1) UNIQUE, c1 INT);\n  INSERT INTO t0 VALUES('');\n  SELECT quote(c0), quote(c1) from t0;\n")
				return
			}
			got := flatten(r)
			want := "1.0 ''"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-17.20"
			r = db.Query("\n  SELECT *, (1 BETWEEN CAST(t0.c0 AS TEXT) AND t0.c0) FROM t0;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT *, (1 BETWEEN CAST(t0.c0 AS TEXT) AND t0.c0) FROM t0;\n")
				return
			}
			got := flatten(r)
			want := "1.0 {} 0"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-17.30"
			r = db.Query("\n  SELECT * FROM t0 WHERE (1 BETWEEN CAST(t0.c0 AS TEXT) AND t0.c0);\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t0 WHERE (1 BETWEEN CAST(t0.c0 AS TEXT) AND t0.c0);\n")
			}
		}
		{ // "gencol1-17.40"
			r = db.Query("\n  CREATE TABLE t1(a TEXT AS(b) COLLATE nocase, b TEXT, c INT, d DEFAULT 1);\n  INSERT INTO t1(b,c) VALUES('abc',11),('DEF',22),('ghi',33);\n  SELECT a FROM t1 WHERE b='DEF' AND a='def';\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t1(a TEXT AS(b) COLLATE nocase, b TEXT, c INT, d DEFAULT 1);\n  INSERT INTO t1(b,c) VALUES('abc',11),('DEF',22),('ghi',33);\n  SELECT a FROM t1 WHERE b='DEF' AND a='def';\n")
				return
			}
			got := flatten(r)
			want := "DEF"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-17.50"
			r = db.Query("\n  CREATE INDEX t1bca ON t1(b,c,a);\n  SELECT a FROM t1 WHERE b='DEF' AND a='def';\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE INDEX t1bca ON t1(b,c,a);\n  SELECT a FROM t1 WHERE b='DEF' AND a='def';\n")
				return
			}
			got := flatten(r)
			want := "DEF"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		db.Close()
		os.Remove("test.db")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		{ // "gencol1-18.10"
			r = db.Query("\n  CREATE TABLE t0(c0 UNIQUE AS(0), c1, c2);\n  INSERT INTO t0(c1) VALUES(0);\n  SELECT * FROM t0;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t0(c0 UNIQUE AS(0), c1, c2);\n  INSERT INTO t0(c1) VALUES(0);\n  SELECT * FROM t0;\n")
				return
			}
			got := flatten(r)
			want := "0 0 {}"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-18.20"
			r = db.Query("\n  UPDATE t0 SET c1=0, c2=0 WHERE c0>=0;\n  SELECT * FROM t0;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  UPDATE t0 SET c1=0, c2=0 WHERE c0>=0;\n  SELECT * FROM t0;\n")
				return
			}
			got := flatten(r)
			want := "0 0 0"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		db.Close()
		os.Remove("test.db")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		{ // "gencol1-19.10"
			_res = db.Exec("\n  CREATE TABLE t0(\n    c0 INT AS(2) UNIQUE,\n    c1 TEXT UNIQUE,\n    FOREIGN KEY(c0) REFERENCES t0(c1)\n  );\n  INSERT INTO t0(c1) VALUES(0.16334143182538696), (0);\n")
			if _res.Error == nil || !strings.Contains(_res.Error.Error(), "UNIQUE constraint failed: t0.c0") {
				t.Errorf("expected error containing %q, got: %v\n  sql: %s", "UNIQUE constraint failed: t0.c0", _res.Error, "\n  CREATE TABLE t0(\n    c0 INT AS(2) UNIQUE,\n    c1 TEXT UNIQUE,\n    FOREIGN KEY(c0) REFERENCES t0(c1)\n  );\n  INSERT INTO t0(c1) VALUES(0.16334143182538696), (0);\n")
			}
		}
		{ // "gencol1-20.1"
			r = db.Query("\n  CREATE TEMPORARY TABLE tab (\n    prim DATE PRIMARY KEY,\n    a INTEGER,\n    comp INTEGER AS (a),\n    b INTEGER,\n    x INTEGER\n  );\n  -- Add some data\n  INSERT INTO tab (prim, a, b) VALUES ('2001-01-01', 0, 0);\n  -- Check that each column is 0 like I expect\n  SELECT * FROM tab;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TEMPORARY TABLE tab (\n    prim DATE PRIMARY KEY,\n    a INTEGER,\n    comp INTEGER AS (a),\n    b INTEGER,\n    x INTEGER\n  );\n  -- Add some data\n  INSERT INTO tab (prim, a, b) VALUES ('2001-01-01', 0, 0);\n  -- Check that each column is 0 like I expect\n  SELECT * FROM tab;\n")
				return
			}
			got := flatten(r)
			want := "2001-01-01 0 0 0 {}"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "gencol1-20.2"
			r = db.Query("\n  -- Do an UPSERT on the b column\n  INSERT INTO tab (prim, b)\n  VALUES ('2001-01-01',5)\n      ON CONFLICT(prim) DO UPDATE SET  b=excluded.b;\n  -- Now b is NULL rather than 5\n  SELECT * FROM tab;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  -- Do an UPSERT on the b column\n  INSERT INTO tab (prim, b)\n  VALUES ('2001-01-01',5)\n      ON CONFLICT(prim) DO UPDATE SET  b=excluded.b;\n  -- Now b is NULL rather than 5\n  SELECT * FROM tab;\n")
				return
			}
			got := flatten(r)
			want := "2001-01-01 0 0 5 {}"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		db.Close()
		os.Remove("test.db")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		{ // "gencol1-21.1" — skipped: pragma_table_xinfo type for stray Always word is version-dependent N-A (no-side-effects)
		}
		db.Close()
		os.Remove("test.db")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		{ // "gencol1-22.1"
			r = db.Query("\n  CREATE TABLE t0(a PRIMARY KEY,b TEXT AS ('2') UNIQUE);\n  INSERT INTO t0(a) VALUES(2);\n  SELECT * FROM t0 AS x JOIN t0 AS y\n   WHERE x.b='2'\n     AND (y.a=2 OR (x.b LIKE '2*' AND y.a=x.b));\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t0(a PRIMARY KEY,b TEXT AS ('2') UNIQUE);\n  INSERT INTO t0(a) VALUES(2);\n  SELECT * FROM t0 AS x JOIN t0 AS y\n   WHERE x.b='2'\n     AND (y.a=2 OR (x.b LIKE '2*' AND y.a=x.b));\n")
				return
			}
			got := flatten(r)
			want := "2 2 2 2"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		vtab.TclVarSet("typelist", "", "ANY INT REAL BLOB TEXT {}")
		typelist = "ANY INT REAL BLOB TEXT {}"
		_ = typelist // suppress unused warning
		vtab.TclVarSet("cnt", "", "0")
		cnt = "0"
		_ = cnt // suppress unused warning
		for _, t1 := range tclSplitList(typelist) {
		_ = t1 // suppress unused warning
			for _, t2 := range tclSplitList(typelist) {
			_ = t2 // suppress unused warning
				// incr cnt 1
				{
					_n, _err := strconv.Atoi(cnt)
					if _err == nil {
						cnt = strconv.Itoa(_n + 1)
					}
				}
				_res = db.Exec("\n      DROP TABLE IF EXISTS t1;\n      CREATE TABLE t1(\n        x " + t1 + ",\n        a " + t2 + " AS (x) VIRTUAL,\n        b BLOB AS (x) VIRTUAL\n      );\n      CREATE INDEX x2 ON t1(a);\n      INSERT INTO t1(x) VALUES(NULL),('1'),(2),(3.5),('xyz');\n    ")
				if _res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n      DROP TABLE IF EXISTS t1;\n      CREATE TABLE t1(\n        x " + t1 + ",\n        a " + t2 + " AS (x) VIRTUAL,\n        b BLOB AS (x) VIRTUAL\n      );\n      CREATE INDEX x2 ON t1(a);\n      INSERT INTO t1(x) VALUES(NULL),('1'),(2),(3.5),('xyz');\n    ")
				}
				x1 = tclSort(tclDbOne(db, "db eval {SELECT typeof(b) FROM t1}"))
				_ = x1 // suppress unused warning
				{ // "gencol1-23.1." + cnt — skipped: do_test lsort db-eval error body not transpiled + EXPLAIN order N-A (no-side-effects)
				}
			}
		}
		{ // "gencol1-23.2" — skipped: EXPLAIN output ordering for INDEXED BY queries N-A (no-side-effects)
		}
		{ // "gencol1-23.3" — skipped: EXPLAIN output ordering for INDEXED BY queries N-A (no-side-effects)
		}
		{ // "gencol1-23.4" — skipped: EXPLAIN output ordering for INDEXED BY queries N-A (no-side-effects)
		}
		{ // "gencol1-23.5" — skipped: EXPLAIN output ordering for INDEXED BY queries N-A (no-side-effects)
		}
}
