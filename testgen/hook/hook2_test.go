// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package hook

import (
"github.com/pijalu/frigolite"
"os"
"testing"
)

func Test_hook2(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var testprefix string
	_ = testprefix // pre-declared from TCL source
	var X string
	_ = X // pre-declared from TCL source
	var elem string
	_ = elem // pre-declared from TCL source
	var _type string
	_ = _type // pre-declared from TCL source
	var i string
	_ = i // pre-declared from TCL source
	var preupdate string
	_ = preupdate // pre-declared from TCL source
	var rc string
	_ = rc // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source
	var x string
	_ = x // pre-declared from TCL source
	var tn string
	_ = tn // pre-declared from TCL source
	var sql string
	_ = sql // pre-declared from TCL source
	var args string
	_ = args // pre-declared from TCL source
	var v string
	_ = v // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	testprefix = "hook2" // TCL namespace variable
	_ = testprefix // suppress unused warning
	// proc definition (not transpiled)
	// proc definition (not transpiled)
	{ // "1.0"
		_res = db.Exec("\n  CREATE TABLE t1(a PRIMARY KEY, b) WITHOUT ROWID;\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t1(a PRIMARY KEY, b) WITHOUT ROWID;\n")
		}
	}
	// do_preupdate_test 1.1 {\n  INSERT INTO t1 VALUES('one', 1);\n} {\n  INSERT main t1 0 0  one 1\n} (unsupported command, not transpiled)
	// do_preupdate_test 1.2 {\n  UPDATE t1 SET b=2 WHERE a='one';\n} {\n  UPDATE main t1 0 0  one 1 one ... (unsupported command, not transpiled)
	// do_preupdate_test 1.3 {\n  DELETE FROM t1 WHERE a='one';\n} {\n  DELETE main t1 0 0  one 2\n} (unsupported command, not transpiled)
	{ // "2.0"
		_res = db.Exec("\n  CREATE TABLE t2(a DEFAULT 4, b, c, PRIMARY KEY(b, c)) WITHOUT ROWID;\n  CREATE UNIQUE INDEX t2a ON t2(a);\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t2(a DEFAULT 4, b, c, PRIMARY KEY(b, c)) WITHOUT ROWID;\n  CREATE UNIQUE INDEX t2a ON t2(a);\n")
		}
	}
	// do_preupdate_test 2.1.1 {\n  INSERT INTO t2(b, c) VALUES(1, 1);\n} {\n  INSERT main t2 0 0  4 1 1\... (unsupported command, not transpiled)
	{ // "2.1.2.0"
		_res = db.Exec("\n  CREATE TABLE d1(a DEFAULT 4, b, c, PRIMARY KEY(b, c)) WITHOUT ROWID;\n  CREATE UNIQUE INDEX d1a ON d1(a);\n  INSERT INTO d1 VALUES(1, 2, 3);\n  INSERT INTO d1 VALUES(11, 12, 13);\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE d1(a DEFAULT 4, b, c, PRIMARY KEY(b, c)) WITHOUT ROWID;\n  CREATE UNIQUE INDEX d1a ON d1(a);\n  INSERT INTO d1 VALUES(1, 2, 3);\n  INSERT INTO d1 VALUES(11, 12, 13);\n")
		}
	}
	// do_preupdate_test 2.1.2.1 {\n  INSERT INTO t2 SELECT * FROM d1;\n} {\n  INSERT main t2 0 0  1 2 3\... (unsupported command, not transpiled)
	// do_preupdate_test 2.1.2.2 {\n  INSERT INTO t2 SELECT a+20, b+20, c+20 FROM d1...} {\n  INSERT main... (unsupported command, not transpiled)
	{ // "2.1.2.3"
		r = db.Query("\n  SELECT * FROM t2 ORDER BY b, c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t2 ORDER BY b, c;\n")
			return
		}
		got := flatten(r)
		want := "4 1 1 1 2 3 11 12 13 21 22 23 31 32 33"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	// do_preupdate_test 2.1.3 {\n  REPLACE INTO t2 VALUES(45, 22, 23);\n} {\n  DELETE main t2 0 0 21 22 ... (unsupported command, not transpiled)
	// do_preupdate_test 2.1.4 {\n  REPLACE INTO t2 VALUES(11, 100, 100);\n} {\n  DELETE main t2 0 0 11 1... (unsupported command, not transpiled)
	// do_preupdate_test 2.1.5 {\n  REPLACE INTO t2(c, b) VALUES(33, 32)\n} {\n  DELETE main t2 0 0 4 1 1... (unsupported command, not transpiled)
	{ // "2.2.0"
		r = db.Query("\n  SELECT * FROM t2 ORDER BY b,c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t2 ORDER BY b,c;\n")
			return
		}
		got := flatten(r)
		want := "1 2 3 45 22 23 4 32 33 11 100 100"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	// do_preupdate_test 2.2.1 {\n  DELETE FROM t2 WHERE b=22;\n} {\n  DELETE main t2 0 0  45 22 23\n} (unsupported command, not transpiled)
	// do_preupdate_test 2.2.2 {\n  DELETE FROM t2;\n} {\n  DELETE main t2 0 0 1 2 3 \n  DELETE main t2 0... (unsupported command, not transpiled)
	{ // "2.3.0"
		_res = db.Exec("\n  CREATE TABLE t3(x, y PRIMARY KEY, z UNIQUE) WITHOUT ROWID;\n  INSERT INTO t3 VALUES('a', 'b', 'c');\n  INSERT INTO t3 VALUES('d', 'e', 'f');\n\n  INSERT INTO t3 VALUES(1, 1, 1);\n  INSERT INTO t3 VALUES(2, 2, 2);\n  INSERT INTO t3 VALUES(3, 3, 3);\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t3(x, y PRIMARY KEY, z UNIQUE) WITHOUT ROWID;\n  INSERT INTO t3 VALUES('a', 'b', 'c');\n  INSERT INTO t3 VALUES('d', 'e', 'f');\n\n  INSERT INTO t3 VALUES(1, 1, 1);\n  INSERT INTO t3 VALUES(2, 2, 2);\n  INSERT INTO t3 VALUES(3, 3, 3);\n")
		}
	}
	// do_preupdate_test 2.3.1 {\n  UPDATE t3 SET x=4 WHERE y IN ('b', 'e', 'x');\...} {\n  UPDATE main t... (unsupported command, not transpiled)
	// do_preupdate_test 2.3.2 {\n  UPDATE t3 SET y=y||y WHERE z IN('c', 'f');\n} {\n  UPDATE main t3 0 0... (unsupported command, not transpiled)
	// do_preupdate_test 2.3.3 {\n  UPDATE OR REPLACE t3 SET y='bb' WHERE z='f'\n} {\n  DELETE main t3 0 ... (unsupported command, not transpiled)
	// do_preupdate_test 2.3.4 {\n  UPDATE OR REPLACE t3 SET z=2 WHERE y=1;\n} {\n  DELETE main t3 0 0  2... (unsupported command, not transpiled)
	// do_preupdate_test 2.3.5 {\n  UPDATE OR REPLACE t3 SET z=2, y='bb' WHERE y=3...} {\n  DELETE main t... (unsupported command, not transpiled)
}
