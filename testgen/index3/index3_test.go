// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package index3

import (
"github.com/pijalu/frigolite"
"os"
"strings"
"testing"
)

func Test_index3(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	{ // do_test "index3-1.1"
		r = db.Query("\n    CREATE TABLE t1(a);\n    INSERT INTO t1 VALUES(1);\n    INSERT INTO t1 VALUES(1);\n    SELECT * FROM t1;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    CREATE TABLE t1(a);\n    INSERT INTO t1 VALUES(1);\n    INSERT INTO t1 VALUES(1);\n    SELECT * FROM t1;\n  ")
		}
	}
	{ // do_test "index3-1.2"
		_res = db.Exec("\n    BEGIN;\n    CREATE UNIQUE INDEX i1 ON t1(a);\n  ")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "UNIQUE constraint failed: t1.a") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "UNIQUE constraint failed: t1.a", _res.Error, "\n    BEGIN;\n    CREATE UNIQUE INDEX i1 ON t1(a);\n  ")
		}
	}
	{ // do_test "index3-1.3"
		_res = db.Exec("COMMIT")
		if _res.Error != nil {
			t.Errorf("expected success, got error: %v\n  sql: %s", _res.Error, "COMMIT")
		}
	}
	_res = db.Exec("PRAGMA integrity_check")
	if _res.Error != nil { t.Errorf("integrity check: %v", _res.Error) }
	{ // "index3-2.1"
		_res = db.Exec("\n  DROP TABLE t1;\n  CREATE TABLE t1(a, b, c, d, e, \n                  PRIMARY KEY('a'), UNIQUE('b' COLLATE nocase DESC));\n  CREATE INDEX t1c ON t1('c');\n  CREATE INDEX t1d ON t1('d' COLLATE binary ASC);\n  WITH RECURSIVE c(x) AS (VALUES(1) UNION SELECT x+1 FROM c WHERE x<30)\n    INSERT INTO t1(a,b,c,d,e) \n      SELECT x, printf('ab%03xxy',x), x, x, x FROM c;\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  DROP TABLE t1;\n  CREATE TABLE t1(a, b, c, d, e, \n                  PRIMARY KEY('a'), UNIQUE('b' COLLATE nocase DESC));\n  CREATE INDEX t1c ON t1('c');\n  CREATE INDEX t1d ON t1('d' COLLATE binary ASC);\n  WITH RECURSIVE c(x) AS (VALUES(1) UNION SELECT x+1 FROM c WHERE x<30)\n    INSERT INTO t1(a,b,c,d,e) \n      SELECT x, printf('ab%03xxy',x), x, x, x FROM c;\n")
		}
	}
	{ // "index3-2.2"
		r = db.Query("\n  SELECT a FROM t1 WHERE b='ab005xy' COLLATE nocase;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT a FROM t1 WHERE b='ab005xy' COLLATE nocase;\n")
			return
		}
		got := flatten(r)
		want := "5"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "index3-2.2eqp" — skipped: EXPLAIN QUERY PLAN USING INDEX not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT a FROM t1 WHERE b='ab005xy' COLLATE nocase;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "index3-2.3"
		r = db.Query("\n  SELECT name FROM sqlite_master WHERE tbl_name='t1' ORDER BY name\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT name FROM sqlite_master WHERE tbl_name='t1' ORDER BY name\n")
			return
		}
		got := flatten(r)
		want := "sqlite_autoindex_t1_1 sqlite_autoindex_t1_2 t1 t1c t1d"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "index3-2.4"
		_res = db.Exec("\n  CREATE TABLE t2a(a integer, b, PRIMARY KEY(a));\n  CREATE TABLE t2b(\"a\" integer, b, PRIMARY KEY(\"a\"));\n  CREATE TABLE t2c([a] integer, b, PRIMARY KEY([a]));\n  CREATE TABLE t2d('a' integer, b, PRIMARY KEY('a'));\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t2a(a integer, b, PRIMARY KEY(a));\n  CREATE TABLE t2b(\"a\" integer, b, PRIMARY KEY(\"a\"));\n  CREATE TABLE t2c([a] integer, b, PRIMARY KEY([a]));\n  CREATE TABLE t2d('a' integer, b, PRIMARY KEY('a'));\n")
		}
	}
	{ // "index3-2.5"
		r = db.Query("\n  SELECT name FROM sqlite_master WHERE tbl_name LIKE 't2_' ORDER BY name\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT name FROM sqlite_master WHERE tbl_name LIKE 't2_' ORDER BY name\n")
			return
		}
		got := flatten(r)
		want := "t2a t2b t2c t2d"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "index3-99.1" (prepare-step internals; SQL side effects only)
		db.SetDefensive(false)
		r = db.Query("\n    PRAGMA writable_schema=on;\n    UPDATE sqlite_master SET sql='nonsense' WHERE name='t1d'\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    PRAGMA writable_schema=on;\n    UPDATE sqlite_master SET sql='nonsense' WHERE name='t1d'\n  ")
		}
		db.Close()
		{
			var _catchErr error
			_ = _catchErr // suppress unused warning
			db, err = frigolite.Open("test.db")
			if err != nil { t.Fatal(err) }
		}
		_res = db.Exec(" DROP INDEX t1c ")
		_ = _res // catchsql
	}
}
