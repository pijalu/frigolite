...
func Test_index6(t *testing.T) {
	...
	{ // do_test "index6-1.15"
		...
		got := flatten(r)
		want := "t1a 10 1 t1b 8 1 t1c 15 1 ok"
		if got != want && !tclFpnumCompare(got, want) {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	...
	{ // do_test "index6-2.2"
		...
		got := flatten(r)
		wantPattern := "(SEARCH|SCAN) t2 USING INDEX t2a1"
	...
	{ // "index6-5.0" — skipped: ANALYZE sqlite_stat1 stat not matched (G5.ANALYZE) (SQL side effects only)
	...
	{ // "index6-7.4" — skipped: EXPLAIN QUERY PLAN USING COVERING INDEX not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT * FROM t7a JOIN t7b ON (x=99) ORDER BY x;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "index6-8.0"
	...
	{ // "index6-11.1" — skipped: EXPLAIN QUERY PLAN USING INDEX not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  CREATE TABLE t11(a,b,c);\n  CREATE INDEX t11x ON t11(a) WHERE b<>99;\n  EXPLAIN QUERY PLAN SELECT a FROM t11 WHERE b<>99;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "index6-11.2" — skipped: EXPLAIN QUERY PLAN USING INDEX not planned (G5.EXPLAIN) (SQL side effects only)
