// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package indexA

import (
"github.com/pijalu/frigolite"
"github.com/pijalu/frigolite/internal/vtab"
"os"
"strings"
"testing"
)

func Test_indexA(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var testprefix string
	_ = testprefix // pre-declared from TCL source
	var tn string
	_ = tn // pre-declared from TCL source
	var idx string
	_ = idx // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source
	var lhs string
	_ = lhs // pre-declared from TCL source
	var rhs string
	_ = rhs // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	vtab.TclVarSet("testprefix", "", "indexA")
	testprefix = "indexA"
	_ = testprefix // suppress unused warning
	{ // "1.0"
		_res = db.Exec("\n  CREATE TABLE t1(a TEXT, b, c);\n  CREATE INDEX i1 ON t1(b, c) WHERE a='abc';\n  INSERT INTO t1 VALUES('abc', 1, 2);\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t1(a TEXT, b, c);\n  CREATE INDEX i1 ON t1(b, c) WHERE a='abc';\n  INSERT INTO t1 VALUES('abc', 1, 2);\n")
		}
	}
	{ // "1.1"
		r = db.Query("\n  SELECT * FROM t1 WHERE a='abc'\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t1 WHERE a='abc'\n")
			return
		}
		got := flatten(r)
		want := "abc 1 2"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "1.2"
		r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT * FROM t1 WHERE a='abc'\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT * FROM t1 WHERE a='abc'\n")
		}
	}
	{ // "1.3"
		r = db.Query("\n  CREATE INDEX i2 ON t1(b, c) WHERE a=5;\n  INSERT INTO t1 VALUES(5, 4, 3);\n\n  SELECT a, typeof(a), b, c FROM t1 WHERE a=5;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE INDEX i2 ON t1(b, c) WHERE a=5;\n  INSERT INTO t1 VALUES(5, 4, 3);\n\n  SELECT a, typeof(a), b, c FROM t1 WHERE a=5;\n")
			return
		}
		got := flatten(r)
		want := "5 text 4 3"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "1.4"
		_res = db.Exec("\n  CREATE TABLE t2(x);\n  INSERT INTO t2 VALUES('v');\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t2(x);\n  INSERT INTO t2 VALUES('v');\n")
		}
	}
	{ // "1.5"
		r = db.Query("\n  SELECT x, a, b, c FROM t2 LEFT JOIN t1 ON (a=5 AND b=x)\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT x, a, b, c FROM t2 LEFT JOIN t1 ON (a=5 AND b=x)\n")
			return
		}
		got := flatten(r)
		want := "v {} {} {}"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "1.6"
		r = db.Query("\n  SELECT x, a, b, c FROM t2 RIGHT JOIN t1 ON (t1.a=5 AND t1.b=t2.x)\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT x, a, b, c FROM t2 RIGHT JOIN t1 ON (t1.a=5 AND t1.b=t2.x)\n")
			return
		}
		got := flatten(r)
		want := "{} abc 1 2 {} 5 4 3"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "1.7"
		r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT x, a, b, c FROM t2 RIGHT JOIN t1 ON (t1.a=5 AND t1.b=t2.x)\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT x, a, b, c FROM t2 RIGHT JOIN t1 ON (t1.a=5 AND t1.b=t2.x)\n")
		}
	}
	db.Close()
	os.Remove("test.db")
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	tcl_nullvalue = "{}" // fresh connection resets nullvalue
	{ // "2.0"
		_res = db.Exec("\n  CREATE TABLE x1(a TEXT, b, c);\n  INSERT INTO x1 VALUES('2', 'two', 'ii');\n  INSERT INTO x1 VALUES('2.0', 'twopointoh', 'ii.0');\n\n  CREATE TABLE x2(a NUMERIC, b, c);\n  INSERT INTO x2 VALUES('2', 'two', 'ii');\n  INSERT INTO x2 VALUES('2.0', 'twopointoh', 'ii.0');\n\n  CREATE TABLE x3(a REAL, b, c);\n  INSERT INTO x3 VALUES('2', 'two', 'ii');\n  INSERT INTO x3 VALUES('2.0', 'twopointoh', 'ii.0');\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE x1(a TEXT, b, c);\n  INSERT INTO x1 VALUES('2', 'two', 'ii');\n  INSERT INTO x1 VALUES('2.0', 'twopointoh', 'ii.0');\n\n  CREATE TABLE x2(a NUMERIC, b, c);\n  INSERT INTO x2 VALUES('2', 'two', 'ii');\n  INSERT INTO x2 VALUES('2.0', 'twopointoh', 'ii.0');\n\n  CREATE TABLE x3(a REAL, b, c);\n  INSERT INTO x3 VALUES('2', 'two', 'ii');\n  INSERT INTO x3 VALUES('2.0', 'twopointoh', 'ii.0');\n")
		}
	}
	// foreach {tn idx} "0 {\n  }\n  1 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a=2;\n    CREATE INDEX i2 ON x2(b, c) WHERE a=2;\n    CREATE INDEX i3 ON x3(b, c) WHERE a=2;\n  }\n  2 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a=2.0;\n    CREATE INDEX i2 ON x2(b, c) WHERE a=2.0;\n    CREATE INDEX i3 ON x3(b, c) WHERE a=2.0;\n  }\n  3 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a='2.0';\n    CREATE INDEX i2 ON x2(b, c) WHERE a='2.0';\n    CREATE INDEX i3 ON x3(b, c) WHERE a='2.0';\n  }\n  4 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a='2';\n    CREATE INDEX i2 ON x2(b, c) WHERE a='2';\n    CREATE INDEX i3 ON x3(b, c) WHERE a='2';\n  }"
	_items0 := tclSplitList("0 {\n  }\n  1 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a=2;\n    CREATE INDEX i2 ON x2(b, c) WHERE a=2;\n    CREATE INDEX i3 ON x3(b, c) WHERE a=2;\n  }\n  2 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a=2.0;\n    CREATE INDEX i2 ON x2(b, c) WHERE a=2.0;\n    CREATE INDEX i3 ON x3(b, c) WHERE a=2.0;\n  }\n  3 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a='2.0';\n    CREATE INDEX i2 ON x2(b, c) WHERE a='2.0';\n    CREATE INDEX i3 ON x3(b, c) WHERE a='2.0';\n  }\n  4 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a='2';\n    CREATE INDEX i2 ON x2(b, c) WHERE a='2';\n    CREATE INDEX i3 ON x3(b, c) WHERE a='2';\n  }")
	for _idx0 := 0; _idx0+2 <= len(_items0); _idx0 += 2 {
		tn := _items0[_idx0+0]
		_ = tn // suppress unused warning
		idx := _items0[_idx0+1]
		_ = idx // suppress unused warning
		_ = _idx0
			_res = db.Exec(" DROP INDEX IF EXISTS i1 ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " DROP INDEX IF EXISTS i1 ")
			}
			_res = db.Exec(" DROP INDEX IF EXISTS i2 ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " DROP INDEX IF EXISTS i2 ")
			}
			_res = db.Exec(" DROP INDEX IF EXISTS i3 ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " DROP INDEX IF EXISTS i3 ")
			}
			_res = db.Exec(idx)
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, idx)
			}
			{ // "2.1." + tn + ".1"
				r = db.Query("\n    SELECT *, typeof(a) FROM x1 WHERE a=2\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT *, typeof(a) FROM x1 WHERE a=2\n  ")
					return
				}
				got := flatten(r)
				want := "2 two ii text"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "2.1." + tn + ".2"
				r = db.Query("\n    SELECT *, typeof(a) FROM x1 WHERE a=2.0\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT *, typeof(a) FROM x1 WHERE a=2.0\n  ")
					return
				}
				got := flatten(r)
				want := "2.0 twopointoh ii.0 text"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "2.1." + tn + ".3"
				r = db.Query("\n    SELECT *, typeof(a) FROM x1 WHERE a='2'\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT *, typeof(a) FROM x1 WHERE a='2'\n  ")
					return
				}
				got := flatten(r)
				want := "2 two ii text"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "2.1." + tn + ".4"
				r = db.Query("\n    SELECT *, typeof(a) FROM x1 WHERE a='2.0'\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT *, typeof(a) FROM x1 WHERE a='2.0'\n  ")
					return
				}
				got := flatten(r)
				want := "2.0 twopointoh ii.0 text"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "2.1." + tn + ".5"
				r = db.Query("\n    SELECT *, typeof(a) FROM x2 WHERE a=2\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT *, typeof(a) FROM x2 WHERE a=2\n  ")
					return
				}
				got := flatten(r)
				want := "2 two ii integer 2 twopointoh ii.0 integer"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "2.1." + tn + ".6"
				r = db.Query("\n    SELECT *, typeof(a) FROM x2 WHERE a=2.0\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT *, typeof(a) FROM x2 WHERE a=2.0\n  ")
					return
				}
				got := flatten(r)
				want := "2 two ii integer 2 twopointoh ii.0 integer"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "2.1." + tn + ".7"
				r = db.Query("\n    SELECT *, typeof(a) FROM x2 WHERE a='2'\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT *, typeof(a) FROM x2 WHERE a='2'\n  ")
					return
				}
				got := flatten(r)
				want := "2 two ii integer 2 twopointoh ii.0 integer"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "2.1." + tn + ".8"
				r = db.Query("\n    SELECT *, typeof(a) FROM x2 WHERE a='2.0'\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT *, typeof(a) FROM x2 WHERE a='2.0'\n  ")
					return
				}
				got := flatten(r)
				want := "2 two ii integer 2 twopointoh ii.0 integer"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "2.1." + tn + ".9"
				r = db.Query("\n    SELECT *, typeof(a) FROM x3 WHERE a=2\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT *, typeof(a) FROM x3 WHERE a=2\n  ")
					return
				}
				got := flatten(r)
				want := "2.0 two ii real 2.0 twopointoh ii.0 real"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "2.1." + tn + ".10"
				r = db.Query("\n    SELECT *, typeof(a) FROM x3 WHERE a=2.0\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT *, typeof(a) FROM x3 WHERE a=2.0\n  ")
					return
				}
				got := flatten(r)
				want := "2.0 two ii real 2.0 twopointoh ii.0 real"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "2.1." + tn + ".11"
				r = db.Query("\n    SELECT *, typeof(a) FROM x3 WHERE a='2'\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT *, typeof(a) FROM x3 WHERE a='2'\n  ")
					return
				}
				got := flatten(r)
				want := "2.0 two ii real 2.0 twopointoh ii.0 real"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "2.1." + tn + ".12"
				r = db.Query("\n    SELECT *, typeof(a) FROM x3 WHERE a='2.0'\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT *, typeof(a) FROM x3 WHERE a='2.0'\n  ")
					return
				}
				got := flatten(r)
				want := "2.0 two ii real 2.0 twopointoh ii.0 real"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
		}
		db.Close()
		os.Remove("test.db")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		{ // "3.0"
			_res = db.Exec("\n  CREATE TABLE x1(a TEXT, d PRIMARY KEY, b, c) WITHOUT ROWID;\n  INSERT INTO x1 VALUES('2', 1, 'two', 'ii');\n  INSERT INTO x1 VALUES('2.0', 2, 'twopointoh', 'ii.0');\n\n  CREATE TABLE x2(a NUMERIC, b, c, d PRIMARY KEY) WITHOUT ROWID;\n  INSERT INTO x2 VALUES('2', 'two', 'ii', 1);\n  INSERT INTO x2 VALUES('2.0', 'twopointoh', 'ii.0', 2);\n\n  CREATE TABLE x3(d PRIMARY KEY, a REAL, b, c) WITHOUT ROWID;\n  INSERT INTO x3 VALUES(34, '2', 'two', 'ii');\n  INSERT INTO x3 VALUES(35, '2.0', 'twopointoh', 'ii.0');\n")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE x1(a TEXT, d PRIMARY KEY, b, c) WITHOUT ROWID;\n  INSERT INTO x1 VALUES('2', 1, 'two', 'ii');\n  INSERT INTO x1 VALUES('2.0', 2, 'twopointoh', 'ii.0');\n\n  CREATE TABLE x2(a NUMERIC, b, c, d PRIMARY KEY) WITHOUT ROWID;\n  INSERT INTO x2 VALUES('2', 'two', 'ii', 1);\n  INSERT INTO x2 VALUES('2.0', 'twopointoh', 'ii.0', 2);\n\n  CREATE TABLE x3(d PRIMARY KEY, a REAL, b, c) WITHOUT ROWID;\n  INSERT INTO x3 VALUES(34, '2', 'two', 'ii');\n  INSERT INTO x3 VALUES(35, '2.0', 'twopointoh', 'ii.0');\n")
			}
		}
		// foreach {tn idx} "0 {\n  }\n  1 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a=2;\n    CREATE INDEX i2 ON x2(b, c) WHERE a=2;\n    CREATE INDEX i3 ON x3(b, c) WHERE a=2;\n  }\n  2 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a=2.0;\n    CREATE INDEX i2 ON x2(b, c) WHERE a=2.0;\n    CREATE INDEX i3 ON x3(b, c) WHERE a=2.0;\n  }\n  3 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a='2.0';\n    CREATE INDEX i2 ON x2(b, c) WHERE a='2.0';\n    CREATE INDEX i3 ON x3(b, c) WHERE a='2.0';\n  }\n  4 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a='2';\n    CREATE INDEX i2 ON x2(b, c) WHERE a='2';\n    CREATE INDEX i3 ON x3(b, c) WHERE a='2';\n  }"
		_items1 := tclSplitList("0 {\n  }\n  1 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a=2;\n    CREATE INDEX i2 ON x2(b, c) WHERE a=2;\n    CREATE INDEX i3 ON x3(b, c) WHERE a=2;\n  }\n  2 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a=2.0;\n    CREATE INDEX i2 ON x2(b, c) WHERE a=2.0;\n    CREATE INDEX i3 ON x3(b, c) WHERE a=2.0;\n  }\n  3 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a='2.0';\n    CREATE INDEX i2 ON x2(b, c) WHERE a='2.0';\n    CREATE INDEX i3 ON x3(b, c) WHERE a='2.0';\n  }\n  4 {\n    CREATE INDEX i1 ON x1(b, c) WHERE a='2';\n    CREATE INDEX i2 ON x2(b, c) WHERE a='2';\n    CREATE INDEX i3 ON x3(b, c) WHERE a='2';\n  }")
		for _idx1 := 0; _idx1+2 <= len(_items1); _idx1 += 2 {
			tn := _items1[_idx1+0]
			_ = tn // suppress unused warning
			idx := _items1[_idx1+1]
			_ = idx // suppress unused warning
			_ = _idx1
				_res = db.Exec(" DROP INDEX IF EXISTS i1 ")
				if _res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", _res.Error, " DROP INDEX IF EXISTS i1 ")
				}
				_res = db.Exec(" DROP INDEX IF EXISTS i2 ")
				if _res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", _res.Error, " DROP INDEX IF EXISTS i2 ")
				}
				_res = db.Exec(" DROP INDEX IF EXISTS i3 ")
				if _res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", _res.Error, " DROP INDEX IF EXISTS i3 ")
				}
				_res = db.Exec(idx)
				if _res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", _res.Error, idx)
				}
				{ // "3.1." + tn + ".1"
					r = db.Query("\n    SELECT a, b, c, typeof(a) FROM x1 WHERE a=2\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a, b, c, typeof(a) FROM x1 WHERE a=2\n  ")
						return
					}
					got := flatten(r)
					want := "2 two ii text"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // "3.1." + tn + ".2"
					r = db.Query("\n    SELECT a, b, c, typeof(a) FROM x1 WHERE a=2.0\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a, b, c, typeof(a) FROM x1 WHERE a=2.0\n  ")
						return
					}
					got := flatten(r)
					want := "2.0 twopointoh ii.0 text"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // "3.1." + tn + ".3"
					r = db.Query("\n    SELECT a, b, c, typeof(a) FROM x1 WHERE a='2'\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a, b, c, typeof(a) FROM x1 WHERE a='2'\n  ")
						return
					}
					got := flatten(r)
					want := "2 two ii text"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // "3.1." + tn + ".4"
					r = db.Query("\n    SELECT a, b, c, typeof(a) FROM x1 WHERE a='2.0'\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a, b, c, typeof(a) FROM x1 WHERE a='2.0'\n  ")
						return
					}
					got := flatten(r)
					want := "2.0 twopointoh ii.0 text"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // "3.1." + tn + ".5"
					r = db.Query("\n    SELECT a, b, c, typeof(a) FROM x2 WHERE a=2\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a, b, c, typeof(a) FROM x2 WHERE a=2\n  ")
						return
					}
					got := flatten(r)
					want := "2 two ii integer 2 twopointoh ii.0 integer"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // "3.1." + tn + ".6"
					r = db.Query("\n    SELECT a, b, c, typeof(a) FROM x2 WHERE a=2.0\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a, b, c, typeof(a) FROM x2 WHERE a=2.0\n  ")
						return
					}
					got := flatten(r)
					want := "2 two ii integer 2 twopointoh ii.0 integer"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // "3.1." + tn + ".7"
					r = db.Query("\n    SELECT a, b, c, typeof(a) FROM x2 WHERE a='2'\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a, b, c, typeof(a) FROM x2 WHERE a='2'\n  ")
						return
					}
					got := flatten(r)
					want := "2 two ii integer 2 twopointoh ii.0 integer"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // "3.1." + tn + ".8"
					r = db.Query("\n    SELECT a, b, c, typeof(a) FROM x2 WHERE a='2.0'\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a, b, c, typeof(a) FROM x2 WHERE a='2.0'\n  ")
						return
					}
					got := flatten(r)
					want := "2 two ii integer 2 twopointoh ii.0 integer"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // "3.1." + tn + ".9"
					r = db.Query("\n    SELECT a, b, c, typeof(a) FROM x3 WHERE a=2\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a, b, c, typeof(a) FROM x3 WHERE a=2\n  ")
						return
					}
					got := flatten(r)
					want := "2.0 two ii real 2.0 twopointoh ii.0 real"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // "3.1." + tn + ".10"
					r = db.Query("\n    SELECT a, b, c, typeof(a) FROM x3 WHERE a=2.0\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a, b, c, typeof(a) FROM x3 WHERE a=2.0\n  ")
						return
					}
					got := flatten(r)
					want := "2.0 two ii real 2.0 twopointoh ii.0 real"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // "3.1." + tn + ".11"
					r = db.Query("\n    SELECT a, b, c, typeof(a) FROM x3 WHERE a='2'\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a, b, c, typeof(a) FROM x3 WHERE a='2'\n  ")
						return
					}
					got := flatten(r)
					want := "2.0 two ii real 2.0 twopointoh ii.0 real"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // "3.1." + tn + ".12"
					r = db.Query("\n    SELECT a, b, c, typeof(a) FROM x3 WHERE a='2.0'\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a, b, c, typeof(a) FROM x3 WHERE a='2.0'\n  ")
						return
					}
					got := flatten(r)
					want := "2.0 two ii real 2.0 twopointoh ii.0 real"
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
			}
			db.Close()
			os.Remove("test.db")
			db, err = frigolite.Open("test.db")
			if err != nil { t.Fatal(err) }
			tcl_nullvalue = "{}" // fresh connection resets nullvalue
			{ // "4.0"
				_res = db.Exec("\n  CREATE TABLE t2(a INTEGER, b TEXT);\n  INSERT INTO t2 VALUES(1, 'two');\n  INSERT INTO t2 VALUES(2, 'two');\n  INSERT INTO t2 VALUES(3, 'two');\n  INSERT INTO t2 VALUES(1, 'three');\n  INSERT INTO t2 VALUES(2, 'three');\n  INSERT INTO t2 VALUES(3, 'three');\n\n  CREATE INDEX t2a_two ON t2(a) WHERE b='two';\n")
				if _res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t2(a INTEGER, b TEXT);\n  INSERT INTO t2 VALUES(1, 'two');\n  INSERT INTO t2 VALUES(2, 'two');\n  INSERT INTO t2 VALUES(3, 'two');\n  INSERT INTO t2 VALUES(1, 'three');\n  INSERT INTO t2 VALUES(2, 'three');\n  INSERT INTO t2 VALUES(3, 'three');\n\n  CREATE INDEX t2a_two ON t2(a) WHERE b='two';\n")
				}
			}
			{ // "4.1.1"
				r = db.Query("\n  SELECT sum(a), b FROM t2 WHERE b='two'\n")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT sum(a), b FROM t2 WHERE b='two'\n")
					return
				}
				got := flatten(r)
				want := "6 two"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "4.1.2"
				r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT sum(a), b FROM t2 WHERE b='two'\n")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT sum(a), b FROM t2 WHERE b='two'\n")
				}
			}
			db.Close()
			os.Remove("test.db")
			db, err = frigolite.Open("test.db")
			if err != nil { t.Fatal(err) }
			tcl_nullvalue = "{}" // fresh connection resets nullvalue
			{ // "indexA-5.0" — skipped: unknown type PRIMQRY stored verbatim by SQLite (type-noise, table still created) (SQL side effects only)
				_res = db.Exec("\n  CREATE TABLE t1(a INTEGER PRIMQRY KEY, b, c);\n")
				_ = _res.Error // tolerate unsupported-feature errors in skipped tests
			}
			{ // "indexA-5.1" — skipped: depends on 5.0 PRIMQRY table creation
			}
			// proc xyz collation (registered via db collate)
			db.RegisterCollation("xyz", func(a, b string) int { return strings.Compare(a, b) })
			{ // "indexA-5.2" — skipped: depends on 5.0 PRIMQRY table creation (xyz collation index) (SQL side effects only)
				_res = db.Exec("\n  CREATE INDEX ex1 ON t1(c) WHERE b IS 'abc' COLLATE xyz;\n")
				_ = _res.Error // tolerate unsupported-feature errors in skipped tests
			}
			db.Close()
			db, err = frigolite.Open("test.db")
			if err != nil { t.Fatal(err) }
			{ // "indexA-5.3" — skipped: depends on 5.0 PRIMQRY table creation (reopen select) (SQL side effects only)
				_res = db.Exec("\n  SELECT * FROM t1\n")
				_ = _res.Error // tolerate unsupported-feature errors in skipped tests
			}
			db.Close()
			os.Remove("test.db")
			db, err = frigolite.Open("test.db")
			if err != nil { t.Fatal(err) }
			tcl_nullvalue = "{}" // fresh connection resets nullvalue
			{ // "6.0"
				_res = db.Exec("\n  CREATE TABLE t1(a INTEGER PRIMARY KEY, b, c);\n  CREATE TABLE t2(x INTEGER PRIMARY KEY, y INTEGER, z INTEGER);\n  INSERT INTO t1 VALUES(1, 1, 1);\n  INSERT INTO t1 VALUES(2, 1, 2);\n  INSERT INTO t2 VALUES(1, 5, 1);\n  INSERT INTO t2 VALUES(2, 5, 2);\n\n  CREATE INDEX t2z ON t2(z) WHERE y=5;\n")
				if _res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t1(a INTEGER PRIMARY KEY, b, c);\n  CREATE TABLE t2(x INTEGER PRIMARY KEY, y INTEGER, z INTEGER);\n  INSERT INTO t1 VALUES(1, 1, 1);\n  INSERT INTO t1 VALUES(2, 1, 2);\n  INSERT INTO t2 VALUES(1, 5, 1);\n  INSERT INTO t2 VALUES(2, 5, 2);\n\n  CREATE INDEX t2z ON t2(z) WHERE y=5;\n")
				}
			}
			{ // "6.1"
				_res = db.Exec("\n  ANALYZE;\n  UPDATE sqlite_stat1 SET stat = '50 1' WHERE idx='t2z';\n  UPDATE sqlite_stat1 SET stat = '50' WHERE tbl='t2' AND idx IS NULL;\n  UPDATE sqlite_stat1 SET stat = '5000' WHERE tbl='t1' AND idx IS NULL;\n  ANALYZE sqlite_schema;\n")
				if _res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  ANALYZE;\n  UPDATE sqlite_stat1 SET stat = '50 1' WHERE idx='t2z';\n  UPDATE sqlite_stat1 SET stat = '50' WHERE tbl='t2' AND idx IS NULL;\n  UPDATE sqlite_stat1 SET stat = '5000' WHERE tbl='t1' AND idx IS NULL;\n  ANALYZE sqlite_schema;\n")
				}
			}
			{ // "6.2"
				r = db.Query("\n  SELECT * FROM t1, t2 WHERE b=1 AND z=c AND y=5;\n")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t1, t2 WHERE b=1 AND z=c AND y=5;\n")
					return
				}
				got := flatten(r)
				want := "1 1 1 1 5 1 2 1 2 2 5 2"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "6.3"
				r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT * FROM t1, t2 WHERE b=1 AND z=c AND y=5;\n")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT * FROM t1, t2 WHERE b=1 AND z=c AND y=5;\n")
				}
			}
			{ // "6.4"
				r = db.Query("\n  SELECT * FROM t1 LEFT JOIN t2 ON (y=5) WHERE b=1 AND z IS c;\n")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t1 LEFT JOIN t2 ON (y=5) WHERE b=1 AND z IS c;\n")
					return
				}
				got := flatten(r)
				want := "1 1 1 1 5 1 2 1 2 2 5 2"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // "6.5"
				r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT * FROM t1 LEFT JOIN t2 ON (y=5) WHERE b=1 AND z IS c;\n")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT * FROM t1 LEFT JOIN t2 ON (y=5) WHERE b=1 AND z IS c;\n")
				}
			}
			{ // "6.6"
				_res = db.Exec("\n  CREATE INDEX t2yz ON t2(y, z) WHERE y=5;\n")
				if _res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE INDEX t2yz ON t2(y, z) WHERE y=5;\n")
				}
			}
			{ // "6.7"
				r = db.Query("\n  SELECT * FROM t1 LEFT JOIN t2 ON (y=5) WHERE b=1 AND z IS c;\n")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t1 LEFT JOIN t2 ON (y=5) WHERE b=1 AND z IS c;\n")
					return
				}
				got := flatten(r)
				want := "1 1 1 1 5 1 2 1 2 2 5 2"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			db.Close()
			os.Remove("test.db")
			db, err = frigolite.Open("test.db")
			if err != nil { t.Fatal(err) }
			tcl_nullvalue = "{}" // fresh connection resets nullvalue
			{ // "7.0"
				r = db.Query("\n  CREATE TABLE t1(i INTEGER PRIMARY KEY, b TEXT, c TEXT);\n  CREATE INDEX i1 ON t1(c) WHERE b='abc' AND i=5;\n  INSERT INTO t1 VALUES(5, 'abc', 'xyz');\n  SELECT * FROM t1 INDEXED BY i1 WHERE b='abc' AND i=5 ORDER BY c;\n")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t1(i INTEGER PRIMARY KEY, b TEXT, c TEXT);\n  CREATE INDEX i1 ON t1(c) WHERE b='abc' AND i=5;\n  INSERT INTO t1 VALUES(5, 'abc', 'xyz');\n  SELECT * FROM t1 INDEXED BY i1 WHERE b='abc' AND i=5 ORDER BY c;\n")
					return
				}
				got := flatten(r)
				want := "5 abc xyz"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			db.Close()
			os.Remove("test.db")
			db, err = frigolite.Open("test.db")
			if err != nil { t.Fatal(err) }
			tcl_nullvalue = "{}" // fresh connection resets nullvalue
			{ // "8.0"
				_res = db.Exec("\n  CREATE TABLE t1(a, b, c);\n  CREATE INDEX ex2 ON t1(a, 4);\n  CREATE INDEX ex1 ON t1(a) WHERE 4=b;\n  INSERT INTO t1 VALUES(1, 4, 1);\n  INSERT INTO t1 VALUES(1, 5, 1);\n  INSERT INTO t1 VALUES(2, 4, 2);\n")
				if _res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t1(a, b, c);\n  CREATE INDEX ex2 ON t1(a, 4);\n  CREATE INDEX ex1 ON t1(a) WHERE 4=b;\n  INSERT INTO t1 VALUES(1, 4, 1);\n  INSERT INTO t1 VALUES(1, 5, 1);\n  INSERT INTO t1 VALUES(2, 4, 2);\n")
				}
			}
			{ // "8.1"
				r = db.Query("\n  SELECT * FROM t1 WHERE b=4;\n")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t1 WHERE b=4;\n")
					return
				}
				got := flatten(r)
				want := "1 4 1 2 4 2"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
}
