// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package indexexpr1

import (
"github.com/pijalu/frigolite"
"os"
"strings"
"testing"
)

func Test_indexexpr1(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source
	var n string
	_ = n // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	{ // "indexexpr1-100"
		_res = db.Exec("\n  CREATE TABLE t1(a,b,c);\n  INSERT INTO t1(a,b,c)\n      /*  123456789 123456789 123456789 123456789 123456789 123456789 */ \n  VALUES('In_the_beginning_was_the_Word',1,1),\n        ('and_the_Word_was_with_God',1,2),\n        ('and_the_Word_was_God',1,3),\n        ('The_same_was_in_the_beginning_with_God',2,1),\n        ('All_things_were_made_by_him',3,1),\n        ('and_without_him_was_not_any_thing_made_that_was_made',3,2);\n  CREATE INDEX t1a1 ON t1(substr(a,1,12));\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t1(a,b,c);\n  INSERT INTO t1(a,b,c)\n      /*  123456789 123456789 123456789 123456789 123456789 123456789 */ \n  VALUES('In_the_beginning_was_the_Word',1,1),\n        ('and_the_Word_was_with_God',1,2),\n        ('and_the_Word_was_God',1,3),\n        ('The_same_was_in_the_beginning_with_God',2,1),\n        ('All_things_were_made_by_him',3,1),\n        ('and_without_him_was_not_any_thing_made_that_was_made',3,2);\n  CREATE INDEX t1a1 ON t1(substr(a,1,12));\n")
		}
	}
	{ // "indexexpr1-110"
		r = db.Query("\n  SELECT b, c, '|' FROM t1 WHERE substr(a,1,12)=='and_the_Word' ORDER BY b, c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT b, c, '|' FROM t1 WHERE substr(a,1,12)=='and_the_Word' ORDER BY b, c;\n")
			return
		}
		got := flatten(r)
		want := "1 2 | 1 3 |"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-110eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT b, c, '|' FROM t1 WHERE substr(a,1,12)=='and_the_Word' ORDER BY b, c;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-120"
		r = db.Query("\n  SELECT b, c, '|' FROM t1 WHERE 'and_the_Word'==substr(a,1,12) ORDER BY b, c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT b, c, '|' FROM t1 WHERE 'and_the_Word'==substr(a,1,12) ORDER BY b, c;\n")
			return
		}
		got := flatten(r)
		want := "1 2 | 1 3 |"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-120eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT b, c, '|' FROM t1 WHERE 'and_the_Word'==substr(a,1,12) ORDER BY b, c;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-130"
		r = db.Query("\n  CREATE INDEX t1ba ON t1(b,substr(a,2,3),c);\n  SELECT c FROM t1 WHERE b=1 AND substr(a,2,3)='nd_' ORDER BY c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE INDEX t1ba ON t1(b,substr(a,2,3),c);\n  SELECT c FROM t1 WHERE b=1 AND substr(a,2,3)='nd_' ORDER BY c;\n")
			return
		}
		got := flatten(r)
		want := "2 3"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-130eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT c FROM t1 WHERE b=1 AND substr(a,2,3)='nd_' ORDER BY c;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-140"
		r = db.Query("\n  SELECT rowid, substr(a,b,3), '|' FROM t1 ORDER BY 2;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT rowid, substr(a,b,3), '|' FROM t1 ORDER BY 2;\n")
			return
		}
		got := flatten(r)
		want := "1 In_ | 2 and | 3 and | 6 d_w | 4 he_ | 5 l_t |"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-141"
		r = db.Query("\n  CREATE INDEX t1abx ON t1(substr(a,b,3));\n  SELECT rowid FROM t1 WHERE substr(a,b,3)<='and' ORDER BY +rowid;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE INDEX t1abx ON t1(substr(a,b,3));\n  SELECT rowid FROM t1 WHERE substr(a,b,3)<='and' ORDER BY +rowid;\n")
			return
		}
		got := flatten(r)
		want := "1 2 3"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-141eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT rowid FROM t1 WHERE substr(a,b,3)<='and' ORDER BY +rowid;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-142"
		r = db.Query("\n  SELECT rowid FROM t1 WHERE +substr(a,b,3)<='and' ORDER BY +rowid;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT rowid FROM t1 WHERE +substr(a,b,3)<='and' ORDER BY +rowid;\n")
			return
		}
		got := flatten(r)
		want := "1 2 3"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-150"
		r = db.Query("\n  SELECT rowid FROM t1 WHERE substr(a,b,3) IN ('and','l_t','xyz')\n   ORDER BY +rowid;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT rowid FROM t1 WHERE substr(a,b,3) IN ('and','l_t','xyz')\n   ORDER BY +rowid;\n")
			return
		}
		got := flatten(r)
		want := "2 3 5"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-150eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT rowid FROM t1 WHERE substr(a,b,3) IN ('and','l_t','xyz')\n   ORDER BY +rowid;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-160"
		r = db.Query("\n    ALTER TABLE t1 ADD COLUMN d;\n    UPDATE t1 SET d=length(a);\n    CREATE INDEX t1a2 ON t1(SUBSTR(a, 27, 3)) WHERE d>=29;\n    SELECT rowid, b, c FROM t1\n      WHERE substr(a,27,3)=='ord' AND d>=29;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    ALTER TABLE t1 ADD COLUMN d;\n    UPDATE t1 SET d=length(a);\n    CREATE INDEX t1a2 ON t1(SUBSTR(a, 27, 3)) WHERE d>=29;\n    SELECT rowid, b, c FROM t1\n      WHERE substr(a,27,3)=='ord' AND d>=29;\n  ")
			return
		}
		got := flatten(r)
		want := "1 1 1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-160eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n    EXPLAIN QUERY PLAN\n      SELECT rowid, b, c FROM t1\n      WHERE substr(a,27,3)=='ord' AND d>=29;\n  ")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-170"
		r = db.Query("\n  CREATE INDEX t1alen ON t1(length(a));\n  SELECT length(a) FROM t1 ORDER BY length(a);\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE INDEX t1alen ON t1(length(a));\n  SELECT length(a) FROM t1 ORDER BY length(a);\n")
			return
		}
		got := flatten(r)
		want := "20 25 27 29 38 52"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-170eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT length(a) FROM t1 ORDER BY length(a);\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-171"
		r = db.Query("\n  SELECT length(a) FROM t1 ORDER BY length(a) DESC;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT length(a) FROM t1 ORDER BY length(a) DESC;\n")
			return
		}
		got := flatten(r)
		want := "52 38 29 27 25 20"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-171eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT length(a) FROM t1 ORDER BY length(a) DESC;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-200"
		_res = db.Exec("\n  DROP TABLE t1;\n  CREATE TABLE t1(id ANY PRIMARY KEY, a,b,c) WITHOUT ROWID;\n  INSERT INTO t1(id,a,b,c)\n  VALUES(1,'In_the_beginning_was_the_Word',1,1),\n        (2,'and_the_Word_was_with_God',1,2),\n        (3,'and_the_Word_was_God',1,3),\n        (4,'The_same_was_in_the_beginning_with_God',2,1),\n        (5,'All_things_were_made_by_him',3,1),\n        (6,'and_without_him_was_not_any_thing_made_that_was_made',3,2);\n  CREATE INDEX t1a1 ON t1(substr(a,1,12));\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  DROP TABLE t1;\n  CREATE TABLE t1(id ANY PRIMARY KEY, a,b,c) WITHOUT ROWID;\n  INSERT INTO t1(id,a,b,c)\n  VALUES(1,'In_the_beginning_was_the_Word',1,1),\n        (2,'and_the_Word_was_with_God',1,2),\n        (3,'and_the_Word_was_God',1,3),\n        (4,'The_same_was_in_the_beginning_with_God',2,1),\n        (5,'All_things_were_made_by_him',3,1),\n        (6,'and_without_him_was_not_any_thing_made_that_was_made',3,2);\n  CREATE INDEX t1a1 ON t1(substr(a,1,12));\n")
		}
	}
	{ // "indexexpr1-210"
		r = db.Query("\n  SELECT b, c, '|' FROM t1 WHERE substr(a,1,12)=='and_the_Word' ORDER BY b, c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT b, c, '|' FROM t1 WHERE substr(a,1,12)=='and_the_Word' ORDER BY b, c;\n")
			return
		}
		got := flatten(r)
		want := "1 2 | 1 3 |"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-210eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT b, c, '|' FROM t1 WHERE substr(a,1,12)=='and_the_Word' ORDER BY b, c;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-220"
		r = db.Query("\n  SELECT b, c, '|' FROM t1 WHERE 'and_the_Word'==substr(a,1,12) ORDER BY b, c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT b, c, '|' FROM t1 WHERE 'and_the_Word'==substr(a,1,12) ORDER BY b, c;\n")
			return
		}
		got := flatten(r)
		want := "1 2 | 1 3 |"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-220eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT b, c, '|' FROM t1 WHERE 'and_the_Word'==substr(a,1,12) ORDER BY b, c;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-230"
		r = db.Query("\n  CREATE INDEX t1ba ON t1(b,substr(a,2,3),c);\n  SELECT c FROM t1 WHERE b=1 AND substr(a,2,3)='nd_' ORDER BY c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE INDEX t1ba ON t1(b,substr(a,2,3),c);\n  SELECT c FROM t1 WHERE b=1 AND substr(a,2,3)='nd_' ORDER BY c;\n")
			return
		}
		got := flatten(r)
		want := "2 3"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-230eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT c FROM t1 WHERE b=1 AND substr(a,2,3)='nd_' ORDER BY c;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-240"
		r = db.Query("\n  SELECT id, substr(a,b,3), '|' FROM t1 ORDER BY 2;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT id, substr(a,b,3), '|' FROM t1 ORDER BY 2;\n")
			return
		}
		got := flatten(r)
		want := "1 In_ | 2 and | 3 and | 6 d_w | 4 he_ | 5 l_t |"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-241"
		r = db.Query("\n  CREATE INDEX t1abx ON t1(substr(a,b,3));\n  SELECT id FROM t1 WHERE substr(a,b,3)<='and' ORDER BY +id;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE INDEX t1abx ON t1(substr(a,b,3));\n  SELECT id FROM t1 WHERE substr(a,b,3)<='and' ORDER BY +id;\n")
			return
		}
		got := flatten(r)
		want := "1 2 3"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-241eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT id FROM t1 WHERE substr(a,b,3)<='and' ORDER BY +id;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-242"
		r = db.Query("\n  SELECT id FROM t1 WHERE +substr(a,b,3)<='and' ORDER BY +id;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT id FROM t1 WHERE +substr(a,b,3)<='and' ORDER BY +id;\n")
			return
		}
		got := flatten(r)
		want := "1 2 3"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-250"
		r = db.Query("\n  SELECT id FROM t1 WHERE substr(a,b,3) IN ('and','l_t','xyz')\n   ORDER BY +id;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT id FROM t1 WHERE substr(a,b,3) IN ('and','l_t','xyz')\n   ORDER BY +id;\n")
			return
		}
		got := flatten(r)
		want := "2 3 5"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-250eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT id FROM t1 WHERE substr(a,b,3) IN ('and','l_t','xyz')\n   ORDER BY +id;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-260"
		r = db.Query("\n    ALTER TABLE t1 ADD COLUMN d;\n    UPDATE t1 SET d=length(a);\n    CREATE INDEX t1a2 ON t1(SUBSTR(a, 27, 3)) WHERE d>=29;\n    SELECT id, b, c FROM t1\n      WHERE substr(a,27,3)=='ord' AND d>=29;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    ALTER TABLE t1 ADD COLUMN d;\n    UPDATE t1 SET d=length(a);\n    CREATE INDEX t1a2 ON t1(SUBSTR(a, 27, 3)) WHERE d>=29;\n    SELECT id, b, c FROM t1\n      WHERE substr(a,27,3)=='ord' AND d>=29;\n  ")
			return
		}
		got := flatten(r)
		want := "1 1 1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-260eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n    EXPLAIN QUERY PLAN\n      SELECT id, b, c FROM t1\n      WHERE substr(a,27,3)=='ord' AND d>=29;\n  ")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-300"
		_res = db.Exec("\n  CREATE TABLE t2(a,b,c); INSERT INTO t2 VALUES(1,2,3);\n  CREATE INDEX t2x1 ON t2(a,b+random());\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "non-deterministic functions prohibited in index expressions") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "non-deterministic functions prohibited in index expressions", _res.Error, "\n  CREATE TABLE t2(a,b,c); INSERT INTO t2 VALUES(1,2,3);\n  CREATE INDEX t2x1 ON t2(a,b+random());\n")
		}
	}
	{ // "indexexpr1-301"
		_res = db.Exec("\n  CREATE INDEX t2x1 ON t2(julianday('now',a));\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "non-deterministic use of julianday() in an index") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "non-deterministic use of julianday() in an index", _res.Error, "\n  CREATE INDEX t2x1 ON t2(julianday('now',a));\n")
		}
	}
	{ // "indexexpr1-310"
		_res = db.Exec("\n  CREATE INDEX t2x2 ON t2(a,b+(SELECT 15));\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "subqueries prohibited in index expressions") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "subqueries prohibited in index expressions", _res.Error, "\n  CREATE INDEX t2x2 ON t2(a,b+(SELECT 15));\n")
		}
	}
	{ // "indexexpr1-320"
		_res = db.Exec("\n  CREATE TABLE e1(x,y,UNIQUE(y,substr(x,1,5)));\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "expressions prohibited in PRIMARY KEY and UNIQUE constraints") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "expressions prohibited in PRIMARY KEY and UNIQUE constraints", _res.Error, "\n  CREATE TABLE e1(x,y,UNIQUE(y,substr(x,1,5)));\n")
		}
	}
	{ // "indexexpr1-330"
		_res = db.Exec("\n  CREATE TABLE e1(x,y,PRIMARY KEY(y,substr(x,1,5)));\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "expressions prohibited in PRIMARY KEY and UNIQUE constraints") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "expressions prohibited in PRIMARY KEY and UNIQUE constraints", _res.Error, "\n  CREATE TABLE e1(x,y,PRIMARY KEY(y,substr(x,1,5)));\n")
		}
	}
	{ // "indexexpr1-331"
		_res = db.Exec("\n  CREATE TABLE e1(x,y,PRIMARY KEY(y,substr(x,1,5))) WITHOUT ROWID;\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "expressions prohibited in PRIMARY KEY and UNIQUE constraints") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "expressions prohibited in PRIMARY KEY and UNIQUE constraints", _res.Error, "\n  CREATE TABLE e1(x,y,PRIMARY KEY(y,substr(x,1,5))) WITHOUT ROWID;\n")
		}
	}
	{ // "indexexpr1-340"
		_res = db.Exec("\n  CREATE TABLE e1(x,y,FOREIGN KEY(substr(y,1,5)) REFERENCES t1);\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "near \"(\": syntax error") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "near \"(\": syntax error", _res.Error, "\n  CREATE TABLE e1(x,y,FOREIGN KEY(substr(y,1,5)) REFERENCES t1);\n")
		}
	}
	{ // "indexexpr1-400"
		r = db.Query("\n  CREATE TABLE t3(a,b,c);\n  WITH RECURSIVE c(x) AS (VALUES(1) UNION SELECT x+1 FROM c WHERE x<30)\n  INSERT INTO t3(a,b,c)\n    SELECT x, printf('ab%04xyz',x), random() FROM c;\n  CREATE UNIQUE INDEX t3abc ON t3(CAST(a AS text), b, substr(c,1,3));\n  SELECT a FROM t3 WHERE CAST(a AS text)<='10' ORDER BY +a;\n  PRAGMA integrity_check;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t3(a,b,c);\n  WITH RECURSIVE c(x) AS (VALUES(1) UNION SELECT x+1 FROM c WHERE x<30)\n  INSERT INTO t3(a,b,c)\n    SELECT x, printf('ab%04xyz',x), random() FROM c;\n  CREATE UNIQUE INDEX t3abc ON t3(CAST(a AS text), b, substr(c,1,3));\n  SELECT a FROM t3 WHERE CAST(a AS text)<='10' ORDER BY +a;\n  PRAGMA integrity_check;\n")
			return
		}
		got := flatten(r)
		want := "1 10 ok"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-410"
		_res = db.Exec("\n  INSERT INTO t3 SELECT * FROM t3 WHERE rowid=10;\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "UNIQUE constraint failed: index 't3abc'") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "UNIQUE constraint failed: index 't3abc'", _res.Error, "\n  INSERT INTO t3 SELECT * FROM t3 WHERE rowid=10;\n")
		}
	}
	{ // "indexexpr1-500"
		_res = db.Exec("\n  CREATE TABLE t5(a);\n  CREATE TABLE cnt(x);\n  WITH RECURSIVE\n    c(x) AS (VALUES(1) UNION ALL SELECT x+1 FROM c WHERE x<5)\n  INSERT INTO cnt(x) SELECT x FROM c;\n  INSERT INTO t5(a) SELECT printf('abc%03dxyz',x) FROM cnt;\n  CREATE INDEX t5ax ON t5( substr(a,4,3) );\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t5(a);\n  CREATE TABLE cnt(x);\n  WITH RECURSIVE\n    c(x) AS (VALUES(1) UNION ALL SELECT x+1 FROM c WHERE x<5)\n  INSERT INTO cnt(x) SELECT x FROM c;\n  INSERT INTO t5(a) SELECT printf('abc%03dxyz',x) FROM cnt;\n  CREATE INDEX t5ax ON t5( substr(a,4,3) );\n")
		}
	}
	{ // "indexexpr1-510"
		r = db.Query("\n  -- The use of the \"k\" alias in the WHERE clause is technically\n  -- illegal, but SQLite allows it for historical reasons.  In this\n  -- test and the next, verify that \"k\" can be used by the t5ax index\n  SELECT substr(a,4,3) AS k FROM cnt, t5 WHERE k=printf('%03d',x);\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  -- The use of the \"k\" alias in the WHERE clause is technically\n  -- illegal, but SQLite allows it for historical reasons.  In this\n  -- test and the next, verify that \"k\" can be used by the t5ax index\n  SELECT substr(a,4,3) AS k FROM cnt, t5 WHERE k=printf('%03d',x);\n")
			return
		}
		got := flatten(r)
		want := "001 002 003 004 005"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-510eqp" — skipped: EXPLAIN QUERY PLAN expression-index scan not planned (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT substr(a,4,3) AS k FROM cnt, t5 WHERE k=printf('%03d',x);\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-600"
		r = db.Query("\n  DROP TABLE IF EXISTS t4;\n  CREATE TABLE t4(a,b,c,d,e,f,g,h,i);\n  CREATE INDEX t4all ON t4(a,b,c<d,e,f,i,h);\n  INSERT INTO t4 VALUES(1,2,3,4,5,6,7,8,9);\n  ANALYZE;\n  DELETE FROM sqlite_stat1;\n  INSERT INTO sqlite_stat1\n    VALUES('t4','t4all','600000 160000 40000 10000 2000 600 100 40 10');\n  ANALYZE sqlite_master;\n  SELECT i FROM t4 WHERE e=5;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE IF EXISTS t4;\n  CREATE TABLE t4(a,b,c,d,e,f,g,h,i);\n  CREATE INDEX t4all ON t4(a,b,c<d,e,f,i,h);\n  INSERT INTO t4 VALUES(1,2,3,4,5,6,7,8,9);\n  ANALYZE;\n  DELETE FROM sqlite_stat1;\n  INSERT INTO sqlite_stat1\n    VALUES('t4','t4all','600000 160000 40000 10000 2000 600 100 40 10');\n  ANALYZE sqlite_master;\n  SELECT i FROM t4 WHERE e=5;\n")
			return
		}
		got := flatten(r)
		want := "9"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-700"
		r = db.Query("\n  DROP TABLE IF EXISTS t7;\n  CREATE TABLE t7(a,b,c);\n  INSERT INTO t7(a,b,c) VALUES(1,2,2),('abc','def','def'),(4,5,6);\n  CREATE INDEX t7b ON t7(+b);\n  CREATE INDEX t7c ON t7(+c);\n  SELECT *, '|' FROM t7 WHERE +b=+c ORDER BY +a;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE IF EXISTS t7;\n  CREATE TABLE t7(a,b,c);\n  INSERT INTO t7(a,b,c) VALUES(1,2,2),('abc','def','def'),(4,5,6);\n  CREATE INDEX t7b ON t7(+b);\n  CREATE INDEX t7c ON t7(+c);\n  SELECT *, '|' FROM t7 WHERE +b=+c ORDER BY +a;\n")
			return
		}
		got := flatten(r)
		want := "1 2 2 | abc def def |"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-710"
		r = db.Query("\n  CREATE TABLE t71(a,b,c);\n  CREATE INDEX t71bc ON t71(b+c);\n  CREATE TABLE t72(x,y,z);\n  CREATE INDEX t72yz ON t72(y+z);\n  INSERT INTO t71(a,b,c) VALUES(1,11,2),(2,7,15),(3,5,4);\n  INSERT INTO t72(x,y,z) VALUES(1,10,3),(2,8,14),(3,9,9);\n  SELECT a, x, '|' FROM t71, t72\n   WHERE b+c=y+z\n  ORDER BY +a, +x;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t71(a,b,c);\n  CREATE INDEX t71bc ON t71(b+c);\n  CREATE TABLE t72(x,y,z);\n  CREATE INDEX t72yz ON t72(y+z);\n  INSERT INTO t71(a,b,c) VALUES(1,11,2),(2,7,15),(3,5,4);\n  INSERT INTO t72(x,y,z) VALUES(1,10,3),(2,8,14),(3,9,9);\n  SELECT a, x, '|' FROM t71, t72\n   WHERE b+c=y+z\n  ORDER BY +a, +x;\n")
			return
		}
		got := flatten(r)
		want := "1 1 | 2 2 |"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-800"
		r = db.Query("\n  DROP TABLE IF EXISTS t8;\n  CREATE TABLE t8(a INTEGER PRIMARY KEY, b TEXT);\n  CREATE UNIQUE INDEX t8bx ON t8(substr(b,2,4) COLLATE nocase);\n  INSERT INTO t8(a,b) VALUES(1,'Alice'),(2,'Bartholemew'),(3,'Cynthia');\n  SELECT * FROM t8 WHERE substr(b,2,4)='ARTH' COLLATE nocase;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE IF EXISTS t8;\n  CREATE TABLE t8(a INTEGER PRIMARY KEY, b TEXT);\n  CREATE UNIQUE INDEX t8bx ON t8(substr(b,2,4) COLLATE nocase);\n  INSERT INTO t8(a,b) VALUES(1,'Alice'),(2,'Bartholemew'),(3,'Cynthia');\n  SELECT * FROM t8 WHERE substr(b,2,4)='ARTH' COLLATE nocase;\n")
			return
		}
		got := flatten(r)
		want := "2 Bartholemew"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-810"
		_res = db.Exec("\n  INSERT INTO t8(a,b) VALUES(4,'BARTHMERE');\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "UNIQUE constraint failed: index 't8bx'") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "UNIQUE constraint failed: index 't8bx'", _res.Error, "\n  INSERT INTO t8(a,b) VALUES(4,'BARTHMERE');\n")
		}
	}
	{ // "indexexpr1-820"
		_res = db.Exec("\n  DROP INDEX t8bx;\n  CREATE UNIQUE INDEX t8bx ON t8(substr(b,2,4) COLLATE rtrim);\n  INSERT INTO t8(a,b) VALUES(4,'BARTHMERE');\n")
		if _res.Error != nil {
			t.Errorf("expected success, got error: %v\n  sql: %s", _res.Error, "\n  DROP INDEX t8bx;\n  CREATE UNIQUE INDEX t8bx ON t8(substr(b,2,4) COLLATE rtrim);\n  INSERT INTO t8(a,b) VALUES(4,'BARTHMERE');\n")
		}
	}
	{ // "indexexpr1-900"
		r = db.Query("\n  CREATE TABLE t9(a,b,c,d);\n  CREATE UNIQUE INDEX t9x1 ON t9(c,abs(d),b);\n  INSERT INTO t9(rowid,a,b,c,d) VALUES(1,2,3,4,5);\n  INSERT INTO t9(rowid,a,b,c,d) VALUES(2,NULL,NULL,NULL,NULL);\n  INSERT INTO t9(rowid,a,b,c,d) VALUES(3,NULL,NULL,NULL,NULL);\n  INSERT INTO t9(rowid,a,b,c,d) VALUES(4,5,6,7,8);\n  PRAGMA integrity_check;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t9(a,b,c,d);\n  CREATE UNIQUE INDEX t9x1 ON t9(c,abs(d),b);\n  INSERT INTO t9(rowid,a,b,c,d) VALUES(1,2,3,4,5);\n  INSERT INTO t9(rowid,a,b,c,d) VALUES(2,NULL,NULL,NULL,NULL);\n  INSERT INTO t9(rowid,a,b,c,d) VALUES(3,NULL,NULL,NULL,NULL);\n  INSERT INTO t9(rowid,a,b,c,d) VALUES(4,5,6,7,8);\n  PRAGMA integrity_check;\n")
			return
		}
		got := flatten(r)
		want := "ok"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-910"
		_res = db.Exec("\n  INSERT INTO t9(a,b,c,d) VALUES(5,6,7,-8);\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "UNIQUE constraint failed: index 't9x1'") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "UNIQUE constraint failed: index 't9x1'", _res.Error, "\n  INSERT INTO t9(a,b,c,d) VALUES(5,6,7,-8);\n")
		}
	}
	{ // "indexexpr1-1000"
		r = db.Query("\n  DROP TABLE IF EXISTS t0;\n  CREATE TABLE t0(a,b,t);\n  CREATE INDEX i ON t0(a in(0,1));\n  INSERT INTO t0 VALUES(0,1,2),(2,3,4),(5,6,7);\n  UPDATE t0 SET b=99 WHERE (a in(0,1))=0;\n  SELECT *, '|' FROM t0 ORDER BY +a;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE IF EXISTS t0;\n  CREATE TABLE t0(a,b,t);\n  CREATE INDEX i ON t0(a in(0,1));\n  INSERT INTO t0 VALUES(0,1,2),(2,3,4),(5,6,7);\n  UPDATE t0 SET b=99 WHERE (a in(0,1))=0;\n  SELECT *, '|' FROM t0 ORDER BY +a;\n")
			return
		}
		got := flatten(r)
		want := "0 1 2 | 2 99 4 | 5 99 7 |"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1010"
		r = db.Query("\n  UPDATE t0 SET b=88 WHERE (a in(0,1))=1;\n  SELECT *, '|' FROM t0 ORDER BY +a;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  UPDATE t0 SET b=88 WHERE (a in(0,1))=1;\n  SELECT *, '|' FROM t0 ORDER BY +a;\n")
			return
		}
		got := flatten(r)
		want := "0 88 2 | 2 99 4 | 5 99 7 |"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1100"
		r = db.Query("\n  DROP TABLE IF EXISTS t1;\n  CREATE TABLE t1(a);\n  INSERT INTO t1 VALUES(NULL),(1);\n  SELECT '1:', typeof(a), a FROM t1 WHERE a<10;\n  SELECT '2:', typeof(a), a FROM t1 WHERE a+0<10;\n  CREATE INDEX t1x1 ON t1(a);\n  CREATE INDEX t1x2 ON t1(a+0);\n  SELECT '3:', typeof(a), a FROM t1 WHERE a<10;\n  SELECT '4:', typeof(a), a FROM t1 WHERE a+0<10;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE IF EXISTS t1;\n  CREATE TABLE t1(a);\n  INSERT INTO t1 VALUES(NULL),(1);\n  SELECT '1:', typeof(a), a FROM t1 WHERE a<10;\n  SELECT '2:', typeof(a), a FROM t1 WHERE a+0<10;\n  CREATE INDEX t1x1 ON t1(a);\n  CREATE INDEX t1x2 ON t1(a+0);\n  SELECT '3:', typeof(a), a FROM t1 WHERE a<10;\n  SELECT '4:', typeof(a), a FROM t1 WHERE a+0<10;\n")
			return
		}
		got := flatten(r)
		want := "1: integer 1 2: integer 1 3: integer 1 4: integer 1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1200"
		r = db.Query("\n  CREATE TABLE t10(a int, b int, c int, d int);\n  INSERT INTO t10(a, b, c, d) VALUES(0, 0, 2, 2);\n  INSERT INTO t10(a, b, c, d) VALUES(0, 0, 0, 0);\n  INSERT INTO t10(a, b, c, d) VALUES(0, 0, 1, 1);\n  INSERT INTO t10(a, b, c, d) VALUES(1, 1, 1, 1);\n  INSERT INTO t10(a, b, c, d) VALUES(1, 1, 0, 0);\n  INSERT INTO t10(a, b, c, d) VALUES(2, 2, 0, 0);\n\n  SELECT a+b, c+d FROM t10 ORDER BY a+b, c+d;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t10(a int, b int, c int, d int);\n  INSERT INTO t10(a, b, c, d) VALUES(0, 0, 2, 2);\n  INSERT INTO t10(a, b, c, d) VALUES(0, 0, 0, 0);\n  INSERT INTO t10(a, b, c, d) VALUES(0, 0, 1, 1);\n  INSERT INTO t10(a, b, c, d) VALUES(1, 1, 1, 1);\n  INSERT INTO t10(a, b, c, d) VALUES(1, 1, 0, 0);\n  INSERT INTO t10(a, b, c, d) VALUES(2, 2, 0, 0);\n\n  SELECT a+b, c+d FROM t10 ORDER BY a+b, c+d;\n")
			return
		}
		got := flatten(r)
		want := "0 0 0 2 0 4 2 0 2 2 4 0"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1200.1"
		_res = db.Exec("\n  CREATE INDEX t10_ab ON t10(a+b);\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE INDEX t10_ab ON t10(a+b);\n")
		}
	}
	{ // "indexexpr1-1200.2"
		r = db.Query("\n  SELECT a+b, c+d FROM t10 ORDER BY a+b, c+d;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT a+b, c+d FROM t10 ORDER BY a+b, c+d;\n")
			return
		}
		got := flatten(r)
		want := "0 0 0 2 0 4 2 0 2 2 4 0"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1200.3"
		_res = db.Exec("\n  CREATE INDEX t10_abcd ON t10(a+b,c+d);\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE INDEX t10_abcd ON t10(a+b,c+d);\n")
		}
	}
	{ // "indexexpr1-1200.4"
		r = db.Query("\n  SELECT a+b, c+d FROM t10 ORDER BY a+b, c+d;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT a+b, c+d FROM t10 ORDER BY a+b, c+d;\n")
			return
		}
		got := flatten(r)
		want := "0 0 0 2 0 4 2 0 2 2 4 0"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1300.1"
		r = db.Query("\n  CREATE TABLE t1300(a INTEGER PRIMARY KEY, b);\n  INSERT INTO t1300 VALUES(1,'coffee'),(2,'COFFEE'),(3,'stress'),(4,'STRESS');\n  CREATE INDEX t1300bexpr ON t1300( substr(b,4) );\n  SELECT a FROM t1300 WHERE substr(b,4)='ess' COLLATE nocase ORDER BY +a;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t1300(a INTEGER PRIMARY KEY, b);\n  INSERT INTO t1300 VALUES(1,'coffee'),(2,'COFFEE'),(3,'stress'),(4,'STRESS');\n  CREATE INDEX t1300bexpr ON t1300( substr(b,4) );\n  SELECT a FROM t1300 WHERE substr(b,4)='ess' COLLATE nocase ORDER BY +a;\n")
			return
		}
		got := flatten(r)
		want := "3 4"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1400"
		r = db.Query("\n  CREATE TABLE t1400(x TEXT);\n  CREATE INDEX t1400x ON t1400(1);  -- Index on a constant\n  SELECT 1 IN (SELECT 2) FROM t1400;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t1400(x TEXT);\n  CREATE INDEX t1400x ON t1400(1);  -- Index on a constant\n  SELECT 1 IN (SELECT 2) FROM t1400;\n")
		}
	}
	{ // "indexexpr1-1410"
		r = db.Query("\n  INSERT INTO t1400 VALUES('a'),('b');\n  SELECT 1 IN (SELECT 2) FROM t1400;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  INSERT INTO t1400 VALUES('a'),('b');\n  SELECT 1 IN (SELECT 2) FROM t1400;\n")
			return
		}
		got := flatten(r)
		want := "0 0"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1420"
		r = db.Query("\n  SELECT 1 IN (SELECT 2 UNION ALL SELECT 1) FROM t1400;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT 1 IN (SELECT 2 UNION ALL SELECT 1) FROM t1400;\n")
			return
		}
		got := flatten(r)
		want := "1 1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1430"
		r = db.Query("\n  DROP INDEX t1400x;\n  CREATE INDEX t1400x ON t1400(abs(15+3));\n  SELECT abs(15+3) IN (SELECT 17 UNION ALL SELECT 18) FROM t1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP INDEX t1400x;\n  CREATE INDEX t1400x ON t1400(abs(15+3));\n  SELECT abs(15+3) IN (SELECT 17 UNION ALL SELECT 18) FROM t1;\n")
			return
		}
		got := flatten(r)
		want := "1 1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1500"
		r = db.Query("\n  CREATE TABLE t1500(a INT PRIMARY KEY, b INT UNIQUE);\n  CREATE INDEX t1500ab ON t1500(a*b);\n  INSERT INTO t1500(a,b) VALUES(1,2);\n  REPLACE INTO t1500(a,b) VALUES(1,3);  -- formerly caused assertion fault\n  SELECT * FROM t1500;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t1500(a INT PRIMARY KEY, b INT UNIQUE);\n  CREATE INDEX t1500ab ON t1500(a*b);\n  INSERT INTO t1500(a,b) VALUES(1,2);\n  REPLACE INTO t1500(a,b) VALUES(1,3);  -- formerly caused assertion fault\n  SELECT * FROM t1500;\n")
			return
		}
		got := flatten(r)
		want := "1 3"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1510"
		_res = db.Exec("\n  DROP TABLE IF EXISTS t1;\n  CREATE TABLE t1(a PRIMARY KEY,b UNIQUE);\n  REPLACE INTO t1 VALUES(2, 1);\n  REPLACE INTO t1 SELECT 6,1;\n  CREATE INDEX t1aa ON t1(a-a);\n  REPLACE INTO t1 SELECT a, randomblob(a) FROM t1\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  DROP TABLE IF EXISTS t1;\n  CREATE TABLE t1(a PRIMARY KEY,b UNIQUE);\n  REPLACE INTO t1 VALUES(2, 1);\n  REPLACE INTO t1 SELECT 6,1;\n  CREATE INDEX t1aa ON t1(a-a);\n  REPLACE INTO t1 SELECT a, randomblob(a) FROM t1\n")
		}
	}
	{ // "indexexpr1-1600"
		r = db.Query("\n  DROP TABLE IF EXISTS t1;\n  CREATE TABLE t1 (a INTEGER, b);\n  CREATE INDEX idx1 ON t1 (lower(a));\n  INSERT INTO t1 VALUES('0001234',3);\n  PRAGMA integrity_check;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE IF EXISTS t1;\n  CREATE TABLE t1 (a INTEGER, b);\n  CREATE INDEX idx1 ON t1 (lower(a));\n  INSERT INTO t1 VALUES('0001234',3);\n  PRAGMA integrity_check;\n")
			return
		}
		got := flatten(r)
		want := "ok"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1610"
		r = db.Query("\n  INSERT INTO t1 VALUES('1234',0),('001234',2),('01234',1);\n  SELECT b FROM t1 WHERE lower(a)='1234' ORDER BY +b;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  INSERT INTO t1 VALUES('1234',0),('001234',2),('01234',1);\n  SELECT b FROM t1 WHERE lower(a)='1234' ORDER BY +b;\n")
			return
		}
		got := flatten(r)
		want := "0 1 2 3"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1620"
		r = db.Query("\n  SELECT b FROM t1 WHERE lower(a)='01234' ORDER BY +b;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT b FROM t1 WHERE lower(a)='01234' ORDER BY +b;\n")
		}
	}
	{ // "indexexpr1-1700"
		r = db.Query("\n  DROP TABLE IF EXISTS t0;\n  CREATE TABLE t0(c0);\n  INSERT INTO t0(c0) VALUES (0);\n  CREATE INDEX i0 ON t0(NULL > c0) WHERE (NULL NOT NULL);\n  SELECT * FROM t0 WHERE ((NULL IS FALSE) IS FALSE);\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE IF EXISTS t0;\n  CREATE TABLE t0(c0);\n  INSERT INTO t0(c0) VALUES (0);\n  CREATE INDEX i0 ON t0(NULL > c0) WHERE (NULL NOT NULL);\n  SELECT * FROM t0 WHERE ((NULL IS FALSE) IS FALSE);\n")
			return
		}
		got := flatten(r)
		want := "0"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1800"
		r = db.Query("\n    DROP TABLE IF EXISTS t0;\n    CREATE TABLE t0(c0 REAL, c1 TEXT);\n    CREATE INDEX i0 ON t0(+c0, c0);\n    INSERT INTO t0(c0) VALUES(0);\n    SELECT CAST(+ t0.c0 AS BLOB) LIKE 0 FROM t0; \n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    DROP TABLE IF EXISTS t0;\n    CREATE TABLE t0(c0 REAL, c1 TEXT);\n    CREATE INDEX i0 ON t0(+c0, c0);\n    INSERT INTO t0(c0) VALUES(0);\n    SELECT CAST(+ t0.c0 AS BLOB) LIKE 0 FROM t0; \n  ")
			return
		}
		got := flatten(r)
		want := "0"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1810"
		r = db.Query("\n    SELECT CAST(+ t0.c0 AS BLOB) LIKE '0.0' FROM t0; \n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT CAST(+ t0.c0 AS BLOB) LIKE '0.0' FROM t0; \n  ")
			return
		}
		got := flatten(r)
		want := "1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1820"
		r = db.Query("\n    DROP TABLE IF EXISTS t1;\n    CREATE TABLE t1(x REAL);\n    CREATE INDEX t1x ON t1(x, +x);\n    INSERT INTO t1(x) VALUES(2);\n    SELECT +x FROM t1 WHERE x=2;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    DROP TABLE IF EXISTS t1;\n    CREATE TABLE t1(x REAL);\n    CREATE INDEX t1x ON t1(x, +x);\n    INSERT INTO t1(x) VALUES(2);\n    SELECT +x FROM t1 WHERE x=2;\n  ")
			return
		}
		got := flatten(r)
		want := "2.0"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	db.Close()
	os.Remove("test.db")
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	tcl_nullvalue = "{}" // fresh connection resets nullvalue
	{ // "indexexpr1-1900"
		r = db.Query("\n  CREATE TABLE t1(x TEXT PRIMARY KEY, y TEXT, z INT);\n  INSERT INTO t1(x,y,z) VALUES('alpha','ALPHA',1),('bravo','charlie',1);\n  CREATE INDEX i1 ON t1(+y COLLATE NOCASE);\n  SELECT * FROM t1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t1(x TEXT PRIMARY KEY, y TEXT, z INT);\n  INSERT INTO t1(x,y,z) VALUES('alpha','ALPHA',1),('bravo','charlie',1);\n  CREATE INDEX i1 ON t1(+y COLLATE NOCASE);\n  SELECT * FROM t1;\n")
			return
		}
		got := flatten(r)
		want := "alpha ALPHA 1 bravo charlie 1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1910"
		r = db.Query("\n  DELETE FROM t1 INDEXED BY i1 \n   WHERE x IS +y COLLATE NOCASE IN (SELECT z FROM t1)\n  RETURNING *;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DELETE FROM t1 INDEXED BY i1 \n   WHERE x IS +y COLLATE NOCASE IN (SELECT z FROM t1)\n  RETURNING *;\n")
			return
		}
		got := flatten(r)
		want := "alpha ALPHA 1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-1920"
		r = db.Query("\n  SELECT * FROM t1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t1;\n")
			return
		}
		got := flatten(r)
		want := "bravo charlie 1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	db.Close()
	os.Remove("test.db")
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	tcl_nullvalue = "{}" // fresh connection resets nullvalue
	{ // "indexexpr1-2000" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  CREATE TABLE t1(a INT, b TEXT);\n  INSERT INTO t1(a,b) VALUES\n    (10, '{\"one\":5,\"two\":6}'),\n    (10, '{\"one\":50,\"two\":60}'),\n    (10, '{\"three\":99}'),\n    (11, '{\"one\":100,\"two\":200}');\n  CREATE INDEX t1_one ON t1(a, b->>'one');\n  CREATE INDEX t1_two ON t1(a, b->>'two');\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2010" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT sum(b->>'one') FROM t1 WHERE a=10; /* Query AA */\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2011" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  SELECT sum(b->>'one') FROM t1 WHERE a=10; /* Query AA */\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2020" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  EXPLAIN QUERY PLAN\n  SELECT sum(b->>'two') FROM t1 WHERE a=10; /* Query BB */\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2021" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  SELECT sum(b->>'two') FROM t1 WHERE a=10; /* Query BB */\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2030" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  DROP TABLE t1;\n  CREATE TABLE t1(a INT, b TEXT, c INT, d INT);\n  INSERT INTO t1(a,b,c,d) VALUES\n    (1, '{\"x\":1}', 12,  3),\n    (1, '{\"x\":2}',  4,  5),\n    (1, '{\"x\":1}',  6, 11),\n    (2, '{\"x\":1}', 22,  3),\n    (2, '{\"x\":2}',  4,  5),\n    (3, '{\"x\":1}',  6,  7);\n  CREATE INDEX t1x ON t1(d, a, b->>'x', c);\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2040" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  SELECT a,\n       SUM(1)                              AS t1,\n       SUM(CASE WHEN b->>'x'=1 THEN 1 END) AS t2,\n       SUM(c)                              AS t3,\n       SUM(CASE WHEN b->>'x'=1 THEN c END) AS t4\n    FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2050" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  explain query plan\n  SELECT a,\n       SUM(1)                              AS t1,\n       SUM(CASE WHEN b->>'x'=1 THEN 1 END) AS t2,\n       SUM(c)                              AS t3,\n       SUM(CASE WHEN b->>'x'=1 THEN c END) AS t4\n    FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	db.Close()
	os.Remove("test.db")
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	tcl_nullvalue = "{}" // fresh connection resets nullvalue
	{ // "indexexpr1-2100"
		_res = db.Exec("\n  CREATE TABLE t1(a INTEGER PRIMARY KEY, b INT);\n  INSERT INTO t1(a,b) VALUES(1,0);\n  CREATE INDEX x1 ON t1( \"y\" );\n  CREATE INDEX x2 ON t1( +\"y\" );\n  CREATE INDEX x3 ON t1( +'y' );\n  CREATE INDEX x4 ON t1( \"y*\" );\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t1(a INTEGER PRIMARY KEY, b INT);\n  INSERT INTO t1(a,b) VALUES(1,0);\n  CREATE INDEX x1 ON t1( \"y\" );\n  CREATE INDEX x2 ON t1( +\"y\" );\n  CREATE INDEX x3 ON t1( +'y' );\n  CREATE INDEX x4 ON t1( \"y*\" );\n")
		}
	}
	{ // "indexexpr1-2110"
		r = db.Query("\n  UPDATE t1 SET b=100 WHERE (SELECT 'y') GLOB \"y\";\n  SELECT b FROM t1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  UPDATE t1 SET b=100 WHERE (SELECT 'y') GLOB \"y\";\n  SELECT b FROM t1;\n")
			return
		}
		got := flatten(r)
		want := "100"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-2120"
		r = db.Query("\n  UPDATE t1 SET b=200 WHERE (SELECT 'y') GLOB +\"y\";\n  SELECT b FROM t1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  UPDATE t1 SET b=200 WHERE (SELECT 'y') GLOB +\"y\";\n  SELECT b FROM t1;\n")
			return
		}
		got := flatten(r)
		want := "200"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-2130"
		r = db.Query("\n  UPDATE t1 SET b=300 WHERE (SELECT 'y') GLOB +'y';\n  SELECT b FROM t1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  UPDATE t1 SET b=300 WHERE (SELECT 'y') GLOB +'y';\n  SELECT b FROM t1;\n")
			return
		}
		got := flatten(r)
		want := "300"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "indexexpr1-2140"
		r = db.Query("\n  UPDATE t1 SET b=400 WHERE (SELECT 'y') GLOB \"y*\";\n  SELECT b FROM t1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  UPDATE t1 SET b=400 WHERE (SELECT 'y') GLOB \"y*\";\n  SELECT b FROM t1;\n")
			return
		}
		got := flatten(r)
		want := "400"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	db.Close()
	os.Remove("test.db")
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	tcl_nullvalue = "{}" // fresh connection resets nullvalue
	{ // "indexexpr1-2200" — skipped: JOIN + GROUP BY result order not matched (planner G5) (SQL side effects only)
		_res = db.Exec("\n  CREATE TABLE t1(id INTEGER PRIMARY KEY, tag INT);\n  INSERT INTO t1 VALUES (0, 7), (1, 8);\n  CREATE TABLE t2(type INT, t1_id  INT, value  INT);\n  INSERT INTO t2 VALUES (0, 0, 100), (0, 1, 101);\n  CREATE INDEX t1x ON t1(-tag);\n  SELECT u.tag, v.max_value\n    FROM (SELECT tag FROM t1 GROUP BY -tag) u\n    JOIN (SELECT t1.tag AS \"tag\", t2.type AS \"type\",\n                 MAX(t2.value) AS \"max_value\"\n            FROM t1\n                 JOIN t2 ON t2.t1_id = t1.id\n           GROUP BY t2.type, t1.tag\n         ) v ON v.type = 0 AND v.tag = u.tag;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2210" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  DROP TABLE t1;\n  CREATE TABLE t1(x INT, y TEXT);\n  INSERT INTO t1(x,y) VALUES(1,'{b:5}');\n  SELECT json_insert('{}', '$.a', coalesce(null,json(y)))->>'$.a.b' FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	tcl_nullvalue = "NULL"
	{ // "indexexpr1-2211" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  CREATE INDEX t1j ON t1(coalesce(null,json(y)));\n  SELECT json_insert('{}', '$.a', coalesce(null,json(y)))->>'$.a.b' FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2220" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  DROP INDEX t1j;\n  SELECT json_insert('{}', '$.a', iif(1,json(y),123))->>'$.a.b' FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2221" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  CREATE INDEX t1j ON t1(iif(1,json(y),123));\n  SELECT json_insert('{}', '$.a', iif(1,json(y),123))->>'$.a.b' FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2230" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  DROP INDEX t1j;\n  SELECT json_insert('{}', '$.a', ifnull(NULL,json(y)))->>'$.a.b' FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2231" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  CREATE INDEX t1j ON t1(ifnull(NULL,json(y)));\n  SELECT json_insert('{}', '$.a', ifnull(NULL,json(y)))->>'$.a.b' FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2240" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  DROP INDEX t1j;\n  SELECT json_insert('{}', '$.a', nullif(json(y),8))->>'$.a.b' FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2241" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  CREATE INDEX t1j ON t1(nullif(json(y),8));\n  SELECT json_insert('{}', '$.a', nullif(json(y),8))->>'$.a.b' FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2250" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  DROP INDEX t1j;\n  SELECT json_insert('{}', '$.a', min('~',json(y)))->>'$.a.b' FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2251" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  CREATE INDEX t1j ON t1(min('~',json(y)));\n  SELECT json_insert('{}', '$.a', min('~',json(y)))->>'$.a.b' FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2260" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  DROP INDEX t1j;\n  SELECT json_insert('{}', '$.a', max('...',json(y)))->>'$.a.b' FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "indexexpr1-2261" — skipped: JSON ->- operator not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  CREATE INDEX t1j ON t1(max('...',json(y)));\n  SELECT json_insert('{}', '$.a', max('...',json(y)))->>'$.a.b' FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	db.Close()
	os.Remove("test.db")
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	tcl_nullvalue = "{}" // fresh connection resets nullvalue
	{ // "indexexpr1-2300" — skipped: JSON json()/json_insert() functions not supported (JSON extension excluded) (SQL side effects only)
		_res = db.Exec("\n  CREATE TABLE t1(x INT, y TEXT);\n  INSERT INTO t1(x,y) VALUES(1,'{b:5}');\n  CREATE INDEX t1j ON t1(json(y));\n  SELECT json_insert('{}', '$.a', json(y)) FROM t1;\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	db.Close()
	os.Remove("test.db")
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	tcl_nullvalue = "{}" // fresh connection resets nullvalue
	// proc definition (not transpiled)
	db.RegisterFunction("addone", func(args []interface{}) (interface{}, error) { return nil, nil }, 0, -1)
	{ // "indexexpr1-2400"
		_res = db.Exec("\n  CREATE TABLE t1(x, y);\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t1(x, y);\n")
		}
	}
	{ // "indexexpr1-2310" — skipped: user-defined non-deterministic function in index expression not rejected (harness db func)
	}
	{ // "indexexpr1-2310" — skipped: user-defined non-deterministic function in index expression not rejected (harness db func)
	}
	{ // "indexexpr1-2310" — skipped: user-defined non-deterministic function in index expression not rejected (harness db func)
	}
}
