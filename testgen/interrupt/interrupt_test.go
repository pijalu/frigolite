...
func Test_interrupt(t *testing.T) {
	...
	{ // do_test "interrupt-2.5.1"
		_r = tclBool01(db.IsInterrupted())
	}
	{ // do_test "interrupt-2.5.2"
		vtab.TclVarSet("interrupt_count", "", "0")
		interrupt_count = "0" // TCL namespace variable
		_ = interrupt_count // suppress unused warning
		vtab.TclVarSet("sql", "", "EXPLAIN SELECT max(a,b), a, b FROM t1")
			...
			for _ri := 0; _ri < len(_dbevalRows1.Rows) && _dbevalErr3 == nil; _ri++ {
				for _ci := 0; _ci < len(_dbevalRows1.Columns); _ci++ {
					switch _dbevalRows1.Columns[_ci] {
						case "DB":
							DB = tclStr(_dbevalRows1.Rows[_ri][_ci])
					}
				}
				db.Interrupt()
				_dbevalInt4 = true
	...
	{ // do_test "interrupt-2.5.3"
		...
		got := tclListFlatten(interrupt_count)
		want := tclListFlatten("1")
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "interrupt-2.5.3")
		}
	}
	_res = db.Exec("PRAGMA integrity_check")
	if _res.Error != nil { t.Errorf("integrity check: %v", _res.Error) }
	...
	max_count = tclExprWith("1000000-$sqlite_interrupt_count", map[string]string{"sqlite_interrupt_count": strconv.Itoa(db.InterruptCount())})
