...
func Test_intpkey(t *testing.T) {
	...
	{ // "intpkey-18.6" — skipped: rowid = 2^63 float boundary is version-dependent (3.51: no match) N-A (no-side-effects)
	}
	{ // "intpkey-18.7" — skipped: rowid = 2^63 float boundary is version-dependent (3.51: no match) N-A (no-side-effects)
	}
	{ // "intpkey-18.8"
		r = db.Query("\n  SELECT x FROM t1 WHERE rowid = +9223372036854777856.0;\n")
		if r.Error != nil {
