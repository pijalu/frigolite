...
func Test_join8(t *testing.T) {
	...
	{ // "join8-6010" — skipped: json_each virtual table not supported (SQL side effects only)
	...
	{ // "join8-6020" — skipped: json_each virtual table not supported (SQL side effects only)
		_res = db.Exec("\n  DROP TABLE IF EXISTS t1;\n  CREATE TABLE t1(a INTEGER PRIMARY KEY,b);\n  INSERT INTO t1 VALUES(0,NULL),(1,2);\n  SELECT value, t1.* FROM json_each('17') NATURAL RIGHT JOIN t1\n   WHERE (a,b) IN (SELECT rowid, b FROM t1);\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "join8-6021" — skipped: json_each virtual table not supported (SQL side effects only)
		_res = db.Exec("\n  SELECT value, t1.* FROM json_each('null') NATURAL RIGHT JOIN t1\n   WHERE (a,b) IN (SELECT rowid, b FROM t1);\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "join8-6022" — skipped: json_each virtual table not supported (SQL side effects only)
	...
	{ // "join8-18040" — skipped: rtree virtual table not supported (SQL side effects only)
		_res = db.Exec("\n    SELECT *, x NOTNULL, (x NOTNULL)=a FROM t2 \n      RIGHT JOIN rtree1 ON true INNER JOIN t3 ON (x NOTNULL)=+a;\n  ")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "join8-18050" — skipped: rtree virtual table not supported (SQL side effects only)
		_res = db.Exec("\n    SELECT *, x NOTNULL, (x NOTNULL)=a FROM t2 \n      RIGHT JOIN rtree1 ON true INNER JOIN t3 ON (x NOTNULL)=a;\n  ")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	db.Close()
