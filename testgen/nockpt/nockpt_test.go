// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package nockpt

import (
"github.com/pijalu/frigolite"
"github.com/pijalu/frigolite/internal/vtab"
"os"
"strconv"
"testing"
)

func Test_nockpt(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var testprefix string
	_ = testprefix // pre-declared from TCL source
	var stmt string
	_ = stmt // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	if tclBool("" + "==\"journaltest\" || " + "" + "==\"inmemory_journal\"") {
		return
	}
	vtab.TclVarSet("testprefix", "", "nockpt")
	testprefix = "nockpt"
	_ = testprefix // suppress unused warning
	{ // "1.0"
		r = db.Query("\n  PRAGMA auto_vacuum=OFF;\n  PRAGMA page_size = 1024;\n  PRAGMA journal_mode = wal;\n  CREATE TABLE c1(x, y, z);\n  INSERT INTO c1 VALUES(1, 2, 3);\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  PRAGMA auto_vacuum=OFF;\n  PRAGMA page_size = 1024;\n  PRAGMA journal_mode = wal;\n  CREATE TABLE c1(x, y, z);\n  INSERT INTO c1 VALUES(1, 2, 3);\n")
			return
		}
		got := flatten(r)
		want := "wal"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // do_test "1.1"
		// file exists "test.db-wal"
	}
	{ // do_test "1.2" (file size test.db-wal)
		got := strconv.Itoa(tclFileSize("test.db-wal"))
		if got != "wal_file_size 3 1024" {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, "wal_file_size 3 1024", "1.2")
		}
	}
	{ // do_test "1.3"
		db.Close()
	}
	{ // do_test "1.4"
		// file exists "test.db-wal"
	}
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	{ // "1.5"
		_res = db.Exec("\n  INSERT INTO c1 VALUES(4, 5, 6);\n  INSERT INTO c1 VALUES(7, 8, 9);\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  INSERT INTO c1 VALUES(4, 5, 6);\n  INSERT INTO c1 VALUES(7, 8, 9);\n")
		}
	}
	{ // do_test "1.6"
		// file exists "test.db-wal"
	}
	{ // do_test "1.7"
		// sqlite3_db_config NO_CKPT_ON_CLOSE (unhandled flag)
	}
	{ // do_test "1.8" (file size test.db-wal)
		got := strconv.Itoa(tclFileSize("test.db-wal"))
		if got != "wal_file_size 2 1024" {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, "wal_file_size 2 1024", "1.8")
		}
	}
	{ // do_test "1.9"
		db.Close()
	}
	{ // do_test "1.10"
		// file exists "test.db-wal"
	}
	{ // do_test "1.11" (file size test.db-wal)
		got := strconv.Itoa(tclFileSize("test.db-wal"))
		if got != "wal_file_size 2 1024" {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, "wal_file_size 2 1024", "1.11")
		}
	}
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	{ // "1.12"
		r = db.Query("\n  SELECT * FROM c1\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM c1\n")
			return
		}
		got := flatten(r)
		want := "1 2 3 4 5 6 7 8 9"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "1.13"
		r = db.Query(" PRAGMA main.journal_mode ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA main.journal_mode ")
			return
		}
		got := flatten(r)
		want := "wal"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // do_test "1.14"
		// sqlite3_db_config NO_CKPT_ON_CLOSE (unhandled flag)
	}
	{ // "1.14"
		r = db.Query(" PRAGMA main.journal_mode = delete ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA main.journal_mode = delete ")
			return
		}
		got := flatten(r)
		want := "delete"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // do_test "1.15"
		// file exists "test.db-wal"
	}
	if tcl_platform_platform != "windows" {
		db.Close()
		os.Remove("test.db")
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		tcl_nullvalue = "{}" // fresh connection resets nullvalue
		db.Close()
		// set ::db1 (skipped, DB connection)
		{ // do_test "2.0"
			_r = tclLIndex("sqlite3_exec $::db1 {\n      PRAGMA journal_mode = wal;\n      CREATE TABLE t1(x PRIMARY KEY, y UNIQUE, z);\n      INSERT INTO t1 VALUES(1, 2, 3);\n      PRAGMA wal_checkpoint;\n    }", "0") // lindex result
			if _r != "0" {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", _r, "0", "2.0")
			}
		}
		// prepared stmt: SELECT * FROM t1 (bind/step emulation)
		tclPrepareStep(db1, "SELECT * FROM t1", "stmt")
		_ = stmt // prepared statement handle
		// sqlite3_close_v2 $::db1 (unsupported command, not transpiled)
		{
			var _catchErr error
			_ = _catchErr // suppress unused warning
			os.Remove("test.db")
		}
		{
			var _catchErr error
			_ = _catchErr // suppress unused warning
			os.Remove("test.db-wal")
		}
		{
			var _catchErr error
			_ = _catchErr // suppress unused warning
			os.Remove("test.db-shm")
		}
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		db2, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
		{ // "2.1"
			r = db.Query("\n  PRAGMA auto_vacuum=OFF;\n  PRAGMA journal_mode = wal;\n  CREATE TABLE y1(a PRIMARY KEY, b UNIQUE, c);\n  INSERT INTO y1 VALUES('a', 'b', 'c');\n  INSERT INTO y1 VALUES('d', 'e', 'f');\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  PRAGMA auto_vacuum=OFF;\n  PRAGMA journal_mode = wal;\n  CREATE TABLE y1(a PRIMARY KEY, b UNIQUE, c);\n  INSERT INTO y1 VALUES('a', 'b', 'c');\n  INSERT INTO y1 VALUES('d', 'e', 'f');\n")
				return
			}
			got := flatten(r)
			want := "wal"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "2.2"
			r = db2.Query("\n  BEGIN;\n    SELECT * FROM y1;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  BEGIN;\n    SELECT * FROM y1;\n")
				return
			}
			got := flatten(r)
			want := "a b c d e f"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "2.3"
			r = db.Query("\n  UPDATE y1 SET c='g' WHERE a='d';\n  PRAGMA wal_checkpoint;\n")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  UPDATE y1 SET c='g' WHERE a='d';\n  PRAGMA wal_checkpoint;\n")
				return
			}
			got := flatten(r)
			want := "0 11 10"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
		{ // "2.4"
			_res = db2.Exec("\n  COMMIT\n")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  COMMIT\n")
			}
		}
		{ // "2.5" (prepare-step internals; SQL side effects only)
			tclFinalizePrepared("stmt")
			// sqlite3_finalize $stmt
			db3, err = frigolite.Open("test.db")
			if err != nil { t.Fatal(err) }
			r = db3.Query(" \n    PRAGMA integrity_check; \n    SELECT * FROM y1;\n  ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " \n    PRAGMA integrity_check; \n    SELECT * FROM y1;\n  ")
			}
		}
	}
}
