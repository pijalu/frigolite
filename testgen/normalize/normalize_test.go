...
func Test_normalize(t *testing.T) {
	...
	for _idx0 := 0; _idx0+3 <= len(_items0); _idx0 += 3 {
		...
		_ = _idx0
			{ // do_test tnum
				// [list sqlite3_normalize $sql] (test-harness C API, not transpiled)
			}
		}
		{ // do_test "200"
			_res = db.Exec("\n    CREATE TABLE t1(a,b);\n  ")
		...
		{ // "201" (prepare-step internals; SQL side effects only)
			_ = STMT // prepared statement handle
			// sqlite3_bind_null $STMT (unknown prepared statement)
		}
		{ // "202" (prepare-step internals; SQL side effects only)
		...
		{ // "210" (prepare-step internals; SQL side effects only)
			_ = STMT // prepared statement handle
			// sqlite3_bind_null $STMT (unknown prepared statement)
		}
		{ // "211" (prepare-step internals; SQL side effects only)
			// sqlite3_normalized_sql $STMT (unsupported command, not transpiled)
		}
		...
		{ // "220" (prepare-step internals; SQL side effects only)
			_ = STMT // prepared statement handle
		}
		{ // "221" (prepare-step internals; SQL side effects only)
			// sqlite3_normalized_sql $STMT (unsupported command, not transpiled)
		}
		...
		for _idx1 := 0; _idx1+4 <= len(_items1); _idx1 += 4 {
			...
			_ = norm // suppress unused warning
			_ = _idx1
				{ // tnum (prepare-step internals; SQL side effects only)
	...
	_ = res // suppress unused warning
					...
					if tclBool(tclBool01(vtab.TclVarExists("STMT", ""))) {
						tclFinalizePrepared("STMT")
						// sqlite3_finalize $STMT
					}
