// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package notify

import (
"github.com/pijalu/frigolite"
"os"
"strings"
"testing"
)

func Test_notify3(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var esc string
	_ = esc // pre-declared from TCL source
	var stmt string
	_ = stmt // pre-declared from TCL source
	var when string
	_ = when // pre-declared from TCL source
	var res string
	_ = res // pre-declared from TCL source
	var noerr string
	_ = noerr // pre-declared from TCL source
	var tn string
	_ = tn // pre-declared from TCL source
	var db1_loaded string
	_ = db1_loaded // pre-declared from TCL source
	var db2_loaded string
	_ = db2_loaded // pre-declared from TCL source
	var enable_extended_errors string
	_ = enable_extended_errors // pre-declared from TCL source
	var result string
	_ = result // pre-declared from TCL source
	var error1 string
	_ = error1 // pre-declared from TCL source
	var error2 string
	_ = error2 // pre-declared from TCL source
	var invoked string
	_ = invoked // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	esc = "sqlite3_enable_shared_cache 1"
	_ = esc // suppress unused warning
	_dbtmp0, err := frigolite.Open("test.db")
	_ = _dbtmp0 // sqlite3 db connection
	if err != nil { t.Fatal(err) }
	os.Remove("test.db2")
	db2, err = frigolite.Open("test.db2")
	if err != nil { t.Fatal(err) }
	{ // do_test "notify3-1.1"
		_res = db.Exec(" \n    CREATE TABLE t1(a, b); \n    INSERT INTO t1 VALUES('t1 A', 't1 B');\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, " \n    CREATE TABLE t1(a, b); \n    INSERT INTO t1 VALUES('t1 A', 't1 B');\n  ")
		}
	}
	{ // do_test "notify3-1.2"
		_res = db2.Exec(" \n    CREATE TABLE t2(a, b);\n    INSERT INTO t2 VALUES('t2 A', 't2 B');\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, " \n    CREATE TABLE t2(a, b);\n    INSERT INTO t2 VALUES('t2 A', 't2 B');\n  ")
		}
	}
	{ // do_test "notify3-1.3"
		_res = db2.Exec(" \n    BEGIN EXCLUSIVE;\n    INSERT INTO t2 VALUES('t2 C', 't2 D');\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, " \n    BEGIN EXCLUSIVE;\n    INSERT INTO t2 VALUES('t2 C', 't2 D');\n  ")
		}
	}
	{ // do_test "notify3-1.4"
		_res = db.Exec(" ATTACH 'test.db2' AS aux ")
		_ = _res // catchsql
	}
	{ // do_test "notify3-1.5"
		_res = db.Exec(" SELECT * FROM t2 ")
		_ = _res // catchsql
	}
	{ // do_test "notify3-1.6"
		_list := tclList([]string{"0", "sqlite3_extended_errcode db"})
		_ = _list
	}
	{ // do_test "notify3-1.7"
		// sqlite3_extended_result_codes db 1 (unsupported command, not transpiled)
		{
			var msg string // catch result ("0"=ok, "1"=error)
			var _catchErrMsg string // catch error message
			_ = msg // suppress unused warning
			_ = _catchErrMsg // suppress unused warning
			var _catchErr error
			stmt = "" // TCL namespace variable
			_ = stmt // suppress unused warning
			if _catchErr != nil {
				msg = "1"
				_catchErrMsg = _catchErr.Error()
			} else {
				msg = "0"
				_catchErrMsg = ""
			}
		}
	}
	{ // do_test "notify3-1.8"
		when = "1" // TCL namespace variable
		_ = when // suppress unused warning
		when = "2" // TCL namespace variable
		_ = when // suppress unused warning
		_res = db2.Exec(" COMMIT ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, " COMMIT ")
		}
		_ = res // TCL namespace variable (query)
	}
	{ // do_test "notify3-1.9"
		_res = db.Exec(" SELECT * FROM t2 ")
		_ = _res // catchsql
	}
	db.Close()
	var _err_tcl string
	_err_tcl = "{1 {unable to open database: test.db2}}"
	_ = _err_tcl // suppress unused warning
	noerr = "{0 {}}"
	_ = noerr // suppress unused warning
	if tclBool("presql" + " == \"\"") {
		// foreach {tn db1_loaded db2_loaded enable_extended_errors result error1 error2} "0   0 0 0   " + _err_tcl + "     SQLITE_LOCKED               SQLITE_LOCKED_SHAREDCACHE\n    1   0 0 1   " + _err_tcl + "     SQLITE_LOCKED_SHAREDCACHE   SQLITE_LOCKED_SHAREDCACHE\n    2   0 1 0   " + noerr + "   SQLITE_OK                   SQLITE_OK\n    3   0 1 1   " + noerr + "   SQLITE_OK                   SQLITE_OK\n    4   1 0 0   " + _err_tcl + "     SQLITE_LOCKED               SQLITE_LOCKED_SHAREDCACHE\n    5   1 0 1   " + _err_tcl + "     SQLITE_LOCKED_SHAREDCACHE   SQLITE_LOCKED_SHAREDCACHE\n    6   1 1 0   " + noerr + "   SQLITE_OK                   SQLITE_OK\n    7   1 1 1   " + noerr + "   SQLITE_OK                   SQLITE_OK"
		_items0 := tclSplitList("0   0 0 0   " + _err_tcl + "     SQLITE_LOCKED               SQLITE_LOCKED_SHAREDCACHE\n    1   0 0 1   " + _err_tcl + "     SQLITE_LOCKED_SHAREDCACHE   SQLITE_LOCKED_SHAREDCACHE\n    2   0 1 0   " + noerr + "   SQLITE_OK                   SQLITE_OK\n    3   0 1 1   " + noerr + "   SQLITE_OK                   SQLITE_OK\n    4   1 0 0   " + _err_tcl + "     SQLITE_LOCKED               SQLITE_LOCKED_SHAREDCACHE\n    5   1 0 1   " + _err_tcl + "     SQLITE_LOCKED_SHAREDCACHE   SQLITE_LOCKED_SHAREDCACHE\n    6   1 1 0   " + noerr + "   SQLITE_OK                   SQLITE_OK\n    7   1 1 1   " + noerr + "   SQLITE_OK                   SQLITE_OK")
		for _idx0 := 0; _idx0+7 <= len(_items0); _idx0 += 7 {
			tn := _items0[_idx0+0]
			_ = tn // suppress unused warning
			db1_loaded := _items0[_idx0+1]
			_ = db1_loaded // suppress unused warning
			db2_loaded := _items0[_idx0+2]
			_ = db2_loaded // suppress unused warning
			enable_extended_errors := _items0[_idx0+3]
			_ = enable_extended_errors // suppress unused warning
			result := _items0[_idx0+4]
			_ = result // suppress unused warning
			error1 := _items0[_idx0+5]
			_ = error1 // suppress unused warning
			error2 := _items0[_idx0+6]
			_ = error2 // suppress unused warning
			_ = _idx0
				{ // do_test "notify3-2." + tn + ".1"
					{
						var _catchErr error
						_ = _catchErr // suppress unused warning
						db1.Close()
					}
					{
						var _catchErr error
						_ = _catchErr // suppress unused warning
						db2.Close()
					}
					db1 = db // sqlite3 db1 test.db: alias to main in-memory db
					_ = db1
					db2, err = frigolite.Open("test.db2")
					if err != nil { t.Fatal(err) }
					// sqlite3_extended_result_codes db1 $enable_extended_errors (unsupported command, not transpiled)
					// sqlite3_extended_result_codes db2 $enable_extended_errors (unsupported command, not transpiled)
					if tclBool(db1_loaded) {
						_res = db1.Exec("SELECT * FROM sqlite_master")
						if _res.Error != nil { t.Errorf("exec error: %v", _res.Error) }
					}
					if tclBool(db2_loaded) {
						_res = db2.Exec("SELECT * FROM sqlite_master")
						if _res.Error != nil { t.Errorf("exec error: %v", _res.Error) }
					}
					_res = db2.Exec("BEGIN EXCLUSIVE")
					if _res.Error != nil { t.Errorf("exec error: %v", _res.Error) }
					_res = db.Exec("ATTACH 'test.db2' AS two")
					_ = _res // catchsql
					if _res.Error == nil || !strings.Contains(_res.Error.Error(), result) {
						t.Errorf("expected error containing %s, got: %v\n  body: do_test %s", result, _res.Error, "notify3-2." + tn + ".1")
					}
				}
				{ // do_test "notify3-2." + tn + ".2"
					_list := tclList([]string{"0", "sqlite3_extended_errcode db1"})
					_ = _list
				}
				{ // do_test "notify3-2." + tn + ".3"
					// db1.unlock_notify (db command)
					invoked = "0"
					_ = invoked // suppress unused warning
					_res = db2.Exec("commit")
					if _res.Error != nil { t.Errorf("exec error: %v", _res.Error) }
				}
			}
		}
		{
			var _catchErr error
			_ = _catchErr // suppress unused warning
			db1.Close()
		}
		{
			var _catchErr error
			_ = _catchErr // suppress unused warning
			db2.Close()
		}
		// sqlite3_enable_shared_cache $esc (unsupported command, not transpiled)
}
