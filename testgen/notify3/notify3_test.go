...
func Test_notify3(t *testing.T) {
	...
	{ // do_test "notify3-1.7"
		...
		want := tclListFlattenCollapse("(262) database schema is locked: aux")
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "notify3-1.7")
		}
	}
	{ // do_test "notify3-1.8"
		vtab.TclVarSet("when", "", "1")
		when = "1" // TCL namespace variable
		_ = when // suppress unused warning
		vtab.TclVarSet("when", "", "2")
		...
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "notify3-1.8")
		}
	}
	{ // do_test "notify3-1.9"
		_res = db.Exec(" SELECT * FROM t2 ")
		...
		for _idx0 := 0; _idx0+7 <= len(_items0); _idx0 += 7 {
			...
			_ = _idx0
				...
				{ // do_test "notify3-2." + tn + ".3"
					...
					vtab.TclVarSet("invoked", "", "0")
					invoked = "0"
					_ = invoked // suppress unused warning
					_res = db2.Exec("commit")
					if _res.Error != nil { t.Errorf("exec error: %v", _res.Error) }
					got := tclListFlatten(invoked)
					want := tclListFlatten(tclLIndex(result, "0"))
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "notify3-2." + tn + ".3")
					}
				}
			}
		}
