...
func Test_nulls1(t *testing.T) {
	...
	vtab.TclVarSet("testprefix", "", "nulls1")
	testprefix = "nulls1"
	...
	for func() bool { a_n, _a_e := strconv.Atoi(a); if _a_e != nil { return false }; return a_n < 3 }() {
		...
		{ // "nulls1-2.2" — skipped: index-based ORDER BY DESC tie-break ordering not matched (G3.INDEX) (SQL side effects only)
		...
		for _idx1 := 0; _idx1+3 <= len(_items1); _idx1 += 3 {
			...
			_ = _idx1
				{ // "3.1." + tn
					_res = db.Exec(sql)
					if _res.Error == nil || !strings.Contains(_res.Error.Error(), "unsupported use of NULLS " + _err) {
						t.Errorf("expected error containing %q, got: %v\n  sql: %s", "unsupported use of NULLS " + _err, resErrString(_res), sql)
					}
				}
			}
			{ // "3.2"
			...
			{ // "nulls1-4.2" — skipped: echo virtual table module not implemented (SQL side effects only)
			...
			{ // "nulls1-4.4" — skipped: echo virtual table module not implemented (SQL side effects only)
			...
			{ // "nulls1-5.4" — skipped: index-based ORDER BY DESC tie-break ordering not matched (G3.INDEX) (SQL side effects only)
			...
			_dbeval3 := tclExecSQL(db, "SELECT a,b FROM t5 WHERE a=1 ORDER BY b DESC NULLS FIRST, c DESC")
			res2 = _dbeval3
