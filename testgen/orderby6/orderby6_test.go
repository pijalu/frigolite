// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package orderby6

import (
"github.com/pijalu/frigolite"
"github.com/pijalu/frigolite/internal/vtab"
"os"
"strings"
"testing"
)

func Test_orderby6(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var testprefix string
	_ = testprefix // pre-declared from TCL source
	var tn string
	_ = tn // pre-declared from TCL source
	var rowidclause string
	_ = rowidclause // pre-declared from TCL source
	var tx string
	_ = tx // pre-declared from TCL source
	var limit string
	_ = limit // pre-declared from TCL source
	var offset string
	_ = offset // pre-declared from TCL source
	var orderby string
	_ = orderby // pre-declared from TCL source
	var sql1 string
	_ = sql1 // pre-declared from TCL source
	var sql2 string
	_ = sql2 // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	vtab.TclVarSet("testprefix", "", "orderby6")
	testprefix = "orderby6" // TCL namespace variable
	_ = testprefix // suppress unused warning
	// foreach {tn rowidclause} "1 {} 2 {WITHOUT ROWID}"
	_items0 := tclSplitList("1 {} 2 {WITHOUT ROWID}")
	for _idx0 := 0; _idx0+2 <= len(_items0); _idx0 += 2 {
		tn := _items0[_idx0+0]
		_ = tn // suppress unused warning
		rowidclause := _items0[_idx0+1]
		_ = rowidclause // suppress unused warning
		_ = _idx0
			db.Close()
			os.Remove("test.db")
			db, err = frigolite.Open("test.db")
			if err != nil { t.Fatal(err) }
			tcl_nullvalue = "{}" // fresh connection resets nullvalue
			{ // do_test tn + ".1"
				_res = db.Exec("CREATE TABLE t1(a,b,c,PRIMARY KEY(b,c)) " + rowidclause + ";")
				_res = db.Exec("\n      WITH RECURSIVE\n       cnt(x) AS (VALUES(1) UNION ALL SELECT x+1 FROM cnt WHERE x<1000)\n     INSERT INTO t1 SELECT x, x%40, x/40 FROM cnt;\n    ")
			}
			{ // tn + ".2"
				r = db.Query("\n    SELECT b,a,c FROM t1 ORDER BY b,a,c;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT b,a,c FROM t1 ORDER BY b,a,c;\n  ")
					return
				}
				got := flatten(r)
				_want1 := db.Query("SELECT b,a,c FROM t1 ORDER BY +b,+a,+c")
				if _want1.Error != nil {
					t.Errorf("expected query error: %v\n  sql: %s", _want1.Error, "SELECT b,a,c FROM t1 ORDER BY +b,+a,+c")
					return
				}
				want := flatten(_want1)
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // tn + ".3"
				r = db.Query("\n    SELECT b,a,c FROM t1 ORDER BY b,c DESC,a;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT b,a,c FROM t1 ORDER BY b,c DESC,a;\n  ")
					return
				}
				got := flatten(r)
				_want2 := db.Query("SELECT b,a,c FROM t1 ORDER BY +b,+c DESC,+a")
				if _want2.Error != nil {
					t.Errorf("expected query error: %v\n  sql: %s", _want2.Error, "SELECT b,a,c FROM t1 ORDER BY +b,+c DESC,+a")
					return
				}
				want := flatten(_want2)
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // tn + ".4"
				r = db.Query("\n    SELECT b,a,c FROM t1 ORDER BY b DESC,c,a;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT b,a,c FROM t1 ORDER BY b DESC,c,a;\n  ")
					return
				}
				got := flatten(r)
				_want3 := db.Query("SELECT b,a,c FROM t1 ORDER BY +b DESC,+c,+a")
				if _want3.Error != nil {
					t.Errorf("expected query error: %v\n  sql: %s", _want3.Error, "SELECT b,a,c FROM t1 ORDER BY +b DESC,+c,+a")
					return
				}
				want := flatten(_want3)
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // tn + ".5"
				r = db.Query("\n    SELECT b,a,c FROM t1 ORDER BY b DESC,a,c;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT b,a,c FROM t1 ORDER BY b DESC,a,c;\n  ")
					return
				}
				got := flatten(r)
				_want4 := db.Query("SELECT b,a,c FROM t1 ORDER BY +b DESC,+a,+c")
				if _want4.Error != nil {
					t.Errorf("expected query error: %v\n  sql: %s", _want4.Error, "SELECT b,a,c FROM t1 ORDER BY +b DESC,+a,+c")
					return
				}
				want := flatten(_want4)
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // tn + ".11"
				r = db.Query("\n    SELECT a FROM t1 ORDER BY b, a LIMIT 10 OFFSET 20;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t1 ORDER BY b, a LIMIT 10 OFFSET 20;\n  ")
					return
				}
				got := flatten(r)
				want := "840 880 920 960 1000 1 41 81 121 161"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // tn + ".11x"
				r = db.Query("\n    SELECT a FROM t1 ORDER BY +b, a LIMIT 10 OFFSET 20;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t1 ORDER BY +b, a LIMIT 10 OFFSET 20;\n  ")
					return
				}
				got := flatten(r)
				want := "840 880 920 960 1000 1 41 81 121 161"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // tn + ".12"
				r = db.Query("\n    SELECT a FROM t1 ORDER BY b DESC, a LIMIT 10 OFFSET 20;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t1 ORDER BY b DESC, a LIMIT 10 OFFSET 20;\n  ")
					return
				}
				got := flatten(r)
				want := "839 879 919 959 999 38 78 118 158 198"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // tn + ".12"
				r = db.Query("\n    SELECT a FROM t1 ORDER BY +b DESC, a LIMIT 10 OFFSET 20;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t1 ORDER BY +b DESC, a LIMIT 10 OFFSET 20;\n  ")
					return
				}
				got := flatten(r)
				want := "839 879 919 959 999 38 78 118 158 198"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // tn + ".13"
				r = db.Query("\n    SELECT a FROM t1 ORDER BY b, a DESC LIMIT 10 OFFSET 45;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t1 ORDER BY b, a DESC LIMIT 10 OFFSET 45;\n  ")
					return
				}
				got := flatten(r)
				want := "161 121 81 41 1 962 922 882 842 802"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // tn + ".13x"
				r = db.Query("\n    SELECT a FROM t1 ORDER BY +b, a DESC LIMIT 10 OFFSET 45;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t1 ORDER BY +b, a DESC LIMIT 10 OFFSET 45;\n  ")
					return
				}
				got := flatten(r)
				want := "161 121 81 41 1 962 922 882 842 802"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // tn + ".14"
				r = db.Query("\n    SELECT a FROM t1 ORDER BY b DESC, a LIMIT 10 OFFSET 45;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t1 ORDER BY b DESC, a LIMIT 10 OFFSET 45;\n  ")
					return
				}
				got := flatten(r)
				want := "838 878 918 958 998 37 77 117 157 197"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			{ // tn + ".14x"
				r = db.Query("\n    SELECT a FROM t1 ORDER BY +b DESC, a LIMIT 10 OFFSET 45;\n  ")
				if r.Error != nil {
					t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t1 ORDER BY +b DESC, a LIMIT 10 OFFSET 45;\n  ")
					return
				}
				got := flatten(r)
				want := "838 878 918 958 998 37 77 117 157 197"
				if got != want {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
				}
			}
			// foreach {tx limit offset orderby} "1  10 24 {+b,+a}\n     2  10 25 {+b,+a}\n     3  10 26 {+b,+a}\n     4  10 39 {+b,+a}\n     5  10 40 {+b,+a}\n     6  10 41 {+b,+a}\n     7  27 24 {+b,+a}\n     8  27 49 {+b,+a}\n     11 10 24 {+b DESC,+a}\n     12 10 25 {+b DESC,+a}\n     13 10 26 {+b DESC,+a}\n     14 10 39 {+b DESC,+a}\n     15 10 40 {+b DESC,+a}\n     16 10 41 {+b DESC,+a}\n     17 27 24 {+b DESC,+a}\n     18 27 49 {+b DESC,+a}\n     21 10 24 {+b,+a DESC}\n     22 10 25 {+b,+a DESC}\n     23 10 26 {+b,+a DESC}\n     24 10 39 {+b,+a DESC}\n     25 10 40 {+b,+a DESC}\n     26 10 41 {+b,+a DESC}\n     27 27 24 {+b,+a DESC}\n     28 27 49 {+b,+a DESC}\n     31 10 24 {+b DESC,+a DESC}\n     32 10 25 {+b DESC,+a DESC}\n     33 10 26 {+b DESC,+a DESC}\n     34 10 39 {+b DESC,+a DESC}\n     35 10 40 {+b DESC,+a DESC}\n     36 10 41 {+b DESC,+a DESC}\n     37 27 24 {+b DESC,+a DESC}\n     38 27 49 {+b DESC,+a DESC}"
			_items5 := tclSplitList("1  10 24 {+b,+a}\n     2  10 25 {+b,+a}\n     3  10 26 {+b,+a}\n     4  10 39 {+b,+a}\n     5  10 40 {+b,+a}\n     6  10 41 {+b,+a}\n     7  27 24 {+b,+a}\n     8  27 49 {+b,+a}\n     11 10 24 {+b DESC,+a}\n     12 10 25 {+b DESC,+a}\n     13 10 26 {+b DESC,+a}\n     14 10 39 {+b DESC,+a}\n     15 10 40 {+b DESC,+a}\n     16 10 41 {+b DESC,+a}\n     17 27 24 {+b DESC,+a}\n     18 27 49 {+b DESC,+a}\n     21 10 24 {+b,+a DESC}\n     22 10 25 {+b,+a DESC}\n     23 10 26 {+b,+a DESC}\n     24 10 39 {+b,+a DESC}\n     25 10 40 {+b,+a DESC}\n     26 10 41 {+b,+a DESC}\n     27 27 24 {+b,+a DESC}\n     28 27 49 {+b,+a DESC}\n     31 10 24 {+b DESC,+a DESC}\n     32 10 25 {+b DESC,+a DESC}\n     33 10 26 {+b DESC,+a DESC}\n     34 10 39 {+b DESC,+a DESC}\n     35 10 40 {+b DESC,+a DESC}\n     36 10 41 {+b DESC,+a DESC}\n     37 27 24 {+b DESC,+a DESC}\n     38 27 49 {+b DESC,+a DESC}")
			for _idx5 := 0; _idx5+4 <= len(_items5); _idx5 += 4 {
				tx := _items5[_idx5+0]
				_ = tx // suppress unused warning
				limit := _items5[_idx5+1]
				_ = limit // suppress unused warning
				offset := _items5[_idx5+2]
				_ = offset // suppress unused warning
				orderby := _items5[_idx5+3]
				_ = orderby // suppress unused warning
				_ = _idx5
					vtab.TclVarSet("sql1", "", "SELECT a FROM t1 ORDER BY " + orderby + " LIMIT " + limit + " OFFSET " + offset + ";")
					sql1 = "SELECT a FROM t1 ORDER BY " + orderby + " LIMIT " + limit + " OFFSET " + offset + ";"
					_ = sql1 // suppress unused warning
					sql2 = strings.ReplaceAll(sql1, "+", "")
					_ = sql2 // suppress unused warning
					{ // do_test tn + ".21." + tx
						r = db.Query(sql2)
						if r.Error != nil {
							t.Errorf("query error: %v\n  sql: %s", r.Error, sql2)
							return
						}
						got := flatten(r)
						_want6 := db.Query(sql1)
						if _want6.Error != nil {
							t.Errorf("expected query error: %v\n  sql: %s", _want6.Error, sql1)
							return
						}
						want := flatten(_want6)
						if got != want {
							t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
						}
					}
				}
				{ // do_test tn + ".31"
					_res = db.Exec("CREATE TABLE t2(a,b,c,d,e,f,PRIMARY KEY(b,c,d,e,f)) " + rowidclause + ";")
					_res = db.Exec("\n      WITH RECURSIVE\n       cnt(x) AS (VALUES(0) UNION ALL SELECT x+1 FROM cnt WHERE x<242)\n     INSERT INTO t2 SELECT x,  x%3, (x/3)%3, (x/9)%3, (x/27)%3, (x/81)%3\n                      FROM cnt;\n    ")
				}
				{ // tn + ".32"
					r = db.Query("\n    SELECT a FROM t2 ORDER BY b,c,d,e,f;\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t2 ORDER BY b,c,d,e,f;\n  ")
						return
					}
					got := flatten(r)
					_want7 := db.Query("SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f;")
					if _want7.Error != nil {
						t.Errorf("expected query error: %v\n  sql: %s", _want7.Error, "SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f;")
						return
					}
					want := flatten(_want7)
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // tn + ".33"
					r = db.Query("\n    SELECT a FROM t2 ORDER BY b,c,d,e,+f;\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t2 ORDER BY b,c,d,e,+f;\n  ")
						return
					}
					got := flatten(r)
					_want8 := db.Query("SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f;")
					if _want8.Error != nil {
						t.Errorf("expected query error: %v\n  sql: %s", _want8.Error, "SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f;")
						return
					}
					want := flatten(_want8)
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // tn + ".34"
					r = db.Query("\n    SELECT a FROM t2 ORDER BY b,c,d,+e,+f;\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t2 ORDER BY b,c,d,+e,+f;\n  ")
						return
					}
					got := flatten(r)
					_want9 := db.Query("SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f;")
					if _want9.Error != nil {
						t.Errorf("expected query error: %v\n  sql: %s", _want9.Error, "SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f;")
						return
					}
					want := flatten(_want9)
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // tn + ".35"
					r = db.Query("\n    SELECT a FROM t2 ORDER BY b,c,+d,+e,+f;\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t2 ORDER BY b,c,+d,+e,+f;\n  ")
						return
					}
					got := flatten(r)
					_want10 := db.Query("SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f;")
					if _want10.Error != nil {
						t.Errorf("expected query error: %v\n  sql: %s", _want10.Error, "SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f;")
						return
					}
					want := flatten(_want10)
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // tn + ".36"
					r = db.Query("\n    SELECT a FROM t2 ORDER BY b,+c,+d,+e,+f;\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t2 ORDER BY b,+c,+d,+e,+f;\n  ")
						return
					}
					got := flatten(r)
					_want11 := db.Query("SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f;")
					if _want11.Error != nil {
						t.Errorf("expected query error: %v\n  sql: %s", _want11.Error, "SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f;")
						return
					}
					want := flatten(_want11)
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // tn + ".37"
					r = db.Query("\n    SELECT a FROM t2 ORDER BY b,c,d,e,f DESC;\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t2 ORDER BY b,c,d,e,f DESC;\n  ")
						return
					}
					got := flatten(r)
					_want12 := db.Query("SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f DESC;")
					if _want12.Error != nil {
						t.Errorf("expected query error: %v\n  sql: %s", _want12.Error, "SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f DESC;")
						return
					}
					want := flatten(_want12)
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // tn + ".38"
					r = db.Query("\n    SELECT a FROM t2 ORDER BY b,c,d,e DESC,f;\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t2 ORDER BY b,c,d,e DESC,f;\n  ")
						return
					}
					got := flatten(r)
					_want13 := db.Query("SELECT a FROM t2 ORDER BY +b,+c,+d,+e DESC,+f;")
					if _want13.Error != nil {
						t.Errorf("expected query error: %v\n  sql: %s", _want13.Error, "SELECT a FROM t2 ORDER BY +b,+c,+d,+e DESC,+f;")
						return
					}
					want := flatten(_want13)
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // tn + ".39"
					r = db.Query("\n    SELECT a FROM t2 ORDER BY b,c,d DESC,e,f;\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t2 ORDER BY b,c,d DESC,e,f;\n  ")
						return
					}
					got := flatten(r)
					_want14 := db.Query("SELECT a FROM t2 ORDER BY +b,+c,+d DESC,+e,+f;")
					if _want14.Error != nil {
						t.Errorf("expected query error: %v\n  sql: %s", _want14.Error, "SELECT a FROM t2 ORDER BY +b,+c,+d DESC,+e,+f;")
						return
					}
					want := flatten(_want14)
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // tn + ".40"
					r = db.Query("\n    SELECT a FROM t2 ORDER BY b,c DESC,d,e,f;\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t2 ORDER BY b,c DESC,d,e,f;\n  ")
						return
					}
					got := flatten(r)
					_want15 := db.Query("SELECT a FROM t2 ORDER BY +b,+c DESC,+d,+e,+f;")
					if _want15.Error != nil {
						t.Errorf("expected query error: %v\n  sql: %s", _want15.Error, "SELECT a FROM t2 ORDER BY +b,+c DESC,+d,+e,+f;")
						return
					}
					want := flatten(_want15)
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // tn + ".41"
					r = db.Query("\n    SELECT a FROM t2 ORDER BY b DESC,c,d,e,f;\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t2 ORDER BY b DESC,c,d,e,f;\n  ")
						return
					}
					got := flatten(r)
					_want16 := db.Query("SELECT a FROM t2 ORDER BY +b DESC,+c,+d,+e,+f;")
					if _want16.Error != nil {
						t.Errorf("expected query error: %v\n  sql: %s", _want16.Error, "SELECT a FROM t2 ORDER BY +b DESC,+c,+d,+e,+f;")
						return
					}
					want := flatten(_want16)
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // tn + ".42"
					r = db.Query("\n    SELECT a FROM t2 ORDER BY b DESC,c DESC,d,e,f LIMIT 31;\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t2 ORDER BY b DESC,c DESC,d,e,f LIMIT 31;\n  ")
						return
					}
					got := flatten(r)
					_want17 := db.Query("SELECT a FROM t2 ORDER BY +b DESC,+c DESC,+d,+e,+f LIMIT 31")
					if _want17.Error != nil {
						t.Errorf("expected query error: %v\n  sql: %s", _want17.Error, "SELECT a FROM t2 ORDER BY +b DESC,+c DESC,+d,+e,+f LIMIT 31")
						return
					}
					want := flatten(_want17)
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
				{ // tn + ".43"
					r = db.Query("\n    SELECT a FROM t2 ORDER BY b,c,d,e,f DESC LIMIT 8 OFFSET 7;\n  ")
					if r.Error != nil {
						t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a FROM t2 ORDER BY b,c,d,e,f DESC LIMIT 8 OFFSET 7;\n  ")
						return
					}
					got := flatten(r)
					_want18 := db.Query("SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f DESC LIMIT 8 OFFSET 7")
					if _want18.Error != nil {
						t.Errorf("expected query error: %v\n  sql: %s", _want18.Error, "SELECT a FROM t2 ORDER BY +b,+c,+d,+e,+f DESC LIMIT 8 OFFSET 7")
						return
					}
					want := flatten(_want18)
					if got != want {
						t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
					}
				}
			}
}
