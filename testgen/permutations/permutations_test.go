...
func Test_permutations(t *testing.T) {
	...
	// test_suite veryquick -prefix  -description {\n  "Very" quick test suite. Runs in minutes on a ...} -files [\n  test_set $allquicktests -exclude *malloc* *io... (unsupported command, not transpiled)
	// test_suite shell -prefix  -description {\n  Run tests of the command-line shell\n} -files [\n  test_set [glob $testdir/shell*.test]\n] (unsupported command, not transpiled)
	// test_suite extraquick -prefix  -description {\n  "Extra" quick test suite. Runs in a few minute...} -files [\n  test_set $allquicktests -exclude *malloc* *io... (unsupported command, not transpiled)
	// test_suite mmap -prefix mm- -description {\n  Similar to veryquick. Except with memory mappi...} -presql {\n  pragma mmap_size = 268435456;\n} -files [\n  test_set $allquicktests -exclude *malloc* *io... (unsupported command, not transpiled)
	// test_suite valgrind -prefix  -description {\n  Run the "veryquick" test suite with a couple o...} -files [\n  test_set $allquicktests -exclude *malloc* *io... -initialize {\n  set ::G(valgrind) 1\n} -shutdown {\n  unset -nocomplain ::G(valgrind)\n} (unsupported command, not transpiled)
	...
	_ = discard_rate // suppress unused warning
		// test_suite pcache${discard_rate} -description \n    Alternative pcache implementation with ${dis... -initialize \n    catch {db close}\n    sqlite3_shutdown\n    ... -shutdown {\n    catch {db close}\n    sqlite3_shutdown\n    ...} -files $ {perm-alt-pcache-testset} (unsupported command, not transpiled)
	}
	// test_suite journaltest -description {\n  Check that pages are synced before being writt...} -initialize {\n  catch {db close}\n  register_jt_vfs -default "...} -shutdown {\n  unregister_jt_vfs\n} -files [test_set $::allquicktests -exclude {\n  wal* incr... (unsupported command, not transpiled)
	if tclBool("info commands register_demovfs" + " != \"\"") {
		// test_suite demovfs -description {\n    Check that the demovfs (code in test_demovfs...} -initialize {\n    register_demovfs\n  } -shutdown {\n    unregister_demovfs\n  } -files {\n    insert.test   insert2.test  insert3.test rol...} (unsupported command, not transpiled)
	}
	// test_suite wal -description {\n  Run tests with journal_mode=WAL\n} -initialize {\n  set ::G(savepoint6_iterations) 100\n} -shutdown {\n  unset -nocomplain ::G(savepoint6_iterations)\n} -files {\n  savepoint.test     savepoint2.test     savepoi...} (unsupported command, not transpiled)
	...
	// test_suite session_eec -description {\n  All session module related tests with sqlite3_...} -files [\n  glob -nocomplain $::testdir/../ext/session/*.... -dbconfig {\n  sqlite3_extended_result_codes $::dbhandle 1\n} (unsupported command, not transpiled)
	// test_suite session_strm -description {\n  All session module related tests using the str...} -files [\n  glob -nocomplain $::testdir/../ext/session/*.... -dbconfig {\n  set ::sqlite3session_streams 1\n} (unsupported command, not transpiled)
	// test_suite rbu -description {\n  RBU tests.\n} -files [\n  test_set [glob -nocomplain $::testdir/../ext/... (unsupported command, not transpiled)
	// test_suite no_optimization -description {\n  Run test scripts with optimizations disabled u...} -files [\n  test_set \\n    [glob -nocomplain $::testdir/... -dbconfig {\n  optimization_control $::dbhandle all 0\n} (unsupported command, not transpiled)
	// test_suite prepare -description {\n  Run tests with the db connection using sqlite3...} -dbconfig {\n  $::dbhandle version -use-legacy-prepare 1\n  #...} -files [\n  test_set $allquicktests -exclude *malloc* *io... (unsupported command, not transpiled)
	// test_suite sorterref -prefix  -description {\n  Run the "veryquick" test suite with SQLITE_CON...} -files [\n  test_set $allquicktests -exclude *malloc* *io... -initialize {\n  catch {db close}\n  sqlite3_shutdown\n  sqlite...} -shutdown {\n  catch {db close}\n  sqlite3_shutdown\n  sqlite...} (unsupported command, not transpiled)
	// test_suite maindbname -prefix  -description {\n  Run the "veryquick" test suite with SQLITE_DBC...} -files [\n  test_set $allquicktests -exclude *malloc* *io... -dbconfig {\n  dbconfig_maindbname_icecube $::dbhandle\n} (unsupported command, not transpiled)
	// test_suite win_unc_locking -prefix  -description {\n  Run the "wal*" tests with UNC style single fd ...} -files [\n  test_set                                     ... -initialize {\n  set sqlite3_win_test_unc_locking 1\n} -shutdown {\n  set sqlite3_win_test_unc_locking 0\n} (unsupported command, not transpiled)
	// proc definition (not transpiled)
	// proc definition (not transpiled)
	// proc definition (not transpiled)
