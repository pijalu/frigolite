	...
	for _idx0 := 0; _idx0+4 <= len(_items0); _idx0 += 4 {
		...
		_ = _idx0
			{ // do_test "quota-glob-" + testnum + ".1"
				_r = tclQuotaGlob(pattern, text)
				if _r != ans {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", _r, ans, "quota-glob-" + testnum + ".1")
				}
			}
			{ // do_test "quota-glob-" + testnum + ".2"
				_r = tclQuotaGlob(pattern, strings.ReplaceAll(text, "/", "\\"))
				if _r != ans {
					t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", _r, ans, "quota-glob-" + testnum + ".2")
				}
			}
		}
}
