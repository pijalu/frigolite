...
func Test_rowid(t *testing.T) {
	...
	rowid2xMap := map[string]string{}
	_ = rowid2xMap // dynamic-key array from TCL source

	// set testdir: test directory (not used in Go test context)
	vtab.TclVarSet("testprefix", "", "rowid")
	testprefix = "rowid"
	...
	{ // do_test "rowid-1.2"
		...
		_ = _r // suppress unused warning
		vtab.TclVarSet("x2rowid", "1", tclLIndex(_r, "0"))
		x2rowid_1 = tclLIndex(_r, "0")
		_ = x2rowid_1 // suppress unused warning
		vtab.TclVarSet("x2rowid", "3", tclLIndex(_r, "1"))
		x2rowid_3 = tclLIndex(_r, "1")
		_ = x2rowid_3 // suppress unused warning
		rowid2xMap[x2rowid_1] = "1"
		rowid2xMap[x2rowid_3] = "3"
		_ = strconv.Itoa(tclLLength(_r)) // llength result
	}
	{ // do_test "rowid-1.3"
		vtab.TclVarSet("sql", "", "SELECT x FROM t1 WHERE rowid==" + x2rowid_1)
	...
	{ // do_test "rowid-1.4"
		vtab.TclVarSet("sql", "", "SELECT x FROM t1 WHERE rowid==" + x2rowid_3)
	...
	{ // do_test "rowid-1.5"
		vtab.TclVarSet("sql", "", "SELECT x FROM t1 WHERE oid==" + x2rowid_1)
	...
	{ // do_test "rowid-1.6"
		vtab.TclVarSet("sql", "", "SELECT x FROM t1 WHERE OID==" + x2rowid_3)
	...
	{ // do_test "rowid-1.7"
		vtab.TclVarSet("sql", "", "SELECT x FROM t1 WHERE _rowid_==" + x2rowid_1)
	...
	{ // do_test "rowid-2.7"
		vtab.TclVarSet("sql", "", "UPDATE t1 SET x=2 WHERE OID==" + x2rowid_3)
	...
	{ // do_test "rowid-2.8"
		vtab.TclVarSet("sql", "", "UPDATE t1 SET x=3 WHERE _rowid_==" + x2rowid_3)
	...
	for func() bool { i_n, _i_e := strconv.Atoi(i); if _i_e != nil { return false }; return i_n < 100 }() {
		{ // do_test "rowid-12.3." + i
			...
			if flatten(r) != tclListFlatten(i) {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", flatten(r), tclListFlatten(i), "rowid-12.3." + i)
			}
		}
		// incr i 1
		{
	...
	{ // "rowid-13.1" — skipped: C test-harness addrow() function not implemented N-A (no-side-effects)
	}
	{ // "rowid-14.0"
	...
	{ // "rowid-15.1" — skipped: join rowid affinity/ordering edge case N-A (no-side-effects)
	}
	{ // "rowid-15.2" — skipped: ambiguous rowid in join is version-dependent (3.51: ambiguous) N-A (no-side-effects)
	}
	db.Close()
	os.Remove("test.db")
	db, err = frigolite.Open("test.db")
	...
	{ // "rowid-16.5" — skipped: unqualified rowid in mixed rowid/without-rowid join resolution N-A (no-side-effects)
	}
	{ // "16.6"
