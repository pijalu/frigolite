// Code generated by tcl2go; DO NOT EDIT.
package schema

import (
"github.com/pijalu/frigolite"
"testing"
)

func Test_schema(t *testing.T) {
	db, err := frigolite.Open("")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result

	// set testdir: test directory (not used in Go test context)
	{ // do_test "schema-1.1"
		var _STMT = "sqlite3_prepare $::DB {SELECT * FROM sqlite_master} -1 TAIL" // TCL namespace variable
		_ = _STMT // suppress unused warning
		_res = db.Exec("\n    CREATE TABLE abc(a, b, c);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TABLE abc(a, b, c);\n  ")
		}
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_step $::STMT")
	}
	{ // do_test "schema-1.2"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_finalize $::STMT")
	}
	{ // do_test "schema-1.3"
		var _STMT = "sqlite3_prepare $::DB {SELECT * FROM sqlite_master} -1 TAIL" // TCL namespace variable
		_ = _STMT // suppress unused warning
		_res = db.Exec("\n    DROP TABLE abc;\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    DROP TABLE abc;\n  ")
		}
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_step $::STMT")
	}
	{ // do_test "schema-1.4"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_finalize $::STMT")
	}
	{ // do_test "schema-4.1"
		_res = db.Exec("\n    CREATE TABLE abc(a, b, c);\n  ")
		_ = _res // catchsql
		var _STMT = "sqlite3_prepare $::DB {SELECT * FROM sqlite_master} -1 TAIL" // TCL namespace variable
		_ = _STMT // suppress unused warning
		_res = db.Exec("\n    CREATE INDEX abc_index ON abc(a);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE INDEX abc_index ON abc(a);\n  ")
		}
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_step $::STMT")
	}
	{ // do_test "schema-4.2"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_finalize $::STMT")
	}
	{ // do_test "schema-4.3"
		var _STMT = "sqlite3_prepare $::DB {SELECT * FROM sqlite_master} -1 TAIL" // TCL namespace variable
		_ = _STMT // suppress unused warning
		_res = db.Exec("\n    DROP INDEX abc_index;\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    DROP INDEX abc_index;\n  ")
		}
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_step $::STMT")
	}
	{ // do_test "schema-4.4"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_finalize $::STMT")
	}
	{ // do_test "schema-6.1"
		var sql = "SELECT * FROM abc;"
		_ = sql // suppress unused warning
		var _STMT = "sqlite3_prepare $::DB $sql -1 TAIL" // TCL namespace variable
		_ = _STMT // suppress unused warning
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_step $::STMT")
	}
	{ // do_test "schema-6.2"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_reset $::STMT")
	}
	{ // do_test "schema-6.3"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite_delete_function $::DB hello_function")
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_step $::STMT")
	}
	{ // do_test "schema-6.4"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_finalize $::STMT")
	}
	{ // do_test "schema-9.1"
		db2, err := frigolite.Open("test.db")
		defer db2.Close()
		if err != nil { t.Fatal(err) }
		_res = db.Exec("\n    DROP TABLE abc;\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    DROP TABLE abc;\n  ")
		}
		db2.Close()
		_res = db.Exec("\n    SELECT * FROM abc;\n  ")
		_ = _res // catchsql
	}
	_res = db.Exec("\n  CREATE TABLE abc(a, b, c);\n")
	if _res.Error != nil {
		t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE abc(a, b, c);\n")
	}
	{ // do_test "schema-10.1"
		_res = db.Exec("\n    INSERT INTO abc VALUES(1, 2, 3);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    INSERT INTO abc VALUES(1, 2, 3);\n  ")
		}
		var sql = "SELECT * FROM abc"
		_ = sql // suppress unused warning
		var _STMT = "sqlite3_prepare $::DB $sql -1 TAIL" // TCL namespace variable
		_ = _STMT // suppress unused warning
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_step $::STMT")
	}
	{ // do_test "schema-10.2"
		_res = db.Exec("\n    CREATE TABLE t2(a, b, c);\n  ")
		_ = _res // catchsql
	}
	{ // do_test "schema-10.3"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_finalize $::STMT")
	}
	{ // do_test "schema-10.4"
		db2, err := frigolite.Open("test.db")
		defer db2.Close()
		if err != nil { t.Fatal(err) }
		r = db.Query("\n    SELECT * FROM abc\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT * FROM abc\n  ")
		}
	}
	{ // do_test "schema-10.5"
		db2.Close()
	}
	{ // do_test "schema-11.1"
		var sql = "SELECT * FROM abc"
		_ = sql // suppress unused warning
		var _STMT = "sqlite3_prepare $::DB $sql -1 TAIL" // TCL namespace variable
		_ = _STMT // suppress unused warning
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_step $::STMT")
	}
	{ // do_test "schema-11.2"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite_delete_function $::DB tstfunc")
	}
	{ // do_test "schema-11.3"
		var rc string
		var msg string
		_ = msg // suppress unused warning
		{ // catch block
			var _catchErr error
			if _catchErr != nil {
				rc = "1"
				msg = _catchErr.Error()
			} else {
				rc = "0"
				msg = ""
			}
		}
		_list := tclList([]string{rc, msg})
		_ = _list
	}
	{ // do_test "schema-11.4"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_finalize $::STMT")
	}
	{ // do_test "schema-11.5"
		var sql = "SELECT * FROM abc"
		_ = sql // suppress unused warning
		var _STMT = "sqlite3_prepare $::DB $sql -1 TAIL" // TCL namespace variable
		_ = _STMT // suppress unused warning
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_step $::STMT")
	}
	{ // do_test "schema-11.6"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite_delete_collation $::DB tstcollate")
	}
	{ // do_test "schema-11.7"
		var rc string
		var msg string
		_ = msg // suppress unused warning
		{ // catch block
			var _catchErr error
			if _catchErr != nil {
				rc = "1"
				msg = _catchErr.Error()
			} else {
				rc = "0"
				msg = ""
			}
		}
		_list := tclList([]string{rc, msg})
		_ = _list
	}
	{ // do_test "schema-11.8"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3_finalize $::STMT")
	}
	{ // do_test "schema-12.1"
		_res = db.Exec("\n    BEGIN;\n    CREATE TABLE t3(a, b, c);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    BEGIN;\n    CREATE TABLE t3(a, b, c);\n  ")
		}
		var _STMT = "sqlite3_prepare $::DB \"CREATE TABLE t4(a,b,c)\" -1 TAIL" // TCL namespace variable
		_ = _STMT // suppress unused warning
		_res = db.Exec("\n    ROLLBACK;\n    CREATE TABLE t4(a, b, c);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    ROLLBACK;\n    CREATE TABLE t4(a, b, c);\n  ")
		}
		_list := tclList([]string{"SQLITE_ROW", ""})
		_ = _list
	}
}
