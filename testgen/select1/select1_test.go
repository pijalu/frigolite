...
func Test_select1(t *testing.T) {
	...
	{ // do_test "select1-9.2"
		...
		_dbevalRows0 := db.Query("SELECT * FROM test1 WHERE f1<0")
		...
		_r_arr = vtab.TclVarGet("r", "*")
		got := tclListFlatten(_r_arr)
		want := tclListFlatten("f1 f2")
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "select1-9.2")
		}
	}
	{ // do_test "select1-9.3"
		vtab.TclVarSet("r", "*", "")
		_r_arr = ""
		_ = _r_arr // suppress unused warning
		_dbevalRows2 := db.Query("SELECT * FROM test1 WHERE f1<(select count(*) from test2)")
		...
		want := tclListFlatten("f1 f2")
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "select1-9.3")
		}
	}
	{ // do_test "select1-9.4"
		vtab.TclVarSet("r", "*", "")
		_r_arr = ""
		_ = _r_arr // suppress unused warning
		_dbevalRows4 := db.Query("SELECT * FROM test1 ORDER BY f1")
		...
		want := tclListFlatten("f1 f2")
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "select1-9.4")
		}
	}
	{ // do_test "select1-9.5"
		vtab.TclVarSet("r", "*", "")
		_r_arr = ""
		_ = _r_arr // suppress unused warning
		_dbevalRows6 := db.Query("SELECT * FROM test1 WHERE f1<0 ORDER BY f1")
		...
		want := tclListFlatten("f1 f2")
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "select1-9.5")
		}
	}
	...
	_rows8 := db.Query("SELECT name FROM sqlite_master WHERE type = 'table'")
	if _rows8.Error != nil {
		t.Errorf("query error: %v\n  sql: %s", _rows8.Error, "SELECT name FROM sqlite_master WHERE type = 'table'")
	}
