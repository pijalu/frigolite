// Code generated by tcl2go; DO NOT EDIT.
package superlock

import (
"github.com/pijalu/frigolite"
"os"
"strings"
"testing"
)

func Test_superlock(t *testing.T) {
	db, err := frigolite.Open("")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	_ = msg // suppress unused warning

	// set testdir: test directory (not used in Go test context)
	var testprefix = "superlock"
	_ = testprefix // suppress unused warning
	t.Skipf("TODO: %s not implemented in frigolite", "do_not_use_codec")
	{ // "1.1"
		r = db.Query("\n  CREATE TABLE t1(a, b);\n  INSERT INTO t1 VALUES(1, 2);\n  PRAGMA journal_mode = DELETE;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t1(a, b);\n  INSERT INTO t1 VALUES(1, 2);\n  PRAGMA journal_mode = DELETE;\n")
			return
		}
		got := flatten(r)
		want := "delete"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // do_test "1.2"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3demo_superlock unlock test.db")
	}
	{ // "1.3"
		_res = db.Exec(" SELECT * FROM t1 ")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "database is locked") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "database is locked", _res.Error, " SELECT * FROM t1 ")
		}
	}
	{ // do_test "1.4"
		t.Skipf("TODO: %s not implemented in frigolite", "unlock")
	}
	{ // "2.1"
		r = db.Query(" \n  INSERT INTO t1 VALUES(3, 4);\n  PRAGMA journal_mode = WAL;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, " \n  INSERT INTO t1 VALUES(3, 4);\n  PRAGMA journal_mode = WAL;\n")
			return
		}
		got := flatten(r)
		want := "wal"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // do_test "2.2"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3demo_superlock unlock test.db")
	}
	{ // "2.3"
		_res = db.Exec(" SELECT * FROM t1 ")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "database is locked") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "database is locked", _res.Error, " SELECT * FROM t1 ")
		}
	}
	{ // "2.4"
		_res = db.Exec(" INSERT INTO t1 VALUES(5, 6)")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "database is locked") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "database is locked", _res.Error, " INSERT INTO t1 VALUES(5, 6)")
		}
	}
	{ // "2.5"
		_res = db.Exec(" PRAGMA wal_checkpoint ")
		if _res.Error == nil {
			t.Errorf("expected error, got none\n  sql: %s", " PRAGMA wal_checkpoint ")
		}
	}
	{ // do_test "2.6"
		t.Skipf("TODO: %s not implemented in frigolite", "unlock")
	}
	{ // "3.1"
		_res = db.Exec(" INSERT INTO t1 VALUES(3, 4) ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, " INSERT INTO t1 VALUES(3, 4) ")
		}
	}
	{ // do_test "3.2"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3demo_superlock unlock test.db")
	}
	{ // "3.3"
		_res = db.Exec(" SELECT * FROM t1 ")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "database is locked") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "database is locked", _res.Error, " SELECT * FROM t1 ")
		}
	}
	{ // "3.4"
		_res = db.Exec(" INSERT INTO t1 VALUES(5, 6)")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "database is locked") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "database is locked", _res.Error, " INSERT INTO t1 VALUES(5, 6)")
		}
	}
	{ // "3.5"
		_res = db.Exec(" PRAGMA wal_checkpoint ")
		if _res.Error == nil {
			t.Errorf("expected error, got none\n  sql: %s", " PRAGMA wal_checkpoint ")
		}
	}
	{ // do_test "3.6"
		t.Skipf("TODO: %s not implemented in frigolite", "unlock")
	}
	{ // "4.1"
		r = db.Query(" PRAGMA wal_checkpoint ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA wal_checkpoint ")
			return
		}
		got := flatten(r)
		want := "0 1 1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // do_test "4.2"
		t.Skipf("TODO: %s not implemented in frigolite", "sqlite3demo_superlock unlock test.db")
	}
	{ // "4.3"
		_res = db.Exec(" SELECT * FROM t1 ")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "database is locked") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "database is locked", _res.Error, " SELECT * FROM t1 ")
		}
	}
	{ // "4.4"
		_res = db.Exec(" INSERT INTO t1 VALUES(5, 6)")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "database is locked") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "database is locked", _res.Error, " INSERT INTO t1 VALUES(5, 6)")
		}
	}
	{ // "4.5"
		_res = db.Exec(" PRAGMA wal_checkpoint ")
		if _res.Error == nil {
			t.Errorf("expected error, got none\n  sql: %s", " PRAGMA wal_checkpoint ")
		}
	}
	{ // do_test "4.6"
		t.Skipf("TODO: %s not implemented in frigolite", "unlock")
	}
	t.Skipf("TODO: %s not implemented in frigolite", "do_multiclient_test tn {\n\n  proc busyhandler {x} {\n    switch -- $x {\n    ...}")
	// proc definition (not transpiled)
	// proc definition (not transpiled)
	// proc definition (not transpiled)
	os.Remove("test.db")
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	{ // "6.1"
		r = db.Query("\n  ATTACH 'test.db2' AS aux;\n  PRAGMA aux.journal_mode = wal;\n  CREATE TABLE aux.t2(x, y);\n  INSERT INTO aux.t2 VALUES('a', 'b');\n  PRAGMA schema_version = 450;\n  DETACH aux;\n\n  PRAGMA main.journal_mode = wal;\n  CREATE TABLE t1(a, b);\n  INSERT INTO t1 VALUES(1, 2);\n  INSERT INTO t1 VALUES(3, 4);\n  SELECT * FROM t1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  ATTACH 'test.db2' AS aux;\n  PRAGMA aux.journal_mode = wal;\n  CREATE TABLE aux.t2(x, y);\n  INSERT INTO aux.t2 VALUES('a', 'b');\n  PRAGMA schema_version = 450;\n  DETACH aux;\n\n  PRAGMA main.journal_mode = wal;\n  CREATE TABLE t1(a, b);\n  INSERT INTO t1 VALUES(1, 2);\n  INSERT INTO t1 VALUES(3, 4);\n  SELECT * FROM t1;\n")
			return
		}
		got := flatten(r)
		want := "wal wal 1 2 3 4"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	t.Skipf("TODO: %s not implemented in frigolite", "db_swap test.db2 test.db")
	{ // "6.2"
		_res = db.Exec(" SELECT * FROM t1 ")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "no such table: t1") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "no such table: t1", _res.Error, " SELECT * FROM t1 ")
		}
	}
	{ // "6.3"
		_res = db.Exec(" SELECT * FROM t2 ")
		if _res.Error == nil {
			t.Errorf("expected error, got none\n  sql: %s", " SELECT * FROM t2 ")
		}
	}
	t.Skipf("TODO: %s not implemented in frigolite", "db_swap test.db2 test.db")
	{ // "6.4"
		_res = db.Exec(" SELECT * FROM t1 ")
		if _res.Error == nil {
			t.Errorf("expected error, got none\n  sql: %s", " SELECT * FROM t1 ")
		}
	}
	{ // "6.5"
		_res = db.Exec(" SELECT * FROM t2 ")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "no such table: t2") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "no such table: t2", _res.Error, " SELECT * FROM t2 ")
		}
	}
	{ // "6.6"
		r = db.Query(" PRAGMA wal_checkpoint ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA wal_checkpoint ")
			return
		}
		got := flatten(r)
		want := "0 0 0"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	t.Skipf("TODO: %s not implemented in frigolite", "db_swap test.db2 test.db")
	{ // "6.7"
		_res = db.Exec(" SELECT * FROM t1 ")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "no such table: t1") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "no such table: t1", _res.Error, " SELECT * FROM t1 ")
		}
	}
	{ // "6.8"
		_res = db.Exec(" SELECT * FROM t2 ")
		if _res.Error == nil {
			t.Errorf("expected error, got none\n  sql: %s", " SELECT * FROM t2 ")
		}
	}
	t.Skipf("TODO: %s not implemented in frigolite", "db_swap test.db2 test.db")
	{ // "6.9"
		_res = db.Exec(" SELECT * FROM t1 ")
		if _res.Error == nil {
			t.Errorf("expected error, got none\n  sql: %s", " SELECT * FROM t1 ")
		}
	}
	{ // "6.10"
		_res = db.Exec(" SELECT * FROM t2 ")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "no such table: t2") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "no such table: t2", _res.Error, " SELECT * FROM t2 ")
		}
	}
	if tclBool("nonzero_reserved_bytes") {
		{ // "6.11codec"
			_res = db.Exec(" \n    PRAGMA journal_mode = delete;\n    VACUUM;\n    PRAGMA journal_mode = wal;\n    INSERT INTO t1 VALUES(5, 6);\n  ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " \n    PRAGMA journal_mode = delete;\n    VACUUM;\n    PRAGMA journal_mode = wal;\n    INSERT INTO t1 VALUES(5, 6);\n  ")
			}
		}
	} else {
		{ // "6.11"
			_res = db.Exec(" \n    PRAGMA journal_mode = delete;\n    PRAGMA page_size = 512;\n    VACUUM;\n    PRAGMA journal_mode = wal;\n    INSERT INTO t1 VALUES(5, 6);\n  ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, " \n    PRAGMA journal_mode = delete;\n    PRAGMA page_size = 512;\n    VACUUM;\n    PRAGMA journal_mode = wal;\n    INSERT INTO t1 VALUES(5, 6);\n  ")
			}
		}
	}
	t.Skipf("TODO: %s not implemented in frigolite", "db_swap test.db2 test.db")
	{ // "6.12"
		_res = db.Exec(" SELECT * FROM t1 ")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "no such table: t1") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "no such table: t1", _res.Error, " SELECT * FROM t1 ")
		}
	}
	{ // "6.13"
		_res = db.Exec(" SELECT * FROM t2 ")
		if _res.Error == nil {
			t.Errorf("expected error, got none\n  sql: %s", " SELECT * FROM t2 ")
		}
	}
	t.Skipf("TODO: %s not implemented in frigolite", "db_swap test.db2 test.db")
	{ // "6.14"
		_res = db.Exec(" SELECT * FROM t1 ")
		if _res.Error == nil {
			t.Errorf("expected error, got none\n  sql: %s", " SELECT * FROM t1 ")
		}
	}
	{ // "6.15"
		_res = db.Exec(" SELECT * FROM t2 ")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "no such table: t2") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "no such table: t2", _res.Error, " SELECT * FROM t2 ")
		}
	}
}
