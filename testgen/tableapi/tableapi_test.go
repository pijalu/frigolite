...
func Test_tableapi(t *testing.T) {
	...
	{ // do_test "tableapi-1.0"
		dbx, err := frigolite.Open("test.db")
	...
	{ // do_test "tableapi-2.4"
		...
		manyquote += manyquote
		manyquote += manyquote
		vtab.TclVarSet("big_str", "", manyquote + " Hello " + manyquote)
	...
	{ // do_test "tableapi-5.1"
		vtab.TclVarSet("sql", "", "CREATE TABLE t2(")
		sql = "CREATE TABLE t2("
		...
		vtab.TclVarSet("sql", "", "INSERT INTO t2 VALUES(")
	...
	{ // do_test "tableapi-5.2"
		...
		_ = i // suppress unused warning
		for func() bool { i_n, _i_e := strconv.Atoi(i); if _i_e != nil { return false }; return i_n <= 100 }() {
			sql += sep + tclExprWith("$i+1000", map[string]string{"i": i})
			vtab.TclVarSet("sep", "", ",")
			sep = ","
			_ = sep // suppress unused warning
			// incr i 1
	...
	{ // do_test "tableapi-99.0"
		_r = tclCloseDB(dbx)
		if _r != "SQLITE_OK" {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", _r, "SQLITE_OK", "tableapi-99.0")
		}
	}
