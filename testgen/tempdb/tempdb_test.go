// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package tempdb

import (
"github.com/pijalu/frigolite"
"os"
"testing"
)

func Test_tempdb(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var jrnl_in_memory string
	_ = jrnl_in_memory // pre-declared from TCL source
	var subj_in_memory string
	_ = subj_in_memory // pre-declared from TCL source
	var sqlite_open_file_count string
	_ = sqlite_open_file_count // pre-declared from TCL source
	var Id_ string
	_ = Id_ // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	if tclBool("atomic_batch_write test.db") {
		return
	}
	db.Close()
	db, err = frigolite.Open("")
	if err != nil { t.Fatal(err) }
	{ // do_test "tempdb-1.1"
		_res = db.Exec("\n    BEGIN;\n    CREATE TABLE t1(x UNIQUE);\n    CREATE TABLE t2(y);\n    INSERT INTO t2 VALUES('hello');\n    INSERT INTO t2 VALUES(NULL);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    BEGIN;\n    CREATE TABLE t1(x UNIQUE);\n    CREATE TABLE t2(y);\n    INSERT INTO t2 VALUES('hello');\n    INSERT INTO t2 VALUES(NULL);\n  ")
		}
		_res = db.Exec("\n    INSERT INTO t1\n    SELECT CASE WHEN y IS NULL THEN test_error('oops', 11) ELSE y END\n      FROM t2;\n  ")
		_ = _res // catchsql
	}
	{ // do_test "tempdb-1.2"
		r = db.Query("\n    SELECT * FROM t1\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT * FROM t1\n  ")
		}
	}
	{ // do_test "tempdb-2.1"
		jrnl_in_memory = tclExpr("[permutation] eq \"inmemory_journal\"")
		_ = jrnl_in_memory // suppress unused warning
		subj_in_memory = tclExprWith("$jrnl_in_memory || $TEMP_STORE>=2", map[string]string{"jrnl_in_memory": jrnl_in_memory, "TEMP_STORE": TEMP_STORE})
		_ = subj_in_memory // suppress unused warning
		db.Close()
		db, err = frigolite.Open("test.db")
		if err != nil { t.Fatal(err) }
	}
	{ // do_test "tempdb-2.2"
		_res = db.Exec("\n    CREATE TABLE t1 (a PRIMARY KEY, b, c);\n    CREATE TABLE t2 (a, b, c);\n    BEGIN;\n      INSERT INTO t1 VALUES(1, 2, 3);\n      INSERT INTO t1 VALUES(4, 5, 6);\n      INSERT INTO t2 VALUES(7, 8, 9);\n      INSERT INTO t2 SELECT * FROM t1;\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TABLE t1 (a PRIMARY KEY, b, c);\n    CREATE TABLE t2 (a, b, c);\n    BEGIN;\n      INSERT INTO t1 VALUES(1, 2, 3);\n      INSERT INTO t1 VALUES(4, 5, 6);\n      INSERT INTO t2 VALUES(7, 8, 9);\n      INSERT INTO t2 SELECT * FROM t1;\n  ")
		}
		_res = db.Exec(" INSERT INTO t1 SELECT * FROM t2 ")
		_ = _res // catchsql
		got := tclListFlatten(sqlite_open_file_count)
		want := tclListFlatten(tclExprWith("1 + (0==$jrnl_in_memory)", map[string]string{"jrnl_in_memory": jrnl_in_memory}))
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "tempdb-2.2")
		}
	}
	{ // do_test "tempdb-2.3"
		r = db.Query("\n    PRAGMA temp_store = 'memory';\n    ROLLBACK;\n    BEGIN;\n      INSERT INTO t1 VALUES(1, 2, 3);\n      INSERT INTO t1 VALUES(4, 5, 6);\n      INSERT INTO t2 SELECT * FROM t1;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    PRAGMA temp_store = 'memory';\n    ROLLBACK;\n    BEGIN;\n      INSERT INTO t1 VALUES(1, 2, 3);\n      INSERT INTO t1 VALUES(4, 5, 6);\n      INSERT INTO t2 SELECT * FROM t1;\n  ")
		}
		_res = db.Exec(" INSERT INTO t1 SELECT * FROM t2 ")
		_ = _res // catchsql
		got := tclListFlatten(sqlite_open_file_count)
		want := tclListFlatten(tclExprWith("1 + (0==$jrnl_in_memory)", map[string]string{"jrnl_in_memory": jrnl_in_memory}))
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]\n  body: do_test %s", got, want, "tempdb-2.3")
		}
	}
}
