// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package thread003

import (
"github.com/pijalu/frigolite"
"github.com/pijalu/frigolite/internal/vtab"
"os"
"strconv"
"testing"
)

func Test_thread003(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var ii string
	_ = ii // pre-declared from TCL source
	var nSecond string
	_ = nSecond // pre-declared from TCL source
	var zFile string
	_ = zFile // pre-declared from TCL source
	var SCRIPT string
	_ = SCRIPT // pre-declared from TCL source
	var sqlite_open_file_count string
	_ = sqlite_open_file_count // pre-declared from TCL source
	var Id_ string
	_ = Id_ // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source
	var iEnd string
	_ = iEnd // pre-declared from TCL source
	var iQuery string
	_ = iQuery // pre-declared from TCL source
	var DB string
	_ = DB // pre-declared from TCL source
	var thread_procs string
	_ = thread_procs // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	if tclBool("run_thread_tests" + "==0") {
		return
	}
	{ // do_test "thread003.1.1"
		_res = db.Exec("\n    BEGIN;\n    CREATE TABLE t1(a, b, c);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    BEGIN;\n    CREATE TABLE t1(a, b, c);\n  ")
		}
		vtab.TclVarSet("ii", "", "0")
		ii = "0"
		_ = ii // suppress unused warning
		for func() bool { ii_n, _ii_e := strconv.Atoi(ii); if _ii_e != nil { return false }; return ii_n < 5000 }() {
			_res = db.Exec("INSERT INTO t1 VALUES(" + sqlLiteral(ii) + ", randomblob(200), randomblob(200))")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "INSERT INTO t1 VALUES(" + sqlLiteral(ii) + ", randomblob(200), randomblob(200))")
			}
			// incr ii 1
			{
				_n, _err := strconv.Atoi(ii)
				if _err == nil {
					ii = strconv.Itoa(_n + 1)
				}
			}
		}
		_res = db.Exec(" \n    CREATE INDEX i1 ON t1(a, b); \n    COMMIT;\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, " \n    CREATE INDEX i1 ON t1(a, b); \n    COMMIT;\n  ")
		}
	}
	{ // do_test "thread003.1.2"
		// expr ([file size test.db] / 1024) > 2000 (not evaluated)
	}
	{ // do_test "thread003.1.3"
		db.Close()
		os.Remove("test2.db")
		db, err = frigolite.Open("test2.db")
		if err != nil { t.Fatal(err) }
	}
	{ // do_test "thread003.1.4"
		_res = db.Exec("\n    BEGIN;\n    CREATE TABLE t1(a, b, c);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    BEGIN;\n    CREATE TABLE t1(a, b, c);\n  ")
		}
		vtab.TclVarSet("ii", "", "0")
		ii = "0"
		_ = ii // suppress unused warning
		for func() bool { ii_n, _ii_e := strconv.Atoi(ii); if _ii_e != nil { return false }; return ii_n < 5000 }() {
			_res = db.Exec("INSERT INTO t1 VALUES(" + sqlLiteral(ii) + ", randomblob(200), randomblob(200))")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "INSERT INTO t1 VALUES(" + sqlLiteral(ii) + ", randomblob(200), randomblob(200))")
			}
			// incr ii 1
			{
				_n, _err := strconv.Atoi(ii)
				if _err == nil {
					ii = strconv.Itoa(_n + 1)
				}
			}
		}
		_res = db.Exec(" \n    CREATE INDEX i1 ON t1(a, b); \n    COMMIT;\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, " \n    CREATE INDEX i1 ON t1(a, b); \n    COMMIT;\n  ")
		}
	}
	{ // do_test "thread003.1.5"
		// expr ([file size test.db] / 1024) > 2000 (not evaluated)
	}
	{ // do_test "thread003.1.6"
		db.Close()
	}
	vtab.TclVarSet("nSecond", "", "30")
	nSecond = "30"
	_ = nSecond // suppress unused warning
	_putsMsg := "Starting thread003.2 (should run for ~" + nSecond + " seconds)"
	_ = _putsMsg
	{ // "thread003.2" (prepare-step internals; SQL side effects only)
		for _, zFile := range tclSplitList("test.db test2.db") {
		_ = zFile // suppress unused warning
			SCRIPT = tclFormat("\n      set iEnd " + tclExpr("[clock_seconds] + %d") + "\n      set ::DB " + "sqlthread open %s xyzzy" + "\n  \n      # Set the cache size to 15 pages per cache. 30 available globally.\n      execsql { PRAGMA cache_size = 15 }\n  \n      while {" + "clock_seconds" + " < " + iEnd + "} {\n        set iQuery " + "0" + "\n        execsql \" SELECT * FROM t1 WHERE a = " + iQuery + " \"\n      }\n  \n      sqlite3_close " + DB + "\n      expr 1\n    ", nSecond, zFile)
			_ = SCRIPT // suppress unused warning
			// thread_spawn finished($zFile) $thread_procs $SCRIPT (unsupported command, not transpiled)
		}
		for _, zFile := range tclSplitList("test.db test2.db") {
		_ = zFile // suppress unused warning
			if tclBool("!" + tclBool01(finishedMap[zFile] != "")) {
			}
		}
		// expr 0 → "0"
	}
	vtab.TclVarSet("nSecond", "", "30")
	nSecond = "30"
	_ = nSecond // suppress unused warning
	_putsMsg = "Starting thread003.3 (should run for ~" + nSecond + " seconds)"
	_ = _putsMsg
	{ // "thread003.3" (prepare-step internals; SQL side effects only)
		for _, zFile := range tclSplitList("test.db test2.db") {
		_ = zFile // suppress unused warning
			SCRIPT = tclFormat("\n      set iStart " + "clock_seconds" + "\n      set iEnd " + tclExpr("[clock_seconds] + %d") + "\n      set ::DB " + "sqlthread open %s xyzzy" + "\n  \n      # Set the cache size to 15 pages per cache. 30 available globally.\n      execsql { PRAGMA cache_size = 15 }\n  \n      while {" + "clock_seconds" + " < " + iEnd + "} {\n        set iQuery " + "0" + "\n        execsql \"SELECT * FROM t1 WHERE a = " + iQuery + "\"\n        execsql \"UPDATE t1 SET b = randomblob(200) \n                 WHERE a < " + iQuery + " AND a > " + iQuery + " + 20\n        \"\n      }\n  \n      sqlite3_close " + DB + "\n      expr 1\n    ", nSecond, zFile)
			_ = SCRIPT // suppress unused warning
			// thread_spawn finished($zFile) $thread_procs $SCRIPT (unsupported command, not transpiled)
		}
		for _, zFile := range tclSplitList("test.db test2.db") {
		_ = zFile // suppress unused warning
			if tclBool("!" + tclBool01(finishedMap[zFile] != "")) {
			}
		}
		// expr 0 → "0"
	}
	vtab.TclVarSet("nSecond", "", "30")
	nSecond = "30"
	_ = nSecond // suppress unused warning
	_putsMsg = "Starting thread003.4 (should run for ~" + nSecond + " seconds)"
	_ = _putsMsg
	{ // "thread003.4" (prepare-step internals; SQL side effects only)
		// thread_spawn finished(1) $thread_procs [format {\n    set iEnd [expr {[clock_seconds] + %... (unsupported command, not transpiled)
		// thread_spawn finished(2) [format {\n    set iEnd [expr {[clock_seconds] + %... (unsupported command, not transpiled)
		for _, ii := range tclSplitList("1 2") {
		_ = ii // suppress unused warning
			if tclBool("!" + tclBool01(finishedMap[ii] != "")) {
			}
		}
		// expr 0 → "0"
	}
	vtab.TclVarSet("sqlite_open_file_count", "", "0")
	sqlite_open_file_count = "0"
	_ = sqlite_open_file_count // suppress unused warning
}
