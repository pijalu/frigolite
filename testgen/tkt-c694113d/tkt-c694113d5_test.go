...
func Test_tkt_c694113d5(t *testing.T) {
	...
	{ // do_test "tkt-c694113d5.100"
		...
		answer := ""
		res = db.Exec("SELECT a FROM t1 WHERE NOT EXISTS(SELECT 1 FROM t2 WHERE d=a)")
		if res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", res.Error, "SELECT a FROM t1 WHERE NOT EXISTS(SELECT 1 FROM t2 WHERE d=a)")
		}
	}
}
