	...
	from := _items[_idx+1]
		...
		where := _items[_idx+1]
			{ // do_test "tkt-fc7bd6358f.110." + a + "." + b + ".1"
				res = db.Exec("PRAGMA automatic_index=ON")
				if res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", res.Error, "PRAGMA automatic_index=ON")
				}
				res = db.Exec("SELECT t1.textid, i.intid, t2.textid " + from + " " + where)
				if res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", res.Error, "SELECT t1.textid, i.intid, t2.textid " + from + " " + where)
				}
			}
			{ // do_test "tkt-fc7bd6358f.110." + a + "." + b + ".2"
				res = db.Exec("PRAGMA automatic_index=OFF")
				if res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", res.Error, "PRAGMA automatic_index=OFF")
				}
				res = db.Exec("SELECT t1.textid, i.intid, t2.textid " + from + " " + where)
				if res.Error != nil {
					t.Errorf("exec error: %v\n  sql: %s", res.Error, "SELECT t1.textid, i.intid, t2.textid " + from + " " + where)
