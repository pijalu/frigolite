// Code generated by tcl2go; DO NOT EDIT.
//go:build testgen
// +build testgen

package trigger6

import (
"github.com/pijalu/frigolite"
"github.com/pijalu/frigolite/internal/vtab"
"os"
"testing"
)

func Test_trigger6(t *testing.T) {
	if err := os.Chdir(t.TempDir()); err != nil { t.Fatal(err) }
	db, err := frigolite.Open("test.db")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result
	var msg string
	var _r string
	var _berr error
	_ = _berr // suppress unused warning
	_ = msg // suppress unused warning
	_ = _res // suppress unused warning
	_ = r    // suppress unused warning
	_ = _r   // suppress unused warning
	tcl_nullvalue = "{}" // default NULL rendering

	var db1 *frigolite.DB
	_ = db1
	var db2 *frigolite.DB
	_ = db2
	var db3 *frigolite.DB
	_ = db3
	var db4 *frigolite.DB
	_ = db4
	var db5 *frigolite.DB
	_ = db5
	var db6 *frigolite.DB
	_ = db6
	var db7 *frigolite.DB
	_ = db7
	var db8 *frigolite.DB
	_ = db8
	var db9 *frigolite.DB
	_ = db9

	var testdir string
	_ = testdir // pre-declared from TCL source
	var trigger6_cnt string
	_ = trigger6_cnt // pre-declared from TCL source
	var Id_ string
	_ = Id_ // pre-declared from TCL source
	var argv0 string
	_ = argv0 // pre-declared from TCL source

	// set testdir: test directory (not used in Go test context)
	{ // do_test "trigger6-1.1"
		_res = db.Exec("\n    CREATE TABLE t1(x, y);\n    CREATE TABLE log(a, b, c);\n    CREATE TRIGGER r1 BEFORE INSERT ON t1 BEGIN\n      INSERT INTO log VALUES(1, new.x, new.y);\n    END;\n    CREATE TRIGGER r2 BEFORE UPDATE ON t1 BEGIN\n      INSERT INTO log VALUES(2, new.x, new.y);\n    END;\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TABLE t1(x, y);\n    CREATE TABLE log(a, b, c);\n    CREATE TRIGGER r1 BEFORE INSERT ON t1 BEGIN\n      INSERT INTO log VALUES(1, new.x, new.y);\n    END;\n    CREATE TRIGGER r2 BEFORE UPDATE ON t1 BEGIN\n      INSERT INTO log VALUES(2, new.x, new.y);\n    END;\n  ")
		}
		vtab.TclVarSet("trigger6_cnt", "", "0")
		trigger6_cnt = "0" // TCL namespace variable
		_ = trigger6_cnt // suppress unused warning
		// proc trigger6_counter increments counter var trigger6_cnt_____return__trigger6_cnt (registered via db func)
		var trigger6_cnt_____return__trigger6_cntCounter int64
		db.RegisterFunction("counter", func(args []interface{}) (interface{}, error) { trigger6_cnt_____return__trigger6_cntCounter++; return trigger6_cnt_____return__trigger6_cntCounter, nil }, 0, -1)
		r = db.Query("\n    INSERT INTO t1 VALUES(1,counter());\n    SELECT * FROM t1;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    INSERT INTO t1 VALUES(1,counter());\n    SELECT * FROM t1;\n  ")
		}
	}
	{ // do_test "trigger6-1.2"
		r = db.Query("\n    SELECT * FROM log;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT * FROM log;\n  ")
		}
	}
	{ // do_test "trigger6-1.3"
		r = db.Query("\n    DELETE FROM t1;\n    DELETE FROM log;\n    INSERT INTO t1 VALUES(2,counter(2,3)+4);\n    SELECT * FROM t1;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    DELETE FROM t1;\n    DELETE FROM log;\n    INSERT INTO t1 VALUES(2,counter(2,3)+4);\n    SELECT * FROM t1;\n  ")
		}
	}
	{ // do_test "trigger6-1.4"
		r = db.Query("\n    SELECT * FROM log;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT * FROM log;\n  ")
		}
	}
	{ // do_test "trigger6-1.5"
		r = db.Query("\n    DELETE FROM log;\n    UPDATE t1 SET y=counter(5);\n    SELECT * FROM t1;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    DELETE FROM log;\n    UPDATE t1 SET y=counter(5);\n    SELECT * FROM t1;\n  ")
		}
	}
	{ // do_test "trigger6-1.6"
		r = db.Query("\n    SELECT * FROM log;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT * FROM log;\n  ")
		}
	}
}
