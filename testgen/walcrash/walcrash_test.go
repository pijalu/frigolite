// Code generated by tcl2go; DO NOT EDIT.
package walcrash

import (
"github.com/pijalu/frigolite"
"os"
"strconv"
"testing"
)

func Test_walcrash(t *testing.T) {
	db, err := frigolite.Open("")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result

	// set testdir: test directory (not used in Go test context)
	var seed = "0"
	_ = seed // suppress unused warning
	var REPEATS = "100"
	_ = REPEATS // suppress unused warning
	var i = "1"
	_ = i // suppress unused warning
	for func() bool { i_n, _i_e := strconv.Atoi(i); if _i_e != nil { return false }; REPEATS_n, _REPEATS_e := strconv.Atoi(REPEATS); if _REPEATS_e != nil { return false }; return i_n < REPEATS_n }() {
		os.Remove("test.db")
		{ // do_test "walcrash-1." + i + ".1"
			t.Skipf("TODO: %s not implemented in frigolite", "crashsql -delay 4 -file test.db-wal -seed [incr seed] {\n      PRAGMA journal_mode = WAL;\n      CREATE TAB...}")
		}
		{ // do_test "walcrash-1." + i + ".2"
			db, err := frigolite.Open("test.db")
			defer db.Close()
			if err != nil { t.Fatal(err) }
			r = db.Query(" SELECT sum(a)==max(b) FROM t1 ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " SELECT sum(a)==max(b) FROM t1 ")
			}
		}
		_res = db.Exec("PRAGMA integrity_check")
		if _res.Error != nil { t.Errorf("integrity check: %v", _res.Error) }
		{ // do_test "walcrash-1." + i + ".4"
			t.Skipf("TODO: %s not implemented in frigolite", "crashsql -delay 2 -file test.db-wal -seed [incr seed] {\n      INSERT INTO t1 VALUES(4, (SELECT sum(a) FRO...}")
		}
		{ // do_test "walcrash-1." + i + ".5"
			db, err := frigolite.Open("test.db")
			defer db.Close()
			if err != nil { t.Fatal(err) }
			r = db.Query(" SELECT sum(a)==max(b) FROM t1 ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " SELECT sum(a)==max(b) FROM t1 ")
			}
		}
		_res = db.Exec("PRAGMA integrity_check")
		if _res.Error != nil { t.Errorf("integrity check: %v", _res.Error) }
		{ // do_test "walcrash-1." + i + ".7"
			r = db.Query(" PRAGMA main.journal_mode ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA main.journal_mode ")
			}
		}
		// incr i 1
		{
			_n, _err := strconv.Atoi(i)
			if _err == nil {
				i = strconv.Itoa(_n + 1)
			}
		}
	}
	var i = "1"
	_ = i // suppress unused warning
	for func() bool { i_n, _i_e := strconv.Atoi(i); if _i_e != nil { return false }; REPEATS_n, _REPEATS_e := strconv.Atoi(REPEATS); if _REPEATS_e != nil { return false }; return i_n < REPEATS_n }() {
		os.Remove("test.db")
		{ // do_test "walcrash-2." + i + ".1"
			t.Skipf("TODO: %s not implemented in frigolite", "crashsql -delay 5 -file test.db-wal -seed [incr seed] {\n      PRAGMA journal_mode = WAL;\n      CREATE TAB...}")
		}
		{ // do_test "walcrash-2." + i + ".2"
			db, err := frigolite.Open("test.db")
			defer db.Close()
			if err != nil { t.Fatal(err) }
			r = db.Query(" SELECT sum(a)==max(b) FROM t1 ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " SELECT sum(a)==max(b) FROM t1 ")
			}
		}
		_res = db.Exec("PRAGMA integrity_check")
		if _res.Error != nil { t.Errorf("integrity check: %v", _res.Error) }
		{ // do_test "walcrash-2." + i + ".4"
			t.Skipf("TODO: %s not implemented in frigolite", "crashsql -delay 2 -file test.db-wal -seed [incr seed] {\n      INSERT INTO t1 VALUES(6, (SELECT sum(a) FRO...}")
		}
		{ // do_test "walcrash-2." + i + ".5"
			db, err := frigolite.Open("test.db")
			defer db.Close()
			if err != nil { t.Fatal(err) }
			r = db.Query(" SELECT sum(a)==max(b) FROM t1 ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " SELECT sum(a)==max(b) FROM t1 ")
			}
		}
		_res = db.Exec("PRAGMA integrity_check")
		if _res.Error != nil { t.Errorf("integrity check: %v", _res.Error) }
		{ // do_test "walcrash-2." + i + ".6"
			r = db.Query(" PRAGMA main.journal_mode ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA main.journal_mode ")
			}
		}
		// incr i 1
		{
			_n, _err := strconv.Atoi(i)
			if _err == nil {
				i = strconv.Itoa(_n + 1)
			}
		}
	}
	var i = "1"
	_ = i // suppress unused warning
	for func() bool { i_n, _i_e := strconv.Atoi(i); if _i_e != nil { return false }; REPEATS_n, _REPEATS_e := strconv.Atoi(REPEATS); if _REPEATS_e != nil { return false }; return i_n < REPEATS_n }() {
		os.Remove("test.db")
		os.Remove("test2.db")
		{ // do_test "walcrash-4." + i + ".1"
			t.Skipf("TODO: %s not implemented in frigolite", "crashsql -delay 4 -file test.db-wal -seed [incr seed] -blocksize 4096 {\n      PRAGMA journal_mode = WAL;\n      PRAGMA pag...}")
		}
		{ // do_test "walcrash-4." + i + ".2"
			db, err := frigolite.Open("test.db")
			defer db.Close()
			if err != nil { t.Fatal(err) }
			r = db.Query(" \n      SELECT * FROM t1 WHERE a = 1;\n    ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " \n      SELECT * FROM t1 WHERE a = 1;\n    ")
			}
		}
		{ // do_test "walcrash-4." + i + ".3"
			r = db.Query(" PRAGMA main.integrity_check ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA main.integrity_check ")
			}
		}
		{ // do_test "walcrash-4." + i + ".4"
			r = db.Query(" PRAGMA main.journal_mode ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA main.journal_mode ")
			}
		}
		// incr i 1
		{
			_n, _err := strconv.Atoi(i)
			if _err == nil {
				i = strconv.Itoa(_n + 1)
			}
		}
	}
	var i = "1"
	_ = i // suppress unused warning
	for func() bool { i_n, _i_e := strconv.Atoi(i); if _i_e != nil { return false }; REPEATS_n, _REPEATS_e := strconv.Atoi(REPEATS); if _REPEATS_e != nil { return false }; return i_n < REPEATS_n }() {
		os.Remove("test.db")
		os.Remove("test2.db")
		{ // do_test "walcrash-5." + i + ".1"
			t.Skipf("TODO: %s not implemented in frigolite", "crashsql -delay 13 -file test.db-wal -seed [incr seed] -blocksize 4096 {\n      PRAGMA journal_mode = WAL;\n      PRAGMA pag...}")
		}
		{ // do_test "walcrash-5." + i + ".2"
			db, err := frigolite.Open("test.db")
			defer db.Close()
			if err != nil { t.Fatal(err) }
			r = db.Query(" SELECT count(*)==33 OR count(*)==34 FROM t1 WHERE x != 1 ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " SELECT count(*)==33 OR count(*)==34 FROM t1 WHERE x != 1 ")
			}
		}
		{ // do_test "walcrash-5." + i + ".3"
			r = db.Query(" PRAGMA main.integrity_check ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA main.integrity_check ")
			}
		}
		{ // do_test "walcrash-5." + i + ".4"
			r = db.Query(" PRAGMA main.journal_mode ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA main.journal_mode ")
			}
		}
		// incr i 1
		{
			_n, _err := strconv.Atoi(i)
			if _err == nil {
				i = strconv.Itoa(_n + 1)
			}
		}
	}
	var i = "1"
	_ = i // suppress unused warning
	for func() bool { i_n, _i_e := strconv.Atoi(i); if _i_e != nil { return false }; REPEATS_n, _REPEATS_e := strconv.Atoi(REPEATS); if _REPEATS_e != nil { return false }; return i_n < REPEATS_n }() {
		os.Remove("test.db")
		os.Remove("test2.db")
		{ // do_test "walcrash-6." + i + ".1"
			t.Skipf("TODO: %s not implemented in frigolite", "crashsql -delay 14 -file test.db-wal -seed [incr seed] -blocksize 512 {\n      PRAGMA journal_mode = WAL;\n      PRAGMA pag...}")
		}
		{ // do_test "walcrash-6." + i + ".2"
			db, err := frigolite.Open("test.db")
			defer db.Close()
			if err != nil { t.Fatal(err) }
			r = db.Query(" SELECT count(*) BETWEEN 34 AND 36 FROM t1 WHERE x != 1 ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " SELECT count(*) BETWEEN 34 AND 36 FROM t1 WHERE x != 1 ")
			}
		}
		{ // do_test "walcrash-6." + i + ".3"
			r = db.Query(" PRAGMA main.integrity_check ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA main.integrity_check ")
			}
		}
		{ // do_test "walcrash-6." + i + ".4"
			r = db.Query(" PRAGMA main.journal_mode ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA main.journal_mode ")
			}
		}
		// incr i 1
		{
			_n, _err := strconv.Atoi(i)
			if _err == nil {
				i = strconv.Itoa(_n + 1)
			}
		}
	}
	var i = "1"
	_ = i // suppress unused warning
	for func() bool { i_n, _i_e := strconv.Atoi(i); if _i_e != nil { return false }; REPEATS_n, _REPEATS_e := strconv.Atoi(REPEATS); if _REPEATS_e != nil { return false }; return i_n < REPEATS_n }() {
		os.Remove("test.db")
		var pgsz = "lindex {512 1024 2048 4096 8192 16384} [expr $i%6]"
		_ = pgsz // suppress unused warning
		{ // do_test "walcrash-7." + i + ".1"
			t.Skipf("TODO: %s not implemented in frigolite", "crashsql -delay 3 -file test.db -seed [incr seed] -blocksize 512 \n      PRAGMA page_size = $pgsz;\n      PRAGMA jour...")
		}
		{ // do_test "walcrash-7." + i + ".2"
			db, err := frigolite.Open("test.db")
			defer db.Close()
			if err != nil { t.Fatal(err) }
			r = db.Query(" SELECT b FROM t1 WHERE a = 1 ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " SELECT b FROM t1 WHERE a = 1 ")
			}
		}
		{ // do_test "walcrash-7." + i + ".3"
			r = db.Query(" PRAGMA main.integrity_check ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA main.integrity_check ")
			}
		}
		{ // do_test "walcrash-7." + i + ".4"
			r = db.Query(" PRAGMA main.journal_mode ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, " PRAGMA main.journal_mode ")
			}
		}
		// incr i 1
		{
			_n, _err := strconv.Atoi(i)
			if _err == nil {
				i = strconv.Itoa(_n + 1)
			}
		}
	}
}
