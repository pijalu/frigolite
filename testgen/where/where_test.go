// Code generated by tcl2go; DO NOT EDIT.
package where

import (
"github.com/pijalu/frigolite"
"strconv"
"strings"
"testing"
)

func Test_where(t *testing.T) {
	db, err := frigolite.Open("")
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()

	var _res *frigolite.Result
	var r *frigolite.Result

	// set testdir: test directory (not used in Go test context)
	{ // do_test "where-1.0"
		_res = db.Exec("\n    CREATE TABLE t1(w int, x int, y int);\n    CREATE TABLE t2(p int, q int, r int, s int);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TABLE t1(w int, x int, y int);\n    CREATE TABLE t2(p int, q int, r int, s int);\n  ")
		}
		var i = "1"
		_ = i // suppress unused warning
		for func() bool { i_n, _i_e := strconv.Atoi(i); if _i_e != nil { return false }; return i_n <= 100 }() {
			var w = i
			_ = w // suppress unused warning
			x := "int(log($i)/log(2))"
			y := "$i*$i + 2*$i + 1"
			_res = db.Exec("INSERT INTO t1 VALUES(" + w + "," + x + "," + y + ")")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "INSERT INTO t1 VALUES(" + w + "," + x + "," + y + ")")
			}
			// incr i 1
			{
				_n, _err := strconv.Atoi(i)
				if _err == nil {
					i = strconv.Itoa(_n + 1)
				}
			}
		}
		_res = db.Exec("\n    CREATE INDEX i1w ON t1(\"w\");  -- Verify quoted identifier names\n    CREATE INDEX i1xy ON t1(`x`,'y' ASC); -- Old MySQL compatibility\n    CREATE INDEX i2p ON t2(p);\n    CREATE INDEX i2r ON t2(r);\n    CREATE INDEX i2qs ON t2(q, s);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE INDEX i1w ON t1(\"w\");  -- Verify quoted identifier names\n    CREATE INDEX i1xy ON t1(`x`,'y' ASC); -- Old MySQL compatibility\n    CREATE INDEX i2p ON t2(p);\n    CREATE INDEX i2r ON t2(r);\n    CREATE INDEX i2qs ON t2(q, s);\n  ")
		}
	}
	// proc definition (not transpiled)
	{ // do_test "where-1.1.1"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y, w FROM t1 WHERE w=10}")
	}
	{ // do_test "where-1.1.1b"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y, w FROM t1 WHERE w IS 10}")
	}
	{ // "where-1.1.2"
		r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT x, y, w FROM t1 WHERE w=10\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT x, y, w FROM t1 WHERE w=10\n")
		}
	}
	{ // "where-1.1.2b"
		r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT x, y, w FROM t1 WHERE w IS 10\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT x, y, w FROM t1 WHERE w IS 10\n")
		}
	}
	{ // do_test "where-1.1.3"
	}
	{ // do_test "where-1.1.4"
		_res = db.Exec("SELECT x, y, w FROM t1 WHERE +w=10")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "SELECT x, y, w FROM t1 WHERE +w=10")
		}
	}
	{ // do_test "where-1.1.5"
	}
	{ // "where-1.1.6"
		r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT x, y, w FROM t1 WHERE +w=10\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT x, y, w FROM t1 WHERE +w=10\n")
		}
	}
	{ // do_test "where-1.1.7"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y, w AS abc FROM t1 WHERE abc=10}")
	}
	{ // "where-1.1.8"
		r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT x, y, w AS abc FROM t1 WHERE abc=10\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT x, y, w AS abc FROM t1 WHERE abc=10\n")
		}
	}
	{ // do_test "where-1.1.9"
	}
	{ // do_test "where-1.2.1"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y, w FROM t1 WHERE w=11}")
	}
	{ // do_test "where-1.2.2"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y, w AS abc FROM t1 WHERE abc=11}")
	}
	{ // do_test "where-1.3.1"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y, w AS abc FROM t1 WHERE 11=w}")
	}
	{ // do_test "where-1.3.2"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y, w AS abc FROM t1 WHERE 11=abc}")
	}
	{ // do_test "where-1.3.3"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y, w AS abc FROM t1 WHERE 11 IS abc}")
	}
	{ // do_test "where-1.4.1"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w, x, y FROM t1 WHERE 11=w AND x>2}")
	}
	{ // do_test "where-1.4.1b"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w, x, y FROM t1 WHERE 11 IS w AND x>2}")
	}
	{ // "where-1.4.2"
		r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT w, x, y FROM t1 WHERE 11=w AND x>2\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT w, x, y FROM t1 WHERE 11=w AND x>2\n")
		}
	}
	{ // "where-1.4.2b"
		r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT w, x, y FROM t1 WHERE 11 IS w AND x>2\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT w, x, y FROM t1 WHERE 11 IS w AND x>2\n")
		}
	}
	{ // do_test "where-1.4.3"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w AS a, x AS b, y FROM t1 WHERE 11=a AND b>...}")
	}
	{ // "where-1.4.4"
		r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT w AS a, x AS b, y FROM t1 WHERE 11=a AND b>2\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT w AS a, x AS b, y FROM t1 WHERE 11=a AND b>2\n")
		}
	}
	{ // do_test "where-1.5"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y FROM t1 WHERE y<200 AND w=11 AND x>2}")
	}
	{ // "where-1.5.2"
		r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT x, y FROM t1 WHERE y<200 AND w=11 AND x>2\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT x, y FROM t1 WHERE y<200 AND w=11 AND x>2\n")
		}
	}
	{ // do_test "where-1.6"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y FROM t1 WHERE y<200 AND x>2 AND w=11}")
	}
	{ // do_test "where-1.7"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y FROM t1 WHERE w=11 AND y<200 AND x>2}")
	}
	{ // do_test "where-1.8"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y FROM t1 WHERE w>10 AND y=144 AND x=3}")
	}
	{ // "where-1.8.2"
		r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT x, y FROM t1 WHERE w>10 AND y=144 AND x=3\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT x, y FROM t1 WHERE w>10 AND y=144 AND x=3\n")
		}
	}
	{ // "where-1.8.3"
		r = db.Query("EXPLAIN QUERY PLAN " + "\n  SELECT x, y FROM t1 WHERE y=144 AND x=3\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "EXPLAIN QUERY PLAN "+"\n  SELECT x, y FROM t1 WHERE y=144 AND x=3\n")
		}
	}
	{ // do_test "where-1.9"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y FROM t1 WHERE y=144 AND w>10 AND x=3}")
	}
	{ // do_test "where-1.10"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y FROM t1 WHERE x=3 AND w>=10 AND y=121}")
	}
	{ // do_test "where-1.11"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y FROM t1 WHERE x=3 AND y=100 AND w<10}")
	}
	{ // do_test "where-1.11b"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT x, y FROM t1 WHERE x IS 3 AND y IS 100 AND ...}")
	}
	{ // do_test "where-1.12"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND y<100}")
	}
	{ // do_test "where-1.12b"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x IS 3 AND y<100}")
	}
	{ // do_test "where-1.13"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND 100>y}")
	}
	{ // do_test "where-1.14"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE 3=x AND y<100}")
	}
	{ // do_test "where-1.14b"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE 3 IS x AND y<100}")
	}
	{ // do_test "where-1.15"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE 3=x AND 100>y}")
	}
	{ // do_test "where-1.16"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND y<=100}")
	}
	{ // do_test "where-1.17"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND 100>=y}")
	}
	{ // do_test "where-1.18"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND y>225}")
	}
	{ // do_test "where-1.18b"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x IS 3 AND y>225}")
	}
	{ // do_test "where-1.19"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND 225<y}")
	}
	{ // do_test "where-1.20"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND y>=225}")
	}
	{ // do_test "where-1.21"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND 225<=y}")
	}
	{ // do_test "where-1.22"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND y>121 AND y<196}")
	}
	{ // do_test "where-1.22b"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x IS 3 AND y>121 AND y<196}")
	}
	{ // do_test "where-1.23"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND y>=121 AND y<=196}")
	}
	{ // do_test "where-1.24"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND 121<y AND 196>y}")
	}
	{ // do_test "where-1.25"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND 121<=y AND 196>=y}")
	}
	{ // do_test "where-1.27"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x=3 AND y+1==122}")
	}
	{ // do_test "where-1.28"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE x+1=4 AND y+1==122}")
	}
	{ // do_test "where-1.29"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE y==121}")
	}
	{ // do_test "where-1.30"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE w>97}")
	}
	{ // do_test "where-1.31"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE w>=97}")
	}
	{ // do_test "where-1.33"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE w==97}")
	}
	{ // do_test "where-1.33.1"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE w<=97 AND w==97}")
	}
	{ // do_test "where-1.33.2"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE w<98 AND w==97}")
	}
	{ // do_test "where-1.33.3"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE w>=97 AND w==97}")
	}
	{ // do_test "where-1.33.4"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE w>96 AND w==97}")
	}
	{ // do_test "where-1.33.5"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE w==97 AND w==97}")
	}
	{ // do_test "where-1.34"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE w+1==98}")
	}
	{ // do_test "where-1.35"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE w<3}")
	}
	{ // do_test "where-1.36"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE w<=3}")
	}
	{ // do_test "where-1.37"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT w FROM t1 WHERE w+1<=4 ORDER BY w}")
	}
	{ // do_test "where-1.38"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT (w) FROM t1 WHERE (w)>(97)}")
	}
	{ // do_test "where-1.39"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT (w) FROM t1 WHERE (w)>=(97)}")
	}
	{ // do_test "where-1.40"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT (w) FROM t1 WHERE (w)==(97)}")
	}
	{ // do_test "where-1.41"
		t.Skipf("TODO: %s not implemented in frigolite", "count {SELECT (w) FROM t1 WHERE ((w)+(1))==(98)}")
	}
	{ // do_test "where-2.1"
		t.Skipf("TODO: %s not implemented in frigolite", "count {\n    SELECT w, p FROM t2, t1\n    WHERE x=q AND y=s...}")
	}
	{ // do_test "where-2.2"
		t.Skipf("TODO: %s not implemented in frigolite", "count {\n    SELECT w, p FROM t2, t1\n    WHERE x=q AND s=y...}")
	}
	{ // do_test "where-2.3"
		t.Skipf("TODO: %s not implemented in frigolite", "count {\n    SELECT w, p FROM t2, t1\n    WHERE x=q AND s=y...}")
	}
	{ // do_test "where-2.4"
		t.Skipf("TODO: %s not implemented in frigolite", "count {\n    SELECT w, p FROM t2, t1\n    WHERE p<80 AND x=...}")
	}
	{ // do_test "where-2.5"
		t.Skipf("TODO: %s not implemented in frigolite", "count {\n    SELECT w, p FROM t2, t1\n    WHERE p<80 AND x=...}")
	}
	{ // do_test "where-2.6"
		t.Skipf("TODO: %s not implemented in frigolite", "count {\n    SELECT w, p FROM t2, t1\n    WHERE x=q AND p=7...}")
	}
	{ // do_test "where-2.7"
		t.Skipf("TODO: %s not implemented in frigolite", "count {\n    SELECT w, p FROM t1, t2\n    WHERE x=q AND p>7...}")
	}
	{ // do_test "where-3.1"
		t.Skipf("TODO: %s not implemented in frigolite", "count {\n    SELECT A.w, B.p, C.w FROM t1 as A, t2 as B, t...}")
	}
	{ // do_test "where-3.2"
		t.Skipf("TODO: %s not implemented in frigolite", "count {\n    SELECT A.w, B.p, C.w FROM t1 as A, t2 as B, t...}")
	}
	{ // do_test "where-3.3"
		t.Skipf("TODO: %s not implemented in frigolite", "count {\n    SELECT A.w, B.p, C.w FROM t1 as A, t2 as B, t...}")
	}
	{ // do_test "where-4.1"
		t.Skipf("TODO: %s not implemented in frigolite", "count {\n    SELECT * FROM t1 WHERE 0\n  }")
	}
	{ // do_test "where-4.2"
		t.Skipf("TODO: %s not implemented in frigolite", "count {\n    SELECT * FROM t1 WHERE 1 LIMIT 1\n  }")
	}
	{ // do_test "where-4.3"
		r = db.Query("\n    SELECT 99 WHERE 0\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT 99 WHERE 0\n  ")
		}
	}
	{ // do_test "where-4.4"
		r = db.Query("\n    SELECT 99 WHERE 1\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT 99 WHERE 1\n  ")
		}
	}
	{ // do_test "where-4.5"
		r = db.Query("\n    SELECT 99 WHERE 0.1\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT 99 WHERE 0.1\n  ")
		}
	}
	{ // do_test "where-4.6"
		r = db.Query("\n    SELECT 99 WHERE 0.0\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT 99 WHERE 0.0\n  ")
		}
	}
	{ // do_test "where-4.7"
		r = db.Query("\n    SELECT count(*) FROM t1 WHERE t1.w\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT count(*) FROM t1 WHERE t1.w\n  ")
		}
	}
	// proc definition (not transpiled)
	{ // do_test "where-6.1"
		_res = db.Exec("\n    CREATE TABLE t3(a,b,c);\n    CREATE INDEX t3a ON t3(a);\n    CREATE INDEX t3bc ON t3(b,c);\n    CREATE INDEX t3acb ON t3(a,c,b);\n    INSERT INTO t3 SELECT w, 101-w, y FROM t1;\n    SELECT count(*), sum(a), sum(b), sum(c) FROM t3;\n    ANALYZE;\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TABLE t3(a,b,c);\n    CREATE INDEX t3a ON t3(a);\n    CREATE INDEX t3bc ON t3(b,c);\n    CREATE INDEX t3acb ON t3(a,c,b);\n    INSERT INTO t3 SELECT w, 101-w, y FROM t1;\n    SELECT count(*), sum(a), sum(b), sum(c) FROM t3;\n    ANALYZE;\n  ")
		}
	}
	{ // do_test "where-6.2"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 ORDER BY a LIMIT 3\n  }")
	}
	{ // do_test "where-6.3"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 ORDER BY a+1 LIMIT 3\n  }")
	}
	{ // do_test "where-6.4"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a<10 ORDER BY a LIMIT ...}")
	}
	{ // do_test "where-6.5"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a>0 AND a<10 ORDER BY ...}")
	}
	{ // do_test "where-6.6"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a>0 ORDER BY a LIMIT 3...}")
	}
	{ // do_test "where-6.7.1"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE b>0 ORDER BY a LIMIT 1...}")
	}
	{ // do_test "where-6.7.2"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE b>0 ORDER BY a LIMIT 1...}")
	}
	{ // do_test "where-6.9.1"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a=1 AND c>0 ORDER BY a...}")
	}
	{ // do_test "where-6.9.1.1"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a>=1 AND a=1 AND c>0 O...}")
	}
	{ // do_test "where-6.9.1.2"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a<2 AND a=1 AND c>0 OR...}")
	}
	{ // do_test "where-6.9.2"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a=1 AND c>0 ORDER BY a...}")
	}
	{ // do_test "where-6.9.3"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a=1 AND c>0 ORDER BY c...}")
	}
	{ // do_test "where-6.9.4"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a=1 AND c>0 ORDER BY a...}")
	}
	{ // do_test "where-6.9.5"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a=1 AND c>0 ORDER BY a...}")
	}
	{ // do_test "where-6.9.6"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a=1 AND c>0 ORDER BY c...}")
	}
	{ // do_test "where-6.9.7"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a=1 AND c>0 ORDER BY c...}")
	}
	{ // do_test "where-6.9.8"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a=1 AND c>0 ORDER BY a...}")
	}
	{ // do_test "where-6.9.9"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a=1 AND c>0 ORDER BY a...}")
	}
	{ // do_test "where-6.10"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a=1 AND c>0 ORDER BY a...}")
	}
	{ // do_test "where-6.11"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a=1 AND c>0 ORDER BY a...}")
	}
	{ // do_test "where-6.12"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a=1 AND c>0 ORDER BY a...}")
	}
	{ // do_test "where-6.13"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a>0 ORDER BY a DESC LI...}")
	}
	{ // do_test "where-6.13.1"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 WHERE a>0 ORDER BY -a LIMIT ...}")
	}
	{ // do_test "where-6.14"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t3 ORDER BY b LIMIT 3\n  }")
	}
	{ // do_test "where-6.15"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT t3.a, t1.x FROM t3, t1 WHERE t3.a=t1.w...}")
	}
	{ // do_test "where-6.16"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT t3.a, t1.x FROM t3, t1 WHERE t3.a=t1.w...}")
	}
	{ // do_test "where-6.19"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 ORDER BY w LIMIT 3;\n  }")
	}
	{ // do_test "where-6.20"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 ORDER BY rowid LIMIT 3;\n  }")
	}
	{ // do_test "where-6.21"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 ORDER BY rowid, y LIMIT 3;\n ...}")
	}
	{ // do_test "where-6.22"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 ORDER BY rowid, y DESC LIMIT...}")
	}
	{ // do_test "where-6.23"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 WHERE y>4 ORDER BY rowid, w,...}")
	}
	{ // do_test "where-6.24"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 WHERE y>=9 ORDER BY rowid, x...}")
	}
	{ // do_test "where-6.25"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 WHERE y>4 AND y<25 ORDER BY ...}")
	}
	{ // do_test "where-6.26"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 WHERE y>=4 AND y<=25 ORDER B...}")
	}
	{ // do_test "where-6.27"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 WHERE y<=25 ORDER BY _rowid_...}")
	}
	{ // do_test "where-7.1"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 ORDER BY y;\n  }")
	}
	{ // do_test "where-7.2"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 ORDER BY y DESC;\n ...}")
	}
	{ // do_test "where-7.3"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>100 ORDER BY...}")
	}
	{ // do_test "where-7.4"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>100 ORDER BY...}")
	}
	{ // do_test "where-7.5"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>121 ORDER BY...}")
	}
	{ // do_test "where-7.6"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>=121 ORDER B...}")
	}
	{ // do_test "where-7.7"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>=121 AND y<1...}")
	}
	{ // do_test "where-7.8"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>=121 AND y<=...}")
	}
	{ // do_test "where-7.9"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>121 AND y<=1...}")
	}
	{ // do_test "where-7.10"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>100 AND y<19...}")
	}
	{ // do_test "where-7.11"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>=121 AND y<1...}")
	}
	{ // do_test "where-7.12"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>=121 AND y<=...}")
	}
	{ // do_test "where-7.13"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>121 AND y<=1...}")
	}
	{ // do_test "where-7.14"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>100 AND y<19...}")
	}
	{ // do_test "where-7.15"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y<81 ORDER BY ...}")
	}
	{ // do_test "where-7.16"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y<=81 ORDER BY...}")
	}
	{ // do_test "where-7.17"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>256 ORDER BY...}")
	}
	{ // do_test "where-7.18"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>=256 ORDER B...}")
	}
	{ // do_test "where-7.19"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y<81 ORDER BY ...}")
	}
	{ // do_test "where-7.20"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y<=81 ORDER BY...}")
	}
	{ // do_test "where-7.21"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>256 ORDER BY...}")
	}
	{ // do_test "where-7.22"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=3 AND y>=256 ORDER B...}")
	}
	{ // do_test "where-7.23"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=0 AND y<4 ORDER BY y...}")
	}
	{ // do_test "where-7.24"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=0 AND y<=4 ORDER BY ...}")
	}
	{ // do_test "where-7.25"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=6 AND y>10201 ORDER ...}")
	}
	{ // do_test "where-7.26"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=6 AND y>=10201 ORDER...}")
	}
	{ // do_test "where-7.27"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=0 AND y<4 ORDER BY y...}")
	}
	{ // do_test "where-7.28"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=0 AND y<=4 ORDER BY ...}")
	}
	{ // do_test "where-7.29"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=6 AND y>10201 ORDER ...}")
	}
	{ // do_test "where-7.30"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t1 WHERE x=6 AND y>=10201 ORDER...}")
	}
	{ // do_test "where-7.31"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 ORDER BY rowid DESC LIMIT 3\n...}")
	}
	{ // do_test "where-7.32"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 WHERE y<25 ORDER BY rowid DE...}")
	}
	{ // do_test "where-7.33"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 WHERE y<=25 ORDER BY rowid D...}")
	}
	{ // do_test "where-7.34"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 WHERE y<25 AND y>4 ORDER BY ...}")
	}
	{ // do_test "where-7.35"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT y FROM t1 WHERE y<25 AND y>=4 ORDER BY...}")
	}
	{ // do_test "where-8.1"
		_res = db.Exec("\n    CREATE TABLE t4 AS SELECT * FROM t1;\n    CREATE INDEX i4xy ON t4(x,y);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TABLE t4 AS SELECT * FROM t1;\n    CREATE INDEX i4xy ON t4(x,y);\n  ")
		}
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t4 WHERE x=4 and y<1000 ORDER B...}")
	}
	{ // do_test "where-8.2"
		_res = db.Exec("\n    DELETE FROM t4;\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    DELETE FROM t4;\n  ")
		}
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT w FROM t4 WHERE x=4 and y<1000 ORDER B...}")
	}
	{ // do_test "where-9.1"
		r = db.Query("\n    CREATE TABLE t5(x PRIMARY KEY);\n    SELECT * FROM t5 WHERE x<10;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    CREATE TABLE t5(x PRIMARY KEY);\n    SELECT * FROM t5 WHERE x<10;\n  ")
		}
	}
	{ // do_test "where-9.2"
		r = db.Query("\n    SELECT * FROM t5 WHERE x<10 ORDER BY x DESC;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT * FROM t5 WHERE x<10 ORDER BY x DESC;\n  ")
		}
	}
	{ // do_test "where-9.3"
		r = db.Query("\n    SELECT * FROM t5 WHERE x=10;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT * FROM t5 WHERE x=10;\n  ")
		}
	}
	{ // do_test "where-10.1"
		r = db.Query("\n    SELECT 1 WHERE abs(random())<0\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT 1 WHERE abs(random())<0\n  ")
		}
	}
	{ // do_test "where-10.2"
		// proc definition (not transpiled)
		var _v1 = "0" // TCL namespace variable
		_ = _v1 // suppress unused warning
		r = db.Query("\n    SELECT count(*) FROM t1 WHERE tclvar('v1');\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT count(*) FROM t1 WHERE tclvar('v1');\n  ")
		}
	}
	{ // do_test "where-10.3"
		var _v1 = "1" // TCL namespace variable
		_ = _v1 // suppress unused warning
		r = db.Query("\n    SELECT count(*) FROM t1 WHERE tclvar('v1');\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT count(*) FROM t1 WHERE tclvar('v1');\n  ")
		}
	}
	{ // do_test "where-10.4"
		var _v1 = "1" // TCL namespace variable
		_ = _v1 // suppress unused warning
		// proc definition (not transpiled)
		r = db.Query("\n    SELECT count(*) FROM t1 WHERE tclvar('v1');\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT count(*) FROM t1 WHERE tclvar('v1');\n  ")
		}
	}
	{ // do_test "where-11.1"
		_res = db.Exec("\n   CREATE TABLE t99(Dte INT, X INT);\n   DELETE FROM t99 WHERE (Dte = 2451337) OR (Dte = 2451339) OR\n     (Dte BETWEEN 2451345 AND 2451347) OR (Dte = 2451351) OR \n     (Dte BETWEEN 2451355 AND 2451356) OR (Dte = 2451358) OR\n     (Dte = 2451362) OR (Dte = 2451365) OR (Dte = 2451367) OR\n     (Dte BETWEEN 2451372 AND 2451376) OR (Dte BETWEEN 2451382 AND 2451384) OR\n     (Dte = 2451387) OR (Dte BETWEEN 2451389 AND 2451391) OR \n     (Dte BETWEEN 2451393 AND 2451395) OR (Dte = 2451400) OR \n     (Dte = 2451402) OR (Dte = 2451404) OR (Dte BETWEEN 2451416 AND 2451418) OR \n     (Dte = 2451422) OR (Dte = 2451426) OR (Dte BETWEEN 2451445 AND 2451446) OR\n     (Dte = 2451456) OR (Dte = 2451458) OR (Dte BETWEEN 2451465 AND 2451467) OR\n     (Dte BETWEEN 2451469 AND 2451471) OR (Dte = 2451474) OR\n     (Dte BETWEEN 2451477 AND 2451501) OR (Dte BETWEEN 2451503 AND 2451509) OR\n     (Dte BETWEEN 2451511 AND 2451514) OR (Dte BETWEEN 2451518 AND 2451521) OR\n     (Dte BETWEEN 2451523 AND 2451531) OR (Dte BETWEEN 2451533 AND 2451537) OR\n     (Dte BETWEEN 2451539 AND 2451544) OR (Dte BETWEEN 2451546 AND 2451551) OR\n     (Dte BETWEEN 2451553 AND 2451555) OR (Dte = 2451557) OR\n     (Dte BETWEEN 2451559 AND 2451561) OR (Dte = 2451563) OR\n     (Dte BETWEEN 2451565 AND 2451566) OR (Dte BETWEEN 2451569 AND 2451571) OR \n     (Dte = 2451573) OR (Dte = 2451575) OR (Dte = 2451577) OR (Dte = 2451581) OR\n     (Dte BETWEEN 2451583 AND 2451586) OR (Dte BETWEEN 2451588 AND 2451592) OR \n     (Dte BETWEEN 2451596 AND 2451598) OR (Dte = 2451600) OR\n     (Dte BETWEEN 2451602 AND 2451603) OR (Dte = 2451606) OR (Dte = 2451611);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n   CREATE TABLE t99(Dte INT, X INT);\n   DELETE FROM t99 WHERE (Dte = 2451337) OR (Dte = 2451339) OR\n     (Dte BETWEEN 2451345 AND 2451347) OR (Dte = 2451351) OR \n     (Dte BETWEEN 2451355 AND 2451356) OR (Dte = 2451358) OR\n     (Dte = 2451362) OR (Dte = 2451365) OR (Dte = 2451367) OR\n     (Dte BETWEEN 2451372 AND 2451376) OR (Dte BETWEEN 2451382 AND 2451384) OR\n     (Dte = 2451387) OR (Dte BETWEEN 2451389 AND 2451391) OR \n     (Dte BETWEEN 2451393 AND 2451395) OR (Dte = 2451400) OR \n     (Dte = 2451402) OR (Dte = 2451404) OR (Dte BETWEEN 2451416 AND 2451418) OR \n     (Dte = 2451422) OR (Dte = 2451426) OR (Dte BETWEEN 2451445 AND 2451446) OR\n     (Dte = 2451456) OR (Dte = 2451458) OR (Dte BETWEEN 2451465 AND 2451467) OR\n     (Dte BETWEEN 2451469 AND 2451471) OR (Dte = 2451474) OR\n     (Dte BETWEEN 2451477 AND 2451501) OR (Dte BETWEEN 2451503 AND 2451509) OR\n     (Dte BETWEEN 2451511 AND 2451514) OR (Dte BETWEEN 2451518 AND 2451521) OR\n     (Dte BETWEEN 2451523 AND 2451531) OR (Dte BETWEEN 2451533 AND 2451537) OR\n     (Dte BETWEEN 2451539 AND 2451544) OR (Dte BETWEEN 2451546 AND 2451551) OR\n     (Dte BETWEEN 2451553 AND 2451555) OR (Dte = 2451557) OR\n     (Dte BETWEEN 2451559 AND 2451561) OR (Dte = 2451563) OR\n     (Dte BETWEEN 2451565 AND 2451566) OR (Dte BETWEEN 2451569 AND 2451571) OR \n     (Dte = 2451573) OR (Dte = 2451575) OR (Dte = 2451577) OR (Dte = 2451581) OR\n     (Dte BETWEEN 2451583 AND 2451586) OR (Dte BETWEEN 2451588 AND 2451592) OR \n     (Dte BETWEEN 2451596 AND 2451598) OR (Dte = 2451600) OR\n     (Dte BETWEEN 2451602 AND 2451603) OR (Dte = 2451606) OR (Dte = 2451611);\n  ")
		}
	}
	{ // do_test "where-12.1"
		_res = db.Exec("\n    CREATE TABLE t6(a INTEGER PRIMARY KEY, b TEXT);\n    INSERT INTO t6 VALUES(1,'one');\n    INSERT INTO t6 VALUES(4,'four');\n    CREATE INDEX t6i1 ON t6(b);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TABLE t6(a INTEGER PRIMARY KEY, b TEXT);\n    INSERT INTO t6 VALUES(1,'one');\n    INSERT INTO t6 VALUES(4,'four');\n    CREATE INDEX t6i1 ON t6(b);\n  ")
		}
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t6 ORDER BY b;\n  }")
	}
	{ // do_test "where-12.2"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t6 ORDER BY b, a;\n  }")
	}
	{ // do_test "where-12.3"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t6 ORDER BY a;\n  }")
	}
	{ // do_test "where-12.4"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t6 ORDER BY a, b;\n  }")
	}
	{ // do_test "where-12.5"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t6 ORDER BY b DESC;\n  }")
	}
	{ // do_test "where-12.6"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t6 ORDER BY b DESC, a DESC;\n  }")
	}
	{ // do_test "where-12.7"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t6 ORDER BY b DESC, a ASC;\n  }")
	}
	{ // do_test "where-12.8"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t6 ORDER BY b ASC, a DESC;\n  }")
	}
	{ // do_test "where-12.9"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t6 ORDER BY a DESC;\n  }")
	}
	{ // do_test "where-12.10"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t6 ORDER BY a DESC, b DESC;\n  }")
	}
	{ // do_test "where-12.11"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t6 ORDER BY a DESC, b ASC;\n  }")
	}
	{ // do_test "where-12.12"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t6 ORDER BY a ASC, b DESC;\n  }")
	}
	{ // do_test "where-13.1"
		_res = db.Exec("\n    CREATE TABLE t7(a INTEGER PRIMARY KEY, b TEXT);\n    INSERT INTO t7 VALUES(1,'one');\n    INSERT INTO t7 VALUES(4,'four');\n    CREATE INDEX t7i1 ON t7(b);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TABLE t7(a INTEGER PRIMARY KEY, b TEXT);\n    INSERT INTO t7 VALUES(1,'one');\n    INSERT INTO t7 VALUES(4,'four');\n    CREATE INDEX t7i1 ON t7(b);\n  ")
		}
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t7 ORDER BY b;\n  }")
	}
	{ // do_test "where-13.2"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t7 ORDER BY b, a;\n  }")
	}
	{ // do_test "where-13.3"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t7 ORDER BY a;\n  }")
	}
	{ // do_test "where-13.4"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t7 ORDER BY a, b;\n  }")
	}
	{ // do_test "where-13.5"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t7 ORDER BY b DESC;\n  }")
	}
	{ // do_test "where-13.6"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t7 ORDER BY b DESC, a DESC;\n  }")
	}
	{ // do_test "where-13.7"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t7 ORDER BY b DESC, a ASC;\n  }")
	}
	{ // do_test "where-13.8"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t7 ORDER BY b ASC, a DESC;\n  }")
	}
	{ // do_test "where-13.9"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t7 ORDER BY a DESC;\n  }")
	}
	{ // do_test "where-13.10"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t7 ORDER BY a DESC, b DESC;\n  }")
	}
	{ // do_test "where-13.11"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t7 ORDER BY a DESC, b ASC;\n  }")
	}
	{ // do_test "where-13.12"
		t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT * FROM t7 ORDER BY a ASC, b DESC;\n  }")
	}
	if tclBool("permutation" + " != \"no_optimization\"") {
		{ // do_test "where-14.1"
			_res = db.Exec("\n    CREATE TABLE t8(a INTEGER PRIMARY KEY, b TEXT UNIQUE, c CHAR(100));\n    INSERT INTO t8(a,b) VALUES(1,'one');\n    INSERT INTO t8(a,b) VALUES(4,'four');\n  ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TABLE t8(a INTEGER PRIMARY KEY, b TEXT UNIQUE, c CHAR(100));\n    INSERT INTO t8(a,b) VALUES(1,'one');\n    INSERT INTO t8(a,b) VALUES(4,'four');\n  ")
			}
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.2"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.3"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.4"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.5"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.6"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.7"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.7.1"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.7.2"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.8"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.9"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.10"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.11"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
		{ // do_test "where-14.12"
			t.Skipf("TODO: %s not implemented in frigolite", "cksort {\n    SELECT x.a || '/' || y.a FROM t8 x, t8 y ORDE...}")
		}
	}
	{ // do_test "where-15.1"
		r = db.Query("\n    CREATE TEMP TABLE t1 (a, b, c, d, e);\n    CREATE TEMP TABLE t2 (f);\n    SELECT t1.e AS alias FROM t2, t1 WHERE alias = 1 ;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    CREATE TEMP TABLE t1 (a, b, c, d, e);\n    CREATE TEMP TABLE t2 (f);\n    SELECT t1.e AS alias FROM t2, t1 WHERE alias = 1 ;\n  ")
		}
	}
	{ // do_test "where-16.1"
		_res = db.Exec("\n    CREATE TABLE a1(id INTEGER PRIMARY KEY, v);\n    CREATE TABLE a2(id INTEGER PRIMARY KEY, v);\n    INSERT INTO a1 VALUES(1, 'one');\n    INSERT INTO a1 VALUES(2, 'two');\n    INSERT INTO a2 VALUES(1, 'one');\n    INSERT INTO a2 VALUES(2, 'two');\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TABLE a1(id INTEGER PRIMARY KEY, v);\n    CREATE TABLE a2(id INTEGER PRIMARY KEY, v);\n    INSERT INTO a1 VALUES(1, 'one');\n    INSERT INTO a1 VALUES(2, 'two');\n    INSERT INTO a2 VALUES(1, 'one');\n    INSERT INTO a2 VALUES(2, 'two');\n  ")
		}
	}
	{ // do_test "where-16.2"
		r = db.Query("\n    SELECT * FROM a2 CROSS JOIN a1 WHERE a1.id=1 AND a1.v='one';\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT * FROM a2 CROSS JOIN a1 WHERE a1.id=1 AND a1.v='one';\n  ")
		}
	}
	{ // do_test "where-16.3"
		_res = db.Exec("\n    CREATE TEMP TABLE foo(idx INTEGER);\n    INSERT INTO foo VALUES(1);\n    INSERT INTO foo VALUES(1);\n    INSERT INTO foo VALUES(1);\n    INSERT INTO foo VALUES(2);\n    INSERT INTO foo VALUES(2);\n    CREATE TEMP TABLE bar(stuff INTEGER);\n    INSERT INTO bar VALUES(100);\n    INSERT INTO bar VALUES(200);\n    INSERT INTO bar VALUES(300);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TEMP TABLE foo(idx INTEGER);\n    INSERT INTO foo VALUES(1);\n    INSERT INTO foo VALUES(1);\n    INSERT INTO foo VALUES(1);\n    INSERT INTO foo VALUES(2);\n    INSERT INTO foo VALUES(2);\n    CREATE TEMP TABLE bar(stuff INTEGER);\n    INSERT INTO bar VALUES(100);\n    INSERT INTO bar VALUES(200);\n    INSERT INTO bar VALUES(300);\n  ")
		}
	}
	{ // do_test "where-16.4"
		r = db.Query("\n    SELECT bar.RowID id FROM foo, bar WHERE foo.idx = bar.RowID AND id = 2;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT bar.RowID id FROM foo, bar WHERE foo.idx = bar.RowID AND id = 2;\n  ")
		}
	}
	_res = db.Exec("PRAGMA integrity_check")
	if _res.Error != nil { t.Errorf("integrity check: %v", _res.Error) }
	{ // do_test "where-17.1"
		_res = db.Exec("\n    CREATE TABLE tbooking (\n      id INTEGER PRIMARY KEY,\n      eventtype INTEGER NOT NULL\n    );\n    INSERT INTO tbooking VALUES(42, 3);\n    INSERT INTO tbooking VALUES(43, 4);\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    CREATE TABLE tbooking (\n      id INTEGER PRIMARY KEY,\n      eventtype INTEGER NOT NULL\n    );\n    INSERT INTO tbooking VALUES(42, 3);\n    INSERT INTO tbooking VALUES(43, 4);\n  ")
		}
	}
	{ // do_test "where-17.2"
		r = db.Query("\n    SELECT a.id\n    FROM tbooking AS a\n    WHERE a.eventtype=3;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a.id\n    FROM tbooking AS a\n    WHERE a.eventtype=3;\n  ")
		}
	}
	{ // do_test "where-17.3"
		r = db.Query("\n    SELECT a.id, (SELECT b.id FROM tbooking AS b WHERE b.id>a.id)\n    FROM tbooking AS a\n    WHERE a.eventtype=3;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a.id, (SELECT b.id FROM tbooking AS b WHERE b.id>a.id)\n    FROM tbooking AS a\n    WHERE a.eventtype=3;\n  ")
		}
	}
	{ // do_test "where-17.4"
		r = db.Query("\n    SELECT a.id, (SELECT b.id FROM tbooking AS b WHERE b.id>a.id)\n    FROM (SELECT 1.5 AS id) AS a\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a.id, (SELECT b.id FROM tbooking AS b WHERE b.id>a.id)\n    FROM (SELECT 1.5 AS id) AS a\n  ")
		}
	}
	{ // do_test "where-17.5"
		r = db.Query("\n    CREATE TABLE tother(a, b);\n    INSERT INTO tother VALUES(1, 3.7);\n    SELECT id, a FROM tbooking, tother WHERE id>a;\n  ")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    CREATE TABLE tother(a, b);\n    INSERT INTO tother VALUES(1, 3.7);\n    SELECT id, a FROM tbooking, tother WHERE id>a;\n  ")
		}
	}
	{ // "where-18.1"
		r = db.Query("\n  CREATE TABLE t181(a);\n  CREATE TABLE t182(b,c);\n  INSERT INTO t181 VALUES(1);\n  SELECT DISTINCT a FROM t181 LEFT JOIN t182 ON a=b ORDER BY c IS NULL;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t181(a);\n  CREATE TABLE t182(b,c);\n  INSERT INTO t181 VALUES(1);\n  SELECT DISTINCT a FROM t181 LEFT JOIN t182 ON a=b ORDER BY c IS NULL;\n")
			return
		}
		got := flatten(r)
		want := "1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-18.1rj"
		r = db.Query("\n  SELECT DISTINCT a FROM t182 RIGHT JOIN t181 ON a=b ORDER BY c IS NULL;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT DISTINCT a FROM t182 RIGHT JOIN t181 ON a=b ORDER BY c IS NULL;\n")
			return
		}
		got := flatten(r)
		want := "1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-18.2"
		r = db.Query("\n  SELECT DISTINCT a FROM t181 LEFT JOIN t182 ON a=b ORDER BY +c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT DISTINCT a FROM t181 LEFT JOIN t182 ON a=b ORDER BY +c;\n")
			return
		}
		got := flatten(r)
		want := "1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-18.3"
		r = db.Query("\n  SELECT DISTINCT a FROM t181 LEFT JOIN t182 ON a=b ORDER BY c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT DISTINCT a FROM t181 LEFT JOIN t182 ON a=b ORDER BY c;\n")
			return
		}
		got := flatten(r)
		want := "1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-18.3rj"
		r = db.Query("\n  SELECT DISTINCT a FROM t182 RIGHT JOIN t181 ON a=b ORDER BY c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT DISTINCT a FROM t182 RIGHT JOIN t181 ON a=b ORDER BY c;\n")
			return
		}
		got := flatten(r)
		want := "1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-18.4"
		r = db.Query("\n  INSERT INTO t181 VALUES(1),(1),(1),(1);\n  SELECT DISTINCT a FROM t181 LEFT JOIN t182 ON a=b ORDER BY +c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  INSERT INTO t181 VALUES(1),(1),(1),(1);\n  SELECT DISTINCT a FROM t181 LEFT JOIN t182 ON a=b ORDER BY +c;\n")
			return
		}
		got := flatten(r)
		want := "1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-18.4rj"
		r = db.Query("\n  SELECT DISTINCT a FROM t182 RIGHT JOIN t181 ON a=b ORDER BY +c;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT DISTINCT a FROM t182 RIGHT JOIN t181 ON a=b ORDER BY +c;\n")
			return
		}
		got := flatten(r)
		want := "1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-18.5"
		r = db.Query("\n  INSERT INTO t181 VALUES(2);\n  SELECT DISTINCT a FROM t181 LEFT JOIN t182 ON a=b ORDER BY c IS NULL, +a;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  INSERT INTO t181 VALUES(2);\n  SELECT DISTINCT a FROM t181 LEFT JOIN t182 ON a=b ORDER BY c IS NULL, +a;\n")
			return
		}
		got := flatten(r)
		want := "1 2"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-18.6"
		r = db.Query("\n  INSERT INTO t181 VALUES(2);\n  SELECT DISTINCT a FROM t181 LEFT JOIN t182 ON a=b ORDER BY +a, +c IS NULL;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  INSERT INTO t181 VALUES(2);\n  SELECT DISTINCT a FROM t181 LEFT JOIN t182 ON a=b ORDER BY +a, +c IS NULL;\n")
			return
		}
		got := flatten(r)
		want := "1 2"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-19.0"
		r = db.Query("\n  CREATE TABLE t191(a INT UNIQUE NOT NULL, b INT UNIQUE NOT NULL,c,d);\n  CREATE INDEX t191a ON t1(a);\n  CREATE INDEX t191b ON t1(b);\n  CREATE TABLE t192(x INTEGER PRIMARY KEY,y INT, z INT);\n\n  EXPLAIN QUERY PLAN\n  SELECT t191.rowid FROM t192, t191 WHERE (a=y OR b=y) AND x=?1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t191(a INT UNIQUE NOT NULL, b INT UNIQUE NOT NULL,c,d);\n  CREATE INDEX t191a ON t1(a);\n  CREATE INDEX t191b ON t1(b);\n  CREATE TABLE t192(x INTEGER PRIMARY KEY,y INT, z INT);\n\n  EXPLAIN QUERY PLAN\n  SELECT t191.rowid FROM t192, t191 WHERE (a=y OR b=y) AND x=?1;\n")
			return
		}
		got := flatten(r)
		want := "/.* sqlite_autoindex_t191_1 .* sqlite_autoindex_t191_2 .*/"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-20.0"
		r = db.Query("\n  CREATE TABLE t201(x);\n  CREATE TABLE t202(y, z);\n  INSERT INTO t201 VALUES('key');\n  INSERT INTO t202 VALUES('key', -1);\n  CREATE INDEX t202i ON t202(y, ifnull(z, 0));\n  SELECT count(*) FROM t201 LEFT JOIN t202 ON (x=y) WHERE ifnull(z, 0) >=0;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t201(x);\n  CREATE TABLE t202(y, z);\n  INSERT INTO t201 VALUES('key');\n  INSERT INTO t202 VALUES('key', -1);\n  CREATE INDEX t202i ON t202(y, ifnull(z, 0));\n  SELECT count(*) FROM t201 LEFT JOIN t202 ON (x=y) WHERE ifnull(z, 0) >=0;\n")
			return
		}
		got := flatten(r)
		want := "0"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-21.0"
		_res = db.Exec("\n  CREATE TABLE t12(a, b, c);\n  CREATE TABLE t13(x);\n  CREATE INDEX t12ab ON t12(b, a);\n  CREATE INDEX t12ac ON t12(c, a);\n\n  INSERT INTO t12 VALUES(4, 0, 1);\n  INSERT INTO t12 VALUES(4, 1, 0);\n  INSERT INTO t12 VALUES(5, 0, 1);\n  INSERT INTO t12 VALUES(5, 1, 0);\n\n  INSERT INTO t13 VALUES(1), (2), (3), (4);\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t12(a, b, c);\n  CREATE TABLE t13(x);\n  CREATE INDEX t12ab ON t12(b, a);\n  CREATE INDEX t12ac ON t12(c, a);\n\n  INSERT INTO t12 VALUES(4, 0, 1);\n  INSERT INTO t12 VALUES(4, 1, 0);\n  INSERT INTO t12 VALUES(5, 0, 1);\n  INSERT INTO t12 VALUES(5, 1, 0);\n\n  INSERT INTO t13 VALUES(1), (2), (3), (4);\n")
		}
	}
	{ // "where-21.1"
		r = db.Query("\n  SELECT * FROM t12 WHERE \n  a = (SELECT * FROM (SELECT count(*) FROM t13 LIMIT 5) ORDER BY 1 LIMIT 10) \n  AND (b=1 OR c=1);\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t12 WHERE \n  a = (SELECT * FROM (SELECT count(*) FROM t13 LIMIT 5) ORDER BY 1 LIMIT 10) \n  AND (b=1 OR c=1);\n")
			return
		}
		got := flatten(r)
		want := "\n  4 1 0\n  4 0 1\n"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	db, err = frigolite.Open(":memory:")
	if err != nil { t.Fatal(err) }
	{ // "where-22.1"
		r = db.Query("\n  CREATE TABLE t1(a INT);\n  CREATE INDEX t1a ON t1(a);\n  INSERT INTO t1(a) VALUES(NULL),(NULL),(42),(NULL),(NULL);\n  CREATE TABLE t2(dummy INT);\n  SELECT count(*) FROM t1 LEFT JOIN t2 ON a IS NOT NULL;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t1(a INT);\n  CREATE INDEX t1a ON t1(a);\n  INSERT INTO t1(a) VALUES(NULL),(NULL),(42),(NULL),(NULL);\n  CREATE TABLE t2(dummy INT);\n  SELECT count(*) FROM t1 LEFT JOIN t2 ON a IS NOT NULL;\n")
			return
		}
		got := flatten(r)
		want := "5"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-23.0"
		r = db.Query("\n  DROP TABLE IF EXISTS t1;\n  DROP TABLE IF EXISTS t2;\n  CREATE TABLE t1(a INTEGER PRIMARY KEY);\n  INSERT INTO t1(a) VALUES(1),(2),(3);\n  CREATE TABLE t2(x INTEGER PRIMARY KEY, y INT);\n  INSERT INTO t2(y) VALUES(2),(3);\n  SELECT * FROM t1, t2 WHERE a=y AND y=3;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  DROP TABLE IF EXISTS t1;\n  DROP TABLE IF EXISTS t2;\n  CREATE TABLE t1(a INTEGER PRIMARY KEY);\n  INSERT INTO t1(a) VALUES(1),(2),(3);\n  CREATE TABLE t2(x INTEGER PRIMARY KEY, y INT);\n  INSERT INTO t2(y) VALUES(2),(3);\n  SELECT * FROM t1, t2 WHERE a=y AND y=3;\n")
			return
		}
		got := flatten(r)
		want := "3 2 3"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	db.Close()
	db, err = frigolite.Open("")
	if err != nil { t.Fatal(err) }
	{ // "where-24.0"
		_res = db.Exec("\n  CREATE TABLE t1(a INTEGER PRIMARY KEY, b);\n  INSERT INTO t1 VALUES(1, 'one');\n  INSERT INTO t1 VALUES(2, 'two');\n  INSERT INTO t1 VALUES(3, 'three');\n  INSERT INTO t1 VALUES(4, 'four');\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t1(a INTEGER PRIMARY KEY, b);\n  INSERT INTO t1 VALUES(1, 'one');\n  INSERT INTO t1 VALUES(2, 'two');\n  INSERT INTO t1 VALUES(3, 'three');\n  INSERT INTO t1 VALUES(4, 'four');\n")
		}
	}
	// foreach {tn sql res} "\n  1 \"SELECT b FROM t1\"                   {one two three four}\n  2 \"SELECT b FROM t1 WHERE a<4\"         {one two three}\n  3 \"SELECT b FROM t1 WHERE a>1\"         {two three four}\n  4 \"SELECT b FROM t1 WHERE a>1 AND a<4\" {two three}\n\n  5 \"SELECT b FROM t1 WHERE a>? AND a<4\" {}\n  6 \"SELECT b FROM t1 WHERE a>1 AND a<?\" {}\n  7 \"SELECT b FROM t1 WHERE a>? AND a<?\" {}\n\n  7 \"SELECT b FROM t1 WHERE a>=? AND a<=4\" {}\n  8 \"SELECT b FROM t1 WHERE a>=1 AND a<=?\" {}\n  9 \"SELECT b FROM t1 WHERE a>=? AND a<=?\" {}\n"
	_items := []string{"\n  1 \"SELECT b FROM t1\"                   {one two three four}\n  2 \"SELECT b FROM t1 WHERE a<4\"         {one two three}\n  3 \"SELECT b FROM t1 WHERE a>1\"         {two three four}\n  4 \"SELECT b FROM t1 WHERE a>1 AND a<4\" {two three}\n\n  5 \"SELECT b FROM t1 WHERE a>? AND a<4\" {}\n  6 \"SELECT b FROM t1 WHERE a>1 AND a<?\" {}\n  7 \"SELECT b FROM t1 WHERE a>? AND a<?\" {}\n\n  7 \"SELECT b FROM t1 WHERE a>=? AND a<=4\" {}\n  8 \"SELECT b FROM t1 WHERE a>=1 AND a<=?\" {}\n  9 \"SELECT b FROM t1 WHERE a>=? AND a<=?\" {}\n"}
	for _idx := 0; _idx+3 <= len(_items); _idx += 3 {
	tn := _items[_idx+0]
	sql := _items[_idx+1]
	res := _items[_idx+2]
		var rev = "list"
		_ = rev // suppress unused warning
		for _, r := range []string{res} {
			var rev = "concat $r $rev"
			_ = rev // suppress unused warning
		}
		{ // "where-24." + tn + ".1"
			_res = db.Exec(sql)
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, sql)
			}
		}
		{ // "where-24." + tn + ".2"
			_res = db.Exec(sql + " ORDER BY rowid")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, sql + " ORDER BY rowid")
			}
		}
		{ // "where-24." + tn + ".3"
			_res = db.Exec(sql + " ORDER BY rowid DESC")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, sql + " ORDER BY rowid DESC")
			}
		}
		{ // "where-24-" + tn + ".4"
			_res = db.Exec("\n    BEGIN;\n      DELETE FROM t1;\n      " + sql + ";\n      " + sql + " ORDER BY rowid;\n      " + sql + " ORDER BY rowid DESC;\n    ROLLBACK;\n  ")
			if _res.Error != nil {
				t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    BEGIN;\n      DELETE FROM t1;\n      " + sql + ";\n      " + sql + " ORDER BY rowid;\n      " + sql + " ORDER BY rowid DESC;\n    ROLLBACK;\n  ")
			}
		}
	}
	}
	db.Close()
	db, err = frigolite.Open("")
	if err != nil { t.Fatal(err) }
	{ // "where-25.0"
		_res = db.Exec("\n  CREATE TABLE t1(a INTEGER PRIMARY KEY, b, c);\n  CREATE UNIQUE INDEX i1 ON t1(c);\n  INSERT INTO t1 VALUES(1, 'one', 'i');\n  INSERT INTO t1 VALUES(2, 'two', 'ii');\n\n  CREATE TABLE t2(a INTEGER PRIMARY KEY, b, c);\n  CREATE UNIQUE INDEX i2 ON t2(c);\n  INSERT INTO t2 VALUES(1, 'one', 'i');\n  INSERT INTO t2 VALUES(2, 'two', 'ii');\n  INSERT INTO t2 VALUES(3, 'three', 'iii');\n\n  PRAGMA writable_schema = 1;\n  UPDATE sqlite_schema SET rootpage = (\n    SELECT rootpage FROM sqlite_schema WHERE name = 'i2'\n  ) WHERE name = 'i1';\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t1(a INTEGER PRIMARY KEY, b, c);\n  CREATE UNIQUE INDEX i1 ON t1(c);\n  INSERT INTO t1 VALUES(1, 'one', 'i');\n  INSERT INTO t1 VALUES(2, 'two', 'ii');\n\n  CREATE TABLE t2(a INTEGER PRIMARY KEY, b, c);\n  CREATE UNIQUE INDEX i2 ON t2(c);\n  INSERT INTO t2 VALUES(1, 'one', 'i');\n  INSERT INTO t2 VALUES(2, 'two', 'ii');\n  INSERT INTO t2 VALUES(3, 'three', 'iii');\n\n  PRAGMA writable_schema = 1;\n  UPDATE sqlite_schema SET rootpage = (\n    SELECT rootpage FROM sqlite_schema WHERE name = 'i2'\n  ) WHERE name = 'i1';\n")
		}
	}
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	{ // "where-25.1"
		_res = db.Exec("\n  DELETE FROM t1 WHERE c='iii'\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "database disk image is malformed") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "database disk image is malformed", _res.Error, "\n  DELETE FROM t1 WHERE c='iii'\n")
		}
	}
	{ // "where-25.2"
		_res = db.Exec("\n  INSERT INTO t1 VALUES(4, 'four', 'iii') \n    ON CONFLICT(c) DO UPDATE SET b=NULL\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "database disk image is malformed") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "database disk image is malformed", _res.Error, "\n  INSERT INTO t1 VALUES(4, 'four', 'iii') \n    ON CONFLICT(c) DO UPDATE SET b=NULL\n")
		}
	}
	db.Close()
	db, err = frigolite.Open("")
	if err != nil { t.Fatal(err) }
	{ // "where-25.3"
		_res = db.Exec("\n  CREATE TABLE t1(a PRIMARY KEY, b, c) WITHOUT ROWID;\n  CREATE UNIQUE INDEX i1 ON t1(c);\n  INSERT INTO t1 VALUES(1, 'one', 'i');\n  INSERT INTO t1 VALUES(2, 'two', 'ii');\n\n  CREATE TABLE t2(a INTEGER PRIMARY KEY, b, c);\n  CREATE UNIQUE INDEX i2 ON t2(c);\n  INSERT INTO t2 VALUES(1, 'one', 'i');\n  INSERT INTO t2 VALUES(2, 'two', 'ii');\n  INSERT INTO t2 VALUES(3, 'three', 'iii');\n\n  PRAGMA writable_schema = 1;\n  UPDATE sqlite_schema SET rootpage = (\n    SELECT rootpage FROM sqlite_schema WHERE name = 'i2'\n  ) WHERE name = 'i1';\n")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n  CREATE TABLE t1(a PRIMARY KEY, b, c) WITHOUT ROWID;\n  CREATE UNIQUE INDEX i1 ON t1(c);\n  INSERT INTO t1 VALUES(1, 'one', 'i');\n  INSERT INTO t1 VALUES(2, 'two', 'ii');\n\n  CREATE TABLE t2(a INTEGER PRIMARY KEY, b, c);\n  CREATE UNIQUE INDEX i2 ON t2(c);\n  INSERT INTO t2 VALUES(1, 'one', 'i');\n  INSERT INTO t2 VALUES(2, 'two', 'ii');\n  INSERT INTO t2 VALUES(3, 'three', 'iii');\n\n  PRAGMA writable_schema = 1;\n  UPDATE sqlite_schema SET rootpage = (\n    SELECT rootpage FROM sqlite_schema WHERE name = 'i2'\n  ) WHERE name = 'i1';\n")
		}
	}
	db, err = frigolite.Open("test.db")
	if err != nil { t.Fatal(err) }
	{ // "where-25.4"
		_res = db.Exec("\n  SELECT * FROM t1 WHERE c='iii'\n")
		if _res.Error == nil {
			t.Errorf("expected error, got none\n  sql: %s", "\n  SELECT * FROM t1 WHERE c='iii'\n")
		}
	}
	{ // "where-25.5"
		_res = db.Exec("\n  INSERT INTO t1 VALUES(4, 'four', 'iii') \n    ON CONFLICT(c) DO UPDATE SET b=NULL\n")
		if _res.Error == nil || !strings.Contains(_res.Error.Error(), "corrupt database") {
			t.Errorf("expected error containing %q, got: %v\n  sql: %s", "corrupt database", _res.Error, "\n  INSERT INTO t1 VALUES(4, 'four', 'iii') \n    ON CONFLICT(c) DO UPDATE SET b=NULL\n")
		}
	}
	db, err = frigolite.Open(":memory:")
	if err != nil { t.Fatal(err) }
	{ // "where-26.1"
		r = db.Query("\n  CREATE TABLE t0(c0 INTEGER PRIMARY KEY, c1 TEXT);\n  INSERT INTO t0(c0, c1) VALUES (1, 'a');\n  CREATE TABLE t1(c0 INT PRIMARY KEY, c1 TEXT);\n  INSERT INTO t1(c0, c1) VALUES (1, 'a');\n  SELECT * FROM t0 WHERE '-1' BETWEEN 0 AND t0.c0;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t0(c0 INTEGER PRIMARY KEY, c1 TEXT);\n  INSERT INTO t0(c0, c1) VALUES (1, 'a');\n  CREATE TABLE t1(c0 INT PRIMARY KEY, c1 TEXT);\n  INSERT INTO t1(c0, c1) VALUES (1, 'a');\n  SELECT * FROM t0 WHERE '-1' BETWEEN 0 AND t0.c0;\n")
			return
		}
		got := flatten(r)
		want := "1 a"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-26.2"
		r = db.Query("\n  SELECT * FROM t1 WHERE '-1' BETWEEN 0 AND t1.c0;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t1 WHERE '-1' BETWEEN 0 AND t1.c0;\n")
			return
		}
		got := flatten(r)
		want := "1 a"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-26.3"
		r = db.Query("\n  SELECT * FROM t0 WHERE '-1'>=0 AND '-1'<=t0.c0;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t0 WHERE '-1'>=0 AND '-1'<=t0.c0;\n")
			return
		}
		got := flatten(r)
		want := "1 a"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-26.4"
		r = db.Query("\n  SELECT * FROM t1 WHERE '-1'>=0 AND '-1'<=t1.c0;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT * FROM t1 WHERE '-1'>=0 AND '-1'<=t1.c0;\n")
			return
		}
		got := flatten(r)
		want := "1 a"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-26.5"
		r = db.Query("\n  SELECT '-1' BETWEEN 0 AND t0.c0 FROM t0;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT '-1' BETWEEN 0 AND t0.c0 FROM t0;\n")
			return
		}
		got := flatten(r)
		want := "1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-26.6"
		r = db.Query("\n  SELECT '-1' BETWEEN 0 AND t1.c0 FROM t1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT '-1' BETWEEN 0 AND t1.c0 FROM t1;\n")
			return
		}
		got := flatten(r)
		want := "1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-26.7"
		r = db.Query("\n  SELECT '-1'>=0 AND '-1'<=t0.c0 FROM t0;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT '-1'>=0 AND '-1'<=t0.c0 FROM t0;\n")
			return
		}
		got := flatten(r)
		want := "1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // "where-26.8"
		r = db.Query("\n  SELECT '-1'>=0 AND '-1'<=t1.c0 FROM t1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  SELECT '-1'>=0 AND '-1'<=t1.c0 FROM t1;\n")
			return
		}
		got := flatten(r)
		want := "1"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	if tclBool("permutation" + "!=\"valgrind\"") {
		db.Close()
		db, err = frigolite.Open("")
		if err != nil { t.Fatal(err) }
		{ // "where-27.1"
			r = db.Query("\n    CREATE TABLE t1(a INTEGER PRIMARY KEY);\n    INSERT INTO t1(a) VALUES(9223372036854775807);\n    SELECT 1 FROM t1 WHERE a>=(9223372036854775807+1);\n  ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    CREATE TABLE t1(a INTEGER PRIMARY KEY);\n    INSERT INTO t1(a) VALUES(9223372036854775807);\n    SELECT 1 FROM t1 WHERE a>=(9223372036854775807+1);\n  ")
			}
		}
		{ // "where-27.2"
			r = db.Query("\n    SELECT a>=9223372036854775807+1 FROM t1;\n  ")
			if r.Error != nil {
				t.Errorf("query error: %v\n  sql: %s", r.Error, "\n    SELECT a>=9223372036854775807+1 FROM t1;\n  ")
				return
			}
			got := flatten(r)
			want := "0"
			if got != want {
				t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
			}
		}
	}
	db.Close()
	db, err = frigolite.Open("")
	if err != nil { t.Fatal(err) }
	{ // "where-28.1"
		r = db.Query("\n  CREATE TABLE t1(a INTEGER PRIMARY KEY, b INT);\n  CREATE INDEX t1b ON t1(b,b,b,b,b,b,b,b,b,b,b,b,b);\n  INSERT INTO t1(a,b) VALUES(1,1),(15,2),(19,5);\n  UPDATE t1 SET b=999 WHERE a IN (SELECT 15) AND b IN (1,2);\n  SELECT * FROM t1;\n")
		if r.Error != nil {
			t.Errorf("query error: %v\n  sql: %s", r.Error, "\n  CREATE TABLE t1(a INTEGER PRIMARY KEY, b INT);\n  CREATE INDEX t1b ON t1(b,b,b,b,b,b,b,b,b,b,b,b,b);\n  INSERT INTO t1(a,b) VALUES(1,1),(15,2),(19,5);\n  UPDATE t1 SET b=999 WHERE a IN (SELECT 15) AND b IN (1,2);\n  SELECT * FROM t1;\n")
			return
		}
		got := flatten(r)
		want := "\n 1  1\n 15 999\n 19 5\n"
		if got != want {
			t.Errorf("result mismatch\n  got:  [%s]\n  want: [%s]", got, want)
		}
	}
	{ // do_test "where-30.1"
		var res = ""
		_ = res // suppress unused warning
		_res = db.Exec("CREATE TABLE raw(country,date,total,delta, UNIQUE(country,date));")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "CREATE TABLE raw(country,date,total,delta, UNIQUE(country,date));")
		}
		_res = db.Exec("\n    EXPLAIN QUERY PLAN\n    WITH\n      -- Find the country and min/max date\n      init(country, date, fin) AS (SELECT country, min(date), max(date)\n         FROM raw WHERE total > 0 GROUP BY country),\n    \n      -- Generate the date stream for each country\n      src(country, date) AS (SELECT raw.country, raw.date\n          FROM raw JOIN init i on raw.country = i.country AND raw.date > i.date\n          ORDER BY raw.country, raw.date),\n    \n      -- Generate the x & y for each entry in the country/date stream\n      vals(country, date, x, y) AS (SELECT src.country, src.date,\n                julianday(raw.date) - julianday(src.date), log(delta+1)\n          FROM src JOIN raw on raw.country = src.country\n                        AND raw.date > date(src.date,'-7 days')\n                        AND raw.date <= src.date AND delta >= 0),\n    \n      -- Accumulate the data we need\n      sums(country, date, x2, x, n, xy, y) AS (SELECT country, date,\n              sum(x*x*1.0), sum(x*1.0), sum(1.0), sum(x*y*1.0), sum(y*1.0)\n         FROM vals GROUP BY 1, 2),\n    \n      -- use these to calculate to divisor for the inverse matrix\n      mult(country, date, m) AS (SELECT country, date, 1.0/(x2 * n - x * x)\n         FROM sums),\n    \n      -- Build the inverse matrix\n      inv(country, date, a,b,c,d) AS (SELECT mult.country, mult.date, n * m,\n                -x * m, -x * m, x2 * m\n          FROM mult JOIN sums on sums.country=mult.country\n                         AND mult.date=sums.date),\n    \n      -- Calculate the coefficients for the least squares fit\n      fit(country, date, a, b) AS (SELECT inv.country, inv.date,\n               a * xy + b * y, c * xy + d * y\n          FROM inv\n          JOIN mult on mult.country = inv.country AND mult.date = inv.date\n          JOIN sums on sums.country = mult.country AND sums.date = mult.date\n    )\n    SELECT *, nFin/nPrev - 1 AS growth, log(2)/log(nFin/nPrev) AS doubling\n      FROM (SELECT f.*, exp(b) - 1 AS nFin, exp(a* (-1) + b) - 1 AS nPrev\n              FROM fit f JOIN init i on i.country = f.country\n                          AND f.date <= date(i.fin,'-3 days'))\n     WHERE nPrev > 0 AND nFin > 0;\n  ")
		if _res.Error != nil {
			t.Errorf("exec error: %v\n  sql: %s", _res.Error, "\n    EXPLAIN QUERY PLAN\n    WITH\n      -- Find the country and min/max date\n      init(country, date, fin) AS (SELECT country, min(date), max(date)\n         FROM raw WHERE total > 0 GROUP BY country),\n    \n      -- Generate the date stream for each country\n      src(country, date) AS (SELECT raw.country, raw.date\n          FROM raw JOIN init i on raw.country = i.country AND raw.date > i.date\n          ORDER BY raw.country, raw.date),\n    \n      -- Generate the x & y for each entry in the country/date stream\n      vals(country, date, x, y) AS (SELECT src.country, src.date,\n                julianday(raw.date) - julianday(src.date), log(delta+1)\n          FROM src JOIN raw on raw.country = src.country\n                        AND raw.date > date(src.date,'-7 days')\n                        AND raw.date <= src.date AND delta >= 0),\n    \n      -- Accumulate the data we need\n      sums(country, date, x2, x, n, xy, y) AS (SELECT country, date,\n              sum(x*x*1.0), sum(x*1.0), sum(1.0), sum(x*y*1.0), sum(y*1.0)\n         FROM vals GROUP BY 1, 2),\n    \n      -- use these to calculate to divisor for the inverse matrix\n      mult(country, date, m) AS (SELECT country, date, 1.0/(x2 * n - x * x)\n         FROM sums),\n    \n      -- Build the inverse matrix\n      inv(country, date, a,b,c,d) AS (SELECT mult.country, mult.date, n * m,\n                -x * m, -x * m, x2 * m\n          FROM mult JOIN sums on sums.country=mult.country\n                         AND mult.date=sums.date),\n    \n      -- Calculate the coefficients for the least squares fit\n      fit(country, date, a, b) AS (SELECT inv.country, inv.date,\n               a * xy + b * y, c * xy + d * y\n          FROM inv\n          JOIN mult on mult.country = inv.country AND mult.date = inv.date\n          JOIN sums on sums.country = mult.country AND sums.date = mult.date\n    )\n    SELECT *, nFin/nPrev - 1 AS growth, log(2)/log(nFin/nPrev) AS doubling\n      FROM (SELECT f.*, exp(b) - 1 AS nFin, exp(a* (-1) + b) - 1 AS nPrev\n              FROM fit f JOIN init i on i.country = f.country\n                          AND f.date <= date(i.fin,'-3 days'))\n     WHERE nPrev > 0 AND nFin > 0;\n  ")
		}
	}
}
