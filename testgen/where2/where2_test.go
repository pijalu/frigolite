...
func Test_where2(t *testing.T) {
	...
	{ // do_test "where2-2.4"
		...
		_dbeval0 := tclExecSQL(db, sql)
		out1 = _dbeval0
		_ = out1 // suppress unused warning
		_dbeval1 := tclExecSQL(db, sql)
		out2 = _dbeval1
		_ = out2 // suppress unused warning
		_dbeval2 := tclExecSQL(db, sql)
		out3 = _dbeval2
		_ = out3 // suppress unused warning
		// expr $out1!=$out2 && $out2!=$out3 (not evaluated)
	}
	{ // "where2-2.5" — skipped: EXPLAIN VDBE opcode output not implemented (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  -- random() is not optimized out\n  EXPLAIN SELECT * FROM x1, x2 WHERE x=1 ORDER BY random();\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "where2-2.5b" — skipped: EXPLAIN VDBE opcode output not implemented (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  -- random() is not optimized out\n  EXPLAIN SELECT * FROM x1, x2 WHERE x=1 ORDER BY random();\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "where2-2.6" — skipped: EXPLAIN VDBE opcode output not implemented (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  -- other constant functions are optimized out\n  EXPLAIN SELECT * FROM x1, x2 WHERE x=1 ORDER BY abs(5);\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "where2-2.6b" — skipped: EXPLAIN VDBE opcode output not implemented (G5.EXPLAIN) (SQL side effects only)
		_res = db.Exec("\n  -- other constant functions are optimized out\n  EXPLAIN SELECT * FROM x1, x2 WHERE x=1 ORDER BY abs(5);\n")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // do_test "where2-3.1"
	...
	if "" != "no_optimization" {
		{ // "where2-12.1" — skipped: EXPLAIN QUERY PLAN join OR not planned (G3.INDEX) (SQL side effects only)
