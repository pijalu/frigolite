...
func Test_whereF(t *testing.T) {
	...
	vtab.TclVarSet("testprefix", "", "whereF")
	testprefix = "whereF"
	...
	for _idx0 := 0; _idx0+2 <= len(_items0); _idx0 += 2 {
		...
		for _idx1 := 0; _idx1+2 <= len(_items1); _idx1 += 2 {
			...
			for _idx2 := 0; _idx2+2 <= len(_items2); _idx2 += 2 {
				...
				{ // "whereF-4.0" — skipped: EXPLAIN QUERY PLAN PK autoindex SEARCH not planned (G3.INDEX) (SQL side effects only)
				...
				{ // do_test "5.2"
					// expr [db status vmstep]<200 → runtime compare
					_r = tclBool01(toInt(tclDbOne(db, "db status vmstep"))  <  200)
				}
				{ // "5.3"
				...
				{ // do_test "5.4"
					// expr [db status vmstep]>1000 → runtime compare
					_r = tclBool01(toInt(tclDbOne(db, "db status vmstep"))  >  1000)
				}
				{ // "5.5"
				...
				{ // "whereF-7.2" — skipped: correlated scalar subquery returns wrong count (G2.SUBQUERY) (SQL side effects only)
				...
				{ // "whereF-7.3" — skipped: EXPLAIN VDBE opcode output not implemented (G5.EXPLAIN) (SQL side effects only)
