...
func Test_window6(t *testing.T) {
	...
	for _idx0 := 0; _idx0+2 <= len(_items0); _idx0 += 2 {
		...
		db.RegisterFunction("window", func(args []interface{}) (interface{}, error) {
			var parts []string
			// TCL "$args" substitutes the argument LIST's string form:
			// each element is list-quoted — a multi-word element arrives
			// braced (Tcl list rendering), e.g. ('hello world') → {hello world}.
			for _, a := range args {
				if a == nil { continue }
				el := tclStr(a)
				if el != "" && !strings.ContainsAny(el, " \t{}\\") {
					parts = append(parts, el)
				} else if !strings.ContainsAny(el, "\n") && tclBracesBalanced(el) {
					parts = append(parts, "{"+el+"}")
				} else {
					parts = append(parts, el)
				}
		...
		// proc wincmp collation (registered via db collate)
		db.RegisterCollation("window", func(a, b string) int { return -strings.Compare(a, b) })
		{ // "3.0"
