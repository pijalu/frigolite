...
func Test_with2(t *testing.T) {
	...
	vtab.TclVarSet("testprefix", "", "with2")
	...
	{ // "with2-4.2" — skipped: depends on TCL genstmt proc (N-column WITH generator) not transpilable (SQL side effects only)
		_res = db.Exec("genstmt 10")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "with2-4.3" — skipped: depends on TCL genstmt proc (N-column WITH generator) not transpilable (SQL side effects only)
		_res = db.Exec("genstmt 100")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "with2-4.4" — skipped: depends on TCL genstmt proc (N-column WITH generator) not transpilable (SQL side effects only)
		_res = db.Exec("genstmt 255")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	nLimit = strconv.Itoa(db.Limit("SQLITE_LIMIT_COLUMN"))
	_ = nLimit // suppress unused warning
	{ // "with2-4.5" — skipped: depends on TCL genstmt proc (N-column WITH generator) + SQLITE_LIMIT_COLUMN (SQL side effects only)
		_res = db.Exec("genstmt [expr $nLimit-1]")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "with2-4.6" — skipped: depends on TCL genstmt proc (N-column WITH generator) + SQLITE_LIMIT_COLUMN (SQL side effects only)
		_res = db.Exec("genstmt $nLimit")
		_ = _res.Error // tolerate unsupported-feature errors in skipped tests
	}
	{ // "with2-4.7" — skipped: depends on TCL genstmt proc (N-column WITH generator) + SQLITE_LIMIT_COLUMN
	}
	// proc definition (not transpiled)
	{ // "5.1"
