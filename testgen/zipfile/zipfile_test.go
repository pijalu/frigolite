...
func Test_zipfile(t *testing.T) {
	...
	if false { // external-tool guard ([exec ...]) unavailable
		os.RemoveAll("test_unzip")
	...
	if tcl_platform_platform == "unix" {
		vtab.TclVarSet("modes", "", "-rw-r--r-x")
		modes = "-rw-r--r-x"
		_ = modes // suppress unused warning
		vtab.TclVarSet("perms", "", "33189")
		perms = "33189"
		_ = perms // suppress unused warning
	} else {
		vtab.TclVarSet("modes", "", "-rw-r--r--")
		modes = "-rw-r--r--"
		_ = modes // suppress unused warning
		vtab.TclVarSet("perms", "", "33188")
		perms = "33188"
		_ = perms // suppress unused warning
	}
	{ // "1.6.3"
	...
	for _idx0 := 0; _idx0+2 <= len(_items0); _idx0 += 2 {
		...
		for _idx1 := 0; _idx1+2 <= len(_items1); _idx1 += 2 {
			...
			db.RegisterFunction("rt", func(args []interface{}) (interface{}, error) {
				...
				if b, ok := args[0].([]byte); ok { return tclRemoveTimestamps(b), nil }
				if s, ok := args[0].(string); ok { return tclRemoveTimestamps(tclHexDecode(s)), nil }
			...
			{ // do_test "7.0"
				...
				{
					var _catchErr error
					_dbevalRows2 := db.Query(" SELECT name, data FROM zipfile('test.zip') ")
					var _dbevalRb3 bool
					...
					for _ri := 0; _ri < len(_dbevalRows2.Rows) && _dbevalErr4 == nil; _ri++ {
						...
						if func() bool { name_n, _name_e := strconv.Atoi(name); if _name_e != nil { return false }; return name_n == 2 }() {
							// close [open test.zip w+]
						}
