...
// generateTokenConstants emits the TK_* token constant block for a grammar.
func generateTokenConstants(grammar *LemonGrammar, tokenCode map[string]int) string {
	var buf strings.Builder
	buf.WriteString("// Token codes\n")
	buf.WriteString("const (\n")
	buf.WriteString("\tYYNOCODE = 0\n")
...
// generateGetParseTables emits the GetParseTables function that wires the
// generated tables into a ParseTables value usable by the engine runtime.
func generateGetParseTables(tables *ParseTables) string {
	var buf strings.Builder
...
// lemparPlaceholders are the marker tokens in lempar.go that the generator
// replaces to produce a complete, self-contained Go parser file.
const (
	lemparPackageMarker       = "package __LEMON_PACKAGE__"
	lemparTokenConstantsMarker = "// __LEMON_TOKEN_CONSTANTS__"
	lemparParseTablesMarker    = "// __LEMON_PARSE_TABLES__"
	lemparActionCodeMarker     = "// __LEMON_ACTION_CODE__"
)

// InstantiateLempar reads the lempar.go template (the Go port of SQLite's
// lempar.c) and fills in the parse tables, token constants, and grammar action
// code for the given grammar. The result is a complete, self-contained Go
// parser file: it embeds the shift/reduce engine runtime plus the generated
// tables, so it does not depend on the go-lemon tool package at runtime.
func InstantiateLempar(tables *ParseTables, grammar *LemonGrammar, tokenCode map[string]int, pkgName string) (string, error) {
	// Locate lempar.go relative to this package source file.
	path, err := lemparTemplatePath()
	if err != nil {
		return "", err
	}
	data, err := os.ReadFile(path)
	if err != nil {
		return "", fmt.Errorf("reading lempar.go template: %w", err)
	}
	tmpl := string(data)

	// The template carries //go:build ignore so it is not compiled as part of
	// the go-lemon package. The generated parser must NOT have that tag.
	tmpl = strings.Replace(tmpl, "//go:build ignore\n", "", 1)

	// Package name.
	tmpl = strings.Replace(tmpl, lemparPackageMarker, "package "+pkgName, 1)

	// Token constants.
	tmpl = strings.Replace(tmpl, lemparTokenConstantsMarker,
		generateTokenConstants(grammar, tokenCode), 1)

	// Parse tables + GetParseTables.
	var tablesBlock strings.Builder
	tablesBlock.WriteString(generateTablesGo(tables))
	tablesBlock.WriteString(generateGetParseTables(tables))
	tmpl = strings.Replace(tmpl, lemparParseTablesMarker, tablesBlock.String(), 1)

	// Action code (yyReduceAction) implementing grammar actions.
	actionCode := "// (no grammar actions)\nfunc yyReduceAction(ruleNo int, p *Parser, lookahead int, lookaheadToken interface{}) {\n\t_ = ruleNo\n\t_ = p\n\t_ = lookahead\n\t_ = lookaheadToken\n}\n"
	if hasActions(grammar) {
		actionCode = generateActionFunction(grammar)
	}
	tmpl = strings.Replace(tmpl, lemparActionCodeMarker, actionCode, 1)

	return tmpl, nil
}

// lemparTemplatePath locates lempar.go (the parser engine runtime template)
// relative to this source file, so InstantiateLempar works both when running
// `go run ./tools/go-lemon/` from the repo root and when the package is
// compiled as part of the go-lemon binary.
func lemparTemplatePath() (string, error) {
	candidates := []string{
		"tools/go-lemon/lempar.go",
		"lempar.go",
		"/Users/muaddib/dev/frigolite/tools/go-lemon/lempar.go",
	}
	...
	return "", fmt.Errorf("lempar.go template not found (tried %v)", candidates)
}
