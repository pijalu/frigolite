...
// processSqlite3AutovacuumPages handles `sqlite3_autovacuum_pages DB [PROC]`
// — register, clear, or query the connection's per-batch autovacuum-pages
// callback. With a proc name, emit a SetAutovacuumPagesCallback call passing
// the Go closure generated by processProc for that proc name
// (autovac_page_callback or autovac_page_callback_off). Without a proc
// name (or with {}), emit SetAutovacuumPagesCallback(nil) to clear.
// P8.INCRVACUUM phase 4.
func (tp *transpiler) processSqlite3AutovacuumPages(args []tcl.RawWord) {
	if len(args) < 1 {
		tp.emitLine("// sqlite3_autovacuum_pages (insufficient args)")
		return
	}
	dbConn := tp.dbArgGo(args[0].Text)
	if len(args) < 2 || strings.TrimSpace(args[1].Text) == "" || strings.TrimSpace(args[1].Text) == "{}" {
		tp.emitLine("%s.SetAutovacuumPagesCallback(nil)", dbConn)
		return
	}
	procName := strings.TrimPrefix(strings.TrimSpace(args[1].Text), "::")
	goName := tclVarToGo(procName)
	if _, ok := tp.autovacCallbacks[procName]; !ok {
		tp.emitLine("// sqlite3_autovacuum_pages %s %s (proc body not transpiled)", dbConn, procName)
		return
	}
	tp.emitLine("%s.SetAutovacuumPagesCallback(%s)", dbConn, goName)
}
