...
var skipTestFiles = map[string]string{
	...
	"shared":     "N-A G7 slice 5: shared-cache is a separate subsystem (global sqlite3_enable_shared_cache C-API + shared pager cache/schema registry + btree table-level locks — none of it in wal.c), deprecated upstream since 3.43.0 (SQLITE_OMIT_SHARED_CACHE); the WAL multi-connection machinery is the supported model. Residual engine gap logged as candidate goal P7.SHAREDCACHE (PORTPLAN Blocker Register); committed-data cross-connection visibility pinned natively by frigolite_shared_test.go; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 5)",
	"shared2":    "N-A G7 slice 5: shared-cache is a separate subsystem (global sqlite3_enable_shared_cache C-API + shared pager cache/schema registry + btree table-level locks — none of it in wal.c), deprecated upstream since 3.43.0 (SQLITE_OMIT_SHARED_CACHE); the WAL multi-connection machinery is the supported model. Residual engine gap logged as candidate goal P7.SHAREDCACHE (PORTPLAN Blocker Register); committed-data cross-connection visibility pinned natively by frigolite_shared_test.go; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 5)",
	"shared3":    "N-A G7 slice 5: shared-cache is a separate subsystem (global sqlite3_enable_shared_cache C-API + shared pager cache/schema registry + btree table-level locks — none of it in wal.c), deprecated upstream since 3.43.0 (SQLITE_OMIT_SHARED_CACHE); the WAL multi-connection machinery is the supported model. Residual engine gap logged as candidate goal P7.SHAREDCACHE (PORTPLAN Blocker Register); committed-data cross-connection visibility pinned natively by frigolite_shared_test.go; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 5)",
	"shared4":    "N-A G7 slice 5: shared-cache is a separate subsystem (global sqlite3_enable_shared_cache C-API + shared pager cache/schema registry + btree table-level locks — none of it in wal.c), deprecated upstream since 3.43.0 (SQLITE_OMIT_SHARED_CACHE); the WAL multi-connection machinery is the supported model. Residual engine gap logged as candidate goal P7.SHAREDCACHE (PORTPLAN Blocker Register); committed-data cross-connection visibility pinned natively by frigolite_shared_test.go; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 5)",
	"shared6":    "N-A G7 slice 5: shared-cache is a separate subsystem (global sqlite3_enable_shared_cache C-API + shared pager cache/schema registry + btree table-level locks — none of it in wal.c), deprecated upstream since 3.43.0 (SQLITE_OMIT_SHARED_CACHE); the WAL multi-connection machinery is the supported model. Residual engine gap logged as candidate goal P7.SHAREDCACHE (PORTPLAN Blocker Register); committed-data cross-connection visibility pinned natively by frigolite_shared_test.go; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 5)",
	"shared7":    "N-A G7 slice 5: shared-cache is a separate subsystem (global sqlite3_enable_shared_cache C-API + shared pager cache/schema registry + btree table-level locks — none of it in wal.c), deprecated upstream since 3.43.0 (SQLITE_OMIT_SHARED_CACHE); the WAL multi-connection machinery is the supported model. Residual engine gap logged as candidate goal P7.SHAREDCACHE (PORTPLAN Blocker Register); committed-data cross-connection visibility pinned natively by frigolite_shared_test.go; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 5)",
	"shared8":    "N-A G7 slice 5: shared-cache is a separate subsystem (global sqlite3_enable_shared_cache C-API + shared pager cache/schema registry + btree table-level locks — none of it in wal.c), deprecated upstream since 3.43.0 (SQLITE_OMIT_SHARED_CACHE); the WAL multi-connection machinery is the supported model. Residual engine gap logged as candidate goal P7.SHAREDCACHE (PORTPLAN Blocker Register); committed-data cross-connection visibility pinned natively by frigolite_shared_test.go; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 5)",
	"shared9":    "N-A G7 slice 5: shared-cache is a separate subsystem (global sqlite3_enable_shared_cache C-API + shared pager cache/schema registry + btree table-level locks — none of it in wal.c), deprecated upstream since 3.43.0 (SQLITE_OMIT_SHARED_CACHE); the WAL multi-connection machinery is the supported model. Residual engine gap logged as candidate goal P7.SHAREDCACHE (PORTPLAN Blocker Register); committed-data cross-connection visibility pinned natively by frigolite_shared_test.go; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 5)",
	"shared_err": "N-A G7 slice 5: shared-cache is a separate subsystem (global sqlite3_enable_shared_cache C-API + shared pager cache/schema registry + btree table-level locks — none of it in wal.c), deprecated upstream since 3.43.0 (SQLITE_OMIT_SHARED_CACHE); the WAL multi-connection machinery is the supported model. Residual engine gap logged as candidate goal P7.SHAREDCACHE (PORTPLAN Blocker Register); committed-data cross-connection visibility pinned natively by frigolite_shared_test.go; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 5)",
	"sharedlock": "N-A G7 slice 5: shared-cache is a separate subsystem (global sqlite3_enable_shared_cache C-API + shared pager cache/schema registry + btree table-level locks — none of it in wal.c), deprecated upstream since 3.43.0 (SQLITE_OMIT_SHARED_CACHE); the WAL multi-connection machinery is the supported model. Residual engine gap logged as candidate goal P7.SHAREDCACHE (PORTPLAN Blocker Register); committed-data cross-connection visibility pinned natively by frigolite_shared_test.go; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 5)",

	"snapshot":       "N-A G7 slice 4 superseded (C-API harness sqlite3_snapshot_get/open/cmp/free + _blob — untranspilable, tcl2go emits empty stubs; engine-visible contract pinned natively: frigolite_walsnapshot_test.go; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 4)",
	"snapshot2":      "N-A G7 slice 4 superseded (C-API harness sqlite3_snapshot_get_blob/open_blob + sqlite3_snapshot_recover + sqlite3_db_config NO_CKPT_ON_CLOSE — untranspilable, tcl2go emits empty stubs; pinned natively: frigolite_walsnapshot_test.go (TestWalSnapshotCmp, TestWalSnapshotRecover, TestWalSnapshotFullCheckpointKeepsStale); evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 4)",
	"snapshot3":      "N-A G7 slice 4 superseded (C-API harness: cross-connection snapshot_open + ERROR_SNAPSHOT via sqlite3_snapshot_* — untranspilable, tcl2go emits empty stubs; pinned natively: frigolite_walsnapshot_test.go (TestWalSnapshotStaleAfterCheckpoint, TestWalSnapshotStaleAfterRestart, TestWalSnapshotFreshConnectionOpen); evidence portplan/NA_EVIDENCE.md §P7.WAL-G7 slice 4)",
	...
	"uri":               "converter gaps: file-isdir helper + error-variable emission (T5 follow-up)",
	"uri2":              "converter + engine gaps: %00-in-URI rejection (ENABLE_URI_00_ERROR) and sqlite3_open/errcode seams (T5 follow-up)",
	...
	"walrestart":        "N-A G7 slice 2 (sqlite3_test_control faultsim injection drives the 1.2/1.4 checkpoint-vs-writer races — untranspilable; race contract covered by frigolite_wallocks_test.go TestWalRestartConcurrent + triple shapes in TestWalCheckpointTruncateZero; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7)",
	"walsetlk":          "N-A G7 slice 2 (testvfs -fullshm + xSleep counting for blocking-lock timing is harness-observable, untranspilable; engine contracts — corrupt -shm recovery, BEGIN EXCLUSIVE busy, TRUNCATE→0 — covered by frigolite_wallocks_test.go TestWalLockCorruptShmRecovery/TestWalLockBusyTimeout/TestWalCheckpointTruncateZero; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7)",
	"walsetlk2":         "N-A G7 slice 2 (sqlite3_setlk_timeout C-API + db .timeout cmd gating — C-API untranspilable; non-blocking F_SETLK + busy-retry contract covered by frigolite_wallocks_test.go TestWalLockBusyTimeout/TestWalLockBusySnapshot; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7)",
	"walsetlk3":         "N-A G7 slice 2 (sqlite3_setlk_timeout blocking-lock C-API with deadline — untranspilable; engine contracts covered by frigolite_wallocks_test.go TestWalLockBusyTimeout/TestWalLockProtocol; evidence portplan/NA_EVIDENCE.md §P7.WAL-G7)",
