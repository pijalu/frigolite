...
func TestVerifyASC(t *testing.T) {
	cases := []string{
		...
		// Note: "CREATE TABLE t1(a UNIQUE(b DESC))" is NOT valid SQLite — a
		// column-level UNIQUE cannot take a parenthesized column list (the
		// oracle rejects it with "near '(': syntax error").
	}
